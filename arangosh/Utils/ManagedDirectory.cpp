...
inline TRI_read_reply_t rawRead(int fd, char* buffer, size_t length, arangodb::Result& status,
                       std::string const& path, int flags) {
  TRI_read_reply_t bytesRead = TRI_READ(fd, buffer, static_cast<TRI_read_t>(length));
  if (bytesRead < 0) {
    status = ::genericError(path, flags);
  }
  return bytesRead;
...
TRI_read_reply_t ManagedDirectory::File::read(char* buffer, size_t length) {
  TRI_read_reply_t bytesRead = -1;
...
std::string ManagedDirectory::File::slurp() {
  ...
  if (::isReadable(_fd, _flags, _path, _status)) {
    ...
    while (true) {
      TRI_read_reply_t bytesRead = read(buffer, ::DefaultIOChunkSize);
      if (_status.ok()) {
...
ssize_t ManagedDirectory::File::offset() const {
  TRI_read_reply_t fileBytesRead = -1;

  if (isGzip()) {
    fileBytesRead = gzoffset(_gzFile);
  } else {
    fileBytesRead = (TRI_read_reply_t)TRI_LSEEK(_fd, 0L, SEEK_CUR);
  } // else
