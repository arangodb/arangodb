...
struct EdgeCrudTest : public Benchmark<EdgeCrudTest> {
  ...
  void buildRequest(size_t threadNumber, size_t threadCounter, size_t globalCounter,
    ...
    if (mod == 0 || mod == 2) {
      ...
      if (mod == 0) {
        requestData.payload.add("_from", Value(_arangobench.collection() + std::string("/testfrom") +
                                               std::to_string(globalCounter)));
        requestData.payload.add("_to", Value(_arangobench.collection() + std::string("/testto") +
                                             std::to_string(globalCounter)));
      }
