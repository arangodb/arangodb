...
struct StreamCursorTest : public Benchmark<StreamCursorTest> {
  ...
  StreamCursorTest(BenchFeature& arangobench)
      : Benchmark<StreamCursorTest>(arangobench) {}
  ...
  void buildRequest(
    ...
    if (globalCounter == 0) {
      ...
      query += " } INTO " + _arangobench.collection() +
               std::string(" OPTIONS { ignoreErrors: true }");
    } else {
      query = "FOR doc IN " + _arangobench.collection() +
  ...
  char const* getDescription() const noexcept override {
    return "creates 500 documents in a collection, and then performs a mix of "
           "AQL update queries (all on the same document) and a streaming AQL "
           "query that returns all documents from the collection. The "
           "--complexity parameter can be used to control the number of "
           "attributes for the inserted documents and the update queries. This "
           "test will trigger a lot of write-write conflicts with "
           "--concurrency bigger than 2.";
  }
