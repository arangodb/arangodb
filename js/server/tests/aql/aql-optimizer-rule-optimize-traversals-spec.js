...
const paramEnabled  = { allPlans: true, verbosePlans: true, optimizer: { rules: [ "-all", "+" + ruleName ] } };
const paramDisabled = { allPlans: true, verbosePlans: true, optimizer: { rules: [ "+all", "-" + ruleName ] } };
...
describe('Rule optimize-traversals' , () => {
  ...
  before(() => {
    cleanup();
    graph = graph_module._create(graphName, [
  ...
  it('should prune when using functions', () => {
    ...
    queries.forEach(function (q) {
      const res = AQL_EXECUTE(q, bindVars, paramDisabled).json;
