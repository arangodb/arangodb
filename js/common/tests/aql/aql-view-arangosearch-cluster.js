...
function IResearchAqlTestSuite(numberOfShards, replicationFactor) {
  ...
  return {
    ...
    testExistsFilterByIdentityAnalyzer: function () {
      var result = db._query("FOR doc IN UnitTestsView SEARCH EXISTS(doc.text, 'analyzer') OPTIONS { forceSync: true } RETURN doc").toArray();
    ...
    testViewInInnerLoop : function() {
      ...
      var result = db._query(
        ...
        "  FOR doc IN UnitTestsView SEARCH adoc.id == doc.c && STARTS_WITH(doc['a'], adoc.a) OPTIONS { forceSync: true } " +
        "RETURN doc"
      ).toArray();
