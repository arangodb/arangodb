...
exports.testDbQuery = function (cn, fillBlockCache) {
  ...
  if (fillBlockCache) {
    assertTrue(newValue >= oldValue + 100 * 1000 * 1000, { oldValue,
newValue });
  } else {
    // allow for some other background things to be run, thus add 1MB of leeway
    assertTrue(newValue <= oldValue + 1000 * 1000, { oldValue,
newValue });
  }
...
exports.testHttpApi = function (cn, fillBlockCache) {
  const oldValue = db._engineStats()["rocksdb.block-cache-usage"];
  arango.POST("/_api/cursor", { query: "FOR doc IN @@collection RETURN doc.value1",
bindVars: { "@collection": cn },
options: { fillBlockCache } });
