...
const testPaths = {
  'shell_api': [ tu.pathForTesting('common/shell'), tu.pathForTesting('client/http'), tu.pathForTesting('client/shell') ],
...
function shellApiClient (options) {
  ...
  testCases = testCases.filter(test => { return test.search('api') >= 0; });
