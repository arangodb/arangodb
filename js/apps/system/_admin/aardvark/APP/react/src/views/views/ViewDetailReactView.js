...
import { escape, omit, truncate } from 'lodash';
...
const ViewDetailReactView = ({ name }) => {
  const { data } = useSWR(`/view/${name}/properties`,
    path => getApiRouteForCurrentDB().get(path));

  const view = data ? omit(data.body, 'error', 'code') : { name };

  useEffect(() => {
    $('#subNavigationBar .breadcrumb').html('View: ' + escape(truncate(name, 64)));
    arangoHelper.buildViewSubNav(name, 'Info');
    // arangoHelper.fixTooltips('.arangoicon', 'left');
  }, [name]);
...
window.ViewDetailReactView = ViewDetailReactView;
