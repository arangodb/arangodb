...
class ShardInfoRow extends Component<Props, IState> {
  ...
  render () {
    ...
    if (moving) {
      return (
        <MoveShard name={name} leader={leader} followers={followers} source={moving}
                   doMove={this.doMove.bind(this)} doCancel={this.cancelMove.bind(this)}/>
      );
    }
    return (
      <Row key={name} columns={4}>
        <LeftAlign>{name}</LeftAlign>
        <LeftAlign><Leader onClick={e => {
          e.preventDefault();
          this.setMoving(leader);
        }} key={leader}>{leader}</Leader></LeftAlign>
        <LeftAlign>
          {followers.map((f, i) => (
