/* global frontendConfig, arangoHelper, _, $, arangoHelper, templateEngine, Joi, JSONEditor, numeral */
...
(function () {
  ...
  window.DocumentsView = window.PaginationView.extend({
        ...
        var callback = function (error, type) {
          if (error) {
            ...
            if (this.collection.getSmartJoinAttribute()) {
              ...
              if (!this.collection.getIsEnterpriseEdge()) {
                // We do show the _key globally for every type of edge collection,
                // except in case we do have an "EnterpriseSmart" edge.
                tableContent.push(
                  window.modalView.createTextEntry(
                    'new-edge-key-attr',
                    '_key',
    ...
    addEdge: function () {
      ...
      var key;
      if (this.collection.getIsEnterpriseEdge()) {
        // We are not allowed to use any key in an "EnterpriseGraph" context.
        key = null;
      } else {
        // In all other cases we try to read the _key from its input field.
        key = $('.modal-body #new-edge-key-attr').last().val();
      }
      var body;
