...
def buildEdition(os, edition, maintainer) {
    ...
    try {
        if (os == 'linux') {
            sh "./Installation/Pipeline/include/build_OS_EDITION_MAINTAINER.sh 64 ${os} ${edition} ${maintainer} 64 ${arch}"
        }
        else if (os == 'mac') {
            sh "./Installation/Pipeline/include/build_OS_EDITION_MAINTAINER.sh 16 ${os} ${edition} ${maintainer} 64 ${arch}"
        }
        else if (os == 'windows') {
