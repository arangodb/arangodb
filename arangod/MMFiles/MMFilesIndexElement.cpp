...
MMFilesHashIndexElement::MMFilesHashIndexElement(
    TRI_voc_rid_t revisionId, std::vector<std::pair<VPackSlice, uint32_t>> const& values)
...
MMFilesHashIndexElement* MMFilesHashIndexElement::initialize(
    MMFilesHashIndexElement* element, TRI_voc_rid_t revisionId,
...
/// in place.
arangodb::velocypack::Slice MMFilesHashIndexElement::slice(IndexLookupContext* context,
...
uint64_t MMFilesHashIndexElement::hash(
    std::vector<std::pair<arangodb::velocypack::Slice, uint32_t>> const& values) {
  uint64_t hash = 0x0123456789abcdef;
  size_t const n = values.size();
...
MMFilesSkiplistIndexElement::MMFilesSkiplistIndexElement(
    TRI_voc_rid_t revisionId, std::vector<std::pair<VPackSlice, uint32_t>> const& values)
...
MMFilesSkiplistIndexElement* MMFilesSkiplistIndexElement::initialize(
...
/// the datafile or WAL file. If offset is 0, then data contains the actual data
/// in place.
arangodb::velocypack::Slice MMFilesSkiplistIndexElement::slice(IndexLookupContext* context,
                                                               size_t position) const {
  MMFilesIndexElementValue const* sub = subObject(position);
...
MMFilesSimpleIndexElement::MMFilesSimpleIndexElement(TRI_voc_rid_t revisionId,
                                                     arangodb::velocypack::Slice const& value,
                                                     uint32_t offset)
    : _revisionId(revisionId),
      _hashAndOffset(hash(value) | (static_cast<uint64_t>(offset) << 32)) {}
