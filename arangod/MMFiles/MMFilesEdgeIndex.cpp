...
/// @brief checks for elements are equal
static bool IsEqualElementEdgeByKey(void* userData, MMFilesSimpleIndexElement const& left,
                                    MMFilesSimpleIndexElement const& right) {
  IndexLookupContext* context = static_cast<IndexLookupContext*>(userData);
  try {
...
MMFilesEdgeIndex::MMFilesEdgeIndex(TRI_idx_iid_t iid, arangodb::LogicalCollection* collection)
  ...
  _edgesFrom = new TRI_MMFilesEdgeIndexHash_t(HashElementKey, HashElementEdge,
                                              ...
                                              IsEqualElementEdgeByKey,
                                              indexBuckets, initialSize, context);

  _edgesTo = new TRI_MMFilesEdgeIndexHash_t(HashElementKey, HashElementEdge, IsEqualKeyEdge,
                                            IsEqualElementEdge, IsEqualElementEdgeByKey,
                                            indexBuckets, initialSize, context);
}
...
double MMFilesEdgeIndex::selectivityEstimateLocal(arangodb::StringRef const* attribute) const {
...
Result MMFilesEdgeIndex::insert(transaction::Methods* trx, TRI_voc_rid_t revisionId,
...
Result MMFilesEdgeIndex::remove(transaction::Methods* trx, TRI_voc_rid_t revisionId,
                                VPackSlice const& doc, bool isRollback) {
  MMFilesSimpleIndexElement fromElement(buildFromElement(revisionId, doc));
  MMFilesSimpleIndexElement toElement(buildToElement(revisionId, doc));
...
void MMFilesEdgeIndex::batchInsert(transaction::Methods* trx,
  ...
  for (auto const& it : documents) {
    ...
    fromElements->emplace_back(MMFilesSimpleIndexElement(
        it.first, value, static_cast<uint32_t>(value.begin() - it.second.begin())));
  }
...
bool MMFilesEdgeIndex::supportsFilterCondition(arangodb::aql::AstNode const* node,
...
IndexIterator* MMFilesEdgeIndex::iteratorForCondition(transaction::Methods* trx,
...
///        entries.
void MMFilesEdgeIndex::expandInSearchValues(VPackSlice const slice, VPackBuilder& builder) const {
...
MMFilesSimpleIndexElement MMFilesEdgeIndex::buildFromElement(TRI_voc_rid_t revisionId,
                                                             VPackSlice const& doc) const {
  TRI_ASSERT(doc.isObject());
  VPackSlice value(transaction::helpers::extractFromFromDocument(doc));
  TRI_ASSERT(value.isString());
  return MMFilesSimpleIndexElement(revisionId, value,
                                   static_cast<uint32_t>(value.begin() - doc.begin()));
}

MMFilesSimpleIndexElement MMFilesEdgeIndex::buildToElement(TRI_voc_rid_t revisionId,
                                                           VPackSlice const& doc) const {
  TRI_ASSERT(doc.isObject());
