...
MMFilesPrimaryIndex::MMFilesPrimaryIndex(arangodb::LogicalCollection* collection)
  ...
  _primaryIndex =
      new MMFilesPrimaryIndexImpl(HashKey, HashElement, IsEqualKeyElement,
                                  IsEqualElementElement, IsEqualElementElement,
...
Result MMFilesPrimaryIndex::insertKey(transaction::Methods* trx, TRI_voc_rid_t revisionId,
...
Result MMFilesPrimaryIndex::insertKey(transaction::Methods* trx, TRI_voc_rid_t revisionId,
                                      VPackSlice const& doc, ManagedDocumentResult& mmdr) {
  IndexLookupContext context(trx, _collection, &mmdr, 1);
  MMFilesSimpleIndexElement element(buildKeyElement(revisionId, doc));
...
Result MMFilesPrimaryIndex::removeKey(transaction::Methods* trx, TRI_voc_rid_t revisionId,
                                      VPackSlice const& doc) {
  ManagedDocumentResult result;
  IndexLookupContext context(trx, _collection, &result, 1);
...
void MMFilesPrimaryIndex::invokeOnAllElements(std::function<bool(DocumentIdentifierToken const&)> work) {
...
bool MMFilesPrimaryIndex::supportsFilterCondition(arangodb::aql::AstNode const* node,
...
MMFilesSimpleIndexElement MMFilesPrimaryIndex::buildKeyElement(TRI_voc_rid_t revisionId,
