...
class MMFilesIndexFillerTask : public basics::LocalTask {
 public:
  MMFilesIndexFillerTask(std::shared_ptr<basics::LocalTaskQueue> queue,
...
arangodb::Result MMFilesCollection::updateProperties(VPackSlice const& slice, bool doSync) {
  ...
  if (slice.hasKey("journalSize")) {
    _journalSize = Helper::getNumericValue<TRI_voc_size_t>(slice, "journalSize", _journalSize);
  } else {
    _journalSize = Helper::getNumericValue<TRI_voc_size_t>(slice, "maximalSize", _journalSize);
  }
...
arangodb::Result MMFilesCollection::persistProperties() {
  ...
  try {
    ...
    MMFilesCollectionMarker marker(TRI_DF_MARKER_VPACK_CHANGE_COLLECTION,
                                   ...
                                   _logicalCollection->cid(), infoBuilder.slice());
...
MMFilesCollection::MMFilesCollection(LogicalCollection* collection, VPackSlice const& info)
    : PhysicalCollection(collection, info),
      ...
      _journalSize(Helper::readNumericValue<TRI_voc_size_t>(
          ...
          Helper::readNumericValue<TRI_voc_size_t>(info, "journalSize", TRI_JOURNAL_DEFAULT_SIZE))),
      _isVolatile(
  ...
  if (!_logicalCollection->deleted() && !_logicalCollection->vocbase()->isDropped()) {
    ...
    if (_initialCount != static_cast<int64_t>(idxSize)) {
      ...
      engine->changeCollection(_logicalCollection->vocbase(),
                               _logicalCollection->cid(), _logicalCollection, doSync);
    }
  }
...
int MMFilesCollection::syncActiveJournal() {
  ...
  if (datafile->isPhysical()) {
    ...
    if (synced < written) {
      ...
      if (ok) {
        LOG_TOPIC(TRACE, Logger::COLLECTOR) << "msync succeeded " << (void*)synced
                                            << ", size " << (written - synced);
...
/// create a new one
int MMFilesCollection::reserveJournalSpace(TRI_voc_tick_t tick, TRI_voc_size_t size,
                                           char*& resultPosition,
...
MMFilesDatafile* MMFilesCollection::createDatafile(TRI_voc_fid_t fid, TRI_voc_size_t journalSize,
                                                   bool isCompactor) {
  TRI_ASSERT(fid > 0);
  ...
  if (!isCompactor && datafile->isPhysical()) {
    ...
    LOG_TOPIC(TRACE, arangodb::Logger::FIXME)
        << "renamed journal from '" << datafile->getName() << "' to '"
        << filename << "'";
  }
...
void MMFilesCollection::fillIndex(std::shared_ptr<arangodb::basics::LocalTaskQueue> queue,
...
void MMFilesCollection::prepareIndexes(VPackSlice indexesSlice) {
  ...
  if (!foundPrimary || (!foundEdge && _logicalCollection->type() == TRI_COL_TYPE_EDGE)) {
...
void MMFilesCollection::addIndexCoordinator(std::shared_ptr<arangodb::Index> idx) {
  addIndex(idx);
}

int MMFilesCollection::restoreIndex(transaction::Methods* trx, VPackSlice const& info,
                                    std::shared_ptr<arangodb::Index>& idx) {
    ...
    if (useDeadlockDetector) {
      try {
        if (!wasBlocked) {
          ...
          TRI_ASSERT(wasBlocked);
          iterations = 0;
          if (_logicalCollection->vocbase()->_deadlockDetector.detectDeadlock(_logicalCollection, true) ==
              TRI_ERROR_DEADLOCK) {
            // deadlock
...
void MMFilesCollection::truncate(transaction::Methods* trx, OperationOptions& options) {
  ...
  auto callback = [&](MMFilesSimpleIndexElement const& element) {
    ...
    if (vpack != nullptr) {
      ...
      newObjectForRemove(trx, oldDoc, TRI_RidToString(oldRevisionId), *builder.get());
...
Result MMFilesCollection::insert(transaction::Methods* trx, VPackSlice const slice,
  ...
  res = Result(TRI_ERROR_NO_ERROR);
  {
    ...
    bool const useDeadlockDetector =
        (lock && !trx->isSingleOperationTransaction() &&
         !trx->state()->hasHint(transaction::Hints::Hint::NO_DLD));
    try {
      arangodb::MMFilesCollectionWriteLocker collectionLocker(this, useDeadlockDetector, lock);
...
uint8_t const* MMFilesCollection::lookupRevisionVPackConditional(TRI_voc_rid_t revisionId,
                                                                 TRI_voc_tick_t maxTick,
...
MMFilesDocumentPosition MMFilesCollection::insertRevision(TRI_voc_rid_t revisionId,
...
void MMFilesCollection::updateRevision(TRI_voc_rid_t revisionId, uint8_t const* dataptr,
                                       TRI_voc_fid_t fid, bool isInWal) {
  TRI_ASSERT(revisionId != 0);
  TRI_ASSERT(dataptr != nullptr);
...
bool MMFilesCollection::updateRevisionConditional(TRI_voc_rid_t revisionId,
...
Result MMFilesCollection::insertPrimaryIndex(transaction::Methods* trx, TRI_voc_rid_t revisionId,
...
int MMFilesCollection::detectIndexes(transaction::Methods* trx) {
  ...
  engine->getCollectionInfo(_logicalCollection->vocbase(),
                            _logicalCollection->cid(), builder, true, UINT64_MAX);
...
Result MMFilesCollection::update(arangodb::transaction::Methods* trx, VPackSlice const newSlice,
                                 ManagedDocumentResult& result, OperationOptions& options,
                                 TRI_voc_tick_t& resultMarkerTick, bool lock,
                                 TRI_voc_rid_t& prevRev, ManagedDocumentResult& previous,
                                 TRI_voc_rid_t const& revisionId, VPackSlice const key) {
  bool const isEdgeCollection = (_logicalCollection->type() == TRI_COL_TYPE_EDGE);
...
Result MMFilesCollection::remove(arangodb::transaction::Methods* trx,
                                 VPackSlice const slice, ManagedDocumentResult& previous,
...
///        it at that moment.
void MMFilesCollection::deferDropCollection(std::function<bool(LogicalCollection*)> callback) {
...
Result MMFilesCollection::removeFastPath(arangodb::transaction::Methods* trx,
  ...
  MMFilesCrudMarker removeMarker(
      TRI_DF_MARKER_VPACK_REMOVE,
      static_cast<MMFilesTransactionState*>(trx->state())->idForMarker(), toRemove);
...
/// the caller must make sure the write lock on the collection is held
Result MMFilesCollection::updateDocument(transaction::Methods* trx, TRI_voc_rid_t oldRevisionId,
                                         VPackSlice const& oldDoc, TRI_voc_rid_t newRevisionId,
                                         VPackSlice const& newDoc,
                                         MMFilesDocumentOperation& operation,
                                         MMFilesWalMarker const* marker, bool& waitForSync) {
