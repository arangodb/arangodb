...
void DatabasePathFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options->addOption("--database.required-directory-state",
                     "required state of database directory at startup "
                     "(non-existing: database directory must not exist, "
                     "existing: database directory must exist, "
                     "empty: database directory must exist but be empty, "
                     "populated: database directory must exist and contain "
                     "specific files already, "
                     "any: any state allowed)",
                     new DiscreteValuesParameter<StringParameter>(
                         &_requiredDirectoryState,
                         std::unordered_set<std::string>{"any", "non-existing",
                                                         "existing", "empty",
                                                         "populated"}));
}
...
void DatabasePathFeature::prepare() {
  ...
  if (_requiredDirectoryState == "non-existing") {
    if (basics::FileUtils::isDirectory(_directory)) {
      LOG_TOPIC(FATAL, arangodb::Logger::STARTUP)
          << "database directory '"
          << _directory << "' already exists, but option '--database.required-directory-state' was set to 'non-existing'";
      FATAL_ERROR_EXIT();
    }
    return;
  }
  ...
  if (!basics::FileUtils::isDirectory(_directory)) {
    LOG_TOPIC(FATAL, arangodb::Logger::STARTUP)
        << "database directory '"
        << _directory << "' does not exist, but option '--database.required-directory-state' was set to '"
        << _requiredDirectoryState << "'";
    FATAL_ERROR_EXIT();
  }
  ...
  if (_requiredDirectoryState == "empty" && !files.empty()) {
    LOG_TOPIC(FATAL, arangodb::Logger::STARTUP)
        << "database directory '"
        << _directory << "' is not empty, but option '--database.required-directory-state' was set to '"
        << _requiredDirectoryState << "'";
    FATAL_ERROR_EXIT();
  }
  ...
  if (_requiredDirectoryState == "populated" &&
    ...
    LOG_TOPIC(FATAL, arangodb::Logger::STARTUP)
        << "database directory '"
        << _directory << "' is not properly populated, but option '--database.required-directory-state' was set to '"
        << _requiredDirectoryState << "'";
    FATAL_ERROR_EXIT();
  }
