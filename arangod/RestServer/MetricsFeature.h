...
class MetricsFeature final : public application_features::ApplicationFeature {
 public:
  ...
  Histogram<Scale>& histogram(std::string const& name, Scale scale,
                              std::string const& help = std::string()) {
    auto metric = std::make_shared<Histogram<Scale>>(std::move(scale), name, help);
    bool success = false;
    {
      std::lock_guard<std::mutex> guard(_lock);
      success =
          _registry.try_emplace(name, std::dynamic_pointer_cast<Metric>(metric)).second;
    }
    if (!success) {
