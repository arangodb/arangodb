...
void UpgradeFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options
      ...
      .setLongDescription(std::string{
          "If you specify this option, then the server performs a database "
          "upgrade instead of starting normally.\n"
          "\n"
          "A database upgrade first compares the version number stored in the "
          "`VERSION` file in the database directory with the current server "
          "version.\n"
          "\n"
          "If the version number found in the database directory is higher "
          "than that of the server, the server considers this is an "
          "unintentional downgrade and warns about this. Using the server in "
          "these conditions is neither recommended nor supported.\n"
          "\n"
          "If the version number found in the database directory is lower than "
          "that of the server, the server checks whether there are any upgrade "
          "tasks to perform. It then executes all required upgrade tasks and "
          "prints the status. If one of the upgrade tasks fails, the server "
          "exits with an error. Re-starting the server with the upgrade option "
          "again triggers the upgrade check and execution until the problem is "
          "fixed.\n"
          "\n"
          "Whether or not you specify this option, the server always perform a "
          "version check on startup. If you running the server with a "
          "non-matching version number in the `VERSION` file, the server "
          "refuses to start."});
