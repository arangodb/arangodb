...
CheckVersionFeature::CheckVersionResult CheckVersionFeature::checkVersionFileForDB(
    TRI_vocbase_t* vocbase, StorageEngine* engine, uint32_t currentVersion) const {
  std::string const versionFilePath = engine->versionFilename(vocbase->id());
  if (!FileUtils::exists(versionFilePath) || !FileUtils::isRegularFile(versionFilePath)) {
    // File not existing
  ...
  LOG_TOPIC(DEBUG, arangodb::Logger::FIXME) << "found version file " << versionFileData;
  std::shared_ptr<VPackBuilder> versionBuilder;
  try {
    versionBuilder = VPackParser::fromJson(versionFileData);
  } catch (...) {
    LOG_TOPIC(ERR, arangodb::Logger::FIXME) << "Cannot parse VERSION file '" << versionFilePath
                                            << "': '" << versionFileData << "'";
  ...
  if (lastVersion / 100 == currentVersion / 100) {
    LOG_TOPIC(DEBUG, arangodb::Logger::FIXME)
        << "version match: last version" << lastVersion << ", current version"
        << currentVersion;
  ...
  LOG_TOPIC(DEBUG, arangodb::Logger::FIXME) << "upgrade: last version" << lastVersion
