  ...
  for (auto& name : databaseFeature->getDatabaseNames()) {
    ...
    if (res.status < 0) {
      LOG_TOPIC(FATAL, arangodb::Logger::FIXME)
          << "Database version check failed for '"
          << vocbase->name() << "'. Please inspect the logs for any errors. If there are no obvious issues in the logs, please retry with option `--log.level startup=trace`";
