...
void CacheFeatureOptionsProvider::declareOptionsImpl(
  ...
  opts->addOption("--cache.size",
                  ...
                  new UInt64Parameter(&options.cacheSize),
...
If there is more, the default is `(system RAM size - 2 GiB) * 0.25`.)");
  ...
  opts->addOption(
          ...
          new UInt64Parameter(&options.rebalancingInterval, /*base*/ 1,
...
used less often and reallocates it to caches which get more activity.)");
  ...
  opts->addOption(
          ...
          new DoubleParameter(&options.idealLowerFillRatio, /*base*/ 1.0,
...
value will be shrunk by the cache rebalancer.)")
  ...
  opts->addOption(
          ...
          new DoubleParameter(&options.idealUpperFillRatio, /*base*/ 1.0,
                              /*minValue*/ 0.0, /*maxValue*/ 1.0),
          arangodb::options::makeFlags(
              arangodb::options::Flags::DefaultNoComponents,
              arangodb::options::Flags::OnDBServer,
...
value will be inflated in size by the cache rebalancer.)")
  ...
  opts->addOption("--cache.min-value-size-for-edge-compression",
                  "The size threshold (in bytes) from which on payloads in the "
                  "edge index cache transparently get LZ4-compressed.",
                  new SizeTParameter(&options.minValueSizeForEdgeCompression, 1,
                                     0, 1073741824ULL),
                  arangodb::options::makeFlags(
                      arangodb::options::Flags::DefaultNoComponents,
                      arangodb::options::Flags::OnDBServer,
...
It is normally not that useful to compress values that are smaller than 100 bytes.)")
  ...
  opts->addOption(
          ...
          "edge cache entries.",
          new UInt32Parameter(&options.accelerationFactorForEdgeCompression, 1,
                              1, 65537),
          arangodb::options::makeFlags(
...
speed.)")
  ...
  opts->addOption(
          ...
          new DoubleParameter(&options.highwaterMultiplier, 1.0, 0.1, 1.0),
          arangodb::options::makeFlags(
              arangodb::options::Flags::Uncommon,
              arangodb::options::Flags::DefaultNoComponents,
              arangodb::options::Flags::OnDBServer,
...
try to free up memory by evicting the oldest entries.)")
  ...
  opts->addOption(
          ...
          new UInt64Parameter(&options.maxSpareAllocation),
          arangodb::options::makeFlags(
              arangodb::options::Flags::Uncommon,
              arangodb::options::Flags::DefaultNoComponents,
              arangodb::options::Flags::OnDBServer,
  ...
  opts->addOption("--cache.max-cache-value-size",
                  "The maximum payload size of an individual cache value "
                  "(excluding the size of the key).",
                  new UInt64Parameter(&options.maxCacheValueSize),
                  arangodb::options::makeFlags(
                      arangodb::options::Flags::Uncommon,
                      arangodb::options::Flags::DefaultNoComponents,
                      arangodb::options::Flags::OnDBServer,
...
void CacheFeatureOptionsProvider::validateOptionsImpl(
  ...
  if (options.idealLowerFillRatio >= options.idealUpperFillRatio) {
