...
std::vector<std::string_view> IndexFactory::supportedIndexes() const {
  // Note: hash, skiplist, fulltext, geo1, geo2 are deprecated and no longer
  // listed here as they cannot be created anymore. Existing indexes of these
  // types are still supported for upgrades.
  std::vector<std::string_view> enabledFeatures{
      "primary",   "edge", "ttl", "persistent", "geo", "mdi", "mdi-prefixed",
