...
bool Index::Compare(VPackSlice const& lhs, VPackSlice const& rhs) {
  ...
  if (type == IndexType::TRI_IDX_TYPE_GEO1_INDEX || type == IndexType::TRI_IDX_TYPE_GEO_INDEX) {
    ...
    if (lhValue.isString() && rhValue.isString()) {
      if (arangodb::basics::VelocyPackHelper::compare(value, rhs.get("view"), false) != 0) {
        ...
        if (ls.empty() || ls.back() != '/' || ls.compare(rs.substr(0, ls.size())) != 0) {
          return false;
        }
      }
    } else {
