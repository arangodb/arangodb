...
std::string defaultIndexName(VPackSlice const& slice) {
  auto type =
      arangodb::Index::type(slice.get(arangodb::StaticStrings::IndexType).stringView());
...
Index::Index(IndexId iid, arangodb::LogicalCollection& collection, VPackSlice slice)
    : _iid(iid),
      _collection(collection),
...
void Index::validateFields(VPackSlice slice) {
  std::string_view type = slice.get(arangodb::StaticStrings::IndexType).stringView();
...
bool Index::implicitlyUnique() const {
  ...
  for (auto const& it : _fields) {
    ...
    // implicitly unique
    if (it.size() == 1 && it[0] == arangodb::basics::AttributeName(StaticStrings::KeyString, false)) {
      return true;
    }
  }
...
std::vector<std::vector<arangodb::basics::AttributeName>> Index::parseFields(VPackSlice fields,
                                                                             bool allowEmpty,
                                                                             bool allowExpansion) {
