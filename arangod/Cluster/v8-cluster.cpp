...
static void PrepareClusterCommRequest(v8::FunctionCallbackInfo<v8::Value> const& args,
  ...
  if (args.Length() > 6 && args[6]->IsObject()) {
    ...
    if (TRI_HasProperty(context, isolate, opt, SingleRequestKey)) {
      singleRequest = TRI_ObjectToBoolean(isolate, opt->Get(context, SingleRequestKey).FromMaybe(v8::Handle<v8::Value>()));
    }
...
static void Return_PrepareClusterCommResultForJS(v8::FunctionCallbackInfo<v8::Value> const& args,
  ...
  if (res.dropped) {
    ...
    r->Set(context, EndpointKey, TRI_V8_STD_STRING(isolate, res.endpoint)).FromMaybe(false);
    TRI_GET_GLOBAL_STRING(SingleRequestKey);
    r->Set(context, SingleRequestKey, v8::Boolean::New(isolate, res.single)).FromMaybe(false);
    TRI_GET_GLOBAL_STRING(ShardIDKey);
    r->Set(context, ShardIDKey, TRI_V8_STD_STRING(isolate, res.shardID)).FromMaybe(false);

    if (res.status == CL_COMM_SUBMITTED) {
      TRI_GET_GLOBAL_STRING(StatusKey);
      r->Set(context, StatusKey, TRI_V8_ASCII_STRING(isolate, "SUBMITTED")).FromMaybe(false);
      ...
      TRI_GET_GLOBAL_STRING(StatusKey);
      r->Set(context, StatusKey, TRI_V8_ASCII_STRING(isolate, "SENT")).FromMaybe(false);
      ...
      for (auto const& i : res.result->getHeaderFields()) {
        h->Set(context, TRI_V8_STD_STRING(isolate, i.first), TRI_V8_STD_STRING(isolate, i.second)).FromMaybe(false);
      }
      ...
      if (body.length() != 0) {
        r->Set(context, TRI_V8_ASCII_STRING(isolate, "body"), TRI_V8_STD_STRING(isolate, body)).FromMaybe(false);
      ...
      if (res.result && res.result->isComplete()) {
        ...
        details->Set(context,
                     ...
                     v8::Number::New(isolate, res.result->getHttpReturnCode())).FromMaybe(false);
        details->Set(context,
                     TRI_V8_ASCII_STRING(isolate, "message"),
                     TRI_V8_STD_STRING(isolate, res.result->getHttpReturnMessage())).FromMaybe(false);
        arangodb::basics::StringBuffer& body = res.result->getBody();
        details->Set(context,
                     TRI_V8_ASCII_STRING(isolate, "body"),
                     TRI_V8_STD_STRING(isolate, body)).FromMaybe(false);
        V8Buffer* buffer = V8Buffer::New(isolate, body.c_str(), body.length());
        v8::Local<v8::Object> bufferObject =
            v8::Local<v8::Object>::New(isolate, buffer->_handle);
        details->Set(context, TRI_V8_ASCII_STRING(isolate, "rawBody"), bufferObject).FromMaybe(false);

        r->Set(context, TRI_V8_ASCII_STRING(isolate, "details"), details).FromMaybe(false);
        TRI_GET_GLOBAL_STRING(ErrorMessageKey);
        r->Set(context, ErrorMessageKey, TRI_V8_STD_STRING(isolate, res.errorMessage)).FromMaybe(false);
      } else {
        TRI_GET_GLOBAL_STRING(ErrorMessageKey);
        r->Set(context, ErrorMessageKey, TRI_V8_STD_STRING(isolate, res.errorMessage)).FromMaybe(false);
      }
      ...
      TRI_GET_GLOBAL_STRING(StatusKey);
      r->Set(context, StatusKey, TRI_V8_ASCII_STRING(isolate, "DROPPED")).FromMaybe(false);
      TRI_GET_GLOBAL_STRING(ErrorMessageKey);
      r->Set(context,
             ErrorMessageKey,
             TRI_V8_ASCII_STRING(isolate,
                                 "request dropped whilst waiting for answer")).FromMaybe(false);
      ...
      if (!body.empty()) {
        r->Set(context, TRI_V8_ASCII_STRING(isolate, "body"), TRI_V8_ASCII_PAIR_STRING(isolate, body.data(), body.size())).FromMaybe(false);
