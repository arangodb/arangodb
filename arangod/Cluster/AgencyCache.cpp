...
DECLARE_GAUGE(arangodb_agency_cache_callback_count, uint64_t, "Current number of entries in agency cache callbacks table");
...
AgencyCache::AgencyCache(application_features::ApplicationServer& server,
    ...
    _lastSnapshot(0),
    _callbacksCount(
      _server.getFeature<MetricsFeature>().add(arangodb_agency_cache_callback_count{})) {
