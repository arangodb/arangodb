...
void SynchronizeShard::setLeaderInfo(
...
static arangodb::Result replicationSynchronizeCatchup(
  ...
  try {
    std::string const context =
        "catching up delta changes for shard " + database + "/" + collection;
    r = syncer->syncCollectionCatchup(job.leaderInfo(), collection, timeout,
                                      tickReached, didTimeout, context.c_str());
  } catch (arangodb::basics::Exception const& ex) {
...
static arangodb::Result replicationSynchronizeFinalize(
    SynchronizeShard const& job,
    application_features::ApplicationServer& server, VPackSlice const& conf,
  ...
  try {
    std::string const context =
        "finalizing shard " + database + "/" + collection;
    r = syncer->syncCollectionFinalize(job.leaderInfo(), collection,
                                       context.c_str());
  } catch (arangodb::basics::Exception const& ex) {
    r.reset(ex.code(), ex.what());
  } catch (std::exception const& ex) {
...
bool SynchronizeShard::first() {
  ...
  auto& clusterInfo =
      feature().server().getFeature<ClusterFeature>().clusterInfo();
  ...
  try {
    ...
    // old "shortcut" code for getting in sync. This code takes a shortcut if
    // the shard in question is supposed to be empty. in this case it will
    // simply try to add itself as an in-sync follower, without running the full
    // replication protocol. this shortcut relies on the collection counts being
    // correct, and as these can be at least temporarily off, we need to disable
    // it.
#if 0
    if (docCount == 0) {
      // We have a short cut:
    ...
    try {
      ...
      ResultT<TRI_voc_tick_t> tickResult = catchupWithReadLock(
          ep, *collection, clientId, leader, lastTick, builder);
      if (!tickResult.ok()) {
  ...
  if (Logger::isEnabled(LogLevel::INFO, Logger::MAINTENANCE)) {
    ...
    std::stringstream msg;
    AppendShardInformationToMessage(database, shard, planId, startTime, msg);
    LOG_TOPIC("e6780", INFO, Logger::MAINTENANCE)
...
ResultT<TRI_voc_tick_t> SynchronizeShard::catchupWithReadLock(
  ...
  while (
      didTimeout &&
      tries++ <
          18) {  // This will try to sync for at most 1 hour. (200 * 18 == 3600)
    ...
    // recognizing it.
    res = replicationSynchronizeCatchup(*this, feature().server(),
                                        builder.slice(), timeout * 0.6,
                                        tickReached, didTimeout);
...
Result SynchronizeShard::catchupWithExclusiveLock(std::string const& ep,
  ...
  res = replicationSynchronizeFinalize(*this, feature().server(),
                                       builder.slice(), leaderIdWithTerm);
...
void SynchronizeShard::setState(ActionState state) {
  if ((COMPLETE == state || FAILED == state) && _state != state) {
    ...
    auto shardUnlocker =
        scopeGuard([this]() noexcept { _feature.unlockShard(getShard()); });
