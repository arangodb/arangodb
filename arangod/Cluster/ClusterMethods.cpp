...
::ErrorCode distributeBabyOnShards(CreateOperationCtx& opCtx, LogicalCollection& collinfo,
  ...
  if (!value.isObject()) {
    ...
    // Validate the smartJoinAttribute
    // We can only check and validate the smartJoinAttribute if a _key is present
    if (userSpecifiedKey) {
      // This means either, a user has specified the key.
      error = transaction::Methods::validateSmartJoinAttribute(collinfo, value);
    } else {
      // Or it has been auto-generated by us (_key).
      error = transaction::Methods::validateSmartJoinAttribute(collinfo, value,
                                                               VPackStringRef(_key));
    }
