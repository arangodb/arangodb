...
int ClusterInfo::createDatabaseCoordinator(std::string const& name, VPackSlice const& slice,
  ...
  if (!res.successful()) {
    ...
    errorMsg = std::string("Failed to create database with ") + res._clientId +
        ...
        if (result.isArray() && result.length() > 0) {
          if (result[0].isObject()) {
            ...
            if (tres.hasKey(std::vector<std::string>(
              ...
              for (const auto& s : VPackObjectIterator(tres.get(std::vector<std::string>(
...
int ClusterInfo::dropCollectionCoordinator(std::string const& databaseName,
  ...
  if (!clones.empty()) {
    errorMsg +=
        "Collection must not be dropped while it is sharding "
...
int ClusterInfo::ensureIndexCoordinator(std::string const& databaseName,
  ...
  int errorCode =
      ensureIndexCoordinatorWithoutRollback(databaseName, collectionID,
                                            idString, slice, create, compare,
  ...
  do {
    ...
    std::string const planIndexesKey =
        "Plan/Collections/" + databaseName + "/" + collectionID + "/indexes";
    std::vector<AgencyOperation> operations;
    std::vector<AgencyPrecondition> preconditions;
    if (planValue) {
      AgencyOperation planEraser(planIndexesKey, AgencyValueOperationType::ERASE,
                                 planValue->slice());
      TRI_ASSERT(oldPlanIndexes);
      AgencyPrecondition planPrecondition(planIndexesKey, AgencyPrecondition::Type::VALUE,
                                          oldPlanIndexes->slice());
...
int ClusterInfo::ensureIndexCoordinatorWithoutRollback(
...

  {
    ...
    if (indexes.isArray()) {
      ...
      for (auto const& other : VPackArrayIterator(indexes)) {
        if (arangodb::basics::VelocyPackHelper::compare(type, other.get("type"), false) != 0) {
  ...
  AgencyOperation newValue(key, AgencyValueOperationType::SET, newBuilder->slice());
  AgencyOperation incrementVersion("Plan/Version", AgencySimpleOperationType::INCREMENT_OP);
