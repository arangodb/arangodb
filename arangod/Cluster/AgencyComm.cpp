...
bool AgencyComm::send(arangodb::httpclient::GeneralClientConnection* connection,
  ...
  LOG(TRACE) << "sending " << arangodb::rest::GeneralRequest::translateMethod(method).c_str() << " request to agency at endpoint '" << connection->getEndpoint()->getSpecification().c_str() << "', url '" << url.c_str() << "': " << body.c_str();
