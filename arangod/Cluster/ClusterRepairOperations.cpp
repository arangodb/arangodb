...
BeginRepairsOperation::BeginRepairsOperation(
    const DatabaseID& database, const CollectionID& collectionId,
    const std::string& collectionName, const CollectionID& protoCollectionId,
    const std::string& protoCollectionName, size_t collectionReplicationFactor,
    size_t protoReplicationFactor, bool renameDistributeShardsLike)
    : database(database),
      collectionId(collectionId),
      collectionName(collectionName),
      protoCollectionId(protoCollectionId),
      protoCollectionName(protoCollectionName),
      collectionReplicationFactor(collectionReplicationFactor),
      protoReplicationFactor(protoReplicationFactor),
...
MoveShardOperation::MoveShardOperation(const DatabaseID& database,
                                       const CollectionID& collectionId,
                                       const std::string& collectionName,
                                       const ShardID& shard,
                                       const ServerID& from, const ServerID& to,
                                       bool isLeader)
    : database(database),
      collectionId(collectionId),
      collectionName(collectionName),
...
FixServerOrderOperation::FixServerOrderOperation(
    const DatabaseID& database, const CollectionID& collectionId,
    const std::string& collectionName, const CollectionID& protoCollectionId,
    const std::string& protoCollectionName, const ShardID& shard,
    const ShardID& protoShard, const ServerID& leader,
