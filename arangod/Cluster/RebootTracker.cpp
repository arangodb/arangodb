...
//   regularly.
//   In this case, we must move the bidirectional comparison from ClusterInfo.cpp
//   here!
...
bool RebootTracker::notifyChange(RebootTracker::PeerState const& peerState) {
  ...
  if (rv.second) {
    // Inserted a new element. There must be no existing entries for this server.
    TRI_ASSERT(_callbacks.find(peerState.serverId()) == _callbacks.end());
    success = true;
  } else {
    ...
    if (rv.first->second != peerState.rebootId()) {
      ...
      if (success) {
        // Update reboot ID if and only if we successfully fired the callbacks
        rv.first->second = peerState.rebootId();
      }
    } else {
      success = true;
    }
...
void RebootTracker::callMeOnChange(RebootTracker::PeerState const& peerState,
