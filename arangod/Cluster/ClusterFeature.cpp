...
void ClusterFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options->addHiddenOption("--cluster.index-create-timeout",
                           "amount of time (in seconds) the coordinator will "
                           "wait for an index to be created before giving up",
...
void ClusterFeature::start() {
  ...
  LOG_TOPIC(INFO, arangodb::Logger::CLUSTER)
      ...
      << "', internal address: " << _myAddress << ", role: " << role;
  ...
  if (!_disableHeartbeat) {
    ...
    _heartbeatThread =
        std::make_shared<HeartbeatThread>(_agencyCallbackRegistry.get(),
                                          std::chrono::microseconds(_heartbeatInterval * 1000),
                                          5);
  ...
  while (true) {
    ...
    result = comm.setValue("Current/ServersRegistered/" + myId, builder.slice(), 0.0);
