...
#include "ReplicationTimeoutOptionsProvider.h"
...
void ReplicationTimeoutOptionsProvider::declareOptions(
  ...
  options
      ->addOption("--cluster.synchronous-replication-timeout-minimum",
                  ...
                  new DoubleParameter(&opts.lowerLimit),
                  makeDefaultFlags(Flags::Uncommon, Flags::OnDBServer))
...
DB-Servers.)");
  ...
  options
      ->addOption("--cluster.synchronous-replication-timeout-maximum",
                  ...
                  "this value (in seconds).",
                  new DoubleParameter(&opts.upperLimit),
                  makeDefaultFlags(Flags::Uncommon, Flags::OnDBServer))
...
DB-Servers.)");
  ...
  options
      ->addOption(
          ...
          new DoubleParameter(&opts.timeoutFactor),
          makeDefaultFlags(Flags::Uncommon, Flags::OnDBServer))
...
the same value on all DB-Servers.)");
  ...
  options
      ->addOption("--cluster.synchronous-replication-timeout-per-4k",
                  ...
                  new DoubleParameter(&opts.timeoutPer4k),
                  makeDefaultFlags(Flags::Uncommon, Flags::OnDBServer))
      .setLongDescription(R"(**Warning**: If you use multiple DB-Servers, use
the same value on all DB-Servers.)");
  ...
  options
      ->addOption(
          ...
          "leader earlier.",
          new DoubleParameter(&opts.shardSynchronizationAttemptTimeout),
          makeDefaultFlags(Flags::Uncommon, Flags::OnDBServer))
...
void ReplicationTimeoutOptionsProvider::validateOptions(
  ...
  if (opts.upperLimit < opts.lowerLimit) {
    LOG_TOPIC("8a9f3", WARN, Logger::CONFIG)
        ...
        << "min";
    opts.upperLimit = opts.lowerLimit;
  }
}
