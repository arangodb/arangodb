...
struct Summarizing : public SimpleAlgorithm<SCCValue, int8_t, SenderMessage<uint64_t>> {
 public:
  explicit Summarizing(application_features::ApplicationServer& server,
               VPackSlice userParams)
      : SimpleAlgorithm<SCCValue, int8_t, SenderMessage<uint64_t>>(
            server, "summarizing", userParams) {}
