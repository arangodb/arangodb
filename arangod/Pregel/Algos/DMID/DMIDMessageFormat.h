...
struct DMIDMessageFormat : public MessageFormat<DMIDMessage> {
  ...
  void unwrapValue(VPackSlice s, DMIDMessage& message) const override {
    ...
    message.senderId.shard = PregelShard(static_cast<PregelShard::value_type>((*array).getUInt()));
    message.senderId.key = (*(++array)).copyString();
    message.leaderId.shard = PregelShard(static_cast<PregelShard::value_type>((*array).getUInt()));
