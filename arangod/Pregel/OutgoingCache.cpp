...
void OutgoingCache<V, E, M>::sendMessageTo(std::string const& toValue,
  ...
  int res =
      _ci->getResponsibleShard(_collInfo.get(), partial.slice(), true,
  ...
  if (std::find(localShards.begin(), localShards.end(), responsibleShard) !=
    ...
    LOG(INFO) << "Worker: Got messages for myself: " << data;
  } else {
...
void OutgoingCache<V, E, M>::sendMessages() {
  ...
  for (auto const& it : _map) {
    ...
    LOG(INFO) << "Worker: Sending messages to other DBServer "
              << package.toJson();
  }
  size_t nrDone = 0;
