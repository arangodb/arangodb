          ...
          if (eKeys.size() != 1 || eKeys[0] != shardKeyAttribute) {
            return std::make_pair(Result{TRI_ERROR_BAD_PARAMETER,
                                         ...
                                         "by shardKeyAttribute parameter ('" +
                                             shardKeyAttribute + "'), or use SmartGraphs. The current shardKey is: " +
...
void PregelFeature::scheduleGarbageCollection() {
  ...
  auto handle =
      scheduler->queueDelayed(RequestLane::INTERNAL_LOW, offset, [this](bool canceled) {
        if (!canceled) {
...
std::shared_ptr<Conductor> PregelFeature::conductor(uint64_t executionNumber) {
  ...
  return (it != _conductors.end() && ::authorized(it->second.user)) ? it->second.conductor
                                                                    : nullptr;
}
...
void PregelFeature::handleConductorRequest(TRI_vocbase_t& vocbase, std::string const& path,
  ...
  if (!co) {
    ...
    THROW_ARANGO_EXCEPTION_MESSAGE(
        ...
        "Conductor not found, invalid execution number: " + std::to_string(exeNum));
  }
...
uint64_t PregelFeature::numberOfActiveConductors() const {
  ...
  for (auto const& p : _conductors) {
    ...
    if (c->_state == ExecutionState::DEFAULT || c->_state == ExecutionState::RUNNING ||
