...
template <typename V, typename E> struct GraphFormat {
  ...
  virtual greenspun::EvalResult
  buildVertexDocumentWithResult(arangodb::velocypack::Builder &b,
...
public:
  NumberGraphFormat(std::string const &source, std::string const &result,
      ...
      : GraphFormat<V, E>(), _sourceField(source), _resultField(result),
