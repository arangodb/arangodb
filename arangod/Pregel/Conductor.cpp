...
void Conductor::startRecovery() {
  ...
  _boost_timer.reset(new asio_ns::deadline_timer(*ioService, boost::posix_time::seconds(2)));
  _boost_timer->async_wait([this](const boost::system::error_code& error) {
    _boost_timer.reset();

    if (error == asio_ns::error::operation_aborted || _state != ExecutionState::RECOVERING) {
    ...
    if (ServerState::instance()->getRole() == ServerState::ROLE_SINGLE) {
      ...
      if (!w) {
        PregelFeature::instance()->addWorker(
            AlgoRegistry::createWorker(_vocbaseGuard.database(), b.slice()), _executionNumber);
      } else {
