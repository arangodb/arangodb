...
class ChecksumResult : public Result {
 public:
  ...
  explicit ChecksumResult(VPackBuilder&& builder)
      : Result(TRI_ERROR_NO_ERROR), _builder(std::move(builder)) {}

  VPackBuilder builder() { return _builder; }

  VPackSlice slice() { return _builder.slice(); }
...
class LogicalCollection {
 ...
 public:
  ...
  double clusterIndexEstimatesTTL() { return _clusterEstimateTTL; }
  ...
  virtual void toVelocyPackForClusterInventory(velocypack::Builder&,
                                               bool useSystem, bool isReady) const;
  ...
  Result insert(transaction::Methods*, velocypack::Slice const,
                ManagedDocumentResult& result, OperationOptions&, TRI_voc_tick_t&, bool);
