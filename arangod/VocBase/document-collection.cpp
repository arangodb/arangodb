  ...
  while (!TRI_TRY_READ_LOCK_DOCUMENTS_INDEXES_PRIMARY_COLLECTION(this)) {
    try {
      if (!wasBlocked) {
        ...
        wasBlocked = true;
        LOG_TRACE("waiting for read-lock on collection '%s'",
                  _info.namec_str());
  ...
  while (!TRI_TRY_WRITE_LOCK_DOCUMENTS_INDEXES_PRIMARY_COLLECTION(this)) {
    try {
      if (!wasBlocked) {
        ...
        wasBlocked = true;
        LOG_TRACE("waiting for write-lock on collection '%s'",
                  _info.namec_str());
      } else if (++iterations >= 5) {
        // periodically check for deadlocks
        TRI_ASSERT(wasBlocked);
...
static int IterateMarkersCollection(arangodb::Transaction* trx,
  ...
  LOG_TRACE(
      ...
      (unsigned long long)openState._deletions, collection->_info.namec_str());
