...
void TRI_vocbase_t::registerCollection(bool doLock, arangodb::LogicalCollection* collection) {
  ...
  TRI_voc_cid_t cid = collection->cid();
  {
    ...
    if (!it.second) {
      ...
      msg.append(std::string("duplicate entry for collection name '") + name +
                 ...
                 std::to_string(_collectionsByName[name]->cid()));
    ...
    try {
      ...
      if (!it2.second) {
        ...
        msg.append(std::string("duplicate collection identifier ") +
                   std::to_string(collection->cid()) + " for name '" + name +
                   "'");
        LOG_TOPIC(ERR, arangodb::Logger::FIXME) << msg;
...
void TRI_vocbase_t::registerView(bool doLock, std::shared_ptr<arangodb::LogicalView> view) {
...
arangodb::LogicalCollection* TRI_vocbase_t::createCollectionWorker(VPackSlice parameters) {
  std::string name =
...
int TRI_vocbase_t::dropCollectionWorker(arangodb::LogicalCollection* collection,
  ...
  switch (collection->status()) {
    ...
    case TRI_VOC_COL_STATUS_UNLOADING: {
      ...
      arangodb::Result res =
          collection->updateProperties(builder.slice().get("parameters"), doSync);
...
void TRI_vocbase_t::inventory(VPackBuilder& result, TRI_voc_tick_t maxTick,
  ...
  if (collections.size() > 1) {
    ...
    std::sort(collections.begin(), collections.end(),
              [](LogicalCollection const* lhs, LogicalCollection const* rhs) {
  ...
  for (auto& collection : collections) {
    ...
    if (!ServerState::instance()->isRunningInCluster() && collection->cid() > maxTick) {
    ...
    if (collection->cid() <= maxTick) {
      ...
      collection->getIndexesVPack(result, false, false, [](arangodb::Index const* idx) {
        // we have to exclude the primary and the edge index here, because
        // otherwise at least the MMFiles engine will try to create it
      ...
      collection->toVelocyPackIgnore(result,
                                     {"objectId", "path", "statusString"}, true, false);
      result.close();
...
LogicalCollection* TRI_vocbase_t::lookupCollectionNoLock(std::string const& name) const {
  if (name.empty()) {
    return nullptr;
  }
...
arangodb::LogicalCollection* TRI_vocbase_t::createCollection(VPackSlice parameters) {
...
/// @brief locks a collection for usage by name
arangodb::LogicalCollection* TRI_vocbase_t::useCollectionByUuid(std::string const& uuid,
                                                                TRI_vocbase_col_status_e& status) {
  // check that we have an existing name
  arangodb::LogicalCollection* collection = nullptr;
...
std::shared_ptr<arangodb::LogicalView> TRI_vocbase_t::createViewWorker(VPackSlice parameters,
                                                                       TRI_voc_cid_t& id) {
  std::string name =
      arangodb::basics::VelocyPackHelper::getStringValue(parameters, "name", "");
  ...
  std::string type =
      arangodb::basics::VelocyPackHelper::getStringValue(parameters, "type", "");
...
/// using a cid of > 0 is supported to import dumps from other servers etc.
/// but the functionality is not advertised
std::shared_ptr<arangodb::LogicalView> TRI_vocbase_t::createView(VPackSlice parameters,
                                                                 TRI_voc_cid_t id) {
  READ_LOCKER(readLocker, _inventoryLock);
