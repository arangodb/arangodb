...
Result Collections::create(
  ...
  for (auto const& info : infos) {
    ...
    bool const isLocalCollection = (!ServerState::instance()->isCoordinator() &&
                                    Helper::stringUInt64(info.properties.get(
                                        StaticStrings::DataSourcePlanId)) == 0);
    ...
    if (!isLocalCollection) {
      ...
      if (replicationFactorSlice.isNone()) {
        ...
        // the combination if "isSmart" and replicationFactor "satellite" does
        // not make any sense. note: replicationFactor "satellite" can also be
        // expressed as replicationFactor 0.
        ...
        if (s.isBoolean() && s.getBoolean() &&
            ((replicationFactorSlice.isNumber() &&
             ...
             (replicationFactorSlice.isString() &&
              replicationFactorSlice.stringRef() ==
                  StaticStrings::Satellite))) {
          // check for the combination of "satellite" replication factor and
          // "isSmart"
          events::CreateCollection(vocbase.name(), info.name,
      ...
      if (!isSystemName) {
        ...
        if (vocbase.server().getFeature<ClusterFeature>().forceOneShard() ||
          ...
          auto const isSatellite =
              Helper::getStringRef(
                  info.properties, StaticStrings::ReplicationFactor,
