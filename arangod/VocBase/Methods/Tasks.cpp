...
void Task::shutdownTasks() {
  ...
  while (true) {
    ...
    if (++iterations % 10 == 0) {
      LOG_TOPIC("3966b", INFO, Logger::FIXME)
          << "waiting for " << size << " task(s) to complete";
...
std::function<void(bool cancelled)> Task::callbackFunction() {
  return [self = shared_from_this(), this](bool cancelled) {
    ...
    bool allowContinue = _execContext->canUseDatabase(dbname, auth::Level::RW);
  ...
  JavaScriptSecurityContext securityContext =
      _allowUseDatabase
          ? JavaScriptSecurityContext::
                createInternalContext()  // internal context that may access
                                         // internal data
