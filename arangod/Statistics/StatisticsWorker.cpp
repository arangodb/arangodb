...
void StatisticsWorker::generateRawStatistics(VPackBuilder& builder, double const& now) {
  ...
  builder.add("max", VPackValue(v8Counters.max));
  {
    ...
    for (auto memStatistic : memoryStatistics) {
      ...
      builder.add("countOfTimes", VPackValue(static_cast<int32_t>(memStatistic.countOfTimes)));
...
void StatisticsWorker::createCollection(std::string const& collection) const {
  ...
  r = methods::Collections::lookup(&_vocbase, collection, [&](std::shared_ptr<LogicalCollection> const& coll) -> void {
    TRI_ASSERT(coll);
    ...
    if (!idxRes.ok()) {
      LOG_TOPIC(WARN, Logger::STATISTICS)
          << "could not create the skiplist index for statistics collection '"
