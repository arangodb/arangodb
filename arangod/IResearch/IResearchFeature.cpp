...
void registerIndexFactory() {
  ...
  for (auto& entry : factories) {
    ...
    res = indexFactory.emplaceNormalizer(indexType, arangodb::iresearch::IResearchLinkHelper::normalize);

    if (!res.ok()) {
      THROW_ARANGO_EXCEPTION_MESSAGE(
          res.errorNumber(),
          std::string("failure registering IResearch link normalizer with "
...
bool recreateArangoSearchDataForDatabase(TRI_vocbase_t& vocbase) {
  ...
  for (auto& view : vocbase.views()) {
    ...
    builder.openObject();
    res = view->appendVelocyPack(builder, true, false);  // get JSON with end-user definition
    builder.close();
