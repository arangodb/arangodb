...
bool equalAnalyzer(arangodb::iresearch::IResearchAnalyzerFeature::AnalyzerPool const& pool,
  ...
  return type == pool.type()  // same type
         &&
         0 == arangodb::basics::VelocyPackHelper::compare(arangodb::iresearch::slice(normalizedProperties),
...
std::shared_ptr<arangodb::LogicalCollection> getAnalyzerCollection(  // get collection
  ...
  try {
    ...
    LOG_TOPIC("00003", WARN, arangodb::iresearch::TOPIC)
        << "caught exception while looking up Analyzer collection '" << ANALYZER_COLLECTION_NAME
        << "' in vocbase '" << vocbase.name() << "': " << e.what();
    IR_LOG_EXCEPTION();
  } catch (...) {
...
////////////////////////////////////////////////////////////////////////////////
bool setupAnalyzersCollection(TRI_vocbase_t& vocbase,
                              arangodb::velocypack::Slice const& /*upgradeParams*/) {
  return arangodb::methods::Collections::createSystem(vocbase, ANALYZER_COLLECTION_NAME)
      .ok();
}
...
////////////////////////////////////////////////////////////////////////////////
bool dropLegacyAnalyzersCollection(TRI_vocbase_t& vocbase,
                                   arangodb::velocypack::Slice const& /*upgradeParams*/) {
  ...
  auto const lookupRes = arangodb::methods::Collections::lookup(
      vocbase, LEGACY_ANALYZER_COLLECTION_NAME,
  ...
  static const auto resultVisitor =
    ...
    if (!slice.isArray()) {
      return arangodb::Result(  // result
          ...
          std::string("failed to parse contents of collection '") + ANALYZER_COLLECTION_NAME +
    ...
    auto visitor = [this, &analyzers, &vocbase](
      ...
      if (!slice.isObject()) {
        LOG_TOPIC("5c7a5", ERR, arangodb::iresearch::TOPIC)
            ...
            << ANALYZER_COLLECTION_NAME << "' in database '" << vocbase->name()
      ...
      if (slice.hasKey("properties")) {
        ...
        if (subSlice.isArray() || subSlice.isObject()) {
          ...
          LOG_TOPIC("a297e", ERR, arangodb::iresearch::TOPIC)
              ...
              << ANALYZER_COLLECTION_NAME << "' in database '"
