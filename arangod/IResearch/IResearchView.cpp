...
size_t directoryMemory(irs::directory const& directory, TRI_voc_cid_t viewId) noexcept {
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught error while calculating size of arangosearch view '"
        << viewId << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
...
irs::utf8_path getPersistedPath(arangodb::DatabasePathFeature const& dbPathFeature,
                                TRI_vocbase_t const& vocbase, TRI_voc_cid_t id) {
...
/// @return success
////////////////////////////////////////////////////////////////////////////////
arangodb::Result persistProperties(arangodb::LogicalView const& view,
  ...
  if (!engine) {
    return arangodb::Result(TRI_ERROR_INTERNAL,
                            std::string(
                                "failure to get storage engine while "
                                "persisting definition for LogicalView '") +
                                view.name() + "'");
  }
  ...
  if (!engine->inRecovery()) {
    ...
    try {
      ...
      return arangodb::Result(e.code(),
                              std::string(
                                  "caught exception during persistance of "
                                  "properties for arangosearch View '") +
                                  view.name() + "': " + e.what());
    } catch (std::exception const& e) {
      IR_LOG_EXCEPTION();
  ...
  if (!feature) {
    return arangodb::Result(TRI_ERROR_INTERNAL,
                            std::string(
                                "failure to get 'Database' feature while "
                                "persisting definition for LogicalView '") +
                                view.name() + "'");
  }

  return feature->registerPostRecoveryCallback(
      [&view, asyncSelf]() -> arangodb::
                               Result {
                                 auto* engine = arangodb::EngineSelectorFeature::ENGINE;

                                 if (!engine) {
                                   return arangodb::Result(TRI_ERROR_INTERNAL, std::string("failure to get storage engine while persisting definition for LogicalView"));
                                 }

                                 if (!asyncSelf) {
                                   return arangodb::Result(TRI_ERROR_INTERNAL, std::string("invalid view instance passed while persisting definition for LogicalView"));
                                 }

                                 SCOPED_LOCK(asyncSelf->mutex());

                                 if (!asyncSelf->get()) {
                                   LOG_TOPIC(INFO, arangodb::iresearch::TOPIC) << "no view instance available while persisting definition for LogicalView";
                                 ...
                                 try {
                                   ...
                                   return arangodb::Result(e.code(), std::string("caught exception during persistance of properties for arangosearch View '") +
                                                                         view.name() +
                                                                         "': " + e.what());
                                 } catch (std::exception const& e) {
                                   IR_LOG_EXCEPTION();

                                   return arangodb::Result(TRI_ERROR_INTERNAL, std::string("caught exception during persistance of properties for arangosearch View '") +
                                                                                   view.name() +
                                                                                   "': " + e.what());
                                 } catch (...) {
                                   IR_LOG_EXCEPTION();
...
////////////////////////////////////////////////////////////////////////////////
bool consolidateCleanupStore(irs::directory& directory, irs::index_writer& writer,
  ...
  if (policy.policy()) {
    LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
        << "registering consolidation policy '" << policy.properties().toString()
        << "' for store '" << storeName << "' with arangosearch view '"
        << view.name() << "' run id '" << size_t(&runId) << "'";

    try {
      writer.consolidate(policy.policy(), nullptr, progress);
    } catch (arangodb::basics::Exception const& e) {
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception during registration of consolidation policy '"
          << policy.properties().toString() << "' for store '" << storeName
          << "' with arangosearch view '" << view.name() << "': " << e.code()
          << " " << e.what();
      IR_LOG_EXCEPTION();
      ...
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception during registration of consolidation policy '"
          << policy.properties().toString() << "' for store '" << storeName
          << "' with arangosearch view '" << view.name() << "': " << e.what();
      IR_LOG_EXCEPTION();
      ...
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception during registration of consolidation policy '"
          << policy.properties().toString() << "' for store '" << storeName
          << "' with arangosearch view '" << view.name() << "'";
      IR_LOG_EXCEPTION();
    ...
    LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
        << "finished registering consolidation policy '"
        << policy.properties().toString() << "' for store '" << storeName
        << "' with arangosearch view '" << view.name() << "' run id '"
        << size_t(&runId) << "'";
  }
  ...
  LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
      << "starting '" << storeName << "' store cleanup for arangosearch view '"
      << view.name() << "' run id '" << size_t(&runId) << "'";
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during cleanup of '" << storeName << "' store of arangosearch view '"
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during cleanup of '" << storeName
        << "' store of arangosearch view '" << view.name() << "': " << e.what();
    IR_LOG_EXCEPTION();
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during cleanup of '" << storeName
        << "' of arangosearch view '" << view.name() << "'";
    IR_LOG_EXCEPTION();
  ...
  LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
      << "finished '" << storeName << "' store cleanup for arangosearch view '"
      << view.name() << "' run id '" << size_t(&runId) << "'";
...
IResearchView::DataStore& IResearchView::DataStore::operator=(IResearchView::DataStore&& other) noexcept {
  if (this != &other) {
...
struct IResearchView::ViewFactory : public arangodb::ViewFactory {
  virtual arangodb::Result create(arangodb::LogicalView::ptr& view, TRI_vocbase_t& vocbase,
    ...
    if (!impl) {
      return arangodb::Result(
          TRI_ERROR_NO_ERROR == TRI_errno() ? TRI_ERROR_INTERNAL : TRI_errno(), std::string("failure during instantiation while creating arangosearch View in database '") +
                                                                                    vocbase
                                                                                        .name() +
                                                                                    "'");
    }
  ...
  virtual arangodb::Result instantiate(arangodb::LogicalView::ptr& view,
    ...
    if (!feature) {
      return arangodb::Result(
          TRI_ERROR_INTERNAL,
          std::string("failure to find feature 'DatabasePath' while "
                      "constructing arangosearch View in database '") +
              vocbase.name() + "'");
    }
    ...
    if (!meta->init(definition, error) || meta->_version > LATEST_VERSION ||
...
////////////////////////////////////////////////////////////////////////////////
struct IResearchView::ViewStateRead final : public arangodb::TransactionState::Cookie {
  irs::index_reader::ptr _snapshot;
  std::lock_guard<ReadMutex> _viewLock;  // prevent data-store deallocation (lock @ AsyncSelf)

  explicit ViewStateRead(ReadMutex& mutex) noexcept : _viewLock(mutex) {}
};

////////////////////////////////////////////////////////////////////////////////
/// @brief container storing the view 'write' state for a given TransactionState
////////////////////////////////////////////////////////////////////////////////
class IResearchView::ViewStateWrite final : public arangodb::TransactionState::Cookie {
 public:
  std::lock_guard<ReadMutex> _viewLock;  // prevent data-store deallocation (lock @ AsyncSelf)
  irs::index_writer::documents_context _ctx;
  PrimaryKeyFilterContainer _removals;  // list of document removals

  explicit ViewStateWrite(ReadMutex& viewMutex, irs::index_writer& writer) noexcept
      : _viewLock(viewMutex), _ctx(writer.documents()) {}
  ...
  virtual ~ViewStateWrite() noexcept {
    ...
    try {
      ...
      _ctx.remove(irs::filter::make<PrimaryKeyFilterContainer>(std::move(_removals)));
    } catch (std::exception const& e) {
...
class IResearchView::ViewStateHelper {
 public:
  static IResearchView::ViewStateRead* read(arangodb::TransactionState& state,
                                            IResearchView const& view) {
    static_assert(sizeof(IResearchView) > Reader,
                  "'Reader' offset >= sizeof(IResearchView)");
...
// TODO FIXME find a better way to look up a ViewState
  ...
  static bool read(arangodb::TransactionState& state, IResearchView const& view,
  ...
  static IResearchView::ViewStateWrite* write(arangodb::TransactionState& state,
                                              IResearchView const& view) {
    static_assert(sizeof(IResearchView) > Writer,
                  "'Writer' offset >= sizeof(IResearchView)");
...
// TODO FIXME find a better way to look up a ViewState
  ...
  static bool write(arangodb::TransactionState& state, IResearchView const& view,
  ...
  static void commitWrite(arangodb::TransactionState& state,
                          IResearchView const& view, bool rollback) noexcept {
    static_assert(sizeof(IResearchView) > Writer,
                  "'Writer' offset >= sizeof(IResearchView)");
    auto* key = &view + Writer;
...
IResearchView::IResearchView(TRI_vocbase_t& vocbase, arangodb::velocypack::Slice const& info,
  ...
  if (feature) {
    ...
    feature->registerPostRecoveryCallback([view]() -> arangodb::Result {
      ...
      if (viewPtr->_storePersisted) {
        LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
            << "starting persisted-sync sync for arangosearch view '"
            << viewPtr->name() << "'";
        ...
        try {
          ...
          LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
              << "caught exception while committing persisted store for "
                 "arangosearch view '"
              << viewPtr->name() << "': " << e.code() << " " << e.what();
          IR_LOG_EXCEPTION();

          return arangodb::Result(
              e.code(),
              std::string(
                  "error committing persisted store for arangosearch view '") +
                  viewPtr->name() + "'");
        } catch (std::exception const& e) {
          LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
              << "caught exception while committing persisted store for "
        ...
        LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
            << "finished persisted-sync sync for arangosearch view '"
            << viewPtr->name() << "'";
      }
  ...
  if (_asyncFeature) {
    ...
    _asyncFeature->async(self(), [this, state](size_t& timeoutMsec, bool) mutable -> bool {
      ...
      if (_storePersisted &&
          consolidateCleanupStore(*_storePersisted._directory, *_storePersisted._writer,
                                  state._consolidationPolicy, state._progress, runCleanupAfterConsolidation,
  ...
  _trxReadCallback = [&viewRef](arangodb::transaction::Methods& trx,
  ...
  _trxWriteCallback = [&viewRef](arangodb::transaction::Methods& trx,
                                 arangodb::transaction::Status status) -> void {
    ...
    ViewStateHelper::commitWrite(*state, viewRef,
...

  {
    ...
    if (_storePersisted) {
      try {
        ...
        LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
            << "caught exception while closing persisted store for "
               "arangosearch view '"
            << name() << "'";
        IR_LOG_EXCEPTION();
      }
...
arangodb::Result IResearchView::appendVelocyPackDetailed(arangodb::velocypack::Builder& builder,
  ...
  try {
    ...
    if (!state) {
      return arangodb::Result(TRI_ERROR_INTERNAL,
                              std::string(
                                  "failed to get transaction state while "
                                  "generating json for arangosearch view '") +
                                  name() + "'");
    }
...
arangodb::Result IResearchView::drop(TRI_voc_cid_t cid, bool unlink /*= true*/
) {
  ...
  if (!unlink) {
    ...
    if (cid_itr != _metaState._collections.end()) {
      ...
      try {
        ...
        return arangodb::Result(TRI_ERROR_INTERNAL,
                                std::string(
                                    "caught error during persistance of "
                                    "properties for arangosearch View '") +
                                    name() + "' while dropping collection ' " +
                                    std::to_string(cid) + "'");
      }
    }
  ...
  try {
    ...
    return arangodb::Result(
        e.code(),
        std::string(
            "caught exception while removing from arangosearch view '") +
            name() + "', collection '" + std::to_string(cid) + "': " + e.what());
  } catch (std::exception const& e) {
    IR_LOG_EXCEPTION();
    ...
    return arangodb::Result(
        ...
        std::string(
            "caught exception while removing from arangosearch view '") +
            name() + "', collection '" + std::to_string(cid) + "'");
  }
...
arangodb::Result IResearchView::dropImpl() {
  ...
  collections.insert(_metaState._collections.begin(), _metaState._collections.end());
  ...
  for (auto& entry : collections) {
    ...
    if (!collection || !arangodb::iresearch::IResearchLink::find(*collection, *this)) {
      --collectionsCount;
    }
  }
  ...
  try {
    ...
    return arangodb::Result(
        e.code(),
        std::string("caught exception while removing arangosearch view '") +
            name() + "': " + e.what());
  } catch (std::exception const& e) {
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception while removing arangosearch view '" << name()
  ...
  return arangodb::Result(TRI_ERROR_INTERNAL,
                          std::string("failed to remove arangosearch view '") +
                              name() + "'");
}
...
bool IResearchView::emplace(TRI_voc_cid_t cid) {
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during persisting of logical view while emplacing "
           "collection ' "
        << cid << "' into arangosearch View '" << name() << "': " << e.code()
        << " " << e.what();
    IR_LOG_EXCEPTION();
    throw;
  } catch (std::exception const& e) {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during persisting of logical view while emplacing "
           "collection ' "
        << cid << "' into arangosearch View '" << name() << "': " << e.what();
    IR_LOG_EXCEPTION();
    throw;
  } catch (...) {
    _metaState._collections.erase(cid);  // undo meta modification
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during persisting of logical view while emplacing "
           "collection ' "
        << cid << "' into arangosearch View '" << name() << "'";
    IR_LOG_EXCEPTION();
    throw;
  }
...
arangodb::Result IResearchView::commit() {
  ...
  try {
      ...
      if (!reader) {
        ...
        LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
            << "failed to update snapshot after commit, reuse the existing "
               "snapshot for arangosearch view '"
            << name() << "'";
        return {};
      }
    ...
    LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
        << "caught exception while committing store for arangosearch view '"
        << name() << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
  } catch (std::exception const& e) {
    LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
...
int IResearchView::insert(transaction::Methods& trx, TRI_voc_cid_t cid,
                          arangodb::LocalDocumentId const& documentId,
                          arangodb::velocypack::Slice const& doc,
                          IResearchLinkMeta const& meta) {
  TRI_ASSERT(_storePersisted);

  auto insertImpl = [this, cid, &meta, doc,
                     &documentId](irs::index_writer::documents_context& ctx) -> int {
  ...
  if (!ctx) {
    auto ptr = irs::memory::make_unique<ViewStateWrite>(_asyncSelf->mutex(),
                                                        *_storePersisted._writer);  // will aquire read-lock to prevent data-store deallocation
...
int IResearchView::insert(
    transaction::Methods& trx, TRI_voc_cid_t cid,
    std::vector<std::pair<arangodb::LocalDocumentId, arangodb::velocypack::Slice>> const& batch,
    IResearchLinkMeta const& meta) {
  TRI_ASSERT(_storePersisted);

  auto insertImpl = [this, cid, &batch,
                     &meta](irs::index_writer::documents_context& ctx) -> int {
    ...
    try {
      {
        for (FieldIterator body; begin != end; ++begin) {
          ...
          if (!doc) {
            LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
                << "failed inserting batch into arangosearch view '" << id()
                << "', collection '" << cid;
            return TRI_ERROR_INTERNAL;
          }
        }
      ...
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception while inserting batch into arangosearch view '" << id()
          << "', collection '" << cid << "': " << e.code() << " " << e.what();
      IR_LOG_EXCEPTION();
    } catch (std::exception const& e) {
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception while inserting batch into arangosearch view '"
          << id() << "', collection '" << cid << "': " << e.what();
      IR_LOG_EXCEPTION();
    } catch (...) {
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
          << "caught exception while inserting batch into arangosearch view '"
...
void IResearchView::open() {
    ...
    if (format) {
      if (!_storePersisted._path.mkdir()) {
        ...
        throw std::runtime_error(
            std::string(
                "failed to create data directory for arangosearch view '") +
            name() + "' at: '" + _storePersisted._path.utf8() + "' error: '" +
            std::to_string(errno) + "'");
      }
      ...
      if (_storePersisted._directory) {
        ...
        _storePersisted._writer =
            irs::index_writer::make(*(_storePersisted._directory), format,
  ...
  LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
      << "failed to open arangosearch view '" << name()
      << "' at: " << _storePersisted._path.utf8();

  throw std::runtime_error(std::string("failed to open arangosearch view '") +
                           name() + "' at: " + _storePersisted._path.utf8());
}

int IResearchView::remove(transaction::Methods& trx, TRI_voc_cid_t cid,
                          arangodb::LocalDocumentId const& documentId) {
  TRI_ASSERT(_storePersisted);
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        ...
        << "', tid '" << state.id() << "', collection '" << cid << "', revision '"
        << documentId.id() << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
  } catch (std::exception const& e) {
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception while removing from arangosearch view '" << id()
        << "', tid '" << state.id() << "', collection '" << cid
        << "', revision '" << documentId.id() << "': " << e.what();
    IR_LOG_EXCEPTION();
  } catch (...) {
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception while removing from arangosearch view '" << id()
...
irs::index_reader const* IResearchView::snapshot(transaction::Methods& trx,
  ...
  try {
    ...
    if (_storePersisted) {
      cookie->_snapshot = static_cast<irs::index_reader::ptr>(_storePersisted._reader);
    } else {
      cookie->_snapshot = irs::index_reader::ptr(irs::index_reader::ptr(),
...
IResearchView::AsyncSelf::ptr IResearchView::self() const { return _asyncSelf; }
    ...
    if (arangodb::ExecContext::CURRENT) {
      ...
      for (auto& entry : _metaState._collections) {
        ...
        if (collection &&
          ...
          return arangodb::Result(TRI_ERROR_FORBIDDEN,
                                  std::string(
                                      "while updating arangosearch definition, "
                                      "error: collection '") +
                                      collection->name() +
      ...
      if (!meta.init(slice, error, initialMeta)) {
        return arangodb::Result(
            ...
            error.empty()
                ...
                : (std::string("failed to update arangosearch view '") +
                   name() + "' from definition, error in attribute '" + error +
                   "': " + slice.toString()));
      }
...
arangodb::Result IResearchView::updateProperties(std::shared_ptr<AsyncMeta> const& meta) {
  if (!meta) {
    return arangodb::Result(TRI_ERROR_BAD_PARAMETER);
  }
...
void IResearchView::verifyKnownCollections() {
  ...
  for (auto cid : cids) {
    ...
    if (!collection) {
      ...
      LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
          << "collection '" << cid
          << "' no longer exists! removing from arangosearch view '" << id() << "'";
      drop(cid);
    } else {
      ...
      if (!link) {
        LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
            << "collection '" << cid
            << "' no longer linked! removing from arangosearch view '" << id() << "'";
        drop(cid);
      }
    }
  }
