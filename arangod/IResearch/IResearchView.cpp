...
size_t directoryMemory(irs::directory const& directory, TRI_voc_cid_t viewId) noexcept {
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
      << "caught error while calculating size of IResearch view '" << viewId << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
...
bool syncStore(
  ...
  if (policy.policy() && policy.segmentThreshold() > segmentCount.load()) {
    LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
      << "registering consolidation policy '" << size_t(policy.type()) << "for store '" << storeName << "' with IResearch view '" << view.name() << "' run id '" << size_t(&runId) << " segment threshold '" << policy.segmentThreshold() << "' segment count '" << segmentCount.load() << "'";
    ...
    try {
      ...
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during registration of consolidation policy '" << size_t(policy.type()) << "' for store '" << storeName << "' with IResearch view '" << view.name() << "': " << e.code() << " " << e.what();
      IR_LOG_EXCEPTION();
    } catch (std::exception const& e) {
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during registration of consolidation policy '" << size_t(policy.type()) << "' for store '" << storeName << "' with IResearch view '" << view.name() << "': " << e.what();
      IR_LOG_EXCEPTION();
    } catch (...) {
      LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception during registration of consolidation policy '" << size_t(policy.type()) << "' for store '" << storeName << "' with IResearch view '" << view.name() << "'";
      IR_LOG_EXCEPTION();
    }

    LOG_TOPIC(TRACE, arangodb::iresearch::TOPIC)
      << "finished registering consolidation policy '" << size_t(policy.type()) << "' for store '" << storeName << "' with IResearch view '" << view.name() << "' run id '" << size_t(&runId) << "'";
  }
    ...
    feature->registerPostRecoveryCallback([view]()->arangodb::Result {
      ...
      if (viewPtr->_storePersisted) {
        ...
        try {
          ...
          return arangodb::Result(
            e.code(),
            std::string("error committing persisted store for IResearch view '") + viewPtr->name() + "'"
          );
        } catch (std::exception const& e) {
          LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
            << "caught exception while committing persisted store for iResearch view '" << viewPtr->id()
...

  {
    ...
    if (_storePersisted) {
      try {
        ...
        LOG_TOPIC(ERR, arangodb::iresearch::TOPIC)
          << "caught exception while closing persisted store for IResearch view '" << name() << "'";
        IR_LOG_EXCEPTION();
      }
    }
  }
...
arangodb::Result IResearchView::drop(
  ...
  try {
    ...
    return arangodb::Result(
      e.code(),
      std::string("caught exception while removing from IResearch view '") + name() + "', collection '" + std::to_string(cid) + "': " + e.what()
    );
  } catch (std::exception const& e) {
    IR_LOG_EXCEPTION();
    ...
    return arangodb::Result(
      TRI_ERROR_INTERNAL,
      std::string("caught exception while removing from IResearch view '") + name() + "', collection '" + std::to_string(cid) + "'"
    );
  }
...
arangodb::Result IResearchView::dropImpl() {
  ...
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
      << "caught exception while removing IResearch view '" << name() << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();
