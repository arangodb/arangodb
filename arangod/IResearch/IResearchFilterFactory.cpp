...
Result nondeterministicArgs(char const* funcName) {
  return {TRI_ERROR_BAD_PARAMETER,
          "Unable to handle non-deterministic arguments for '"s.append(funcName).append("' function")};
}
...
bool setupGeoFilter(FieldMeta::Analyzer const& a, S2RegionTermIndexer::Options& opts) {
  if (!a._pool) {
    return false;
  }
...
Result evaluateArg(T& out, ScopedAqlValue& value, char const* funcName,
  ...
  static_assert(std::is_same<T, irs::string_ref>::value || std::is_same<T, int64_t>::value ||
                std::is_same<T, double_t>::value || std::is_same<T, bool>::value);
...
Result extractAnalyzerFromArg(arangodb::iresearch::FieldMeta::Analyzer& out,
                              char const* funcName, irs::boolean_filter const* filter,
                              aql::AstNode const& args, size_t i, QueryContext const& ctx) {
...
struct FilterContext {
  FilterContext(arangodb::iresearch::FieldMeta::Analyzer const& analyzer, irs::boost_t boost) noexcept
...
FORCE_INLINE void appendExpression(irs::boolean_filter& filter,
                                   aql::AstNode const& node, QueryContext const& ctx,
...
Result byTerm(irs::by_term* filter, std::string&& name, ScopedAqlValue const& value,
  ...
  switch (value.type()) {
    case arangodb::iresearch::SCOPED_VALUE_TYPE_NULL:
      if (filter) {
        ...
        *filter->mutable_field() = std::move(name);
        ...
        irs::assign(filter->mutable_options()->term, irs::null_token_stream::value_null());
      }
      return {};
    ...
    case arangodb::iresearch::SCOPED_VALUE_TYPE_STRING:
      if (filter) {
        ...
        *filter->mutable_field() = std::move(name);
        filter->boost(filterCtx.boost);
        irs::assign(filter->mutable_options()->term, irs::ref_cast<irs::byte_type>(strValue));
      }
      return {};
    default:
...
Result byTerm(irs::by_term* filter, aql::AstNode const& attribute,
  ...
  if (filter && !arangodb::iresearch::nameFromAttributeAccess(name, attribute, ctx)) {
    return {TRI_ERROR_BAD_PARAMETER, "Failed to generate field name from node "s.append(
...
class ByTermSubFilterFactory {
 public:
  ...
  static Result byValueSubFilter(irs::boolean_filter* filter,
                                 std::string fieldName, const ScopedAqlValue& value,
...
Result fromArrayComparison(irs::boolean_filter*& filter, QueryContext const& ctx,
  ...
  if (aql::NODE_TYPE_ARRAY == valueNode->type) {
    ...
    std::tie(buildRes, arrayExpansionNodeType) =
        buildBinaryArrayComparisonPreFilter(filter, node.type, qualifierNode, n);
    if (!buildRes.ok()) {
      return buildRes;
    }
    if (filter) {
    ...
    FilterContext const subFilterCtx{filterCtx.analyzer, irs::no_boost()};  // reset boost
  ...
  switch (value.type()) {
    case arangodb::iresearch::SCOPED_VALUE_TYPE_ARRAY: {
      ...
      for (size_t i = 0; i < n; ++i) {
        auto rv = SubFilterFactory::byValueSubFilter(filter, fieldName, value.at(i),
...
Result fromFuncAnalyzer(char const* funcName, irs::boolean_filter* filter,
  ...
  auto rv = evaluateArg<decltype(analyzerId), true>(analyzerId, analyzerIdValue, funcName,
...
Result fromFuncBoost(char const* funcName, irs::boolean_filter* filter,
  ...
  auto rv = evaluateArg<decltype(boostValue), true>(boostValue, tmpValue, funcName,
                                                    args, 1, filter != nullptr, ctx);
...
Result fromFuncMinMatch(char const* funcName, irs::boolean_filter* filter,
  ...
  auto rv = evaluateArg<decltype(minMatchCount), true>(minMatchCount, minMatchCountValue,
                                                       funcName, args, lastArg,
                                                       filter != nullptr, ctx);
...
class ArgsTraits<aql::AstNode> {
 public:
  ...
  static Result getMemberValue(aql::AstNode const& arg, size_t idx,
                               char const* funcName, ValueType& value, bool isFilter,
                               QueryContext const& ctx, bool& skippedEvaluation) {
...
class ArgsTraits<VPackSlice> {
 public:
  ...
  static Result getMemberValue(VPackSlice arg, size_t idx, char const* funcName,
                               ValueType& value, bool, QueryContext const&, bool&) {
    TRI_ASSERT(arg.isArray());
  ...
  template <typename T>
  static Result evaluateArg(T& out, ValueType& value, char const* funcName, VPackSlice args,
                            size_t i, bool isFilter, QueryContext const& ctx) {
    static_assert(std::is_same<T, irs::string_ref>::value || std::is_same<T, int64_t>::value ||
                  std::is_same<T, double_t>::value || std::is_same<T, bool>::value);
...
typedef std::function<Result(char const*, size_t const, char const*, irs::by_phrase*,
                             QueryContext const&, VPackSlice, size_t, irs::analysis::analyzer*)>
...
Result fromFuncPhraseTerm(char const* funcName, size_t funcArgumentPosition,
                          ...
                          QueryContext const& ctx, VPackSlice elem, size_t firstOffset,
...
// {<STARTS_WITH>: [ '[' ] <term> [ ']' ] }
Result fromFuncPhraseStartsWith(char const* funcName, size_t funcArgumentPosition,
                                char const* subFuncName, irs::by_phrase* filter,
                                QueryContext const& ctx, VPackSlice elem, size_t firstOffset,
                                irs::analysis::analyzer* /*analyzer*/ = nullptr) {
  irs::string_ref term;
...
// {<WILDCARD>: [ '[' ] <term> [ ']' ] }
Result fromFuncPhraseLike(char const* funcName, size_t const funcArgumentPosition,
                          char const* subFuncName, irs::by_phrase* filter,
                          QueryContext const& ctx, VPackSlice elem, size_t firstOffset,
                          irs::analysis::analyzer* /*analyzer*/ = nullptr) {
  irs::string_ref term;
  ...
  if (filter) {
    auto& wildcard =
        filter->mutable_options()->push_back<irs::by_wildcard_options>(firstOffset);
...
Result fromFuncPhraseLevenshteinMatch(char const* funcName, size_t const funcArgumentPosition,
                                      char const* subFuncName, irs::by_phrase* filter,
  ...
  if (filter) {
    ...
    if (0 != opts.max_terms) {
      ...
      struct top_term_visitor final : irs::filter_visitor {
        ...
        virtual void prepare(const irs::sub_reader& segment, const irs::term_reader& field,
      ...
      irs::visit(*ctx.index, filter->field(), irs::by_edit_distance::visitor(opts), collector);
...
Result fromFuncPhraseTerms(char const* funcName, size_t const funcArgumentPosition,
                           char const* subFuncName, irs::by_phrase* filter,
...
Result fromFuncPhraseInRange(char const* funcName, size_t const funcArgumentPosition,
                             char const* subFuncName, irs::by_phrase* filter,
                             QueryContext const& ctx, VPackSlice array, size_t firstOffset,
                             irs::analysis::analyzer* /*analyzer*/ = nullptr) {
  if (!array.isArray()) {
    return {TRI_ERROR_BAD_PARAMETER,
            "'"s.append(funcName)
...
Result processPhraseArgObjectType(char const* funcName, size_t const funcArgumentPosition,
                                  irs::by_phrase* filter, QueryContext const& ctx,
  ...
  if (ADB_LIKELY(itr.valid())) {
    ...
    if (FCallSystemConversionPhraseHandlers.cend() == entry) {
      return {TRI_ERROR_BAD_PARAMETER, "'"s.append(funcName)
...
Result processPhraseArgs(char const* funcName, irs::by_phrase* phrase,
                         ...
                         size_t offset, bool allowDefaultOffset, bool isInArray) {
  ...
  for (size_t idx = valueArgsBegin; idx < valueArgsEnd; ++idx) {
    ...
    if (valueArg.isArray()) {
      ...
      if (!expectingOffset || allowDefaultOffset) {
        ...
        if (!isInArray) {
          auto subRes = processPhraseArgs(funcName, phrase, ctx, filterCtx,
...
Result fromFuncPhrase(char const* funcName, irs::boolean_filter* filter,
    ...
    if (!analyzer) {
      return {TRI_ERROR_INTERNAL,
              "'"s.append("' AQL function: Unable to instantiate analyzer '")
                  .append(analyzerPool._pool->name())
                  .append("'")};
    }
  ...
  return processPhraseArgs(funcName, phrase, ctx, filterCtx, *valueArgs, valueArgsBegin,
                           valueArgsEnd, analyzer.get(), 0, false, false);
}
      ...
      if (arangodb::iresearch::SCOPED_VALUE_TYPE_STRING == tmpValue.type()) {  // this is analyzer
        ...
        return {TRI_ERROR_BAD_PARAMETER,
                "'"s.append(funcName)
                    ...
                    .append("' ('")
                    .append(ScopedAqlValue::typeString(arangodb::iresearch::SCOPED_VALUE_TYPE_DOUBLE)
                                .c_str())
                    .append("' or '")
                    .append(ScopedAqlValue::typeString(arangodb::iresearch::SCOPED_VALUE_TYPE_STRING)
                                .c_str())
                    .append("' expected)")};
      }
    }
...
Result fromFuncStartsWith(char const* funcName, irs::boolean_filter* filter,
  ...
  // 1st argument defines a field
  auto const* field =
      arangodb::iresearch::checkAttributeAccess(args.getMemberUnchecked(currentArgNum),
                                                *ctx.ref);
...
Result fromFuncLike(char const* funcName, irs::boolean_filter* filter,
  ...
  Result res =
      evaluateArg(pattern, patternValue, funcName, args, 1, filter != nullptr, ctx);
...
Result fromFuncLevenshteinMatch(char const* funcName, irs::boolean_filter* filter,
                                QueryContext const& ctx, FilterContext const& filterCtx,
                                aql::AstNode const& args) {
  TRI_ASSERT(funcName);
...
Result fromFuncGeoContainsIntersect(char const* funcName, irs::boolean_filter* filter,
                                    QueryContext const& ctx, FilterContext const& filterCtx,
                                    aql::AstNode const& args) {
  TRI_ASSERT(funcName);
    ...
    if (shapeValue.isObject()) {
      ...
      return {TRI_ERROR_BAD_PARAMETER,
              "'"s.append(funcName)
                  ...
                  .append("' ('")
                  .append(ScopedAqlValue::typeString(arangodb::iresearch::SCOPED_VALUE_TYPE_OBJECT)
                              .c_str())
                  .append("' or '")
                  .append(ScopedAqlValue::typeString(arangodb::iresearch::SCOPED_VALUE_TYPE_ARRAY)
                              .c_str())
                  .append("' expected)")};
    }
...
/*static*/ Result FilterFactory::filter(irs::boolean_filter* filter,
                                        QueryContext const& ctx, aql::AstNode const& node) {
  if (node.willUseV8()) {
