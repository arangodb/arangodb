...
IResearchLink::IResearchLink(TRI_idx_iid_t iid, arangodb::LogicalCollection& collection)
...
arangodb::Result IResearchLink::drop() {
  ...
  if (!_view) {
    return arangodb::Result(TRI_ERROR_ARANGO_COLLECTION_NOT_LOADED,  // IResearchView required
                            std::string("failed to drop collection '") +
                                _collection.name() +
                                "' from an unset arangosearch view");
  }

  auto res = _view->drop(_collection.id());

  if (!res.ok()) {
    return arangodb::Result(res.errorNumber(),
                            std::string("failed to drop collection '") +
                                _collection.name() +
                                "' from arangosearch view '" + _view->name() +
                                "': " + res.errorMessage());
  }
...
bool IResearchLink::init(arangodb::velocypack::Slice const& definition) {
  ...
  std::string viewId = idSlice.isString() ? idSlice.copyString()
                                          : std::to_string(idSlice.getUInt());
  ...
  if (!logicalView && arangodb::ServerState::instance()->isDBServer()) {
    ...
    if (wiew) {
      ...
      if (_collection.id() == _collection.planId() && _collection.isAStub()) {
        ...
        auto clusterCol =
            ci->getCollectionCurrent(vocbase.name(), std::to_string(_collection.id()));
  ...
  if (!viewSelf) {
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "error read-locking view: '" << viewId << "' for link '" << _id << "'";
  ...
  _dropCollectionInDestructor =
      view->emplace(_collection.id());  // track if this is the instance that called emplace
  _meta = std::move(meta);
  _view = std::move(view);
...
bool IResearchLink::json(arangodb::velocypack::Builder& builder) const {
  ...
  if (_view) {
    builder.add(StaticStrings::ViewIdField, arangodb::velocypack::Value(_view->guid()));
  } else if (!_defaultGuid.empty()) {  // _defaultGuid.empty() == no view name in source jSON
    builder.add(StaticStrings::ViewIdField, VPackValue(_defaultGuid));
  }
...
arangodb::Result IResearchLink::unload() {
  ...
  _dropCollectionInDestructor =
      false;  // valid link (since unload(..) called), should not be dropped
  _defaultGuid = _view->guid();  // remember view ID just in case (e.g. call to toVelocyPack(...) after unload())
  _view = nullptr;     // mark as unassociated
  _viewLock.unlock();  // release read-lock on the IResearch View
...
/*static*/ std::shared_ptr<IResearchLink> IResearchLink::find(LogicalCollection const& collection,
