  ...
  try {
    if (arangodb::ServerState::instance()->isCoordinator()) {
      return modifyLinks<IResearchViewCoordinator, IResearchLinkCoordinator>(
          modified, vocbase, LogicalView::cast<IResearchViewCoordinator>(view), links, stale);
    }
    ...
    if (dbServerView) {
      return modifyLinks<IResearchViewDBServer, IResearchLink>(modified, vocbase, *dbServerView,
                                                               links, stale);
    }

    return modifyLinks<IResearchView, IResearchLink>(modified, vocbase,
