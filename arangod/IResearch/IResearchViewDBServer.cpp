...
std::string generateName(TRI_voc_cid_t viewId, TRI_voc_cid_t collectionId) {
  return VIEW_NAME_PREFIX + std::to_string(collectionId) + "_" + std::to_string(viewId);
}
...
struct IResearchViewDBServer::ViewFactory : public arangodb::ViewFactory {
  virtual arangodb::Result create(arangodb::LogicalView::ptr& view, TRI_vocbase_t& vocbase,
    ...
    std::string error;
    auto resNum = ci->createViewCoordinator(vocbase.name(), std::to_string(impl->id()),
                                            builder.slice(), error);
  ...
  virtual arangodb::Result instantiate(arangodb::LogicalView::ptr& view,
    ...
    if (!getString(name, definition, arangodb::StaticStrings::DataSourceName,
                   seen, irs::string_ref::EMPTY) ||
        !seen) {
      return arangodb::Result(
          TRI_ERROR_BAD_PARAMETER,
          std::string("definition supplied without a 'name' while "
                      "instantiating arangosearch View in database '") +
              vocbase.name() + "'");
    }
    ...
    if (!irs::starts_with(name, VIEW_NAME_PREFIX)) {
      ...
      if (!ci) {
        return arangodb::Result(
            TRI_ERROR_INTERNAL,
            std::string("failure to find 'ClusterInfo' instance while "
                        "constructing arangosearch View in database '") +
                vocbase.name() + "'");
      }
...
arangodb::Result IResearchViewDBServer::drop(TRI_voc_cid_t cid) noexcept {
  try {
    ...
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception while dropping collection '" << cid << "' from arangosearch view'"
        << name() << "': " << e.code() << " " << e.what();
    IR_LOG_EXCEPTION();

    return arangodb::Result(e.code(),
                            std::string("error dropping collection '") + std::to_string(cid) +
                                "' from arangosearch view '" + name() + "'");
  } catch (std::exception const& e) {
    LOG_TOPIC(WARN, arangodb::iresearch::TOPIC)
        << "caught exception while dropping collection '" << cid
...
std::shared_ptr<arangodb::LogicalView> IResearchViewDBServer::ensure(TRI_voc_cid_t cid, bool create /*= true*/
) {
  WriteMutex mutex(_mutex);
  SCOPED_LOCK(mutex);  // 'collections_' can be asynchronously read
  auto itr = _collections.find(cid);
  ...
  builder.openObject();
  builder.add(arangodb::StaticStrings::DataSourceSystem,
              arangodb::velocypack::Value(true));  // required to for use of VIEW_NAME_PREFIX
...
irs::index_reader const* IResearchViewDBServer::snapshot(transaction::Methods& trx,
...
arangodb::Result IResearchViewDBServer::properties(arangodb::velocypack::Slice const& slice,
  ...
  if (partialUpdate) {
    ...
    if (!meta.init(props.slice(), error, *_meta)) {
      return arangodb::Result(TRI_ERROR_BAD_PARAMETER,
                              std::string(
                                  "failure parsing properties while updating "
                                  "arangosearch view in database '") +
                                  vocbase().name() + "'");
    }
...
bool IResearchViewDBServer::visitCollections(CollectionVisitor const& visitor) const {
  ReadMutex mutex(_mutex);
  SCOPED_LOCK(mutex);  // 'collections_' can be asynchronously modified
