...
irs::parametric_description readParametricDescription(std::pair<irs::bytes_ref, size_t> args) {
  auto const rawSize = args.second;
  const auto& data = args.first;

  if (data.size() >= LZ4_MAX_INPUT_SIZE || rawSize >= std::numeric_limits<int>::max()) {
    return {};
  }

  irs::bstring dst(rawSize, 0);
  const auto lz4_size =
      ::LZ4_decompress_safe(reinterpret_cast<char const*>(data.begin()),
                            reinterpret_cast<char*>(&dst[0]),
                            static_cast<int>(data.size()), static_cast<int>(rawSize));
...
irs::parametric_description const DESCRIPTIONS[] = {
    ...
    // distance 3
    readParametricDescription({{pdd30::PDD, pdd30::PDD_COMPRESSED_LEN}, pdd30::PDD_RAW_LEN}),

    readParametricDescription({{pdd31::PDD, pdd31::PDD_COMPRESSED_LEN}, pdd31::PDD_RAW_LEN}),

    // distance 4
    readParametricDescription({{pdd40::PDD, pdd40::PDD_COMPRESSED_LEN}, pdd40::PDD_RAW_LEN}),
};
...
const irs::parametric_description& getParametricDescription(irs::byte_type distance,
