...
SingleRowFetcher<passBlocksThrough>::execute(AqlCallStack& stack) {
  ...
  size_t endIndex = block->size();
  if (block->hasShadowRows()) {
    auto const& idxs = block->getShadowRowIndexes();
    TRI_ASSERT(!idxs.empty());
    endIndex = *idxs.begin();
  }
  ExecutorState exState = ExecutorState::DONE;
  if (state == ExecutionState::HASMORE) {
    exState = ExecutorState::HASMORE;
  }
  return {state, skipped, AqlItemBlockInputRange{exState, block, 0, endIndex}};
}
