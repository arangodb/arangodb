...
auto ExecutionEngine::execute(AqlCallStack const& stack, AqlCallList clientCall)
    -> std::tuple<ExecutionState, SkipResult, SharedAqlItemBlockPtr> {
  if (_query.killed()) {
    THROW_ARANGO_EXCEPTION(TRI_ERROR_QUERY_KILLED);
  }
  auto const res = _root->execute(stack, std::move(clientCall));
#ifdef ARANGODB_ENABLE_MAINTAINER_MODE
...
auto ExecutionEngine::executeForClient(AqlCallStack const& stack,  AqlCallList clientCall, std::string const& clientId)
    -> std::tuple<ExecutionState, SkipResult, SharedAqlItemBlockPtr> {
  if (_query.killed()) {
    THROW_ARANGO_EXCEPTION(TRI_ERROR_QUERY_KILLED);
  }
  ...
  auto const res = rootBlock->executeForClient(stack, std::move(clientCall), clientId);
#ifdef ARANGODB_ENABLE_MAINTAINER_MODE
  if (std::get<ExecutionState>(res) == ExecutionState::WAITING) {
    auto const skipped = std::get<SkipResult>(res);
    auto const& block = std::get<SharedAqlItemBlockPtr>(res);
...
std::pair<ExecutionState, SharedAqlItemBlockPtr> ExecutionEngine::getSome(size_t atMost) {
  ...
  AqlCallStack compatibilityStack{true};
  auto const [state, skipped, block] = _root->execute(compatibilityStack, AqlCallList{AqlCall::SimulateGetSome(atMost)});
...
std::pair<ExecutionState, size_t> ExecutionEngine::skipSome(size_t atMost) {
  ...
  // This will always continue with a fetch-all on underlying subqueries (if any)
  AqlCallStack compatibilityStack{true};
  auto const [state, skipped, block] = _root->execute(compatibilityStack, AqlCallList{AqlCall::SimulateSkipSome(atMost)});
