...
#define CRAP_LOG_SQ LOG_DEVEL_IF(false)
...
auto SubqueryExecutor<isModificationSubquery>::initializeSubquery(AqlItemBlockInputRange& input)
  ...
  if (!_input) {
    ...
    CRAP_LOG_SQ << uint64_t(this) << " nextDataRow: " << _state << " "
                 << _input.isInitialized();
    if (!_input) {
      CRAP_LOG_SQ << uint64_t(this) << "exit, no more input" << _state;
  ...
  if (!_infos.isConst() || _input.isFirstDataRowInBlock()) {
    CRAP_LOG_SQ << "Subquery: Initialize cursor";
    auto [state, result] = _subquery.initializeCursor(_input);
    if (state == ExecutionState::WAITING) {
      CRAP_LOG_SQ << "Waiting on initialize cursor";
      return {state, false};
    }
...
auto SubqueryExecutor<isModificationSubquery>::produceRows(AqlItemBlockInputRange& input,
  ...
  CRAP_LOG_SQ << uint64_t(this) << "produceRows " << output.getClientCall();
  ...
  while (!output.isFull()) {
    if (_subqueryInitialized) {
      ...
      if (_infos.isConst() && !_input.isFirstDataRowInBlock()) {
        // Simply write
        writeOutput(output);
        CRAP_LOG_SQ << uint64_t(this) << "wrote output is const " << _state
                     << " " << getUpstreamCall();
        continue;
      }
      ...
      // We get a result
      CRAP_LOG_SQ << uint64_t(this) << " we get subquery result";
      if (block != nullptr) {
      ...
      if (state == ExecutionState::DONE) {
        writeOutput(output);
        CRAP_LOG_SQ << uint64_t(this) << "wrote output subquery done "
...
auto SubqueryExecutor<true>::skipRowsRange<>(AqlItemBlockInputRange& inputRange, AqlCall& call)
  ...
  CRAP_LOG_SQ << uint64_t(this) << "skipRowsRange " << call;
  ...
  while (isFullCount || _skipped < call.getOffset()) {
    if (_subqueryInitialized) {
      ...
      if (state == ExecutionState::DONE) {
        ...
        _input = InputAqlItemRow(CreateInvalidInputRowHint{});
        _skipped += 1;
        CRAP_LOG_SQ << uint64_t(this) << "did skip one";
      }
