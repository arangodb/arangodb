...
bool attributesMatch(TRI_idx_iid_t& commonIndexId,
                     arangodb::aql::IndexNode const* indexNode, NodeWithAttrs& node) {
...
void traverseReadOnly(
    arangodb::aql::AstNode* node, arangodb::aql::AstNode* parentNode, size_t childNumber,
  ...
  auto preVisitor = [&state](arangodb::aql::AstNode const* node,
    ...
    switch (node->type) {
      case arangodb::aql::NODE_TYPE_ATTRIBUTE_ACCESS:
        if (!state.wasAccess) {
          state.nodeAttrs.attrs.emplace_back(NodeWithAttrs::AttributeAndField{
              std::vector<arangodb::basics::AttributeName>{
                  {std::string(node->getStringValue(), node->getStringLength()), false}},
              parentNode, childNumber, 0, nullptr});
...
void arangodb::aql::lateDocumentMaterializationRule(
    arangodb::aql::Optimizer* opt, std::unique_ptr<arangodb::aql::ExecutionPlan> plan,
  ...
  for (auto limitNode : nodes) {
    ...
    if (arangodb::aql::ExecutionNode::INDEX == loop->getType()) {
      ...
      while (current != loop) {
        ...
        if (sortNode != nullptr &&
            current->getType() != arangodb::aql::ExecutionNode::CALCULATION) {
          ...
          if (currentUsedVars.find(indexNode->outVariable()) != currentUsedVars.end()) {
            // this limit node affects only closest sort, if this sort is
            // invalid we need to check other limit node
            stopSearch = true;
          }
        }
        if (stopSearch) {
      ...
      if (sortNode && !nodesToChange.empty()) {
        ...
        for (auto& node : nodesToChange) {
          std::transform(node.attrs.cbegin(), node.attrs.cend(),
                         std::inserter(uniqueVariables, uniqueVariables.end()),
                         [&ast](auto const& attrAndField) {
                           return std::make_pair(attrAndField.indexField,
                                                 IndexNode::IndexVariable{
                                                     attrAndField.indexFieldNum,
        ...
        auto materializeDependency =
            ServerState::instance()->isCoordinator() ? sortNode : limitNode;
