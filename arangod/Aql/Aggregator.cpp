...
struct AggregatorUnique : public Aggregator {
  ...
  std::unordered_set<velocypack::Slice, basics::VelocyPackHelper::VPackHash,
...
std::unique_ptr<Aggregator> Aggregator::fromTypeString(
    velocypack::Options const* opts, std::string const& type) {
...
std::unique_ptr<Aggregator> Aggregator::fromVPack(
    velocypack::Options const* opts, arangodb::velocypack::Slice const& slice,
