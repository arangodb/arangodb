...
std::pair<ExecutionState, SharedAqlItemBlockPtr> ExecutionBlockImpl<Executor>::getSomeWithoutTrace(size_t atMost) {
  ...
  // output block.
  // Plus, the ConstrainedSortExecutor will report an expectedNumberOfRows according
  // to its heap size, thus resulting in a smaller allocated output block. However,
  // it won't report DONE after, because a LIMIT block with fullCount must continue
  // to count after the sorted output.
