...
RestStatus RestAqlHandler::useQuery(std::string const& operation, std::string const& idString) {
  ...
  // this shared_ptr will protect the ExecutionEngine we originally 
  // leased from the registry while we are executing. it may be populated
  // from our weak_ptr (_engine), later
  std::shared_ptr<ExecutionEngine> engine;
  if (_qId == 0) {
    // have not looked for the engine yet
    
    // the PUT verb
    TRI_ASSERT(this->state() == RestHandler::HandlerState::EXECUTE);
    
    // look up engine in query registry
    engine = findEngine(idString);
    if (engine == nullptr) {
      // error message already generated by findEngine() 
      return RestStatus::DONE;
    }
    // we now have successfully leased an ExecutionEngine

    // register id, so that we don't look up the engine in the registry
    // next time
    _qId = arangodb::basics::StringUtils::uint64(idString);
    
    std::shared_ptr<SharedQueryState> ss = engine->sharedState();

    // create a weak_ptr from the engine, so we can go to sleep safely
    // without blocking the destruction of the engine in the query
    // registry
    _engine = engine;
    ...
    if (!_queryRegistry->validateEngine(_qId)) {
  ...
  auto guard = scopeGuard([&]() {
    if (_shutdownErrorCode >= 0) {
      // return the engine to the registry
      _engine.reset();
      _queryRegistry->closeEngine(_qId);

      // delete the engine from the registry
      _queryRegistry->destroyEngine(_qId, _shutdownErrorCode); // will destroy the query in the end
      _qId = 0;
    }
  });

  if (engine->getQuery().queryOptions().profile >= PROFILE_LEVEL_TRACE_1) {
  ...
  try {
    return handleUseQuery(*engine, operation, querySlice);
...
void RestAqlHandler::shutdownExecute(bool isFinalized) noexcept {
  try {
    if (isFinalized) {
      ...
      if (_qId != 0) {
        _queryRegistry->returnEngine(_qId);
      }
    }
  } catch (arangodb::basics::Exception const& ex) {
...
std::shared_ptr<ExecutionEngine> RestAqlHandler::findEngine(std::string const& idString) {
  TRI_ASSERT(_qId == 0);
  ...
  while (++iterations < MaxIterations) {
    ...
    try {
      // note: openExecutionEngine can return a nullptr!
      engine = _queryRegistry->openExecutionEngine(qId);
  ...
  TRI_ASSERT(engine == nullptr || qId > 0);
...
// handle for useQuery
RestStatus RestAqlHandler::handleUseQuery(ExecutionEngine& engine,
  ...
  VPackOptions const* opts = &engine.getQuery().vpackOptions();
  ...
  auto const rootNodeType = engine.root()->getPlanNode()->getType();
  ...
  if (operation == StaticStrings::AqlRemoteExecute) {
    ...
    if (shardId.empty()) {
      std::tie(state, skipped, items) =
          engine.execute(executeCall.callStack());
      if (state == ExecutionState::WAITING) {
        return RestStatus::WAITING;
      }
    } else {
      std::tie(state, skipped, items) =
          engine.executeForClient(executeCall.callStack(), shardId);
      if (state == ExecutionState::WAITING) {
        return RestStatus::WAITING;
      }
    }
    ...
    result.toVelocyPack(answerBuilder, &engine.getQuery().vpackOptions());
    ...
    TRI_ASSERT(shardId.empty() != (rootNodeType == ExecutionNode::SCATTER ||
                                   rootNodeType == ExecutionNode::DISTRIBUTE));
    if (shardId.empty()) {
      std::tie(state, items) = engine.getSome(atMost);
      if (state == ExecutionState::WAITING) {
        return RestStatus::WAITING;
      }
    } else {
      auto block = dynamic_cast<BlocksWithClients*>(engine.root());
      if (block == nullptr) {
    ...
    if (items.get() == nullptr) {
      ...
      items->toVelocyPack(&engine.getQuery().vpackOptions(), answerBuilder);
    }
    ...
    size_t skipped;
    if (shardId.empty()) {
      auto tmpRes = engine.skipSome(atMost);
    ...
    if (VelocyPackHelper::getBooleanValue(querySlice, "done", true)) {
      auto tmpRes = engine.initializeCursor(nullptr, 0);
      if (tmpRes.first == ExecutionState::WAITING) {
        return RestStatus::WAITING;
      }
      res = tmpRes.second;
    } else {
      auto items = engine.itemBlockManager().requestAndInitBlock(
          querySlice.get("items"));
      auto tmpRes = engine.initializeCursor(std::move(items), pos);
      if (tmpRes.first == ExecutionState::WAITING) {
        return RestStatus::WAITING;
      }
      res = tmpRes.second;
    ...
    // reset the pointer to the default because we shutting down now 
    opts = &engine.getQuery().vpackOptions();
    ...
    stats.toVelocyPack(answerBuilder, engine.getQuery().queryOptions().fullCount);
    
    // return warnings if present
    engine.getQuery().warnings().toVelocyPack(answerBuilder);
