...
RestStatus RestAqlHandler::useQuery(std::string const& operation, std::string const& idString) {
  ...
  TRI_ASSERT((_engine == nullptr && _qId == 0) ||(_engine != nullptr && _qId != 0));
  ...
  try {
    return handleUseQuery(*_engine, operation, querySlice);
...
void RestAqlHandler::shutdownExecute(bool isFinalized) noexcept {
  try {
    if (isFinalized) {
      // this RestHandler has completed, we will fully return the
      // engine to the registry, so it can be opened by another
      // RestHandler for follow-up operations
      if (_engine) {
      ...
      if (qId != 0) {
        ...
        if (_shutdownErrorCode >= 0) {
          _queryRegistry->destroyEngine(qId, _shutdownErrorCode); // will destroy the query in the end
        }
      }
    } else {
      // we are not fully done. this RestHandler will likely 
      // go to sleep now, but continue with the same quer later.
      if (_qId != 0) {
        TRI_ASSERT(_engine != nullptr);
        _queryRegistry->returnExecutionEngine(_qId);
      }
    }
  } catch (arangodb::basics::Exception const& ex) {
