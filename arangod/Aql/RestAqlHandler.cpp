...
RequestLane RestAqlHandler::lane() const {
  if (ServerState::instance()->isCoordinator()) {
    ...
    TRI_ASSERT(PriorityRequestLane(RequestLane::CLUSTER_AQL_INTERNAL_COORDINATOR) == RequestPriority::MED);
  ...
  if (ServerState::instance()->isDBServer()) {
    ...
    if (suffixes.size() == 2 && suffixes[0] == "finish") {
      ...
      TRI_ASSERT(PriorityRequestLane(RequestLane::CONTINUATION) == RequestPriority::MED);
      return RequestLane::CONTINUATION;
    }
  }

  // everything else will run with low priority
  TRI_ASSERT(PriorityRequestLane(RequestLane::CLUSTER_AQL) == RequestPriority::LOW);
