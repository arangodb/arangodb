    ...
    if (member->type == NODE_TYPE_SPREAD) {
      ...
      if (result.isArray()) {
        ...
        for (size_t sub = 0; sub < subLength; ++sub) {
          AqlValue subResult = result.at(int64_t(sub), localMustDestroy, false);
...
AqlValue Expression::executeSimpleExpressionObject(AstNode const* node,
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (member->type == NODE_TYPE_SPREAD) {
      ...
      VPackSlice slice = materializer.slice(result, true);
      ...
      continue; // don't fall through to adding the values again
    }
    
    TRI_ASSERT(member->type != NODE_TYPE_SPREAD);
    ...
    if (arg->type == NODE_TYPE_COLLECTION) {
      ...
      if (a.isArray()) {
        ...
        for (size_t j = 0; j < sub; ++j) {
          bool localMustDestroy;
          params.parameters.emplace_back(a.at(int64_t(j), localMustDestroy, false));
