...
std::pair<ExecutorState, InputAqlItemRow> AqlItemBlockInputRange::peekDataRow(
  ...
  bool AqlItemBlockInputRange::isShadowRowAtIndex(std::size_t index)
      const noexcept {
    TRI_ASSERT(isIndexValid(index));
    return _block->isShadowRow(index);
  }

  arangodb::aql::ShadowAqlItemRow AqlItemBlockInputRange::peekShadowRow()
      const {
    if (hasShadowRow()) {
  ...
  auto AqlItemBlockInputRange::skip(
      std::size_t const toSkip) noexcept->std::size_t {
  ...
  [[nodiscard]] auto AqlItemBlockInputRange::finalState()
      const noexcept->MainQueryState {
    return _finalState;
  }
