...
size_t MatchBuilder::patternEdgeCollectionCount(AstNode const* edgeLabelMember) {
...
AstNode const* MatchBuilder::getPatternEdgeCollection(AstNode const* edgeLabelMember,
                                           size_t index) {
...
AstNode* MatchBuilder::buildPatternEdgeCollectionList(AstNode const* edgeLabelMember) {
...
std::tuple<CalculationNode*, FilterNode*> MatchBuilder::createPropertiesFilter(Variable const* variable,
                                          AstNode* properties,
    ...
    CalculationNode* calc = _plan.createNode<CalculationNode>(
        &_plan, _plan.nextId(), std::make_unique<Expression>(_ast, root), filterVar);
    FilterNode* filter = _plan.createNode<FilterNode>(&_plan, _plan.nextId(), filterVar);
    filter->addDependency(calc);
    return std::make_tuple(calc, filter);
  
}

std::tuple<ExecutionNode*, ExecutionNode*, Variable const*> MatchBuilder::createCollectionAccess(AstNode const* member, Variable const* fullDocumentVariable,
...
ExecutionNode* MatchBuilder::createPatternProjection(AstNode const* member, Variable const* fullDocumentVar,
          std::unordered_map<VariableId, Variable const*> const& subst) {
    ...
    if (projections->type != NODE_TYPE_NOP) {
      ...
      auto* calc = _plan.createNode<CalculationNode>(
          &_plan, _plan.nextId(), std::make_unique<Expression>(_ast, root), variable);
      return calc;
    } else {
...
std::tuple<ExecutionNode*, ExecutionNode*, Variable const*> MatchBuilder::createPatternEdgeEnumerateAccess(AstNode const* edge, Variable const* outputVariable,
          std::unordered_map<VariableId, Variable const*> const& subst) {
        ...
        IndexHint hint(_ast->query(), _ast->createNodeNop(),
                       IndexHint::FromCollectionOperation{});
        auto enumCollection = _plan.createNode<EnumerateCollectionNode>(
            &_plan, _plan.nextId(), collection, outputVariable, false, std::move(hint));
        auto additionalFilter =
...
std::tuple<CalculationNode*, FilterNode*> MatchBuilder::createVertexEdgeFilter(Variable const* leftVertex,
                                          Variable const* edge,
...
std::tuple<ExecutionNode*, ExecutionNode*, Variable const*> MatchBuilder::createTraversalForPattern(Variable const* startNodeVar,                        //
    ...
    auto* traversal =
        _plan.createNode<TraversalNode>(&_plan /* plan */,                         //
                                  _plan.nextId() /* id */,                       //
...
CalculationNode* MatchBuilder::constructPathObject(Variable const* outVariable,
