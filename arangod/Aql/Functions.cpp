...
AqlValue handleBitOperation(ExpressionContext* expressionContext, transaction::Methods* trx,
                            VPackFunctionParameters const& parameters,
                            char const* AFN,
...
AqlValue Functions::BitAnd(ExpressionContext* expressionContext, transaction::Methods* trx,
                           VPackFunctionParameters const& parameters) {
  return handleBitOperation(expressionContext, trx, parameters, "BIT_AND", [](uint64_t value1, uint64_t value2) {
      return value1 & value2;
  });
}

/// @brief function BIT_OR
AqlValue Functions::BitOr(ExpressionContext* expressionContext, transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
  return handleBitOperation(expressionContext, trx, parameters, "BIT_OR", [](uint64_t value1, uint64_t value2) {
      return value1 | value2;
  });
}

/// @brief function BIT_XOR
AqlValue Functions::BitXOr(ExpressionContext* expressionContext, transaction::Methods* trx,
                          VPackFunctionParameters const& parameters) {
  return handleBitOperation(expressionContext, trx, parameters, "BIT_XOR", [](uint64_t value1, uint64_t value2) {
      return value1 ^ value2;
  });
}
  ...
  if (value1.isNumber()) {
    ...
    if (result1.has_value()) {
      ...
      if (value2.isNumber()) {
        ...
        if (result2.has_value() && result2.value() <= static_cast<T2>(Functions::bitFunctionsMaxSupportedBits)) {
...
AqlValue Functions::BitNegate(ExpressionContext* expressionContext, transaction::Methods*,
                              VPackFunctionParameters const& parameters) {
...
AqlValue Functions::BitTest(ExpressionContext* expressionContext, transaction::Methods*,
                            VPackFunctionParameters const& parameters) {
  auto result = binaryBitFunctionParameters<uint64_t, uint64_t>(parameters);
  if (result.has_value()) {
...
AqlValue Functions::BitShiftLeft(ExpressionContext* expressionContext, transaction::Methods*,
                                 VPackFunctionParameters const& parameters) {
  auto result = binaryBitFunctionParameters<uint64_t, uint64_t>(parameters);
  if (result.has_value()) {
    ...
    if (value.isNumber()) {
      ...
      if (width.has_value() && width.value() <= static_cast<uint64_t>(Functions::bitFunctionsMaxSupportedBits)) {
        // mask the lower bits of the result with up to 32 active bits
        return AqlValue(AqlValueHintUInt((testee << shift) & ((uint64_t(1) << width.value()) - 1)));
      }
    }
  }
...
AqlValue Functions::BitShiftRight(ExpressionContext* expressionContext, transaction::Methods*,
                                  VPackFunctionParameters const& parameters) {
  auto result = binaryBitFunctionParameters<uint64_t, uint64_t>(parameters);
  if (result.has_value()) {
    auto [testee, shift] = result.value();
...
AqlValue Functions::BitPopcount(ExpressionContext* expressionContext, transaction::Methods* trx,
                                VPackFunctionParameters const& parameters) {
  AqlValue const& value = extractFunctionParameterValue(parameters, 0);
...
AqlValue Functions::BitConstruct(ExpressionContext* expressionContext, transaction::Methods* trx,
                                 VPackFunctionParameters const& parameters) {
...
AqlValue Functions::BitDeconstruct(ExpressionContext* expressionContext, transaction::Methods* trx,
                                   VPackFunctionParameters const& parameters) {
  AqlValue const& value = extractFunctionParameterValue(parameters, 0);
...
AqlValue Functions::BitToString(ExpressionContext* expressionContext, transaction::Methods*,
                                VPackFunctionParameters const& parameters) {
  auto result = binaryBitFunctionParameters<uint64_t, uint64_t>(parameters);
  if (result.has_value()) {
    auto [testee, index] = result.value();
...
AqlValue Functions::BitFromString(ExpressionContext* expressionContext, transaction::Methods*,
                                  VPackFunctionParameters const& parameters) {
