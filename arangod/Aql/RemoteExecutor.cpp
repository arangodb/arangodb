...
void ExecutionBlockImpl<RemoteExecutor>::traceGetSomeBeginInner(size_t atMost) {
...
void ExecutionBlockImpl<RemoteExecutor>::traceSkipSomeBeginInner(size_t atMost) {
  if (_profile >= PROFILE_LEVEL_BLOCKS) {
    if (_getSomeBegin <= 0.0) {
      _getSomeBegin = TRI_microtime();
    }
...
void ExecutionBlockImpl<RemoteExecutor>::traceGetSomeEndInner(AqlItemBlock const* result,
      ...
      if (_profile >= PROFILE_LEVEL_TRACE_2) {
        if (result == nullptr) {
          ...
          VPackBuilder builder;
          {
            ...
            result->toVelocyPack(_query.trx(), builder); // TODO TOBIAS
          }
...
void ExecutionBlockImpl<RemoteExecutor>::traceSkipSomeEndInner(size_t skipped, ExecutionState state) {
  if (_profile >= PROFILE_LEVEL_BLOCKS) {
    ExecutionStats::Node stats;
    stats.calls = 1;
    stats.items = skipped;
