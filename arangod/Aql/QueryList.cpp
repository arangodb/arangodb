...
void QueryList::remove(Query* query) {
  ...
  try {
    ...
    if (now - started >= _slowQueryThreshold) {
      ...
      if (profile != nullptr) {
        loadTime =
            profile->timers[QueryExecutionState::toNumber(QueryExecutionState::ValueType::LOADING_COLLECTIONS)];
      }
      ...
      if (loadTime >= 0.1) {
        LOG_TOPIC(WARN, Logger::QUERIES)
            << "slow query: '" << q << "'" << bindParameters
            << ", took: " << Logger::FIXED(now - started)
            << ", loading took: " << Logger::FIXED(loadTime);
      } else {
        LOG_TOPIC(WARN, Logger::QUERIES)
            << "slow query: '" << q << "'" << bindParameters
            << ", took: " << Logger::FIXED(now - started);
      ...
      _slow.emplace_back(
          QueryEntryCopy(query->id(), std::move(q),
                         _trackBindVars ? query->bindParameters() : nullptr, started,
                         now - started, QueryExecutionState::ValueType::FINISHED));
...
std::vector<QueryEntryCopy> QueryList::listCurrent() {
...

  {
    ...
    for (auto const& it : _current) {
      ...
      result.emplace_back(QueryEntryCopy(query->id(), extractQueryString(query, maxLength),
                                         _trackBindVars ? query->bindParameters() : nullptr,
                                         started, now - started, query->state()));
    }
  }
