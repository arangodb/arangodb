...
void arangodb::aql::removeCollectVariablesRule(
    ...
    OptimizerRule const& rule) {
  using short_alloc_t =
      ::arangodb::containers::SmallVector<ExecutionNode*>::allocator_type;
...
void arangodb::aql::moveCalculationsUpRule(Optimizer* opt,
  ...
  for (auto const& n : nodes) {
    ...
    if (n->getType() == EN::CALCULATION) {
      ...
      // could end up inserting a smaller amount of documents than what's
      // actually proposed in the query.
