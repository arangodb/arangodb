...
arangodb::futures::Future<Result> EngineInfoContainerDBServerServerBased::buildSetupRequest(
  ...
  auto buildCallback =
      [this, server = server, didCreateEngine = std::move(didCreateEngine),
  ...
  static_assert(std::is_nothrow_move_constructible_v<decltype(buildCallback)>);
  ...
  if (fastPathResult.fail()) {
    ...
    // we got a lock timeout response for the fast path locking...
    {
      ...
      for (auto& tryRes : requests) {
        network::Response response = std::move(tryRes).await_unwrap();
        if (response.fail()) {
    ...
    for (auto& [server, buffer, didCreateEngine] : engineInformation) {
      ...
      if (request.fail()) {
        ...
        // set the proper error reason
        cleanupReason = request.errorNumber();
        return request;
      }
    }
  }
