...
std::map<arangodb::velocypack::StringRef, GatherNode::SortMode> const
    NameToValue{{SortModeMinElement, GatherNode::SortMode::MinElement},
                {SortModeHeap, GatherNode::SortMode::Heap},
                {SortModeUnset, GatherNode::SortMode::Default}};

bool toSortMode(arangodb::velocypack::StringRef const& str,
                GatherNode::SortMode& mode) noexcept {
...
GatherNode::GatherNode(ExecutionPlan* plan,
  ...
  if (!_elements.empty()) {
    ...
    if (!toSortMode(
            VelocyPackHelper::getStringRef(sortModeSlice, VPackStringRef()),
            _sortmode)) {
