...
auto doTheThing(Ast* ast, EnumeratePathsNode* enumeratePathsNode,
                AstNode const* root, Variable const* pathVar) {
  auto appliedChange = false;
  ...
  auto andNode = root->getMemberUnchecked(0);

  if (andNode->type != NODE_TYPE_OPERATOR_NARY_AND) {
    LOG_ENUMERATE_PATHS_OPTIMIZER_RULE
        << fmt::format("expecting andNode to be of type n-ary and, found {}",
  ...
  for (auto i = size_t{0}; i < andNode->numMembers(); ++i) {
    ...
    auto condition = ast->createNodeBinaryOperator(NODE_TYPE_OPERATOR_BINARY_EQ,
                                                   map, rhsValue);
...
auto EnumeratePathsFilterMatcher::before(ExecutionNode* node) -> bool {
  ...
  switch (node->getType()) {
    ...
    case EN::ENUMERATE_PATHS: {
      ...
      auto const* pathVar = &pathsNode->pathOutVariable();
      //      auto const& varsValidInPathEnumeration =
      //      pathsNode->getVarsValid();

      _appliedChange =
          doTheThing(_plan->getAst(), pathsNode, _condition->root(), pathVar);
    } break;
    default: {
