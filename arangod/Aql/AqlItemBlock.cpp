...
void AqlItemBlock::initFromSlice(VPackSlice const slice) {
  ...
  TRI_ASSERT(_nrEffectiveItems == 0);
  ...
  try {
    ...
    // note that the end column is always _nrRegs + 1 here, as the first "real"
    // input column is 1. column 0 contains shadow row information in case the
    // serialization format is not CLASSIC.
    for (RegisterId column = startColumn; column < _nrRegs + 1; column++) {
...
SharedAqlItemBlockPtr AqlItemBlock::cloneDataAndMoveShadow() {
  ...
  auto const numEffectiveRows = _nrEffectiveItems;
...
auto AqlItemBlock::slice(arangodb::containers::SmallVector<std::pair<size_t, size_t>> const& ranges) const
  ...
  size_t targetRow = 0;
  for (auto const& [from, to] : ranges) {
    size_t const effectiveTo = std::min<size_t>(to, _nrEffectiveItems);
...
SharedAqlItemBlockPtr AqlItemBlock::slice(std::vector<size_t> const& chosen,
  ...
  for (size_t chosenIdx = from; chosenIdx < effectiveTo; ++chosenIdx, ++resultRowIdx) {
...
void AqlItemBlock::toVelocyPack(size_t from, size_t to,
  ...
  for (RegisterId column = startColumn; column < _nrRegs || column == subqueryDepthColumn; column++) {
    AqlValue a;
...
void AqlItemBlock::makeShadowRow(size_t row, size_t depth) {
  _nrEffectiveItems = std::max<size_t>(_nrEffectiveItems, row + 1);
...
size_t AqlItemBlock::moveOtherBlockHere(size_t const targetRow, AqlItemBlock& source) {
  ...
  auto const effectiveRows = source._nrEffectiveItems;
  ...
  for (size_t sourceRow = 0; sourceRow < effectiveRows; ++sourceRow, ++thisRow) {
    ...
    _nrEffectiveItems = std::max<size_t>(_nrEffectiveItems, thisRow + 1);
  }
  source.eraseAll();
