...
DECLARE_COUNTER(arangodb_transactions_expired, "Total number of expired transactions");
...
ManagerFeature::ManagerFeature(application_features::ApplicationServer& server)
    : ApplicationFeature(server, "TransactionManager"),
      ...
      _numExpiredTransactions(
        server.getFeature<arangodb::MetricsFeature>().add(arangodb_transactions_expired{})) {
  setOptional(false);
