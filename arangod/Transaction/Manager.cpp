...
void Manager::registerTransaction(TRI_voc_tid_t transactionId, bool isReadOnlyTransaction,
  ...
  TRI_ASSERT(!isFollowerTransaction || isFollowerTransactionId(transactionId) ||
  ...
  if (!isReadOnlyTransaction && !isFollowerTransaction) {
    LOG_TOPIC("ccdea", TRACE, Logger::TRANSACTIONS)
        << "Acquiring read lock for tid " << transactionId;
    _rwLock.lockRead();
  ...
  auto guard = scopeGuard([this, transactionId, &isReadOnlyTransaction,
    ...
    if (!isReadOnlyTransaction && !isFollowerTransaction) {
      ...
      LOG_TOPIC("ccded", TRACE, Logger::TRANSACTIONS)
          << "Released lock for tid " << transactionId
          << " nrReadLocked: " << _nrReadLocked.load(std::memory_order_relaxed);
    }
  });
...
Result Manager::ensureManagedTrx(TRI_vocbase_t& vocbase, TRI_voc_tid_t tid,
...
ResultT<TRI_voc_tid_t> Manager::createManagedTrx(
  ...
  TRI_voc_tid_t tid = ServerState::isSingleServer(role)
                          ? TRI_NewTickServer()
...
Result Manager::ensureManagedTrx(TRI_vocbase_t& vocbase, TRI_voc_tid_t tid,
  ...
  if (isFollowerTransactionId(tid)) {
    options.isFollowerTransaction = true;
  }
    ...
    if (AccessMode::isWriteOrExclusive(mode)) {
      ...
      THROW_ARANGO_EXCEPTION_MESSAGE(
          TRI_ERROR_LOCKED, std::string("cannot read-lock, transaction ") +
                                std::to_string(tid) + " is already in use");
    }
...
bool Manager::garbageCollect(bool abortAll) {
  bool didWork = false;
  ::arangodb::containers::SmallVector<TRI_voc_tid_t, 64>::allocator_type::arena_type a1;
  ::arangodb::containers::SmallVector<TRI_voc_tid_t, 64> toAbort{a1};
  ::arangodb::containers::SmallVector<TRI_voc_tid_t, 64>::allocator_type::arena_type a2;
  ::arangodb::containers::SmallVector<TRI_voc_tid_t, 64> toErase{a2};
...
bool Manager::transactionIdExists(TRI_voc_tid_t const& tid) const {
  size_t const bucket = getBucket(tid);
...
bool Manager::storeManagedState(TRI_voc_tid_t const& tid,
