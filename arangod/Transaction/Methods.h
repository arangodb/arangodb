...
class Methods {
 ...
 public:
  class IndexHandle {
   ...
   public:
    IndexHandle() = default;
    void toVelocyPack(arangodb::velocypack::Builder& builder, bool withFigures) const;
 ...
 public:
  ...
  void registerCallback(std::function<void(arangodb::transaction::Methods* trx)> const& onFinish) {
    _onFinish = onFinish;
  }
  ...
  ENTERPRISE_VIRT OperationResult any(std::string const&, uint64_t skip = 0,
                                      uint64_t limit = 1);
  ...
  ENTERPRISE_VIRT void invokeOnAllElements(
  ...
  OperationCursor* indexScanForCondition(IndexHandle const&, arangodb::aql::AstNode const*,
                                         arangodb::aql::Variable const*,
                                         ManagedDocumentResult*, uint64_t, uint64_t, bool);
  ...
  std::unique_ptr<OperationCursor> indexScan(std::string const& collectionName,
                                             CursorType cursorType, ManagedDocumentResult*,
  ...
  ENTERPRISE_VIRT std::vector<std::shared_ptr<arangodb::Index>> indexesForCollection(std::string const&);
 ...
 private:
  ...
  OperationResult anyLocal(std::string const& collectionName, uint64_t skip, uint64_t limit);
