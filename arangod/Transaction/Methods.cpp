  ...
  if (auto* context = dynamic_cast<transaction::ReplicatedContext*>(
    ...
    auto canReachWriteConcern = basics::catchToResultT([&]() {
      ...
      for (auto& [pid, _] : participantsConfig.participants) {
        ...
        // Server can be removed from Health (health is none), as permanently gone
        // In this case it has to be counted as failed participant
        if (health.isObject() && health.get("Status").copyString() != "FAILED") {
