...
Future<OperationResult> transaction::Methods::insertLocal(
  ...
  CollectionIndexes::ReadLocked indexes =
      collection->getPhysical()->readLockedIndexes();
...
Future<OperationResult> transaction::Methods::removeLocal(
  ...
  bool readPreviousDocument = indexes.size() != 1;
  ...
  auto workForOneDocument = [&](VPackSlice value, bool isArray) -> Result {
    ...
    if (readPreviousDocument) {
      ...
      previousDocumentBuilder->openObject(true);
      previousDocumentBuilder->add(StaticStrings::KeyString, VPackValue(key));
      previousDocumentBuilder->close();
    }

    TRI_ASSERT(previousDocumentBuilder->slice().isObject());

    res = removeLocalHelper(*collection, indexes, value, oldDocumentId,
...
Result transaction::Methods::removeLocalHelper(
    LogicalCollection& collection, CollectionIndexes::ReadLocked const& indexes,
  ...
  Result res = collection.getPhysical()->remove(
      *this, indexes, previousDocumentId, previousRevisionId, previousDocument,
      options);
