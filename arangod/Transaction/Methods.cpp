...
#ifdef USE_ENTERPRISE
    if (collection->isSmart() && collection->type() == TRI_COL_TYPE_EDGE && ServerState::instance()->isSingleServer()) {
      auto vsecol = dynamic_cast<arangodb::VirtualSmartEdgeCollection const*>(collection.get());
