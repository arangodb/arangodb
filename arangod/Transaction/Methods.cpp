    ...
    if (count > 0) {
      ...
      *body = payload->slice().toJson();
      ...
      if (cc != nullptr) {
        ...
        trxTime = TRI_microtime() - trxTime;
        if ((trxTime) >= 1.0) {
          LOG_TOPIC(ERR,::arangodb::Logger::FIXME) << "insert perfRequests after: " << trxTime << " seconds";
        }
    ...
    if (cc != nullptr) {
      ...
      if (count > 0) {
        ...
        *body = payload->slice().toJson();
        ...
        trxTime = TRI_microtime() - trxTime;
        if ((trxTime) >= 1.0) {
          LOG_TOPIC(ERR,::arangodb::Logger::FIXME) << "replace perfRequests after: " << trxTime << " seconds";
        }
        // If any would-be-follower refused to follow there must be a
        // new leader in the meantime, in this case we must not allow
        // this operation to succeed, we simply return with a refusal
