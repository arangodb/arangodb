...
void FoxxFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options
      ...
      .setLongDescription(std::string{
          "If set to `true`, the Foxx queues are available and jobs in the "
          "queues are executed asynchronously.\n"
          "\n"
          "If set to `false`, the queue manager is disabled and any jobs are "
          "prevented from being processed, which may reduce CPU load a bit."});
  ...
  options
      ->addOption("--foxx.queues-poll-interval",
                  ...
                  arangodb::options::makeFlags(
                      ...
                      arangodb::options::Flags::OnSingle))
      .setLongDescription(std::string{
          "Lower values lead to more immediate and more frequent Foxx queue "
          "job execution, but make the queue thread wake up and query the "
          "queues more often. If set to a low value, the queue thread might "
          "cause CPU load.\n"
          "\n"
          "If you don't use Foxx queues much, then you may increase this value "
          "to make the queues thread wake up less."});
  ...
  options
      ...
      .setLongDescription(std::string{
          "If set to `true`, all Foxx services in all databases are "
          "synchronized between multiple Coordinators during the startup "
          "sequence. This ensures that all Foxx services are up-to-date when a "
          "Coordinator reports itself as ready.\n"
          "\n"
          "In case the option is set to `false` (i.e. no waiting), the "
          "Coordinator completes the startup sequence faster, and the Foxx "
          "services are propagated lazily. Until the initialization procedure "
          "has completed for the local Foxx apps, any request to a Foxx app is "
          "responded to with an HTTP 500 error and a message "
          "`waiting for initialization of Foxx services in this database`."
          "This can cause an unavailability window for Foxx services on "
          "Coordinator startup for the initial requests to Foxx apps until the "
          "app propagation has completed.\n"
          "\n"
          "If you don't use Foxx, you should set this option to `false` to "
          "benefit from a faster Coordinator startup. Deployments relying on "
          "Foxx apps being available as soon as a Coordinator is integrated or "
          "responding should set this option to `true`.\n"
          "\n"
          "The option only has an effect for cluster setups. On single servers "
          "and in Active Failover mode, all Foxx apps are available from the "
          "very beginning.\n"
          "\n"
          "**Note**: ArangoDB 3.8 changes the default value to `false` for "
          "this option. In previous versions, this option had a default value "
          "of `true`."});
}
