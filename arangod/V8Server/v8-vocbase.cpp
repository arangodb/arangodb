...
static v8::Handle<v8::Value> DocumentVocbaseCol (bool useCollection,
  ...
  if (res != TRI_ERROR_NO_ERROR || document._dataptr == nullptr) {  // PROTECTED by trx here
    if (res == TRI_ERROR_NO_ERROR) {
...
static v8::Handle<v8::Value> ReplaceVocbaseCol (bool useCollection,
  ...
  if (ServerState::instance()->isDBserver()) {
    ...
    if (res != TRI_ERROR_NO_ERROR || mptr._dataptr == nullptr) {  // PROTECTED by trx here
      TRI_FreeJson(TRI_UNKNOWN_MEM_ZONE, json);
      TRI_FreeString(TRI_CORE_MEM_ZONE, key);
      TRI_V8_EXCEPTION(scope, res);
    }
    ...
    TRI_EXTRACT_SHAPED_JSON_MARKER(shaped, mptr._dataptr);  // PROTECTED by trx here
...
static v8::Handle<v8::Object> AddBasicDocumentAttributes (T& trx,
  ...
  TRI_df_marker_type_t type = ((TRI_df_marker_t*) document->_dataptr)->_type;  // PROTECTED by trx from above

  if (type == TRI_DOC_MARKER_KEY_EDGE) {
    TRI_doc_edge_key_marker_t const* marker = static_cast<TRI_doc_edge_key_marker_t const*>(document->_dataptr);  // PROTECTED by trx from above
    ...
    triagens::wal::edge_marker_t const* marker = static_cast<triagens::wal::edge_marker_t const*>(document->_dataptr);  // PROTECTED by trx from above
...
v8::Handle<v8::Value> TRI_WrapShapedJson (T& trx,
  ...
  TRI_ASSERT_MAINTAINER(document->_dataptr != nullptr);  // PROTECTED by trx from above
  ...
  if (doCopy) {
    ...
    TRI_shaped_json_t json;
    TRI_EXTRACT_SHAPED_JSON_MARKER(json, document->_dataptr);  // PROTECTED by trx from above
  ...
  void* data = const_cast<void*>(document->_dataptr);  // PROTECTED by trx from above
