...
void DepthFirstTraverser::setStartVertex (triagens::arango::traverser::VertexId const& v) {
  ...
  _enumerator.reset(new PathEnumerator<EdgeInfo, VertexId, TRI_doc_mptr_copy_t>(_edgeGetter, _getVertex, v));
  _done = false;
}
...
EdgeIndex* DepthFirstTraverser::EdgeGetter::getEdgeIndex(
  ...
  if (it == _indexCache.end()) {
    ...
    triagens::arango::EdgeIndex* edgeIndex = ecl->edgeIndex();
...
void DepthFirstTraverser::EdgeGetter::operator()(VertexId const& startVertex,
  ...
  while (true) {
    ...
    TRI_voc_cid_t cid;
    triagens::arango::EdgeIndex* edgeIndex = getEdgeIndex(eColName, cid);
