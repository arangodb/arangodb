...
std::unique_ptr<TailingSyncer> GlobalReplicationApplier::buildTailingSyncer(
    TRI_voc_tick_t initialTick, bool useTick, TRI_voc_tick_t barrierId) const {
  return std::make_unique<arangodb::GlobalTailingSyncer>(_configuration, initialTick,
                                                         useTick, barrierId);
}
...
std::string GlobalReplicationApplier::getStateFilename() const {
  ...
  return arangodb::basics::FileUtils::buildFilename(
      engine->databasePath(vocbase), "GLOBAL-REPLICATION-APPLIER-STATE");
}
