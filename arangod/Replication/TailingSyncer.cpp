...
Result TailingSyncer::applyLogMarker(VPackSlice const& slice, TRI_voc_tick_t firstRegularTick,
  ...
  else if (type == REPLICATION_DATABASE_CREATE || type == REPLICATION_DATABASE_DROP) {
  ...
  try {
    ...
    return Result(ex.code(),
                  std::string("continuous synchronization for database '") +
                      _databaseName + "' failed with exception: " + ex.what());
  } catch (std::exception const& ex) {
    ...
    return Result(TRI_ERROR_INTERNAL,
                  std::string("continuous synchronization for database '") +
                      _databaseName + "' failed with unknown exception");
  }
}
...
void TailingSyncer::getLocalState() {
  ...
  // a _masterInfo._serverId value of 0 may occur if no proper connection could
...
Result TailingSyncer::runContinuousSync() {
  ...
  while (true) {
    ...
    Result res = followMasterLog(fetchTick, fromTick, _configuration._ignoreErrors,
                                 worked, masterActive);
...
Result TailingSyncer::fetchOpenTransactions(TRI_voc_tick_t fromTick, TRI_voc_tick_t toTick,
  ...
  if (r.fail()) {
    return Result(
        TRI_ERROR_REPLICATION_INVALID_RESPONSE,
        std::string("got invalid response from master at ") + _masterInfo._endpoint +
...
Result TailingSyncer::followMasterLog(TRI_voc_tick_t& fetchTick,
  ...
  std::string const baseUrl = tailingBaseUrl("tail") +
  ...
  std::string const url =
      ...
      "&includeSystem=" + (_configuration._includeSystem ? "true" : "false");
  ...
  std::string const progress =
      ...
      ", barrier: " + StringUtils::itoa(_barrierId) +
      ", open transactions: " + std::to_string(_ongoingTransactions.size()) +
      ", chunk size " + std::to_string(_configuration._chunkSize);
  ...
  if (!found) {
    return Result(TRI_ERROR_REPLICATION_INVALID_RESPONSE,
                  std::string("got invalid response from master at ") +
                      ...
                      TRI_REPLICATION_HEADER_CHECKMORE + " is missing");
  }
