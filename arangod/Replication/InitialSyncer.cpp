...
int InitialSyncer::handleCollectionDump (string const& cid,
  ...
  while (true) {
    ...
    std::string const typeString = (col->_type == TRI_COL_TYPE_EDGE ? "edge" : "document");
...
int InitialSyncer::handleCollectionSync (std::string const& cid,
  ...
  if (countJson->_value._number <= 0.0) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      errorMsg = std::string("unable to truncate collection '") + collectionName + "': " + TRI_errno_string(res);
  ...
  try {
    res = handleSyncKeys(id, cid, col, collectionName, maxTick, errorMsg);
  }
  catch (triagens::basics::Exception const& ex) {
    res = ex.code();
  }
...
int InitialSyncer::handleSyncKeys (std::string const& keysId,
  ...
  bool const isEdge = (col->_type == TRI_COL_TYPE_EDGE);
   
  std::string progress = "collecting local keys for collection '" + collectionName + "'";
  setProgress(progress);
...
int InitialSyncer::handleCollection (TRI_json_t const* parameters,
  ...
  // -------------------------------------------------------------------------------------
    ...
    if (incremental && getSize(col) > 0) {
      res = handleCollectionSync(StringUtils::itoa(cid), col, masterName, _masterInfo._lastLogTick, errorMsg);
    }
    else {
      res = handleCollectionDump(StringUtils::itoa(cid), col, masterName, _masterInfo._lastLogTick, errorMsg);
    }
