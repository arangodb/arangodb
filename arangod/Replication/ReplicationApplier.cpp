...
struct FullApplierThread final : public ApplierThread {
  FullApplierThread(ReplicationApplier* applier, std::unique_ptr<InitialSyncer>&& syncer)
...
struct TailingApplierThread final : public ApplierThread {
  TailingApplierThread(ReplicationApplier* applier, std::unique_ptr<TailingSyncer>&& syncer)
      : ApplierThread(applier, syncer.get()) {
    syncer.release();
  }
