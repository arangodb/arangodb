...
RocksDBLogValue RocksDBLogValue::DocumentRemoveAsPartOfUpdate(arangodb::StringRef const& key) {
...
RocksDBLogValue::RocksDBLogValue(RocksDBLogType type, uint64_t dbId,
  ...
  switch (type) {
    case RocksDBLogType::IndexCreate: {
      ...
      uint64ToPersistent(_buffer, dbId);
      uint64ToPersistent(_buffer, cid);
      _buffer.append(reinterpret_cast<char const*>(indexInfo.begin()), indexInfo.byteSize());
      break;
    }
    default:
      THROW_ARANGO_EXCEPTION_MESSAGE(TRI_ERROR_BAD_PARAMETER,
...
VPackSlice RocksDBLogValue::indexSlice(rocksdb::Slice const& slice) {
  TRI_ASSERT(slice.size() >= sizeof(RocksDBLogType) + sizeof(uint64_t) * 2);
  RocksDBLogType type = static_cast<RocksDBLogType>(slice.data()[0]);
  TRI_ASSERT(type == RocksDBLogType::IndexCreate || type == RocksDBLogType::ViewCreate ||
             type == RocksDBLogType::ViewChange);
  return VPackSlice(slice.data() + sizeof(RocksDBLogType) + sizeof(uint64_t) * 2);
}

arangodb::StringRef RocksDBLogValue::oldCollectionName(rocksdb::Slice const& slice) {
...
arangodb::StringRef RocksDBLogValue::documentKey(rocksdb::Slice const& slice) {
  RocksDBLogType type = static_cast<RocksDBLogType>(slice.data()[0]);
  TRI_ASSERT(type == RocksDBLogType::SingleRemove || type == RocksDBLogType::DocumentRemove ||
...
bool RocksDBLogValue::containsDatabaseId(RocksDBLogType type) {
  return type == RocksDBLogType::DatabaseCreate || type == RocksDBLogType::DatabaseDrop ||
         ...
         type == RocksDBLogType::SinglePut || type == RocksDBLogType::SingleRemove;
}
