...
RocksDBLogValue RocksDBLogValue::CollectionDrop(
    TRI_voc_tick_t dbid, DataSourceId cid,
    arangodb::velocypack::StringRef const& uuid) {
  return RocksDBLogValue(RocksDBLogType::CollectionDrop, dbid, cid.id(), uuid);
}

RocksDBLogValue RocksDBLogValue::CollectionRename(
    TRI_voc_tick_t dbid, DataSourceId cid,
...
RocksDBLogValue RocksDBLogValue::ViewDrop(
    TRI_voc_tick_t dbid, DataSourceId vid,
    arangodb::velocypack::StringRef const& uuid) {
...
RocksDBLogValue RocksDBLogValue::TrackedDocumentInsert(
    LocalDocumentId docId, VPackSlice const& slice) {
  RocksDBLogValue val{};
...
RocksDBLogValue RocksDBLogValue::TrackedDocumentRemove(
    LocalDocumentId docId, VPackSlice const& slice) {
  RocksDBLogValue val{};
  val._buffer.reserve(sizeof(RocksDBLogType) +
                      sizeof(LocalDocumentId::BaseType) + slice.byteSize());
...
arangodb::velocypack::StringRef dropMarkerUUID(rocksdb::Slice const& slice) {
  ...
  if (slice.size() > off) {
    // have a UUID
    return arangodb::velocypack::StringRef(slice.data() + off,
                                           slice.size() - off);
  }
  // do not have a UUID
...
arangodb::velocypack::StringRef RocksDBLogValue::collectionUUID(
    rocksdb::Slice const& slice) {
  return ::dropMarkerUUID(slice);
}

arangodb::velocypack::StringRef RocksDBLogValue::viewUUID(
    rocksdb::Slice const& slice) {
  return ::dropMarkerUUID(slice);
}

arangodb::velocypack::StringRef RocksDBLogValue::oldCollectionName(
    rocksdb::Slice const& slice) {
  size_t off = sizeof(RocksDBLogType) + sizeof(uint64_t) * 2;
  TRI_ASSERT(slice.size() >= off);
  RocksDBLogType type = static_cast<RocksDBLogType>(slice.data()[0]);
