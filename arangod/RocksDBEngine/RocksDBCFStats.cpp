...
////////////////////////////////////////////////////////////////////////////////
/// toVPack implementations
////////////////////////////////////////////////////////////////////////////////

void CompactionLevelStats::toVPack(velocypack::Builder& builder) const {
  // Adds an object for this level to an array or as a standalone object
  builder.openObject();
  builder.add("level", velocypack::Value(level));
  builder.add("numFiles", velocypack::Value(numFiles));
  builder.add("compactedFiles", velocypack::Value(compactedFiles));
  builder.add("sizeBytes", velocypack::Value(sizeBytes));
  builder.add("score", velocypack::Value(score));
  builder.add("readGB", velocypack::Value(readGB));
  builder.add("rnGB", velocypack::Value(rnGB));
  builder.add("rnp1GB", velocypack::Value(rnp1GB));
  builder.add("writeGB", velocypack::Value(writeGB));
  builder.add("wnewGB", velocypack::Value(wnewGB));
  builder.add("movedGB", velocypack::Value(movedGB));
  builder.add("writeAmp", velocypack::Value(writeAmp));
  builder.add("readMBps", velocypack::Value(readMBps));
  builder.add("writeMBps", velocypack::Value(writeMBps));
  builder.add("compSec", velocypack::Value(compSec));
  builder.add("compMergeCPUSec", velocypack::Value(compMergeCPUSec));
  builder.add("compCount", velocypack::Value(compCount));
  builder.add("avgSec", velocypack::Value(avgSec));
  builder.add("keyIn", velocypack::Value(keyIn));
  builder.add("keyDrop", velocypack::Value(keyDrop));
  builder.add("rblobGB", velocypack::Value(rblobGB));
  builder.add("wblobGB", velocypack::Value(wblobGB));
  builder.close();
}

void WriteStallStats::toVPack(velocypack::Builder& builder) const {
  builder.openObject();
  builder.add("totalDelays", velocypack::Value(totalDelays));
  builder.add("totalStops", velocypack::Value(totalStops));
  builder.add("l0FileCountLimitDelays",
              velocypack::Value(l0FileCountLimitDelays));
  builder.add("l0FileCountLimitStops",
              velocypack::Value(l0FileCountLimitStops));
  builder.add("memtableLimitDelays", velocypack::Value(memtableLimitDelays));
  builder.add("memtableLimitStops", velocypack::Value(memtableLimitStops));
  builder.add("pendingCompactionBytesDelays",
              velocypack::Value(pendingCompactionBytesDelays));
  builder.add("pendingCompactionBytesStops",
              velocypack::Value(pendingCompactionBytesStops));
  builder.add("cfL0FileCountLimitDelaysWithOngoingCompaction",
              velocypack::Value(cfL0FileCountLimitDelaysWithOngoingCompaction));
  builder.add("cfL0FileCountLimitStopsWithOngoingCompaction",
              velocypack::Value(cfL0FileCountLimitStopsWithOngoingCompaction));
  builder.close();
}

void BlockCacheRoleStats::toVPack(velocypack::Builder& builder) const {
  builder.openObject();
  builder.add("count", velocypack::Value(count));
  builder.add("usedBytes", velocypack::Value(usedBytes));
  builder.add("usedPercent", velocypack::Value(usedPercent));
  builder.close();
}

void BlockCacheStats::toVPack(velocypack::Builder& builder) const {
  builder.openObject();
  builder.add("cacheId", velocypack::Value(cacheId));
  builder.add("capacityBytes", velocypack::Value(capacityBytes));
  builder.add("lastCollectionDurationSecs",
              velocypack::Value(lastCollectionDurationSecs));
  builder.add("lastCollectionAgeSecs",
              velocypack::Value(lastCollectionAgeSecs));

  builder.add("roleStats", velocypack::Value(velocypack::ValueType::Object));
  for (auto const& [roleName, roleStats] : roleStats) {
    builder.add(roleName, velocypack::Value(velocypack::ValueType::Object));
    builder.add("count", velocypack::Value(roleStats.count));
    builder.add("usedBytes", velocypack::Value(roleStats.usedBytes));
    builder.add("usedPercent", velocypack::Value(roleStats.usedPercent));
    builder.close();
  }
  builder.close();  // roleStats

  builder.close();
}

void ColumnFamilyStats::toVPack(velocypack::Builder& builder) const {
  // Adds fields to an already-open object (does not open/close the outer
  // object)

  // Compaction stats
  builder.add("compactionStats",
              velocypack::Value(velocypack::ValueType::Object));

  if (sumStats) {
    builder.add("sum", velocypack::Value(velocypack::ValueType::Object));
    builder.add("numFiles", velocypack::Value(sumStats->numFiles));
    builder.add("sizeBytes", velocypack::Value(sumStats->sizeBytes));
    builder.add("readGB", velocypack::Value(sumStats->readGB));
    builder.add("writeGB", velocypack::Value(sumStats->writeGB));
    builder.add("readMBps", velocypack::Value(sumStats->readMBps));
    builder.add("writeMBps", velocypack::Value(sumStats->writeMBps));
    builder.add("compSec", velocypack::Value(sumStats->compSec));
    builder.add("compCount", velocypack::Value(sumStats->compCount));
    builder.add("keyIn", velocypack::Value(sumStats->keyIn));
    builder.add("keyDrop", velocypack::Value(sumStats->keyDrop));
    builder.close();  // sum
  }

  builder.add("levels", velocypack::Value(velocypack::ValueType::Array));
  for (auto const& level : levelStats) {
    level.toVPack(builder);
  }
  builder.close();  // levels

  builder.close();  // compactionStats

  // Write stall stats
  builder.add("writeStall", velocypack::Value(velocypack::ValueType::Object));
  builder.add("totalDelays", velocypack::Value(writeStallStats.totalDelays));
  builder.add("totalStops", velocypack::Value(writeStallStats.totalStops));
  builder.add("l0FileCountLimitDelays",
              velocypack::Value(writeStallStats.l0FileCountLimitDelays));
  builder.add("l0FileCountLimitStops",
              velocypack::Value(writeStallStats.l0FileCountLimitStops));
  builder.add("memtableLimitDelays",
              velocypack::Value(writeStallStats.memtableLimitDelays));
  builder.add("memtableLimitStops",
              velocypack::Value(writeStallStats.memtableLimitStops));
  builder.add("pendingCompactionBytesDelays",
              velocypack::Value(writeStallStats.pendingCompactionBytesDelays));
  builder.add("pendingCompactionBytesStops",
              velocypack::Value(writeStallStats.pendingCompactionBytesStops));
  builder.close();  // writeStall

  // Memory/size stats
  builder.add("numImmutableMemTable", velocypack::Value(numImmutableMemTable));
  builder.add("numImmutableMemTableFlushed",
              velocypack::Value(numImmutableMemTableFlushed));
  builder.add("memTableFlushPending", velocypack::Value(memTableFlushPending));
  builder.add("compactionPending", velocypack::Value(compactionPending));
  builder.add("curSizeActiveMemTable",
              velocypack::Value(curSizeActiveMemTable));
  builder.add("curSizeAllMemTables", velocypack::Value(curSizeAllMemTables));
  builder.add("sizeAllMemTables", velocypack::Value(sizeAllMemTables));
  builder.add("numEntriesActiveMemTable",
              velocypack::Value(numEntriesActiveMemTable));
  builder.add("numEntriesImmMemTables",
              velocypack::Value(numEntriesImmMemTables));
  builder.add("numDeletesActiveMemTable",
              velocypack::Value(numDeletesActiveMemTable));
  builder.add("numDeletesImmMemTables",
              velocypack::Value(numDeletesImmMemTables));
  builder.add("estimateNumKeys", velocypack::Value(estimateNumKeys));
  builder.add("estimateTableReadersMem",
              velocypack::Value(estimateTableReadersMem));
  builder.add("numLiveVersions", velocypack::Value(numLiveVersions));
  builder.add("estimateLiveDataSize", velocypack::Value(estimateLiveDataSize));
  builder.add("liveSstFilesSize", velocypack::Value(liveSstFilesSize));
  builder.add("estimatePendingCompactionBytes",
              velocypack::Value(estimatePendingCompactionBytes));

  // Blob stats
  builder.add("numBlobFiles", velocypack::Value(numBlobFiles));
  builder.add("liveBlobFileSize", velocypack::Value(liveBlobFileSize));
  builder.add("liveBlobFileGarbageSize",
              velocypack::Value(liveBlobFileGarbageSize));
}

void DatabaseStats::toVPack(velocypack::Builder& builder) const {
  // Adds fields to an already-open object (does not open/close the outer
  // object)

  builder.add("numSnapshots", velocypack::Value(numSnapshots));
  builder.add("oldestSnapshotTime", velocypack::Value(oldestSnapshotTime));
  builder.add("numRunningCompactions",
              velocypack::Value(numRunningCompactions));
  builder.add("numRunningFlushes", velocypack::Value(numRunningFlushes));
  builder.add("blockCacheCapacity", velocypack::Value(blockCacheCapacity));
  builder.add("blockCacheUsage", velocypack::Value(blockCacheUsage));
  builder.add("blockCachePinnedUsage",
              velocypack::Value(blockCachePinnedUsage));

  if (blockCacheStats) {
    builder.add("blockCacheStats",
                velocypack::Value(velocypack::ValueType::Object));
    builder.add("cacheId", velocypack::Value(blockCacheStats->cacheId));
    builder.add("capacityBytes",
                velocypack::Value(blockCacheStats->capacityBytes));
    builder.add("lastCollectionDurationSecs",
                velocypack::Value(blockCacheStats->lastCollectionDurationSecs));
    builder.add("lastCollectionAgeSecs",
                velocypack::Value(blockCacheStats->lastCollectionAgeSecs));

    builder.add("roleStats", velocypack::Value(velocypack::ValueType::Object));
    for (auto const& [roleName, roleStats] : blockCacheStats->roleStats) {
      builder.add(roleName, velocypack::Value(velocypack::ValueType::Object));
      builder.add("count", velocypack::Value(roleStats.count));
      builder.add("usedBytes", velocypack::Value(roleStats.usedBytes));
      builder.add("usedPercent", velocypack::Value(roleStats.usedPercent));
