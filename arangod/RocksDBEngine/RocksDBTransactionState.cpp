...
#endif
    RocksDBLogValue header = RocksDBLogValue::BeginTransaction(_vocbase->id(), _id);
...
void RocksDBTransactionState::prepareOperation(TRI_voc_cid_t collectionId,
  ...
  if (collectionId != _lastUsedCollection) {
    ...
    if (!singleOp) {
      if (operationType != TRI_VOC_DOCUMENT_OPERATION_UNKNOWN) {
        RocksDBLogValue logValue = RocksDBLogValue::DocumentOpsPrologue(collectionId);
...
RocksDBOperationResult RocksDBTransactionState::addOperation(
    TRI_voc_cid_t cid, TRI_voc_rid_t revisionId, TRI_voc_document_operation_e operationType,
...
RocksDBOperationResult RocksDBTransactionState::addInternalOperation(uint64_t operationSize,
                                                                     uint64_t keySize) {
  size_t currentSize = _rocksTransaction->GetWriteBatch()->GetWriteBatch()->GetDataSize();
  uint64_t newSize = currentSize + operationSize + keySize;
  if (newSize > _options.maxTransactionSize) {
