...
struct RocksDBMetadata final {
 ...
 public:
  ...
  static Result deleteCollectionMetaBatch(uint64_t objectId, rocksdb::WriteBatch& wb);

  /// @brief remove collection index estimate
  static Result deleteIndexEstimate(rocksdb::DB*, uint64_t objectId);
  static Result deleteIndexEstimateBatch(uint64_t objectId, rocksdb::WriteBatch& wb);
