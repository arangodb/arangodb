    ...
    if (value.length() != dimension) {
      return {TRI_ERROR_TYPE_ERROR,
              std::format(
                  ...
                  transaction::helpers::extractKeyFromDocument(doc), dimension,
                  value.length())};
    }
...
TrainingResult VectorIndexTrainer::restoreFromTrainedData(
  ...
  result.resolvedNLists = static_cast<std::int64_t>(faissIndex->nlist);
  ...
  // would incorrectly use resolvedNLists as a proxy for the original
  // docCount, silently changing query-time defaults after restart).
  result.resolvedDefaultNProbe = static_cast<std::int64_t>(faissIndex->nprobe);
...
TrainingResult VectorIndexTrainer::train(rocksdb::Iterator& it,
  ...
  if (isNListsScaling(_definition.nLists)) {
    LOG_TOPIC("c162b", INFO, Logger::STATISTICS)
        ...
        << ", computed nLists=" << resolvedNLists
        << ", computed defaultNProbe=" << resolvedDefaultNProbe << ".";

    if (_definition.factory) {
      LOG_TOPIC("c163b", INFO, Logger::STATISTICS)
          << "[shard=" << _shardName << ", index=" << _indexId << "] "
          << "Using factory string '" << _definition.factory->c_str()
          << "'.";
    }
  }
  ...
  TrainingResult result;
  result.faissIndex = std::move(faissIndex);
  result.resolvedNLists = resolvedNLists;
  result.resolvedDefaultNProbe = resolvedDefaultNProbe;
