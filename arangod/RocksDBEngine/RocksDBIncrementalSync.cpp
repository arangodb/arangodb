...
Result syncChunkRocksDB(DatabaseInitialSyncer& syncer,
                        SingleCollectionTransaction* trx, IncrementalSyncStats& stats,
  ...
  if (response == nullptr || !response->isComplete()) {
    return Result(TRI_ERROR_REPLICATION_NO_RESPONSE,
                  std::string("could not connect to master at ") +
                      syncer._masterInfo._endpoint + url + ": " +
  ...
  if (response->wasHttpError()) {
    return Result(TRI_ERROR_REPLICATION_MASTER_ERROR,
                  std::string("got invalid response from master at ") +
                      syncer._masterInfo._endpoint + url + ": HTTP " +
    ...
    if (mustRefetch) {
      ...
      if (!documentId.isSet()) {
        ...
        if (physical->readDocument(trx, documentId, mmdr)) {
          currentRevisionId =
              transaction::helpers::extractRevFromDocument(VPackSlice(mmdr.vpack()));
        }
...
Result handleSyncKeysRocksDB(DatabaseInitialSyncer& syncer,
  ...
  progress =
      "fetching remote keys chunks for collection '" + col->name() + "' from " + url;
  syncer.setProgress(progress);
...

  {
    ...
    auto resetChunk = [&]() -> void {
      ...
      if (!chunk.isObject()) {
        THROW_ARANGO_EXCEPTION_MESSAGE(
            ...
            std::string("got invalid response from master at ") +
                syncer._masterInfo._endpoint + url + ": chunk is no object");
      }
    ...
    std::function<void(std::string, std::uint64_t)> compareChunk =
        [&trx, &col, &options, &foundLowKey, &markers, &localHash, &hashString,
         &syncer, &currentChunkId, &numChunks, &keysId, &resetChunk, &compareChunk,
         &lowKey, &highKey, &stats](std::string const& docKey, std::uint64_t docRev) {
