...
void reportPrimaryIndexInconsistency(
    arangodb::Result const& res, arangodb::velocypack::StringRef const& key,
    ...
    for (auto it = _indexes.begin(); it != _indexes.end(); ++it) {
      ...
      if (res.fail()) {
        if (needReversal && !state->isSingleOperation()) {
          ::reverseIdxOps(
              _indexes, it,
              [mthds, trx, &documentId, &doc, &options](RocksDBIndex* rid) {
                return rid->remove(*trx, mthds, documentId, doc, options);
              });
        }
        break;
      }
