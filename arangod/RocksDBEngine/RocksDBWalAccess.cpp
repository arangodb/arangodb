...
WalTailingResult RocksDBWalAccess::tail(uint64_t tickStart, uint64_t tickEnd,
  ...
  auto handler = std::make_unique<MyWALParser>(includeSystem, filter, builder);
  std::unique_ptr<rocksdb::TransactionLogIterator> iterator;  // reader();
  
  rocksdb::TransactionLogIterator::ReadOptions ro(false);
  rocksdb::Status s = rocksutils::globalRocksDB()->GetUpdatesSince(tickStart,
                                                                   &iterator, ro);
  if (!s.ok()) {
  ...
  while (iterator->Valid() && lastTick <= tickEnd &&
    ...
    if (!s.ok()) {
      ...
      return WalTailingResult(converted.errorNumber(), tickStart, lastWrittenTick);
    }
