...
uint64_t defaultMinWriteBufferNumberToMerge(uint64_t totalSize, uint64_t sizePerBuffer,
                                            uint64_t maxBuffers) {
...
RocksDBOptionFeature::RocksDBOptionFeature(application_features::ApplicationServer& server)
  ...
  _maxBackgroundJobs =
      static_cast<int32_t>(std::max(static_cast<size_t>(2), NumberOfCores::getValue()));
...
void RocksDBOptionFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  options
      ->addOption("--rocksdb.cache-index-and-filter-blocks",
                  "if turned on, the RocksDB block cache quota will also "
                  "include RocksDB memtable sizes",
  ...
  options
      ->addOption("--rocksdb.exclusive-writes",
                  "if true, writes are exclusive. This allows the RocksDB "
                  "engine to mimic "
                  "the collection locking behavior of the now-removed MMFiles "
                  "storage engine, "
...
void RocksDBOptionFeature::start() {
  ...
  uint32_t clamped =
      std::max(std::min(static_cast<uint32_t>(NumberOfCores::getValue()), max), 1U);
  // lets test this out
  ...
  LOG_TOPIC("f66e4", TRACE, Logger::ROCKSDB)
      ...
      << ", cache_index_and_filter_blocks_with_high_priority: " << std::boolalpha
      << _cacheIndexAndFilterBlocksWithHighPriority
