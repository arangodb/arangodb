...
HttpsCommTask::HttpsCommTask(HttpsServer* server, TRI_socket_t socket,
    ...
    : 
    // HttpCommTask(server, socket, info, keepAliveTimeout, "HttpsCommTask", GeneralRequest::UNKNOWN, GeneralRequest::REQUEST_ILLEGAL),
    GeneralsCommTask(server, socket, info, keepAliveTimeout, ctx, verificationMode, 
                      "HttpsCommTask", version, request, (*verificationCallback)(int, X509_STORE_CTX*))
    // Task("HttpsCommTask"),
    {
}
