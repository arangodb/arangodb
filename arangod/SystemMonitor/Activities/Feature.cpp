...
DECLARE_COUNTER(arangodb_activities_activities_total,
                "Total number of created activities since database creation");

DECLARE_GAUGE(arangodb_activities_existing_activities, std::uint64_t,
              "Number of currently existing activities");

DECLARE_GAUGE(arangodb_activies_ready_for_deletion_activities, std::uint64_t,
...
auto Feature::create_metrics(metrics::MetricsFeature& metrics_feature)
  ...
  return std::make_shared<RegistryMetrics>(
      metrics_feature.addShared(arangodb_activities_activities_total{}),
      metrics_feature.addShared(arangodb_activities_existing_activities{}),
      metrics_feature.addShared(
          arangodb_activies_ready_for_deletion_activities{}),
