...
#include "ApplicationVelocyServer.h"
...
ApplicationVelocyServer::ApplicationVelocyServer(
...
bool ApplicationVelocyServer::buildServers() {
...
void ApplicationVelocyServer::setupOptions(
    std::map<std::string, ProgramOptionsDescription>& options) {
  // issue #175: add deprecated hidden option for downwards compatibility
  options["Hidden Options"]("server.velocy-port", &_velocyPort,
                            "velocy port for client requests (deprecated)");
  ...
  options["Server Options:help-admin"](
      ...
      "allow VELOCY method override using special headers")(
  ...
  options["SSL Options:help-ssl"]("server.keyfile", &_velocysKeyfile,
...
bool ApplicationVelocyServer::afterOptionParsing(ProgramOptions& options) {
  ...
  if (!_velocyPort.empty()) {
    // issue #175: add hidden option --server.velocy-port for
    // downwards-compatibility
    std::string velocyEndpoint("tcp://" + _velocyPort);
    _endpoints.push_back(velocyEndpoint);
  }
...
std::string ApplicationVelocyServer::getEndpointsFilename() const {
...
ApplicationVelocyServer::getEndpoints() {
...
bool ApplicationVelocyServer::loadEndpoints() {
...
std::vector<std::string> const& ApplicationVelocyServer::getEndpointMapping(
    std::string const& endpoint) {
  return _endpointList.getMapping(endpoint);
}


bool ApplicationVelocyServer::prepare() {
  if (_disabled) {
    return true;
  }
...
bool ApplicationVelocyServer::open() {
  if (_disabled) {
    return true;
  }
...
void ApplicationVelocyServer::close() {
  if (_disabled) {
    return;
  }
...
void ApplicationVelocyServer::stop() {
  if (_disabled) {
    return;
  }
...
bool ApplicationVelocyServer::createSslContext() {
  // check keyfile
  if (_velocysKeyfile.empty()) {
    return true;
  }
