...
class WeightedShortestPathFinder {
 public:
  ...
  using Intersection = detail::Intersection<Step>;
 ...
 public:
  ...
  WeightedShortestPathFinder(WeightedShortestPathFinder const& other) = delete;
  WeightedShortestPathFinder& operator=(
      WeightedShortestPathFinder const& other) = delete;
  WeightedShortestPathFinder(WeightedShortestPathFinder&& other) = delete;
  WeightedShortestPathFinder& operator=(WeightedShortestPathFinder&& other) =
      delete;

  ~WeightedShortestPathFinder();
 ...
 private:
  ...
  // (Left or right ball)
  enum class BallSearchLocation { FORWARD, BACKWARD, FINISH };
  ...
  auto setAlgorithmFinished() -> void { _algorithmFinished = true; }
  auto setAlgorithmUnfinished() -> void { _algorithmFinished = false; }

  auto setInitialFetchVerified() -> void { _handledInitialFetch = true; }
  auto getInitialFetchVerified() -> bool { return _handledInitialFetch; }
  [[nodiscard]] auto isAlgorithmFinished() const -> bool;

 private:
  using Dijkstra = dijkstra::Dijkstra<QueueType, PathStoreType, ProviderType,
                                      PathValidatorType>;
  Dijkstra _forward;
  Dijkstra _backward;
  ...
  // The best path found as of now
  std::optional<Intersection> _bestCandidate{std::nullopt};
