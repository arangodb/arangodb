...
template<class PathStoreImpl>
typename PathStoreImpl::Step PathStoreTracer<PathStoreImpl>::getStep(
...
template<class PathStoreImpl>
typename PathStoreImpl::Step& PathStoreTracer<PathStoreImpl>::getStepReference(
    size_t position) {
  double start = TRI_microtime();
...
auto PathStoreTracer<PathStoreImpl>::reverseBuildPath(
...
auto PathStoreTracer<PathStoreImpl>::visitReversePath(
    const Step& step, const std::function<bool(const Step&)>& visitor) const
    -> bool {
  double start = TRI_microtime();
  auto sg = arangodb::scopeGuard([&]() noexcept {
...
template void ::arangodb::graph::
    PathStoreTracer<PathStore<SingleServerProviderStep>>::buildPath<PathResult<
...
template void arangodb::graph::
    PathStoreTracer<PathStore<SingleServerProviderStep>>::reverseBuildPath<
        ProviderTracer<SingleServerProvider<SingleServerProviderStep>>>(
        ProviderTracer<
            SingleServerProvider<SingleServerProviderStep>>::Step const& vertex,
        PathResult<
...
template void arangodb::graph::
    PathStoreTracer<PathStore<enterprise::SmartGraphStep>>::reverseBuildPath<
        ProviderTracer<SingleServerProvider<enterprise::SmartGraphStep>>>(
        ProviderTracer<SingleServerProvider<enterprise::SmartGraphStep>>::
            Step const& vertex,
        PathResult<
...
template void ::arangodb::graph::PathStoreTracer<
    PathStore<ClusterProviderStep>>::
    buildPath<PathResult<ProviderTracer<enterprise::SmartGraphProvider<
                             ClusterProviderStep>>,
