...
std::unordered_set<std::__cxx11::string> const& Graph::vertexCollections() const {
  return _vertexColls;
}

std::unordered_set<std::__cxx11::string> const& Graph::edgeCollections() const {
  return _edgeColls;
}

void Graph::addEdgeCollection(std::__cxx11::string const& name) {
  _edgeColls.insert(name);
}

void Graph::addVertexCollection(std::__cxx11::string const& name) {
...
Graph::Graph(VPackSlice const& slice) : _vertexColls(), _edgeColls() {
  if (slice.hasKey(_attrEdgeDefs)) {
    ...
    for (auto const& def : VPackArrayIterator(edgeDefs)) {
      ...
      try {
        std::__cxx11::string eCol = arangodb::basics::VelocyPackHelper::getStringValue(
          def, "collection", "");
