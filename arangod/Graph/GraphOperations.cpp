...
std::pair<OperationResult, bool> GraphOperations::validateEdgeContent(
  ...
  if (it == _graph.vertexCollections().end()) {
    ...
    return std::make_pair(OperationResult(TRI_ERROR_GRAPH_FROM_VERTEX_COLLECTION_DOES_NOT_EXIST, options), true);
  }
  it = _graph.vertexCollections().find(toCollectionName);
  if (it == _graph.vertexCollections().end()) {
    // not found to vertex
    return std::make_pair(OperationResult(TRI_ERROR_GRAPH_TO_VERTEX_COLLECTION_DOES_NOT_EXIST, options), true);
  }
