...
ShortestPathOptions::ShortestPathOptions(transaction::Methods* trx, VPackSlice const& info)
    : BaseOptions(trx),
      direction("outbound"),
      weightAttribute(""),
      defaultWeight(1),
...
void ShortestPathOptions::fetchVerticesCoordinator(std::deque<StringRef> const& vertexIds) {
  ...
  if (!fetch.empty()) {
    ...
    fetchVerticesFromEngines(trx()->databaseName(), ch->engines(), fetch, cache,
                             ch->datalake(), *(leased.get()));
  }
}
