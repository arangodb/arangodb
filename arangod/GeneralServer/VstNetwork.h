...
inline std::vector<std::unique_ptr<basics::StringBuffer>> createChunkForNetwork(
  ...
  if (payloadLength < maxChunkBytes - chl) {
    ...
    rv.push_back(createChunkForNetworkDetail(slices, true, 1, id, protocolVersion,
                                             chl + payloadLength));
    return rv;
  } else {
    ...
    // create chunks
    send_many(rv, id, maxChunkBytes, std::move(vstPayload), payloadLength, protocolVersion);
  }
  return rv;
}
