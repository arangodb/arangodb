 ...
 * The main cause of this is the fact that the followers rely on a snapshot sent
 ...
 * recovery process.
 * The purpose of this class is to provide a documented way of handling these
 * errors, such that it is clear what is ignored and what is not.
 */
class DocumentStateErrorHandler {
 public:
  ...
  [[nodiscard]] auto handleOpResult(
      ReplicatedOperation::OperationType const& op, Result const& res) const
      -> Result {
    return std::visit(
  ...
  auto shouldIgnoreDocumentError(arangodb::OperationResult const& res) noexcept
      -> bool;
