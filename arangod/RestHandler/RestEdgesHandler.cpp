...
void RestEdgesHandler::readCursor(aql::AstNode* condition, aql::Variable const* var,
  ...
  std::unique_ptr<OperationCursor> cursor(
      trx.indexScanForCondition(indexId, condition, var, &mmdr, UINT64_MAX, 1000, false));
...
bool RestEdgesHandler::readEdges() {
  ...
  if (ServerState::instance()->isCoordinator()) {
    ...
    int res = getFilteredEdgesOnCoordinator(_vocbase->name(), collectionName, vertexString,
  ...
  generateResult(rest::ResponseCode::OK, resultBuilder.slice(), trx.transactionContext());
