...
void SupportInfoMessageHandler::buildInfoMessage(VPackBuilder& result) {
  if (_isHttp) {
    TRI_ASSERT(_request != nullptr);
  }
  // used for all types of responses
  VPackBuilder hostInfo;
  this->buildHostInfo(hostInfo);
  ...
  bool isLocalValue = _isHttp ? !_request->parsedValue("local", false) : true;
  bool fanout =
  ...
  if (ServerState::instance()->isSingleServer() && !isActiveFailover) {
    ...
    if (fanout) {
      ...
      options.database =
          _isHttp ? _request->databaseName() : StaticStrings::SystemDatabase;
      ...
      for (auto const& server : ci.getServers()) {
        ...
        if (_isHttp) {
          requestPath = _request->requestPath();
        } else {
          requestPath =
              "/_api/support-info";  // TODO: FIX THIS AS IS PROBABLY IS NOT
                                     // CORRECT WHEN IT COMES FROM TELEMETRICS
        }
        auto f = network::sendRequestRetry(
            pool, "server:" + server.first, fuerte::RestVerb::Get, requestPath,
...
void SupportInfoMessageHandler::buildHostInfo(VPackBuilder& result) {
  bool const isActiveFailover =
      _server.getFeature<ReplicationFeature>().isActiveFailoverEnabled();
  ...
  EnvironmentFeature const& ef = _server.getFeature<EnvironmentFeature>();
  ...
  CpuUsageFeature& cpuUsage = _server.getFeature<CpuUsageFeature>();
...
RestStatus RestSupportInfoHandler::execute() {
  ...
  VPackBuilder result;
  SupportInfoMessageHandler infoHandler(_request.get(), _server);
  infoHandler.buildInfoMessage(result);
