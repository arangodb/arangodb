...
void RestBaseHandler::generateResult(TRI_json_t const* json) {
  generateResult(GeneralResponse::OK, json);
}
...
void RestBaseHandler::generateResult(GeneralResponse::HttpResponseCode code,
...
void RestBaseHandler::generateResult(VPackSlice const& slice) {
  generateResult(GeneralResponse::OK, slice);
}
...
void RestBaseHandler::generateCanceled() {
  ...
  try {
    ...
    builder.add("code", VPackValue((int32_t)GeneralResponse::GONE));
    ...
    generateResult(GeneralResponse::GONE, slice);
  } catch (...) {
    generateError(GeneralResponse::SERVER_ERROR, TRI_ERROR_INTERNAL,
                  "cannot generate output");
  }
...
void RestBaseHandler::generateError(GeneralResponse::HttpResponseCode code,
                                    int errorCode) {
