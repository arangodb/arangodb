...
RestAdminClusterHandler::FutureVoid RestAdminClusterHandler::retryTryDeleteServer(
  ...
  if (++ctx->tries < 60) {
    return SchedulerFeature::SCHEDULER->delay(1s).then_bind(
...
RestAdminClusterHandler::FutureVoid RestAdminClusterHandler::tryDeleteServer(
  ...
  return AsyncAgencyComm().sendReadTransaction(20s, std::move(trx))
      .then_bind([this, ctx = std::move(ctx)](AsyncAgencyCommResult&& result) mutable {
    auto rootPath = arangodb::cluster::paths::root()->arango();
...
RestAdminClusterHandler::FutureVoid RestAdminClusterHandler::waitForSupervisionState(
  ...
  return SchedulerFeature::SCHEDULER->delay(1s)
      ...
      .then_bind([this, state, startTime](AgencyReadResult&& result) {
...
RestStatus RestAdminClusterHandler::setMaintenance(bool wantToActivate) {
  ...
  return waitForFuture(
      sendTransaction()
          .then_bind([this, wantToActivate](AsyncAgencyCommResult&& result) {
            if (result.ok() && result.statusCode() == 200) {
...
RestStatus RestAdminClusterHandler::handleHealth() {
  ...
  auto fConfig =
      AsyncAgencyComm()
          ...
          .then_bind([self](AsyncAgencyCommResult&& result) {
            ...
            for (auto member : VPackObjectIterator(result.slice().get(
              ...
              auto future =
                  network::sendRequestRetry(pool, endpoint, fuerte::RestVerb::Get,
                      ...
                      .then_bind([endpoint = std::move(endpoint), memberName = std::move(memberName)](
  ...
  return waitForFuture(
      futures::collect(std::move(fConfig), std::move(fStore))
          .thenValue([this](auto&& result) {
            ...
            if (storeResult.has_value() && storeResult.unwrap().statusCode() == fuerte::StatusOK) {
              VPackBuilder builder;
              {
                VPackObjectBuilder ob(&builder);
