...
RestStatus RestLogHandler::handlePostInsert(ReplicatedLogMethods const& methods,
  ...
  return waitForFuture(
      methods.insert(logId, LogPayload::createFromSlice(payload)).thenValue([this](auto&& waitForResult) {
...
RestStatus RestLogHandler::handlePostInsertMulti(ReplicatedLogMethods const& methods,
                                                 replication2::LogId logId,
                                                 velocypack::Slice payload) {
  if (!payload.isArray()) {
    generateError(ResponseCode::BAD, TRI_ERROR_BAD_PARAMETER, "array expected at top-level");
    return RestStatus::DONE;
  }

  auto iter = replicated_log::LogPayloadIterator{payload};
...
RestStatus RestLogHandler::handlePostRelease(ReplicatedLogMethods const& methods,
                                             replication2::LogId logId) {
  auto idx = LogIndex{basics::StringUtils::uint64(_request->value("index"))};
  return waitForFuture(methods.release(logId, idx).thenValue([this](Result&& res) {
    if (res.fail()) {
      generateError(res);
    } else {
