...
RestStatus RestReplicationHandler::execute() {
  ...
  if (len >= 1) {
    ...
    if (command == LoggerState) {
      ...
      handleCommandSync();
    } else if (command == MakeFollower || command == "make-slave" /*deprecated*/) {
      if (type != rest::RequestType::PUT) {
        goto BAD_CALL;
      }
      ...
      try {
        ...
        if (dropResult.fail()) {
          if (dropResult.is(TRI_ERROR_FORBIDDEN) ||
              dropResult.is(TRI_ERROR_CLUSTER_MUST_NOT_DROP_COLL_OTHER_DISTRIBUTESHARDSLIKE)) {
    ...
    if (pos - ptr > 1) {
      ...
      if (type == REPLICATION_MARKER_DOCUMENT) {
        ...
        for (auto it : VPackObjectIterator(doc, true)) {
          ...
          bool const isKey = checkKey && (arangodb::velocypack::StringRef(it.key) ==
          ...
          } else if (checkRev && arangodb::velocypack::StringRef(it.key) ==
...
Result RestReplicationHandler::parseBatchForSystemCollection(
    ...
    std::unordered_set<std::string>& documentsToRemove, bool generateNewRevisionIds) {
...
Result RestReplicationHandler::processRestoreCoordinatorAnalyzersBatch(bool generateNewRevisionIds) {
  VPackBuilder documentsToInsert;
  std::unordered_set<std::string> documentsToRemove;
  Result res = parseBatchForSystemCollection(StaticStrings::AnalyzersCollection,
                                             documentsToInsert, documentsToRemove,
                                             generateNewRevisionIds);
...
Result RestReplicationHandler::processRestoreUsersBatch(bool generateNewRevisionIds) {
  VPackBuilder documentsToInsert;
  std::unordered_set<std::string> documentsToRemove;
  Result res = parseBatchForSystemCollection(StaticStrings::UsersCollection,
                                             documentsToInsert, documentsToRemove,
                                             generateNewRevisionIds);
  if (res.fail()) {
    return res;
  ...
  if (leaderId != currentLeader) {
    ...
    if (followingTermId.isNumber()) {
      col->followers()->setTheLeader(
          leaderId + "_" + StringUtils::itoa(followingTermId.getNumber<uint64_t>()));
    } else {
...
void RestReplicationHandler::handleCommandHoldReadLockCollection() {
  ...
  bool doSoftLock =
      VelocyPackHelper::getBooleanValue(body, StaticStrings::ReplicationSoftLockOnly, false);
...
ErrorCode RestReplicationHandler::createCollection(VPackSlice slice) {
  ...
  std::string const uuid =
      arangodb::basics::VelocyPackHelper::getStringValue(slice, StaticStrings::DataSourceGuid,
                                                         "");
