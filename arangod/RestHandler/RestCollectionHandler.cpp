...
size_t RestCollectionHandler::queue() const {
...
void RestCollectionHandler::handleCommandGet() {
  ...
  Result found = methods::Collections::lookup(_vocbase, name, [&](LogicalCollection* coll) {
    if (sub == "checksum") {
...
void RestCollectionHandler::handleCommandPost() {
  ...
  Result res = methods::Collections::create(
      _vocbase, name, type, parameters, waitForSyncReplication,
      enforceReplicationFactor, [&](LogicalCollection* coll) {
...
void RestCollectionHandler::handleCommandPut() {
  ...
  Result found = methods::Collections::lookup(_vocbase, name, [&](LogicalCollection* coll) {
    if (sub == "load") {
      ...
      if (flush && coll->status() == TRI_vocbase_col_status_e::TRI_VOC_COL_STATUS_LOADED) {
      ...
      SingleCollectionTransaction trx(ctx, coll->cid(), AccessMode::Type::EXCLUSIVE);
      ...
      auto ctx = transaction::StandaloneContext::Create(_vocbase);
      SingleCollectionTransaction trx(ctx, coll->cid(), AccessMode::Type::WRITE);
      res = trx.begin();
...
void RestCollectionHandler::collectionRepresentation(VPackBuilder& builder,
                                                     std::string const& name, bool showProperties,
                                                     bool showFigures, bool showCount,
                                                     bool aggregateCount) {
  Result r = methods::Collections::lookup(_vocbase, name, [&](LogicalCollection* coll) {
