...
void SchedulerFeature::collectOptions(
  ...
  options
      ->addOption(
          ...
          new UInt64Parameter(&_options.nrMaximalThreads),
...
`--server.maximal-threads`.)");
  ...
  options
      ->addOption("--server.minimal-threads",
                  ...
                  new UInt64Parameter(&_options.nrMinimalThreads),
                  arangodb::options::makeDefaultFlags(
...
request processing threads the server starts and always keeps around.)");
  ...
  options
      ->addOption("--server.ongoing-low-priority-multiplier",
                  ...
                  new DoubleParameter(&_options.ongoingLowPriorityMultiplier),
                  arangodb::options::makeDefaultFlags(
...
This increases memory usage, though.)");
  ...
  options
      ->addOption("--server.maximal-queue-size",
                  ...
                  new UInt64Parameter(&_options.fifo3Size))
...
return HTTP 503 instead of HTTP 200 when their availability API is probed.)");
  ...
  options->addOption(
      ...
      new UInt64Parameter(&_options.queueSize),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));

  options
      ->addOption("--server.prio2-size", "The size of the priority 2 FIFO.",
                  new UInt64Parameter(&_options.fifo2Size),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setIntroducedIn(30800);
  ...
  options->addOption(
      ...
      new UInt64Parameter(&_options.fifo1Size),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));
...
void SchedulerFeature::validateOptions(
  ...
  } else if (_options.nrMaximalThreads == 0) {
    _options.nrMaximalThreads = defaultNumberOfThreads();
  }

  if (_options.nrMinimalThreads < 4) {
    LOG_TOPIC("bf034", WARN, arangodb::Logger::THREADS)
        << "--server.minimal-threads (" << _options.nrMinimalThreads
        << ") must be at least 4";
    _options.nrMinimalThreads = 4;
  }

  if (_options.ongoingLowPriorityMultiplier < 1.0) {
    LOG_TOPIC("0a93a", WARN, arangodb::Logger::THREADS)
        << "--server.ongoing-low-priority-multiplier ("
        << _options.ongoingLowPriorityMultiplier
        << ") is less than 1.0, setting to default (4.0)";
    _options.ongoingLowPriorityMultiplier = 4.0;
  }
  ...
  if (_options.queueSize == 0) {
    // Note that this is way smaller than the default of 4096!
    TRI_ASSERT(_options.nrMaximalThreads > 0);
    _options.queueSize = _options.nrMaximalThreads * 8;
    TRI_ASSERT(_options.queueSize > 0);
  }

  if (_options.fifo1Size < 1) {
    _options.fifo1Size = 1;
  }

  if (_options.fifo2Size < 1) {
    _options.fifo2Size = 1;
  }
