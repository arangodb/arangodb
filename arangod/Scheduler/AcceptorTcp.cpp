...
void AcceptorTcp::open() {
  ...
  if (!err) {
    ...
    if (_endpoint->domain() == AF_INET6) {
      query.reset(new asio::ip::tcp::resolver::query(asio::ip::tcp::v6(), hostname, std::to_string(portNumber)));
    } else if (_endpoint->domain() == AF_INET) {
      query.reset(new asio::ip::tcp::resolver::query(asio::ip::tcp::v4(), hostname, std::to_string(portNumber)));
    } else {
  ...
  _acceptor.set_option(
      asio::ip::tcp::acceptor::reuse_address(
...
void AcceptorTcp::asyncAccept(AcceptHandler const& handler) {
  ...
  if (_endpoint->encryption() == Endpoint::EncryptionType::SSL) {
    _peer.reset(new SocketSslTcp(_ioContext, SslServerFeature::SSL->createSslContext()));
