...
std::vector<std::string> Job::availableServers(Node const& snapshot) {
  ...
  try {
    for (auto const& srv : VPackArrayIterator(snapshot(cleanedPrefix).slice())) {
...
std::vector<Job::shard_t> Job::clones(Node const& snapshot, std::string const& database,
  ...
  for (const auto& colptr : snapshot(databasePath).children()) {  // collections
    ...
    if (otherCollection != collection && col.has("distributeShardsLike") &&
  ...
  for (const auto& clone : cs) {
    ...
    for (const auto& server : VPackArrayIterator(snap(currentShardPath).getArray())) {
      ...
      for (const auto& plannedServer :
           VPackArrayIterator(snap(plannedShardPath).getArray())) {
...
std::string Job::checkServerGood(Node const& snapshot, std::string const& server) {
