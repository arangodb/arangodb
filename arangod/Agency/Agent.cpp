...
AgentInterface::raft_commit_t Agent::waitFor(index_t index, double timeout) {
  ...
  while (true) {
    ...
    LOG_TOPIC(DEBUG, Logger::AGENCY)
        ...
        << " startTime: " << timepointToString(startTime)
        << " now: " << timepointToString(system_clock::now());
    ...
    if (i.first != myid) {  // do not count ourselves
      ...
      LOG_TOPIC(DEBUG, Logger::AGENCY)
          << "challengeLeadership: found "
             "_lastAcked["
          << i.first << "] to be "
          << std::chrono::duration_cast<std::chrono::microseconds>(i.second.time_since_epoch())
                 .count()
...
void Agent::compact() {
  ...
  if (commitIndex > _config.compactionKeepSize()) {
    ...
    if (!_state.compact(commitIndex - _config.compactionKeepSize())) {
      LOG_TOPIC(WARN, Logger::AGENCY)
          << "Compaction for index "
          << commitIndex - _config.compactionKeepSize() << " did not work.";
    }
  }
}
