...
Agent::Agent(application_features::ApplicationServer& server, config_t const& config)
    : Thread(server, "Agent"),
      ...
      _preparing(0),
      _loaded(false),
      _write_ok(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_write_ok_total{})),
      _write_no_leader(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_write_no_leader_total{})),
      _read_ok(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_read_ok_total{})),
      _read_no_leader(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_read_no_leader_total{})),
      _write_hist_msec(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_write_hist{})),
      _commit_hist_msec(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_commit_hist{})),
      _append_hist_msec(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_append_hist{})),
      _compaction_hist_msec(_server.getFeature<arangodb::MetricsFeature>().add(
          arangodb_agency_compaction_hist{})),
      _local_index(_server.getFeature<arangodb::MetricsFeature>().add(
...
priv_rpc_ret_t Agent::requestVote(term_t termOfPeer, std::string const& id,
  ...
  if (timeoutMult != -1 && timeoutMult != _config.timeoutMult()) {
    ...
    LOG_TOPIC("81f2a", WARN, Logger::AGENCY)
        << "Voter: setting timeout multiplier to " << timeoutMult << " for next term.";
  }
...
void Agent::logsForTrigger() {
  ...
  auto builder = std::make_shared<VPackBuilder>();
  {
    ...
    auto const logs =
        _state.get(_lowestPromise, _commitIndex.load(std::memory_order_relaxed));
    ...
    if (followerId != myid && leading()) {
      ...
      if (unconfirmed.empty()) {
        LOG_TOPIC("0b993", ERR, Logger::AGENCY)
            ...
            << "lastConfirmed=" << lastConfirmed << " commitIndex=" << commitIndex;
        TRI_ASSERT(false);
      }
...
std::tuple<futures::Future<query_t>, bool, std::string> Agent::poll(index_t const& index,
  ...
  auto tp = steady_clock::now() + duration_cast<milliseconds>(duration<double>(timeout));
  ...
  try {
    ...
    return std::tuple<futures::Future<query_t>, bool, std::string>{res->second.getFuture(),
                                                                   true, std::string()};
  } catch (...) {
...
void Agent::emptyCbTrashBin() {
  ...
  auto envelope = std::make_shared<VPackBuilder>();
  {
    ...
    auto early = std::chrono::duration_cast<std::chrono::seconds>(clock::now() - _callbackLastPurged)
                     .count() < 10;
