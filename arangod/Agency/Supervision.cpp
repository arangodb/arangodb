  ...
  if (envelope->slice()[0].length() > 0) {
    ...
    if (!res.accepted || (res.indices.size() == 1 && res.indices[0] == 0)) {
      LOG_TOPIC("1232b", INFO, Logger::SUPERVISION)
          << "Failed to remove old transfer jobs: " << envelope->toJson();
    }
  }
}
...
bool Supervision::verifyServerRebootID(std::string const& serverID,
  ...
  LOG_TOPIC("44432", DEBUG, Logger::SUPERVISION)
      << "verifyServerRebootID: coordinatorID=" << serverID
      << " health=" << health;
...
void Supervision::resourceCreatorLost(
  ...
  if (rebootID && coordinatorID) {
    keepResource = Supervision::verifyServerRebootID(*coordinatorID, *rebootID,
                                                     coordinatorFound);
