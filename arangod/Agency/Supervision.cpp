...
struct HealthRecord {
  ...
  HealthRecord(HealthRecord const& other) { *this = other; }
...
std::string Supervision::serverHealth(std::string const& serverName) {
  ...
  return (_snapshot.has(serverStatus)) ? _snapshot(serverStatus).getString()
                                       : std::string();
}
...
void Supervision::handleShutdown() {
  ...
  auto const& serversRegistered = _snapshot(currentServersRegisteredPrefix).children();
...
void Supervision::workJobs() {
  ...
  for (auto const& todoEnt : _snapshot(toDoPrefix).children()) {
    JobContext(TODO, (*todoEnt.second)("jobId").getString(), _snapshot, _agent).run();
  }

  for (auto const& pendEnt : _snapshot(pendingPrefix).children()) {
    JobContext(PENDING, (*pendEnt.second)("jobId").getString(), _snapshot, _agent)
        .run();
  }
}
          ...
          if (actualReplicationFactor != replicationFactor) {
            ...
            for (auto const& pair : todo) {
              ...
              if (job->has("type") &&
                  ...
                  job->has("shard") && (*job)("shard").getString() == shard_.first) {
                found = true;
                LOG_TOPIC(DEBUG, Logger::SUPERVISION)
                    << "already found "
  ...
  if (targetNumDBServers < availServers.size()) {
    ...
    for (auto const& database : databases) {
      for (auto const& collptr : database.second->children()) {
        ...
        if (node.has("replicationFactor") && node("replicationFactor").isUInt()) {
