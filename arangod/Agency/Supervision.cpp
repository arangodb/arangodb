...
static std::string const CONTACT_SUPPORT = " Please report to ArangoDB support immediately!";
static std::string const UPGRADE_FAILED =
  "Failed to upgrade agency. Cluster supervision impared. Supervision cannot "
  "perform its tasks.";
...
void Supervision::upgradeAgency() noexcept {
  ...
  try {
    ...
    report.push_back(UPGRADE_FAILED); // Could throw, but then let's die
    LOG_TOPIC(ERR, Logger::SUPERVISION)
      << UPGRADE_FAILED << ": " << e.what() << CONTACT_SUPPORT;
  }
...
std::vector<check_t> Supervision::check(std::string const& type) noexcept {
    ...
    for (auto const& machine : machinesPlanned) {
      try {
        ...
        LOG_TOPIC(WARN, Logger::AGENCY)
          << "Failed to check machine state for " << machine.first << ": "
          << e.what();
      }
    } // for
  } catch (std::exception const& e) {
    LOG_TOPIC(WARN, Logger::SUPERVISION) <<
      "Failed to check for " << type << ": " << e.what();
  }
...
bool Supervision::updateSnapshot() noexcept {
  _lock.assertLockedByCurrentThread();
...
bool Supervision::doChecks() noexcept {
  _lock.assertLockedByCurrentThread();
    ...
    while (!this->isStopping()) {

      {
        ...
        if (_agent->leading() && (_jobId == 0 || _jobId == _jobIdMax)) {
          if (!getUniqueIds()) { // Without unique ids, we cannot keep working
            continue;
          }
        }

        updateSnapshot(); // noexcept

        if (_agent->leading()) {

          if (!_upgraded) {
            upgradeAgency();
            if (!_upgraded) { // No point doing checks, if agency is not in order
              continue; 
            }
          }
...
// Guarded by caller
void Supervision::workJobs() noexcept {

  _lock.assertLockedByCurrentThread();

  for (auto const& todoEnt : _snapshot(toDoPrefix).children()) {
    if ((*todoEnt.second).has("jobId")) {
      auto const& jobId = (*todoEnt.second)("jobId").getString();
      try {
        JobContext(TODO, jobId, _snapshot, _agent).run();
      } catch (std::exception const& e) {
        LOG_TOPIC(WARN, Logger::SUPERVISION)
          << "Failed to progress todo " << jobId << ": " << e.what()
          << CONTACT_SUPPORT;
      }
    }
  }
  ...
  for (auto const& pendEnt : _snapshot(pendingPrefix).children()) {
    if ((*pendEnt.second).has("jobId")) {
      auto const& jobId = (*pendEnt.second)("jobId").getString();
      try {
        JobContext(PENDING, jobId, _snapshot, _agent).run();
      } catch (std::exception const& e) {
        LOG_TOPIC(WARN, Logger::SUPERVISION)
          << "Failed to progress pending job " << jobId << ": " << e.what()
          << CONTACT_SUPPORT;
      }
    }
  }
...
void Supervision::enforceReplication() {
  try {
    ...
    LOG_TOPIC(WARN, Logger::SUPERVISION) <<
      "Enforcement of replication failed: " << e.what() << CONTACT_SUPPORT;
  }
}
...
void Supervision::shrinkCluster() {
  ...
  try {
    ...
    LOG_TOPIC(WARN, Logger::SUPERVISION)
      << "Shrinking cluster failed. " << e.what() << CONTACT_SUPPORT;
  }
}
...
bool Supervision::getUniqueIds() noexcept {
  _lock.assertLockedByCurrentThread();
  ...
  try {
    ...
    if (ret.accepted) {
      try {
        _jobIdMax = ret.result->slice()[1].get(
          std::vector<std::string>({"arango", "Sync", "LatestID"})).getUInt();
        _jobId = _jobIdMax - n;
      } catch (std::exception const& e) {
        LOG_TOPIC(ERR, Logger::SUPERVISION)
    ...
    LOG_TOPIC(ERR, Logger::SUPERVISION)
      << "Failed to acquire cluster-wide unique ids: " << e.what()
      << CONTACT_SUPPORT;
    return false;
  }
