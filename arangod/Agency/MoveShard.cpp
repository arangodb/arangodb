...
bool MoveShard::start(bool&) {
  ...
  if (_snapshot.has(blockedServersPrefix + _to)) {
    LOG_TOPIC("de054", DEBUG, Logger::SUPERVISION)
        << "server " << _to
        << " is currently"
           " locked, not starting MoveShard job "
        << _jobId;
    return false;
  }
  ...
  if (planned.isArray()) {
    ...
    finish("", "", false,  std::string("shard ") + _shard + " has no planned database servers");
    TRI_ASSERT(false);
  }
  ...
  // Get todo entry
  {
    ...
    if (_jb == nullptr) {
      ...
      if (!tmp_todo.second) {
        ...
        LOG_TOPIC("2482a", INFO, Logger::SUPERVISION) << "Failed to get key " + toDoPrefix + _jobId +
  ...
  // Enter pending, remove todo, block toserver
  {
...

    {
      ...
      doForAllShards(_snapshot, _database, shardsLikeMe,
                     [this, &pending, &failed](Slice plan, Slice current, std::string& planPath, std::string& curPath) {
  ...
  if (plan[0].copyString() == _from) {
    ...
    // We need to ask the old leader to retire:
    {
      VPackArrayBuilder trxArray(&trx);
      {
        ...
        doForAllShards(_snapshot, _database, shardsLikeMe,
                       [this, &trx, &pre, &failed](Slice plan, Slice current, std::string& planPath, std::string& curPath) {
  ...
  // Now look after a PENDING job:
  {
    VPackArrayBuilder arrayForTransactionPair(&trx);
    {
      ...
      if (_isLeader) {
        ...
        doForAllShards(_snapshot, _database, shardsLikeMe,
                       [this, &trx, &failed](Slice plan, Slice current, std::string& planPath, std::string& curPath) {
