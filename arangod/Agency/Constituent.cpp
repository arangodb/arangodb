...
bool Constituent::vote(term_t termOfPeer, std::string const& id,
  ...
  if (_votedFor != NO_LEADER) {  // already voted in this term
    if (_votedFor == id) {
      ...
      _lastHeartbeatSeen = TRI_microtime();
      LOG_TOPIC(TRACE, Logger::AGENCY) << "setting last heartbeat time to now, since we repeated a vote grant: " << _lastHeartbeatSeen;
      return true;
    }
    LOG_TOPIC(DEBUG, Logger::AGENCY)
  ...
  if (prevLogTerm > myLastLogEntry.term ||
    ...
    // to establish itself as a leader, before we call for another election:
    _lastHeartbeatSeen = TRI_microtime();
    LOG_TOPIC(TRACE, Logger::AGENCY) << "setting last heartbeat time to now, since we granted a vote: " << _lastHeartbeatSeen;
    termNoLock(_term, id);
    return true;
  }
  LOG_TOPIC(DEBUG, Logger::AGENCY)
