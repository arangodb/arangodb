...
v8::Handle<v8::Value> V8ClientConnection::sendData (int method,
  ...
  LOGGER_TRACE << "sendData(" << method << ", " << location << ")";
  ...
  if (!result->isComplete()) {
    ...
    message << "\"error\":" << result->getResultType() << ",";
    ...
    if (result->getBody().str().length() == 0) {
      ...
      result->getBody() << "\"error\":" << result->getHttpReturnCode() << ",";        
