...
int H1Connection<ST>::on_headers_complete(llhttp_t* parser) try {
  H1Connection<ST>* self = static_cast<H1Connection<ST>*>(parser->data);
  FUERTE_LOG_HTTPTRACE << "on_headers_complete this=" << self << "\n";
...
int H1Connection<ST>::on_message_complete(llhttp_t* parser) try {
  FUERTE_LOG_HTTPTRACE << "on_message_complete this=" << parser->data << "\n";
...
void H1Connection<ST>::finishConnect() {
  ...
  if (this->_state.compare_exchange_strong(exp, Connection::State::Connected)) {
    ...
    FUERTE_LOG_ERROR << (void*) this << " => " << this->endpoint() << " finishConnect: found state other than 'Connecting': "
                     << static_cast<int>(exp);
...
void H1Connection<ST>::asyncReadCallback(asio_ns::error_code const& ec) {
  ...
  for (auto const& buffer : buffers) {
    ...
    if (err != HPE_OK) {
      ...
      msg.append(llhttp_errno_name(err))
          ...
          .append("'");
      FUERTE_LOG_ERROR << (void*) this << " => " << this->endpoint() << " " << msg << ", this=" << this << "\n";
      // will cleanup _item
  ...
  if (_messageComplete) {
    ...
    if (_item->request->header.restVerb == RestVerb::Head) {
    ...
    try {
      ...
      FUERTE_LOG_ERROR << (void*) this << " => " << this->endpoint() << " unhandled exception in fuerte callback\n";
    }
