...
#if BN_BITS2 == 64
  assert(BN_set_word(bn, v));
#else
  static_assert(BN_BITS2 == 32, "at_least_32_bit_openssl_build_needed");
  assert(BN_set_word(bn, static_cast<uint32_t>(v >> 32)));
  assert(BN_lshift(bn, bn, 32));
  assert(BN_add_word(bn, static_cast<uint32_t>(v)));
#endif
}
...
inline static uint64_t BN_ext_get_uint64(const BIGNUM* bn) {
  assert(BN_num_bytes(bn) <= sizeof(uint64_t));
#if BN_BITS2 == 64
...
ExactFloat ExactFloat::RoundToPowerOf2(int bit_exp, RoundingMode mode) const {
  assert(bit_exp >= kMinExp - kMaxPrec);
  assert(bit_exp <= kMaxExp);
  ...
  assert(BN_rshift(&r.bn_, &bn_, shift));
  if (increment) {
    assert(BN_add_word(&r.bn_, 1));
  }
  r.sign_ = sign_;
  r.Canonicalize();
  return r;
...
int ExactFloat::GetDecimalDigits(int max_digits, std::string* digits) const {
  ...
  if (bn_exp_ >= 0) {
    ...
    assert(BN_lshift(bn, &bn_, bn_exp_));
    bn_exp10 = 0;
  } else {
    // Set bn = bn_ * (5 ** -bn_exp_) and bn_exp10 = bn_exp_.  This is
    // equivalent to the original value of (bn_ * (2 ** bn_exp_)).
    BIGNUM* power = BN_new();
    assert(BN_set_word(power, -bn_exp_));
    assert(BN_set_word(bn, 5));
    BN_CTX* ctx = BN_CTX_new();
    assert(BN_exp(bn, bn, power, ctx));
    assert(BN_mul(bn, bn, &bn_, ctx));
    BN_CTX_free(ctx);
    BN_free(power);
    bn_exp10 = bn_exp_;
  }
...
ExactFloat ExactFloat::SignedSum(int a_sign, const ExactFloat* a,
  ...
  ExactFloat r;
  if (a->bn_exp_ > b->bn_exp_) {
    assert(BN_lshift(&r.bn_, &a->bn_, a->bn_exp_ - b->bn_exp_));
    a = &r;  // The only field of "a" used below is bn_.
  }
  r.bn_exp_ = b->bn_exp_;
  if (a_sign == b_sign) {
    assert(BN_add(&r.bn_, &a->bn_, &b->bn_));
    r.sign_ = a_sign;
  } else {
    ...
    assert(BN_sub(&r.bn_, &a->bn_, &b->bn_));
    if (BN_is_zero(&r.bn_)) {
      r.sign_ = +1;
...
void ExactFloat::Canonicalize() {
  ...
  if (my_exp < kMinExp || BN_is_zero(&bn_)) {
    ...
    assert(!BN_is_zero(&bn_));
    int shift = BN_ext_count_low_zero_bits(&bn_);
    if (shift > 0) {
      assert(BN_rshift(&bn_, &bn_, shift));
      bn_exp_ += shift;
    }
  }
...
ExactFloat operator*(const ExactFloat& a, const ExactFloat& b) {
  ...
  BN_CTX* ctx = BN_CTX_new();
  assert(BN_mul(&r.bn_, &a.bn_, &b.bn_, ctx));
  BN_CTX_free(ctx);
  r.Canonicalize();
  return r;
}
...
int ExactFloat::ScaleAndCompare(const ExactFloat& b) const {
  assert(is_normal() && b.is_normal() && bn_exp_ >= b.bn_exp_);
  ExactFloat tmp = *this;
  assert(BN_lshift(&tmp.bn_, &tmp.bn_, bn_exp_ - b.bn_exp_));
...
T ExactFloat::ToInteger(RoundingMode mode) const {
  static_assert(sizeof(T) <= sizeof(uint64_t), "max_64_bits_supported");
  ...
  if (!r.is_inf()) {
    ...
    if (r.exp() < 64) {
      int64_t value = BN_ext_get_uint64(&r.bn_) << r.bn_exp_;
...
ExactFloat ExactFloat::Unimplemented() {
  std::cerr << "Unimplemented ExactFloat method called";
  assert(false);
  return NaN();
}
