  ...
  class column final : public iresearch::columnstore_writer::column_output {
   public:
    ...
    void flush() {
      ...
      column_props_ &= column_index_.flush(blocks_index_.stream, ctx_->buf_);
   ...
   private:
    ...
    ColumnProperty column_props_{ CP_DENSE | CP_FIXED | CP_MASK }; // aggregated column block index properties
...
bool reader::prepare(
  ...
  for (size_t i = 0, size = columns.capacity(); i < size; ++i) {
    ...
    if (props > IRESEARCH_COUNTOF(g_column_factories)) {
      IR_FRMT_ERROR(
        "Failed to open column id=" IR_SIZE_T_SPECIFIER ", got invalid properties=%d",
    ...
    if (!column || !column->read(*stream, buf)) {
      IR_FRMT_ERROR("Failed to load blocks index for column id=" IR_SIZE_T_SPECIFIER, i);
      return false;
    }
