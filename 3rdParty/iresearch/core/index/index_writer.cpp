...
bool add_document_mask_modified_records(
  ...
  if (!reader) {
    throw index_error(string_utils::to_string(
        "while adding document mask modified records to document_mask of "
...
////////////////////////////////////////////////////////////////////////////////
bool add_document_mask_modified_records(
    modification_contexts_ref& modifications, flush_segment_context& ctx,
  ...
  if (!reader) {
    throw index_error(string_utils::to_string(
        "while adding document mask modified records to flush_segment_context "
        "of segment '%s', error: failed to open segment",
...
const std::string& write_document_mask(directory& dir, segment_meta& meta,
  ...
  const auto& file = *meta.files.emplace(mask_writer->filename(meta))
                          .first;  // new/expected filename
...
index_writer::
    active_segment_context::active_segment_context(segment_context_ptr ctx,
                                                   std::atomic<size_t>&
                                                       segments_active,
                                                   flush_context& flush_ctx,
                                                   size_t
                                                       pending_segment_context_offset /*= std::numeric_limits<size_t>::max()*/) noexcept
    : ctx_(ctx),
      lock_{ctx_->flush_mutex_},
...
index_writer::flush_context_ptr
index_writer::documents_context::update_segment() {
  ...
  if (writer.initialized()) {
    ...
    IR_FRMT_INFO("Flushing segment '%s', docs=" IR_SIZE_T_SPECIFIER
...
void index_writer::flush_context::emplace(active_segment_context&& segment,
                                          size_t base, size_t last_tick) {
  if (!segment.ctx_) {
    return;  // nothing to do
  }

  {
    auto& trxTicks = segment.ctx_->trxTicks;
    auto& trxTicksWithCtx = segment.ctx_->trxTicksWithCtx;
    trxTicksWithCtx.emplace_back(last_tick, segment.flush_ctx_);
    trxTicks.push_back(last_tick);
    segment.ctx_->tick_ = trxTicks.back();
  }
  ...
  assert(  // failure may indicate a dangling 'document' instance
      ...
      || (this == segment.flush_ctx_ && segment.ctx_->dirty_ &&
          segment.ctx_.use_count() ==
              1)  // +1 for 'active_segment_context::ctx_' (flush_context
                  // switching made a full-circle)
      || (this == segment.flush_ctx_ && !segment.ctx_->dirty_ &&
          segment.ctx_.use_count() ==
...

  {
    ...
    if (this != segment.flush_ctx_ || ctx.dirty_) {
      ...
      std::cerr << "EMPLACE " << last_tick << " TO "
                << size_t(segment.flush_ctx_) << "\n";

      // std::cerr << "PUSH " << size_t(segment.flush_ctx_) << "\n";
      ...
      if (segment.flush_ctx_ && this != segment.flush_ctx_) {
        ...
        assert(segment.flush_ctx_
                   ->pending_segment_contexts_
                       [segment.pending_segment_context_offset_]
                   .segment_ ==
               segment.ctx_);  // thread-safe because pending_segment_contexts_
                               // is a deque
    ...
    if (generation_base == std::numeric_limits<size_t>::max()) {
      if (segment.flush_ctx_ && this != segment.flush_ctx_) {
        std::cerr << "VAN\n";
  ...
  for (auto i = ctx.uncomitted_modification_queries_,
            count = ctx.modification_queries_.size();
       i < count; ++i) {
    // must be < modification_count since inserts
    // come after modification
    if (!(ctx.modification_queries_[i].generation < modification_count)) {
      std::cerr << "FCK 1\n mod_gen=" << ctx.modification_queries_[i].generation
                << " mod_count=" << modification_count << "\n";
    }
    ctx.modification_queries_[i].gens.push_back(
        ctx.modification_queries_[i].generation);
  ...
  for (auto i = uncomitted_doc_id_begin - doc_limits::min(),
    ...
    // can == modification_count if inserts come
    // after modification
    if (ctx.flushed_update_contexts_[i].generation <= modification_count) {
      std::cerr << "FCK 2\n flushed_update_context_gen="
  ...
  if (!(uncomitted_doc_id_begin - doc_limits::min() >=
        ctx.flushed_update_contexts_.size())) {
    std::cerr << "FCK 3\n uncomitted_doc_id_begin="
              << uncomitted_doc_id_begin - doc_limits::min()
              << " flushed_updated_context_count="
              << ctx.flushed_update_contexts_.size() << "\n";
  }
  if (!(uncomitted_doc_id_begin - doc_limits::min() <=
  ...
  for (auto doc_id =
                uncomitted_doc_id_begin - ctx.flushed_update_contexts_.size(),
            doc_id_end = writer_docs + doc_limits::min();
       doc_id < doc_id_end; ++doc_id) {
    assert(doc_id <= std::numeric_limits<doc_id_t>::max());
    ...
    // can == modification_count if inserts come
    // after modification
    if (!(writer.doc_context(doc_id).generation <= modification_count)) {
      std::cerr << "FCK 5\n doc_gen=" << writer.doc_context(doc_id).generation
                << " mod_count=" << modification_count << "\n";
    }
  ...
  if (!ctx.dirty_) {
    ...
    // 'pending_segment_context::segment_'
    if (auto uc = segment.ctx_.use_count(); uc != 2) {
      std::cerr << "FCK*6\n uc=" << uc;
    }
...
index_writer::segment_context::segment_context(
    ...
    : active_count_(0),
      ...
      writer_(
          segment_writer::make(dir_, column_info, feature_info, comparator)) {
...
index_writer::ptr index_writer::make(
    ...
    if (OM_CREATE == mode ||
      ...
      try {
        ...
        if (index_exists) {
          ...
          meta.clear();
          meta.last_gen_ = type_limits<type_t::index_gen_t>::
              invalid();  // this meta is for a totaly new index
        }
  ...
  PTR_NAMED(
      index_writer, writer, std::move(lock), std::move(lockfile_ref), dir,
      codec, opts.segment_pool_size, segment_options(opts), opts.comparator,
...
index_writer::~index_writer() noexcept {
  ...
  pending_state_.reset();  // reset pending state (if any) before destroying
                           // flush contexts
...
index_writer::consolidation_result index_writer::consolidate(
  ...
  // validate candidates
  {
    ...
    for (const auto& segment : *committed_meta) {
      const auto it =
          std::lower_bound(candidates.begin(), not_found, &segment.meta);
  ...
  // commit merge
  {
    ...
    if (pending_state_) {
      ...
      IR_FRMT_TRACE("Consolidation id='" IR_SIZE_T_SPECIFIER
                    ...
                    run_id, consolidation_meta.name.c_str(),
      ...
      if (res.first) {
        ...
        if (!map_removals(mappings, merger, cached_readers_, docs_mask)) {
          ...
          IR_FRMT_DEBUG(
              ...
              "' for segment '%s', "
              "due removed documents still present the consolidation "
              "candidates",
              run_id, consolidation_segment.meta.name.c_str());
  ...
  if (!shared) {
    for (;;) {
      auto lock = make_unique_lock(
          ctx->flush_mutex_);  // lock ctx exchange (write-lock)
...
index_writer::active_segment_context index_writer::get_segment_context(
  ...
  auto segment_ctx = segment_writer_pool_
                         .emplace(dir_, std::move(meta_generator), column_info_,
  ...
  size_t i;
  {
    std::lock_guard l{ctx.mutex_};
    i = ctx.pending_segment_contexts_
            .emplace_back(std::move(segment_ctx),
                          ctx.pending_segment_contexts_.size())
            .value;
  }

  return active_segment_context(segment_ctx, segments_active_, ctx, i);
}

std::pair<std::vector<std::unique_lock<std::mutex>>, uint64_t>
index_writer::flush_pending(
  ...
  size_t kkk = ctx.pending_segment_contexts_.size();
  size_t i = 0;

  std::cerr << "CURRENTLY FLUSHING 1 " << size_t(&ctx) << "\n";
  std::vector<size_t> numTicks;
  ...
  for (auto& entry : ctx.pending_segment_contexts_) {
    ...
    numTicks.push_back(entry.segment_->trxTicks.size());
  }

  std::vector<size_t> rdbticks;
  uint64_t rdbMax = 0;

  uint64_t maxBefore = 0;
  for (auto& entry : ctx.pending_segment_contexts_) {
    maxBefore = std::max(maxBefore, entry.segment_->tick_.load());
  }

  if (f) {
    rdbMax = f(maxBefore, rdbticks);
  }

  if (!std::is_sorted(rdbticks.begin(), rdbticks.end())) {
    std::cerr << "PUPA\n";
  }

  std::set<size_t> sortedTicks{rdbticks.begin(), rdbticks.end()};
  std::map<size_t, flush_context*> tickToCtx;
  std::vector<flush_context::pending_segment_context*> ptrs;
  for (auto& ccc : ctx.pending_segment_contexts_) {
    ptrs.push_back(&ccc);
  }

  if (ctx.pending_segment_contexts_.size() != segment_flush_locks.size()) {
    std::cerr << "COUNT MISMATCH\n";
  }

  std::cerr << "CURRENTLY FLUSHING 2 " << size_t(&ctx) << "\n";

  auto numTicksBegin = std::begin(numTicks);
  for (auto& entry : ctx.pending_segment_contexts_) {
    if (!std::is_sorted(entry.segment_->trxTicks.begin(),
                        entry.segment_->trxTicks.end())) {
      std::cerr << "MASTER\n";
    }

    if (entry.segment_->active_count_.load() ||
        entry.segment_.use_count() != 1) {
      std::cerr << "INVALID REF COUNT\n";
    }

    for (auto t : entry.segment_->trxTicks) {
      sortedTicks.erase(t);
    }

    for (auto& t : entry.segment_->trxTicksWithCtx) {
      tickToCtx[t.first] = t.second;
    }

    std::set<size_t> ss;
    for (auto v : entry.segment_->trxTicks) {
      if (!ss.insert(v).second) {
        std::cerr << "DARKHOLME \n";
      }
    }

    if (entry.segment_->trxTicks.size() != *numTicksBegin) {
      std::cerr << "MISMATCH TICKS DETECTED\n";
    }

    ++numTicksBegin;

    auto myTick = entry.segment_->flush();

    // force a flush of the underlying segment_writer

    max_tick = std::max(myTick, max_tick);
    ...
    ticks.insert(ticks.end(), entry.segment_->trxTicks.begin(),
                 entry.segment_->trxTicks.end());
  }

  uint64_t maxAfter = 0;
  for (auto& entry : ctx.pending_segment_contexts_) {
    maxAfter = std::max(maxAfter, entry.segment_->tick_.load());

    entry.segment_->tick_ = 0;
    entry.segment_->trxTicks.clear();
    // entry.segment_->trxTicksWithCtx.clear();
  }

  for (size_t b = 0; auto& ccc : ctx.pending_segment_contexts_) {
    if (ptrs[b] != &ccc) {
      std::cerr << "EBL\n";
    }
  ...
  if (!sortedTicks.empty()) {
    std::stringstream ss;
    ss << "LEFTOVERS:\n";
    for (auto t : sortedTicks) {
      auto ff = tickToCtx.find(t);
      ss << t << " "
         << size_t(ff != tickToCtx.end()
                       ? ff->second
                       : (flush_context*)(std::numeric_limits<size_t>::max()))
         << "\n";
    }
    std::cerr << ss.str();
  }

  if (maxBefore != max_tick) {
    std::cerr << "TICK MISMATCH flush_pending before=" << maxBefore
              << " after=" << max_tick << "\n";
  }

  if (rdbMax != max_tick) {
    std::cerr << "MAX TICK DIFFERS cocksdb=" << rdbMax << " irs=" << max_tick
              << "\n";
  }

  //  std::string str;
  //  for (auto t : ctx.trxTicks) {
  //    str += std::to_string(t);
  //    str += "\n";
  //  }
  //  if (!str.empty()) {
  ...
  //    const auto num = ++kkk;
  //    std::fstream f{"Flushed_" + std::to_string(num), std::fstream::out};
  //    f << "Flushed ticks: \n";
  //    f << str;
  //    f << "Max: ";
  //    f << std::to_string(max_tick);
  //    f << "\n";
  //  }

  return {std::move(segment_flush_locks), maxBefore};
}
...
index_writer::pending_context_t index_writer::flush_all(
  ...
  const auto [segment_flush_locks, max_tick] =
      flush_pending(*ctx, lock, ticks, f);
  ...
  for (auto& existing_segment : meta_) {
    ...
    if (mask_modified) {
      ...
      segment_mask.emplace(
          existing_segment
              .meta);  // mask segment since write_document_mask(...) will
                       // increment version
      ...
      for (auto& flushed : pending_segment_context.segment_->flushed_) {
        ...
        if (!flushed.meta.live_docs_count  // empty segment_meta
            ...
            || pending_segment_context.doc_id_begin_ - doc_limits::min() >=
                   flushed_docs_count) {  // segment_meta fully after the start
                                          // of this flush_context
          continue;
        }
        ...
        ++flushed.meta.version;  // increment version for next run due to
                                 // documents masked from this run, similar to
                                 // write_document_mask(...)
...
bool index_writer::start(
  ...
  auto to_commit = flush_all(ticks, f);
...
void index_writer::finish() {
  ...
  try {
    ...
    static_assert(!noexcept(committed_state_helper::atomic_store(
