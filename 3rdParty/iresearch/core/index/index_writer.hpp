...
class IRESEARCH_API index_writer : private util::noncopyable {
 private:
  ...
  typedef std::unique_ptr<
      ...
      >
      flush_context_ptr;  // unique pointer required since need ponter
                          // declaration before class declaration e.g. for
                          // 'documents_context'
  ...
  //////////////////////////////////////////////////////////////////////////////
  class IRESEARCH_API active_segment_context
   ...
   public:
    ...
    active_segment_context(
        ...
        size_t pending_segment_context_offset = std::numeric_limits<
            size_t>::max()) noexcept;  // the segment offset in
                                       // flush_ctx_->pending_segments_
 ...
 public:
   ...
   public:
    ...
    bool replace(Filter&& filter, Func func) {
      ...
      for (auto i = rollback.size(); i && rollback.any();) {
        if (rollback.test(--i)) {
          rollback.unset(i);  // if new doc_ids at end this allows to terminate
                              // 'for' earlier
  ...
  struct modification_context {
    ...
    std::vector<size_t> gens{generation};
  ...
  using consolidation_policy_t = std::function<void(
      consolidation_t& candidates, const index_meta& meta,
  ...
  ////////////////////////////////////////////////////////////////////////////
  bool begin(std::vector<size_t>* ticks = nullptr) {
    // cppcheck-suppress unreadVariable
    auto lock = make_lock_guard(commit_lock_);

    std::vector<size_t> tmp;
    return start(ticks ? *ticks : tmp, {});
  }
  ...
  bool commit(
      ...
      std::function<uint64_t(uint64_t, std::vector<size_t>&)> const& f) {
    // cppcheck-suppress unreadVariable
    auto lock = make_lock_guard(commit_lock_);

    std::vector<size_t> tmp;

    const bool modified = start(ticks ? *ticks : tmp, f);
    finish();
    return modified;
  }
 ...
 private:
  ...
  //////////////////////////////////////////////////////////////////////////////
  struct IRESEARCH_API
      segment_context {  // IRESEARCH_API because of
                         // make_update_context(...)/remove(...) used by
                         // documents_context::replace(...)/documents_context::remove(...)
    ...
    std::unordered_set<size_t> poppedTicks;
    std::vector<size_t> trxTicks;
    std::vector<std::pair<size_t, flush_context*>> trxTicksWithCtx;
    ...
    std::atomic<bool>
        dirty_;  // true if flush_all() started processing this segment (this
                 // segment should not be used for any new operations), guarded
                 // by the flush_context::flush_mutex_
    ref_tracking_directory
        dir_;  // ref tracking for segment_writer to allow for easy ref removal
               // on segment_writer reset
    ...
    std::vector<segment_writer::update_context>
        flushed_update_contexts_;  // update_contexts to use with 'flushed_'
                                   // sequentially increasing through all
                                   // offsets (sequential doc_id in 'flushed_'
                                   // == offset +
    ...
    size_t uncomitted_modification_queries_;  // staring offset in
                                              // 'modification_queries_' that is
    ...
    segment_context(segment_context&&) = delete;
    segment_context& operator=(segment_context&&) = delete;
  ...
  typedef std::shared_ptr<std::pair<std::shared_ptr<index_meta>, file_refs_t>>
      committed_state_t;
  typedef atomic_shared_ptr_helper<committed_state_t::element_type>
  ...
  struct flush_context {
    ...
    struct pending_segment_context : public freelist_t::node_type {
      const size_t
          doc_id_begin_;   // starting segment_context::document_contexts_ for
      ...
      pending_segment_context(const std::shared_ptr<segment_context>& segment,
          ...
          : doc_id_begin_(segment->uncomitted_doc_id_begin_),
            ...
            modification_offset_begin_(
                segment->uncomitted_modification_queries_),
    ...
    std::mutex mutex_;  // guard for the current context during struct update
                        // operations, e.g. pending_segments_,
    ...
    std::condition_variable
        pending_segment_context_cond_;  // notified when a segment has been
                                        // freed (guarded by mutex_)

    std::vector<size_t> missing_ticks_;

    std::deque<pending_segment_context>
        pending_segment_contexts_;  // segment writers with data pending for
                                    // next commit (all segments that have been
                                    // used by this flush_context) must be
                                    // std::deque to garantee that element
                                    // memory location does not change for use
                                    // with 'pending_segment_contexts_freelist_'
    freelist_t
    ...
    absl::flat_hash_set<readers_cache::key_t, readers_cache::key_hash_t>
        segment_mask_;  // set of segment names to be removed from the index
                        // upon commit
    ...
    void emplace(active_segment_context&& segment, size_t tick,
                 size_t last_tick);
    void reset() noexcept;
  };  // flush_context
  ...
  struct sync_context : util::noncopyable {
    ...
    std::vector<std::pair<size_t, size_t>>
        segments;  // segments to sync (index within index meta + number of
                   // files to sync)
  };               // sync_context
  ...
  struct pending_context_t {
    flush_context_ptr ctx{
        nullptr,
        nullptr};  // reference to flush context held until end of commit
  ...
  active_segment_context get_segment_context(
      flush_context&
          ctx);  // return a usable segment or a nullptr segment if retry is
                 // required (e.g. no free segments available)

  bool start(std::vector<size_t>&,
             std::function<uint64_t(uint64_t, std::vector<size_t>&)> const&
                 f);  // starts transaction
  ...
  std::mutex commit_lock_;  // guard for cached_segment_readers_, commit_pool_,
                            // meta_ (modification during
                            // commit()/defragment()), paylaod_buf_
