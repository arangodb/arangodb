...
for i in `seq 1 1`; do
    for j in  5; do
        ...
        # search
        /usr/bin/time -v ./bin/iresearch-benchmarks -m search --in ${BENCHMARK_RESOURCES_ROOT}/benchmark.tasks --dir-type mmap --index-dir iresearch.data --max-tasks 1 --repeat 2 --format 1_4simd --threads 1 --scorer=bm25 --scored-terms-limit=16 --csv --topN=100 2> iresearch.stderr.${MAX_LINES}.search.log.$i 1> iresearch.stdout.${MAX_LINES}.search.log.$i &
        IRESEARCH_PID=$!
        wait $IRESEARCH_PID
