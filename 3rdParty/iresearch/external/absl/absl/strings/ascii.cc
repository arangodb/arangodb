...
// zero-initialized.
IRESEARCH_ABSL_DLL const unsigned char kPropertyBits[256] = {
...
IRESEARCH_ABSL_DLL const char kToLower[256] = {
...
// identity of the passed character.
IRESEARCH_ABSL_DLL const char kToUpper[256] = {
  '\x00', '\x01', '\x02', '\x03', '\x04', '\x05', '\x06', '\x07',
  '\x08', '\x09', '\x0a', '\x0b', '\x0c', '\x0d', '\x0e', '\x0f',
  '\x10', '\x11', '\x12', '\x13', '\x14', '\x15', '\x16', '\x17',
  '\x18', '\x19', '\x1a', '\x1b', '\x1c', '\x1d', '\x1e', '\x1f',
...
void AsciiStrToLower(std::string* s) {
  for (auto& ch : *s) {
    ch = iresearch_absl::ascii_tolower(ch);
  }
}

void AsciiStrToUpper(std::string* s) {
  for (auto& ch : *s) {
    ch = iresearch_absl::ascii_toupper(ch);
  }
}
...
void RemoveExtraAsciiWhitespace(std::string* str) {
  ...
  for (; input_it < input_end; ++input_it) {
    if (is_ws) {
      ...
      is_ws = iresearch_absl::ascii_isspace(*input_it);
      if (is_ws) --output_it;
    } else {
      is_ws = iresearch_absl::ascii_isspace(*input_it);
    }
