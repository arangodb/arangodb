...
static int LLA_SkiplistLevels(size_t size, size_t base, uint32_t *random) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(level >= 1, "block not big enough for even one level");
  return level;
}
...
static void LLA_SkiplistDelete(AllocList *head, AllocList *e,
  ...
  IRESEARCH_ABSL_RAW_CHECK(e == found, "element not in freelist");
...
struct LowLevelAlloc::Arena {
  ...
  AllocList freelist IRESEARCH_ABSL_GUARDED_BY(mu);
  // Count of allocated blocks
  int32_t allocation_count IRESEARCH_ABSL_GUARDED_BY(mu);
  ...
  // PRNG state
  uint32_t random IRESEARCH_ABSL_GUARDED_BY(mu);
};
...
namespace {
class IRESEARCH_ABSL_SCOPED_LOCKABLE ArenaLock {
 public:
  explicit ArenaLock(LowLevelAlloc::Arena *arena)
      IRESEARCH_ABSL_EXCLUSIVE_LOCK_FUNCTION(arena->mu)
      : arena_(arena) {
#ifndef IRESEARCH_ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
  ...
  ~ArenaLock() { IRESEARCH_ABSL_RAW_CHECK(left_, "haven't left Arena region"); }
  void Leave() IRESEARCH_ABSL_UNLOCK_FUNCTION() {
    arena_->mu.Unlock();
#ifndef IRESEARCH_ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
    if (mask_valid_) {
      const int err = pthread_sigmask(SIG_SETMASK, &mask_, nullptr);
      if (err != 0) {
        IRESEARCH_ABSL_RAW_LOG(FATAL, "pthread_sigmask failed: %d", err);
      }
    }
#endif
    left_ = true;
...
bool LowLevelAlloc::DeleteArena(Arena *arena) {
  ...
  while (arena->freelist.next[0] != nullptr) {
    ...
    IRESEARCH_ABSL_RAW_CHECK(region->header.arena == arena,
                   "bad arena pointer in DeleteArena()");
    IRESEARCH_ABSL_RAW_CHECK(size % arena->pagesize == 0,
                   "empty arena has non-page-aligned block size");
    IRESEARCH_ABSL_RAW_CHECK(reinterpret_cast<uintptr_t>(region) % arena->pagesize == 0,
                   "empty arena has non-page-aligned block");
    int munmap_result;
#ifdef _WIN32
    munmap_result = VirtualFree(region, 0, MEM_RELEASE);
    IRESEARCH_ABSL_RAW_CHECK(munmap_result != 0,
...
#else
    munmap_result = munmap(region, size);
#endif  // IRESEARCH_ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
    if (munmap_result != 0) {
      IRESEARCH_ABSL_RAW_LOG(FATAL, "LowLevelAlloc::DeleteArena: munmap failed: %d",
                   errno);
    }
#endif  // _WIN32
  }
...
static inline uintptr_t CheckedAdd(uintptr_t a, uintptr_t b) {
  uintptr_t sum = a + b;
  IRESEARCH_ABSL_RAW_CHECK(sum >= a, "LowLevelAlloc arithmetic overflow");
  return sum;
}
...
static AllocList *Next(int i, AllocList *prev, LowLevelAlloc::Arena *arena) {
  IRESEARCH_ABSL_RAW_CHECK(i < prev->levels, "too few levels in Next()");
  AllocList *next = prev->next[i];
  if (next != nullptr) {
    IRESEARCH_ABSL_RAW_CHECK(
        next->header.magic == Magic(kMagicUnallocated, &next->header),
        "bad magic number in Next()");
    IRESEARCH_ABSL_RAW_CHECK(next->header.arena == arena, "bad arena pointer in Next()");
    if (prev != &arena->freelist) {
      IRESEARCH_ABSL_RAW_CHECK(prev < next, "unordered freelist");
      IRESEARCH_ABSL_RAW_CHECK(reinterpret_cast<char *>(prev) + prev->header.size <
...
static void AddToFreelist(void *v, LowLevelAlloc::Arena *arena) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(f->header.magic == Magic(kMagicAllocated, &f->header),
                 "bad magic number in AddToFreelist()");
  IRESEARCH_ABSL_RAW_CHECK(f->header.arena == arena,
...
void LowLevelAlloc::Free(void *v) {
  if (v != nullptr) {
    ...
    IRESEARCH_ABSL_RAW_CHECK(arena->allocation_count > 0, "nothing in arena to free");
...
static void *DoAllocWithArena(size_t request, LowLevelAlloc::Arena *arena) {
  ...
  if (request != 0) {
    ...
    for (;;) {      // loop until we find a suitable region
      ...
      IRESEARCH_ABSL_RAW_CHECK(new_pages != nullptr, "VirtualAlloc failed");
#else
#ifndef IRESEARCH_ABSL_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
      if ((arena->flags & LowLevelAlloc::kAsyncSignalSafe) != 0) {
      ...
      if (new_pages == MAP_FAILED) {
        IRESEARCH_ABSL_RAW_LOG(FATAL, "mmap error: %d", errno);
      }
    ...
    s->header.magic = Magic(kMagicAllocated, &s->header);
    IRESEARCH_ABSL_RAW_CHECK(s->header.arena == arena, "");
    arena->allocation_count++;
    section.Leave();
    result = &s->levels;
  }
  IRESEARCH_ABSL_ANNOTATE_MEMORY_IS_UNINITIALIZED(result, request);
  return result;
}
...
void *LowLevelAlloc::AllocWithArena(size_t request, Arena *arena) {
  IRESEARCH_ABSL_RAW_CHECK(arena != nullptr, "must pass a valid arena");
