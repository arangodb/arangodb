...
using iresearch_absl::base_internal::CurrentThreadIdentityIfPresent;
using iresearch_absl::base_internal::PerThreadSynch;
using iresearch_absl::base_internal::SchedulingGuard;
using iresearch_absl::base_internal::ThreadIdentity;
using iresearch_absl::synchronization_internal::GetOrCreateCurrentThreadIdentity;
using iresearch_absl::synchronization_internal::GraphCycles;
using iresearch_absl::synchronization_internal::GraphId;
using iresearch_absl::synchronization_internal::InvalidGraphId;
using iresearch_absl::synchronization_internal::KernelTimeout;
using iresearch_absl::synchronization_internal::PerThreadSem;
...
#if defined(IRESEARCH_ABSL_HAVE_THREAD_SANITIZER)
constexpr OnDeadlockCycle kDeadlockDetectionDefault = OnDeadlockCycle::kIgnore;
#else
constexpr OnDeadlockCycle kDeadlockDetectionDefault = OnDeadlockCycle::kAbort;
#endif

IRESEARCH_ABSL_CONST_INIT std::atomic<OnDeadlockCycle> synch_deadlock_detection(
    kDeadlockDetectionDefault);
IRESEARCH_ABSL_CONST_INIT std::atomic<bool> synch_check_invariants(false);

IRESEARCH_ABSL_INTERNAL_ATOMIC_HOOK_ATTRIBUTES
    iresearch_absl::base_internal::AtomicHook<void (*)(int64_t wait_cycles)>
        submit_profile_data;
IRESEARCH_ABSL_INTERNAL_ATOMIC_HOOK_ATTRIBUTES iresearch_absl::base_internal::AtomicHook<void (*)(
    const char *msg, const void *obj, int64_t wait_cycles)>
    mutex_tracer;
IRESEARCH_ABSL_INTERNAL_ATOMIC_HOOK_ATTRIBUTES
    iresearch_absl::base_internal::AtomicHook<void (*)(const char *msg, const void *cv)>
        cond_var_tracer;
IRESEARCH_ABSL_INTERNAL_ATOMIC_HOOK_ATTRIBUTES iresearch_absl::base_internal::AtomicHook<
    bool (*)(const void *pc, char *out, int out_size)>
    symbolizer(iresearch_absl::Symbolize);
...
struct IRESEARCH_ABSL_CACHELINE_ALIGNED MutexGlobals {
  iresearch_absl::once_flag once;
  int num_cpus = 0;
  int spinloop_iterations = 0;
};

static const MutexGlobals& GetMutexGlobals() {
  IRESEARCH_ABSL_CONST_INIT static MutexGlobals data;
  iresearch_absl::base_internal::LowLevelCallOnce(&data.once, [&]() {
    data.num_cpus = iresearch_absl::base_internal::NumCPUs();
...
int MutexDelay(int32_t c, int mode) {
  ...
  if (c < limit) {
    ...
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_DIVERT(nullptr, 0);
    if (c == limit) {
      // Yield once.
      ...
      // Then wait.
      iresearch_absl::SleepFor(iresearch_absl::Microseconds(10));
      c = 0;
    }
    IRESEARCH_ABSL_TSAN_MUTEX_POST_DIVERT(nullptr, 0);
  }
  return c;
}
}  // namespace synchronization_internal
...
IRESEARCH_ABSL_CONST_INIT static iresearch_absl::base_internal::SpinLock deadlock_graph_mu(
    iresearch_absl::kConstInit, base_internal::SCHEDULE_KERNEL_ONLY);

// Graph used to detect deadlocks.
IRESEARCH_ABSL_CONST_INIT static GraphCycles *deadlock_graph
    IRESEARCH_ABSL_GUARDED_BY(deadlock_graph_mu) IRESEARCH_ABSL_PT_GUARDED_BY(deadlock_graph_mu);
...
IRESEARCH_ABSL_CONST_INIT static iresearch_absl::base_internal::SpinLock synch_event_mu(
    iresearch_absl::kConstInit, base_internal::SCHEDULE_KERNEL_ONLY);
...
static struct SynchEvent {     // this is a trivial hash table for the events
  ...
  int refcount IRESEARCH_ABSL_GUARDED_BY(synch_event_mu);

  // buckets have linear, 0-terminated  chains
  SynchEvent *next IRESEARCH_ABSL_GUARDED_BY(synch_event_mu);
...
} * synch_event[kNSynchEvent] IRESEARCH_ABSL_GUARDED_BY(synch_event_mu);
...
static void PostSynchEvent(void *obj, int ev) {
  ...
  if (e == nullptr || e->log) {
    void *pcs[40];
    int n = iresearch_absl::GetStackTrace(pcs, IRESEARCH_ABSL_ARRAYSIZE(pcs), 1);
    ...
    IRESEARCH_ABSL_RAW_LOG(INFO, "%s%p %s %s", event_properties[ev].msg, obj,
...
static PerThreadSynch *Synch_GetPerThreadAnnotated(Mutex *mu) {
  if (mu) {
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_DIVERT(mu, 0);
  }
  PerThreadSynch *w = Synch_GetPerThread();
  if (mu) {
    IRESEARCH_ABSL_TSAN_MUTEX_POST_DIVERT(mu, 0);
  }
  return w;
}
...
// prevent certain IRESEARCH_ABSL_RAW_CHECK() statements from being triggered when
// re-rentry is detected.  The IRESEARCH_ABSL_RAW_CHECK() statements are those in the
...
static iresearch_absl::Time DeadlineFromTimeout(iresearch_absl::Duration timeout) {
#ifndef _WIN32
  struct timeval tv;
  gettimeofday(&tv, nullptr);
  return iresearch_absl::TimeFromTimeval(tv) + timeout;
#else
...
Mutex::~Mutex() {
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_DESTROY(this, __tsan_mutex_not_static);
}
...
static PerThreadSynch *Enqueue(PerThreadSynch *head,
  ...
  if (head == nullptr) {         // s is the only waiter
    ...
    if (s->next_priority_read_cycles < now_cycles) {
      ...
      if (err != 0) {
        IRESEARCH_ABSL_RAW_LOG(ERROR, "pthread_getschedparam failed: %d", err);
      } else {
...
static PerThreadSynch *DequeueAllWakeable(PerThreadSynch *head,
  ...
  do {
    if (w->wake) {                    // remove this element
      IRESEARCH_ABSL_RAW_CHECK(pw->skip == nullptr, "bad skip in DequeueAllWakeable");
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::Block(PerThreadSynch *s) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(s->waitp != nullptr || s->suppress_fatal_errors,
...
static GraphId GetGraphIdLocked(Mutex *mu)
    IRESEARCH_ABSL_EXCLUSIVE_LOCKS_REQUIRED(deadlock_graph_mu) {
...
static GraphId GetGraphId(Mutex *mu) IRESEARCH_ABSL_LOCKS_EXCLUDED(deadlock_graph_mu) {
...
static void LockEnter(Mutex* mu, GraphId id, SynchLocksHeld *held_locks) {
  ...
  if (i == n) {
    if (n == IRESEARCH_ABSL_ARRAYSIZE(held_locks->locks)) {
...
static char *CurrentStackString(char *buf, int maxlen, bool symbolize) {
  void *pcs[40];
  return StackString(pcs, iresearch_absl::GetStackTrace(pcs, IRESEARCH_ABSL_ARRAYSIZE(pcs), 2), buf,
                     maxlen, symbolize);
}
...
int GetStack(void** stack, int max_depth) {
  return iresearch_absl::GetStackTrace(stack, max_depth, 3);
}
}  // anonymous namespace
...
static GraphId DeadlockCheck(Mutex *mu) {
  ...
  iresearch_absl::base_internal::SpinLockHolder lock(&deadlock_graph_mu);
  ...
  for (int i = 0; i != all_locks->n; i++) {
    ...
    if (!deadlock_graph->InsertEdge(other_node_id, mu_id)) {
      ...
      IRESEARCH_ABSL_RAW_LOG(ERROR, "Potential Mutex deadlock: %s",
      ...
      IRESEARCH_ABSL_RAW_LOG(ERROR, "Acquiring %p    Mutexes held: %s",
                   static_cast<void *>(mu), b->buf);
      IRESEARCH_ABSL_RAW_LOG(ERROR, "Cycle: ");
      int path_len = deadlock_graph->FindPath(
          mu_id, other_node_id, IRESEARCH_ABSL_ARRAYSIZE(b->path), b->path);
      for (int j = 0; j != path_len; j++) {
        ...
        StackString(stack, depth, b->buf + strlen(b->buf),
                    ...
                    symbolize);
        IRESEARCH_ABSL_RAW_LOG(ERROR, "%s", b->buf);
      }
      if (synch_deadlock_detection.load(std::memory_order_acquire) ==
          OnDeadlockCycle::kAbort) {
        deadlock_graph_mu.Unlock();  // avoid deadlock in fatal sighandler
        IRESEARCH_ABSL_RAW_LOG(FATAL, "dying due to potential deadlock");
        return mu_id;
      }
  ...
  if (kDebugMode &&
    ...
    for (int i = 0; i != locks->n; i++) {
      if (locks->locks[i].id == id) {
        ...
        IRESEARCH_ABSL_RAW_LOG(FATAL, "thread should not hold mutex %p %s",
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::Lock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(this, 0);
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(this, 0, 0);
}

ABSL_XRAY_LOG_ARGS(1) void Mutex::ReaderLock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(this, __tsan_mutex_read_lock);
  GraphId id = DebugOnlyDeadlockCheck(this);
  intptr_t v = mu_.load(std::memory_order_relaxed);
  ...
  DebugOnlyLockEnter(this, id);
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(this, __tsan_mutex_read_lock, 0);
}
...
void Mutex::Await(const Condition &cond) {
  if (cond.Eval()) {    // condition already true; nothing to do
    ...
    IRESEARCH_ABSL_RAW_CHECK(this->AwaitCommon(cond, KernelTimeout::Never()),
...
bool Mutex::AwaitWithDeadline(const Condition &cond, iresearch_absl::Time deadline) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(res || t.has_timeout(),
                 "condition untrue on return from Await");
  return res;
}
...
bool Mutex::AwaitCommon(const Condition &cond, KernelTimeout t) {
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(this, TsanFlags(how));
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(this, TsanFlags(how));
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(this, TsanFlags(how));
  this->LockSlowLoop(&waitp, flags);
  bool res = waitp.cond != nullptr ||  // => cond known true from LockSlowLoop
             EvalConditionAnnotated(&cond, this, true, false, how == kShared);
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(this, TsanFlags(how), 0);
  return res;
}

ABSL_XRAY_LOG_ARGS(1) bool Mutex::TryLock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(this, __tsan_mutex_try_lock);
  intptr_t v = mu_.load(std::memory_order_relaxed);
  if ((v & (kMuWriter | kMuReader | kMuEvent)) == 0 &&  // try fast acquire
    ...
    IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(this, __tsan_mutex_try_lock, 0);
    return true;
  }
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(
      this, __tsan_mutex_try_lock | __tsan_mutex_try_lock_failed, 0);
  return false;
}

ABSL_XRAY_LOG_ARGS(1) bool Mutex::ReaderTryLock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(this,
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(this,
                            __tsan_mutex_read_lock | __tsan_mutex_try_lock |
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::Unlock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(this, 0);
  DebugOnlyLockLeave(this);
  intptr_t v = mu_.load(std::memory_order_relaxed);

  if (kDebugMode && ((v & (kMuWriter | kMuReader)) != kMuWriter)) {
    IRESEARCH_ABSL_RAW_LOG(FATAL, "Mutex unlocked when destroyed or not locked: v=0x%x",
  ...
  if (kDebugMode && should_try_cas != (x < y)) {
    ...
    IRESEARCH_ABSL_RAW_LOG(FATAL, "internal logic error %llx %llx %llx\n",
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(this, 0);
}
...
ABSL_XRAY_LOG_ARGS(1) void Mutex::ReaderUnlock() {
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(this, __tsan_mutex_read_lock);
  DebugOnlyLockLeave(this);
  intptr_t v = mu_.load(std::memory_order_relaxed);
  assert((v & (kMuWriter|kMuReader)) == kMuReader);
  if ((v & (kMuReader|kMuWait|kMuEvent)) == kMuReader) {
    ...
    if (mu_.compare_exchange_strong(v, v - clear,
                                    std::memory_order_release,
                                    std::memory_order_relaxed)) {
      IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(this, __tsan_mutex_read_lock);
      return;
    }
  }
  this->UnlockSlow(nullptr /*no waitp*/);  // take slow path
...
static inline bool EvalConditionAnnotated(const Condition *cond, Mutex *mu,
  ...
  if (locking) {
    ...
    // deadlock detector.
    IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(mu, tryflags, 0);
    res = cond->Eval();
    // There is no "try" version of Unlock, so use flags instead of tryflags.
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(mu, flags);
    IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(mu, flags);
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(mu, tryflags);
  } else {
    ...
    IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(mu, flags);
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(mu, flags);
    IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(mu, flags, 0);
    res = cond->Eval();
    IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(mu, flags);
  }
...
static inline bool EvalConditionIgnored(Mutex *mu, const Condition *cond) {
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_DIVERT(mu, 0);
  IRESEARCH_ABSL_ANNOTATE_IGNORE_READS_AND_WRITES_BEGIN();
  bool res = cond->Eval();
  IRESEARCH_ABSL_ANNOTATE_IGNORE_READS_AND_WRITES_END();
  IRESEARCH_ABSL_TSAN_MUTEX_POST_DIVERT(mu, 0);
...
#define RAW_CHECK_FMT(cond, ...)                                   \
  do {                                                             \
    if (IRESEARCH_ABSL_PREDICT_FALSE(!(cond))) {                             \
      IRESEARCH_ABSL_RAW_LOG(FATAL, "Check " #cond " failed: " __VA_ARGS__); \
    }                                                              \
  } while (0)
...
static void CheckForMutexCorruption(intptr_t v, const char* label) {
  ...
  if (IRESEARCH_ABSL_PREDICT_TRUE((w & (w << 3) & (kMuWriter | kMuWrWait)) == 0)) return;
    ...
    if ((v & waitp->how->slow_need_zero) == 0) {
      ...
      if ((v & (kMuSpin|kMuWait)) == 0) {   // no waiters
        ...
        intptr_t nv = (v & zap_desig_waker[flags & kMuHasBlocked] & kMuLow) |
                      kMuWait;
        IRESEARCH_ABSL_RAW_CHECK(new_h != nullptr, "Enqueue to empty list failed");
        ...
        intptr_t wr_wait = 0;
        IRESEARCH_ABSL_RAW_CHECK(new_h != nullptr, "Enqueue to list failed");
        if (waitp->how == kExclusive && (v & kMuReader) != 0) {
...
ABSL_ATTRIBUTE_NOINLINE void Mutex::UnlockSlow(SynchWaitParams *waitp) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(waitp == nullptr || waitp->thread->waitp == nullptr ||
  ...
  for (;;) {
    ...
    if ((v & kMuWriter) != 0 && (v & (kMuWait | kMuDesig)) != kMuWait &&
      ...
      if ((v & kMuWait) == 0) {       // no one to wake
        ...
        IRESEARCH_ABSL_RAW_CHECK(waitp != nullptr,
      ...
      if ((v & kMuReader) != 0 && (h->readers & kMuHigh) > kMuOne) {
        ...
        if (waitp != nullptr) {  // but waitp!=nullptr => must queue ourselves
          ...
          IRESEARCH_ABSL_RAW_CHECK(new_h != nullptr,
      ...
      IRESEARCH_ABSL_RAW_CHECK(old_h == nullptr || h->maybe_unlocking,
      ...
      if (old_h != nullptr &&
        ...
        IRESEARCH_ABSL_RAW_CHECK(old_h->skip == nullptr, "illegal skip from head");
      ...
      if (h->next->waitp->how == kExclusive &&
        ...
        IRESEARCH_ABSL_RAW_CHECK(h->skip == nullptr, "illegal skip from head");
      ...
      IRESEARCH_ABSL_RAW_CHECK(pw->next == w, "pw not w's predecessor");
      ...
      IRESEARCH_ABSL_RAW_CHECK(wake_list != kPerThreadSynchNull,
...
void Mutex::Fer(PerThreadSynch *w) {
  SchedulingGuard::ScopedDisable disable_rescheduling;
  int c = 0;
  IRESEARCH_ABSL_RAW_CHECK(w->waitp->cond == nullptr,
                 "Mutex::Fer while waiting on Condition");
  IRESEARCH_ABSL_RAW_CHECK(!w->waitp->timeout.has_timeout(),
                 "Mutex::Fer while in timed wait");
  IRESEARCH_ABSL_RAW_CHECK(w->waitp->cv_word == nullptr,
...
void Mutex::AssertHeld() const {
  if ((mu_.load(std::memory_order_relaxed) & kMuWriter) == 0) {
    ...
    IRESEARCH_ABSL_RAW_LOG(FATAL, "thread should hold write lock on Mutex %p %s",
                 static_cast<const void *>(this),
                 (e == nullptr ? "" : e->name));
  }
}
...
static void CondVarEnqueue(SynchWaitParams *waitp) {
  ...
  IRESEARCH_ABSL_RAW_CHECK(waitp->thread->waitp == nullptr, "waiting when shouldn't be");
...
bool CondVar::WaitCommon(Mutex *mutex, KernelTimeout t) {
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_UNLOCK(mutex, TsanFlags(mutex_how));
  ...
  IRESEARCH_ABSL_RAW_CHECK(waitp.thread->waitp != nullptr, "not waiting when should be");
  ...
  IRESEARCH_ABSL_TSAN_MUTEX_POST_UNLOCK(mutex, TsanFlags(mutex_how));
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_LOCK(mutex, TsanFlags(mutex_how));
  mutex->Trans(mutex_how);  // Reacquire mutex
  IRESEARCH_ABSL_TSAN_MUTEX_POST_LOCK(mutex, TsanFlags(mutex_how), 0);
  return rc;
}
...
void CondVar::Signal() {
  SchedulingGuard::ScopedDisable disable_rescheduling;
  IRESEARCH_ABSL_TSAN_MUTEX_PRE_SIGNAL(nullptr, 0);
  intptr_t v;
  int c = 0;
  for (v = cv_.load(std::memory_order_relaxed); v != 0;
    ...
    if ((v & kCvSpin) == 0 &&  // attempt to acquire spinlock
      ...
      IRESEARCH_ABSL_TSAN_MUTEX_POST_SIGNAL(nullptr, 0);
      return;
    } else {
      c = synchronization_internal::MutexDelay(c, GENTLE);
    }
...
void ReleasableMutexLock::Release() {
  IRESEARCH_ABSL_RAW_CHECK(this->mu_ != nullptr,
