...
class Builder {
 ...
 public:
  ...
  template<typename T>
  uint8_t* addUnchecked(char const* attrName, std::size_t attrLength,
                        T const& sub) {
    if (_stack.empty()) {
  ...
  inline uint8_t* add(char const* attrName, Value const& sub) {
    return addInternal<Value>(attrName,
                              std::char_traits<char>::length(attrName), sub);
  }
  ...
  inline uint8_t* add(char const* attrName, Slice const& sub) {
    return addInternal<Slice>(attrName,
                              std::char_traits<char>::length(attrName), sub);
  }
  ...
  inline uint8_t* add(char const* attrName, ValuePair const& sub) {
    return addInternal<ValuePair>(
        attrName, std::char_traits<char>::length(attrName), sub);
  }
  ...
  inline uint8_t* add(char const* attrName, Serialize const& sub) {
    return addInternal<Serializable>(
        attrName, std::char_traits<char>::length(attrName), sub._sable);
  }
  ...
  // Add a subvalue into an object from a Value:
  inline uint8_t* addTagged(std::string const& attrName, uint64_t tag,
                            Value const& sub) {
    return addInternalTagged<Value>(attrName, tag, sub);
  }

  inline uint8_t* addTagged(StringRef const& attrName, uint64_t tag,
                            Value const& sub) {
    return addInternalTagged<Value>(attrName, tag, sub);
  }

  inline uint8_t* addTagged(char const* attrName, uint64_t tag,
                            Value const& sub) {
    return addInternalTagged<Value>(
        attrName, std::char_traits<char>::length(attrName), tag, sub);
  }

  inline uint8_t* addTagged(char const* attrName, std::size_t attrLength,
                            uint64_t tag, Value const& sub) {
  ...
  inline uint8_t* addTagged(char const* attrName, uint64_t tag,
                            Slice const& sub) {
    return addInternalTagged<Slice>(
        attrName, std::char_traits<char>::length(attrName), tag, sub);
  }

  inline uint8_t* addTagged(char const* attrName, std::size_t attrLength,
                            uint64_t tag, Slice const& sub) {
  ...
  inline uint8_t* addTagged(char const* attrName, uint64_t tag,
                            ValuePair const& sub) {
    return addInternalTagged<ValuePair>(
        attrName, std::char_traits<char>::length(attrName), tag, sub);
  }

  inline uint8_t* addTagged(char const* attrName, std::size_t attrLength,
                            uint64_t tag, ValuePair const& sub) {
  ...
  inline uint8_t* addTagged(char const* attrName, uint64_t tag,
    ...
    return addInternalTagged<Serializable>(
        attrName, std::char_traits<char>::length(attrName), tag, sub._sable);
  }

  inline uint8_t* addTagged(char const* attrName, std::size_t attrLength,
                            uint64_t tag, Serialize const& sub) {
    return addInternalTagged<Serializable>(attrName, attrLength, tag,
                                           sub._sable);
  }
 ...
 private:
  ...
  template<typename T>
  uint8_t* addInternal(char const* attrName, std::size_t attrLength,
                       T const& sub) {
    bool haveReported = false;
    if (!_stack.empty()) {
  ...
  template<typename T>
  uint8_t* addInternalTagged(std::string const& attrName, uint64_t tag,
                             T const& sub) {
    return addInternalTagged<T>(attrName.data(), attrName.size(), tag, sub);
  }

  template<typename T>
  uint8_t* addInternalTagged(StringRef const& attrName, uint64_t tag,
                             T const& sub) {
    return addInternalTagged<T>(attrName.data(), attrName.size(), tag, sub);
  }

  template<typename T>
  uint8_t* addInternalTagged(char const* attrName, std::size_t attrLength,
                             uint64_t tag, T const& sub) {
    bool haveReported = false;
    if (!_stack.empty()) {
      ValueLength const to = _stack.back().startPos;
      if (VELOCYPACK_UNLIKELY(_start[to] != 0x0b && _start[to] != 0x14)) {
...
struct ObjectBuilder final : public BuilderContainer,
  ...
  ObjectBuilder(Builder* builder, const std::string& attributeName,
...
struct ArrayBuilder final : public BuilderContainer,
  ...
  ArrayBuilder(Builder* builder, const std::string& attributeName,
               bool allowUnindexed = false)
      : BuilderContainer(builder) {
