...
// via skip().
void feed(VectorIndexSampler& s, std::vector<float> const& vector) {
  if (s.wantsItem()) {
    s.consume(vector);
  } else {
    s.skip();
...
TEST(VectorIndexSamplerTest, below_capacity_keeps_everything) {
  VectorIndexSampler s{/*dimension=*/1, /*capacity=*/10, kFixedSeed};
  for (int i = 0; i < 5; ++i) {
...
TEST(VectorIndexSamplerTest, at_capacity_keeps_everything) {
  VectorIndexSampler s{/*dimension=*/1, /*capacity=*/5, kFixedSeed};
  for (int i = 0; i < 5; ++i) {
    feed(s, scalar(static_cast<float>(i)));
  }
  EXPECT_EQ(s.itemsSeen(), 5u);
...
TEST(VectorIndexSamplerTest, over_capacity_size_matches_capacity) {
  constexpr std::size_t k = 10;
  constexpr std::size_t n = 1000;
  VectorIndexSampler s{/*dimension=*/1, /*capacity=*/k, kFixedSeed};
...
TEST(VectorIndexSamplerTest, reservoir_is_subset_of_distinct_inputs) {
...
TEST(VectorIndexSamplerTest, multi_dimensional_slots_are_contiguous) {
  constexpr std::size_t dim = 4;
  constexpr std::size_t k = 8;
  constexpr std::size_t n = 200;
  VectorIndexSampler s{dim, /*capacity=*/k, kFixedSeed};
...
TEST(VectorIndexSamplerTest, fixed_seed_is_deterministic) {
...
TEST(VectorIndexSamplerTest, different_seeds_diverge) {
  constexpr std::size_t k = 32;
  constexpr std::size_t n = 1000;
  auto run = [&](std::uint64_t seed) {
    VectorIndexSampler s{/*dimension=*/1, /*capacity=*/k, seed};
    for (std::size_t i = 0; i < n; ++i) {
      feed(s, scalar(static_cast<float>(i)));
    }
    return std::move(s).release();
...
TEST(VectorIndexSamplerTest, resize_smaller_shrinks_reservoir) {
...
TEST(VectorIndexSamplerTest, resize_larger_is_noop) {
  constexpr std::size_t k = 10;
  VectorIndexSampler s{/*dimension=*/1, /*capacity=*/k, kFixedSeed};
  for (std::size_t i = 0; i < k; ++i) {
    feed(s, scalar(static_cast<float>(i)));
...
TEST(VectorIndexSamplerTest, resize_on_undersized_reservoir_is_noop) {
  // Reservoir never filled past 5 items; resize(10) should leave all 5 in
  // place (new capacity >= current size).
  VectorIndexSampler s{/*dimension=*/1, /*capacity=*/20, kFixedSeed};
...
TEST(VectorIndexSamplerTest, itemsSeen_counts_every_consumed_vector) {
...
TEST(VectorIndexSamplerTest, wants_item_true_through_fill_phase) {
...
TEST(VectorIndexSamplerTest, wants_item_decides_whether_skip_or_consume_runs) {
  ...
  constexpr std::size_t n = 200;
  VectorIndexSampler s{dim, k, kFixedSeed};
