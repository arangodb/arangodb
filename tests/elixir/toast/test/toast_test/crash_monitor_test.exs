defmodule ToastTest.CrashMonitorTest do
  ...
  test "handle_crash sets abort with server_id and signal" do
    CrashMonitor.handle_crash(nil, %{server_id: "db-1", signal: 11, exit_status: 139})
  ...
  test "handle_crash with only exit_status" do
    CrashMonitor.handle_crash(nil, %{server_id: "agent-1", exit_status: 1})
  ...
  test "handle_crash with unknown server" do
    CrashMonitor.handle_crash(nil, %{exit_status: 134, signal: 6})
  ...
  test "handle_crash returns :ok" do
    assert :ok = CrashMonitor.handle_crash(nil, %{server_id: "x"})
  end
