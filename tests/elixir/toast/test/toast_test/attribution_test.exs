defmodule ToastTest.AttributionTest do
  ...
  describe "run/4 — empty inputs" do
    test "no failures, no error, no artifacts returns []" do
      assert [] = Attribution.run(build_test_data(), empty_artifacts(), nil)
    end
  end
  ...
  describe "run/4 — test failures" do
    test "each failure becomes a :test_failure issue" do
      ...
      issues = Attribution.run(test_data, empty_artifacts(), nil)
    ...
    test "failure detail contains the ExUnit.Test struct" do
      ...
      [issue] = Attribution.run(test_data, empty_artifacts(), nil)
  ...
  describe "run/4 — server_crashed error" do
    test "crash with timestamp attributed to test window" do
      ...
      issues =
        Attribution.run(
          build_test_data(),
          empty_artifacts(),
          {:server_crashed, "single1", crash_info},
    ...
    test "crash enriched with coredump analysis" do
      ...
      assert [issue] = issues
      assert issue.type == :crash
      assert issue.detail.signal == "SIGSEGV"
      assert length(issue.detail.threads) == 1
      assert issue.detail.coredump_path == "/tmp/core.1234"
    end
    ...
    test "crash issue produced even when coredump analysis fails" do
      ...
      assert issue.type == :crash
      assert issue.detail.server == "single1"
      # No thread info when analysis fails
      assert issue.detail[:threads] == nil
      assert issue.detail[:signal] == nil
    end
  end

  describe "run/4 — server_unhealthy error" do
    test "produces crash issue with limited info" do
      issues =
        Attribution.run(
          build_test_data(),
          empty_artifacts(),
          {:server_unhealthy, "single1"},
          skip_coredump_analysis: true
        )
  ...
  describe "run/4 — nil error" do
    test "nil error produces no crash issues" do
      assert [] = Attribution.run(build_test_data(), empty_artifacts(), nil)
    end
  end
  ...
  describe "run/4 — sanitizer files" do
    test "each sanitizer file produces a :sanitizer_report issue" do
      ...
      issues = Attribution.run(build_test_data(), artifacts, nil)
  ...
  describe "run/4 — mixed issues" do
    test "test failures + crash + sanitizer all combined" do
      ...
      issues =
        Attribution.run(test_data, artifacts, {:server_crashed, "single1", crash_info},
          skip_coredump_analysis: true
        )
