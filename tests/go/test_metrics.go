...
func main() {
	...
	assert(vstReqCount2 == vstReqCount, "http2 req count did move")
	http1ReqCount = http1ReqCount2

	// Do a few HTTP/2 requests:
	client2 := makeHttp2Client(config)
	for i := 1; i <= 10; i++ {
	...
	assert(http2ReqCount2 >= http2ReqCount + 10, "http2 req count did move")
	assert(vstReqCount2 == vstReqCount, "http2 req count did move")
	http1ReqCount = http1ReqCount2
	http2ReqCount = http2ReqCount2
