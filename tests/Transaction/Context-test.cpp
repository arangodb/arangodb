...
TEST_CASE("TransactionContextTest", "[transaction]") {
  ...
  SECTION("StandaloneSmartContext") {
    ...
    auto params = arangodb::velocypack::Parser::fromJson("{ \"name\": \"testCollection\" }");
    ...
    auto docs = arangodb::velocypack::Parser::fromJson("[{ \"hello\": \"world1\" }, { \"hello\": \"world2\" }]");
    ...
    REQUIRE(trxSlice.isArray());
    REQUIRE(trxSlice.length() == 2);
    ...
    {
      arangodb::aql::Query query(false, vocbase, queryString, bindVars,
      ...
      REQUIRE(aqlSlice.isArray());
      REQUIRE(aqlSlice.length() == 2);
      REQUIRE(aqlSlice.at(0).get("hello").isEqualString("world1"));
    }
    
    REQUIRE(trxSlice.at(1).hasKey(StaticStrings::KeyString));
    ...
    
    {
      ...
      VPackSlice aqlSlice = qres.data->slice();
      REQUIRE(aqlSlice.isArray());
      REQUIRE(aqlSlice.length() == 1);
      REQUIRE(aqlSlice.at(0).get("hello").isEqualString("world2"));
    }
  } // SECTION("StandaloneSmartContext")
}
