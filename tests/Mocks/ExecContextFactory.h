...
struct ExecContextAccessor final : ExecContext {
 ...
 private:
  ExecContextAccessor(ConstructorToken token, AuthMode authMode,
                      VocbasePtr vocbase)
      : ExecContext(token, std::move(authMode), false, std::move(vocbase)) {}
};
