...
std::unique_ptr<fuerte::Response> PreparedRequestResponse::generateResponse() const {
          // TODO what if response is empty;
            ...
            auto code = _response->responseCode();
            (void) code;
...
void PreparedResponseConnectionPool::addPreparedResponses(
    std::pair<std::string, std::string> const& endpoint, std::vector<PreparedRequestResponse> responses) {
  _responses.emplace(endpoint.first + ":" + endpoint.second, std::make_shared<std::vector<PreparedRequestResponse>>(
                                   std::move(responses)));
}
