    ...
    auto res = nextImpl(
        [&](arangodb::LocalDocumentId docId) -> bool {
          auto res = _collection->getPhysical()->read(
              _trx, docId,
              [&](arangodb::LocalDocumentId const& token, arangodb::velocypack::Slice doc, arangodb::velocypack::Slice /*extra*/) -> bool {
 ...
 public:
  ...
  bool nextCoveringImpl(DocumentCallback const& cb, size_t limit) override {
    while (limit && _begin != _end) {
      cb(_begin->first, _begin->second.slice(), arangodb::velocypack::Slice::emptyArraySlice());
      ++_begin;
      --limit;
    }
...
arangodb::Result PhysicalCollectionMock::read(
  ...
  if (it != _documents.end()) {
    cb(it->second.docId(), arangodb::velocypack::Slice(it->second.vptr()), arangodb::velocypack::Slice::emptyArraySlice());
