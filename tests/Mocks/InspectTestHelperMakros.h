...
#define __HELPER_equalsAfterSerializeParseCircle(testee)                    \
  {                                                                         \
    ...
    ASSERT_TRUE(body.ok()) << "Failed to serialize: " << body.error();      \
    auto parsed = parse(body.get());                                        \
    ASSERT_TRUE(parsed.ok()) << "Failed to deserialize " << parsed.error(); \
    EXPECT_EQ(testee, parsed.get())                                         \
        << "SerializeCircle failed on " << body.get().toJson();             \
  }
...
#define __HELPER_assertParsingThrows(attributeName, value)            \
  {                                                                   \
    ...
    EXPECT_FALSE(testee.ok()) << " On body " << body.toJson();        \
  }
