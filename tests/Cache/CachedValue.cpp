...
TEST(CacheCachedValueTest, test_constructor_with_valid_input) {
  ...
  ASSERT_EQ(sizeof(uint64_t), cv->keySize());
  ASSERT_EQ(v.size(), cv->valueSize());
  ASSERT_EQ(sizeof(CachedValue) + padding + sizeof(uint64_t) + v.size(), cv->size());
  ASSERT_EQ(k, *reinterpret_cast<uint64_t const*>(cv->key()));
  ...
  ASSERT_EQ(sizeof(uint64_t), cv->valueSize());
  ASSERT_EQ(sizeof(CachedValue) + padding + sizeof(uint64_t) + v.size(), cv->size());
  ASSERT_EQ(0, memcmp(v.data(), cv->key(), v.size()));
  ASSERT_EQ(k, *reinterpret_cast<uint64_t const*>(cv->value()));
  delete cv;
  ...
  ASSERT_EQ(sizeof(CachedValue) + padding + sizeof(uint64_t), cv->size());
  ASSERT_EQ(k, *reinterpret_cast<uint64_t const*>(cv->key()));
...
TEST(CacheCachedValueTest, copy_should_produce_a_correct_copy) {
  ...
  ASSERT_EQ(sizeof(uint64_t), copy->keySize());
  ASSERT_EQ(v.size(), copy->valueSize());
  ASSERT_EQ(sizeof(CachedValue) + padding + sizeof(uint64_t) + v.size(), copy->size());
  ASSERT_EQ(k, *reinterpret_cast<uint64_t const*>(copy->key()));
...
TEST(CacheCachedValueTest, samekey_method_for_key_comparison_works) {
  ...
  ASSERT_FALSE(cv->sameKey(k2.data(), static_cast<uint32_t>(k2.size())));

  // same length, different key
  ASSERT_FALSE(cv->sameKey(k3.data(), static_cast<uint32_t>(k3.size())));
