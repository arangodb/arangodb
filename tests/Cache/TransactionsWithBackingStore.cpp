...
TEST_CASE("CCacheTransactionsWithBackingStoreTest", "[cache][!hide][longRunning]") {
...
SECTION("tst_single_readonly_hotset") {
  ...
  CHECK(hitRates.first >= 65.0);
  CHECK(hitRates.second >= 85.0);
...
SECTION("tst_single_mixed_hitrate") {
  RandomGenerator::initialize(RandomGenerator::RandomType::MERSENNE);
  MockScheduler scheduler(4);
  ...
  auto hitRates = manager.globalHitRates();
  CHECK(hitRates.first >= 40.0);
  CHECK(hitRates.second >= 60.0);
...
SECTION("tst_single_mixed_transactionality") {
  RandomGenerator::initialize(RandomGenerator::RandomType::MERSENNE);
  MockScheduler scheduler(4);
  Manager manager(scheduler.ioService(), 256 * 1024 * 1024);
  TransactionalStore store(&manager);
