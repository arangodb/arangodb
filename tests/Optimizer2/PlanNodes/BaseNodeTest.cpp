...
TEST_F(Optimizer2BaseNode, construction) {
  ...
  if (!res) {
    ...
    // inVariable
    EXPECT_EQ(BaseNode.inVariable.id, 3u);
    EXPECT_EQ(BaseNode.inVariable.name, "2");
    EXPECT_FALSE(BaseNode.inVariable.isFullDocumentFromCollection);
    EXPECT_FALSE(BaseNode.inVariable.isDataFromCollection);
     */
  }
}