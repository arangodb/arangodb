...
defmodule ToastTest.RunnerTest do
  ...
  describe "check_between_tests dispatch" do
    ...
    test "suite with mode: :manual returns mode: :manual in deployment_config" do
      defmodule ManualModeSuite do
        use ToastTest.Suite, mode: :manual
      end

      config = ManualModeSuite.deployment_config()
      assert Keyword.get(config, :mode) == :manual
    end

    test "suite with default mode returns mode: :auto in deployment_config" do
      defmodule DefaultModeSuite do
        use ToastTest.Suite
      end

      config = DefaultModeSuite.deployment_config()
