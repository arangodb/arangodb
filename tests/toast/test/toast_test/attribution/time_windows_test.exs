defmodule ToastTest.Attribution.TimeWindowsTest do
  ...
  describe "build/1" do
    ...
    test "test window finished_at extends to between_tests_finished_at when set" do
      between_tests_finished = ~U[2026-03-09 10:01:30Z]
        ...
        modules: %{
          OnlyMod => %{
            ...
            tests: [
              %{
                name: :only_test,
                started_at: @test1_started,
                finished_at: @test1_finished,
                between_tests_finished_at: between_tests_finished
              }
            ]
          }
        }
      ...
      windows = TimeWindows.build(test_data)
      assert windows.tests[{OnlyMod, :only_test}].finished_at == to_us(between_tests_finished)
      assert windows.tests[{OnlyMod, :only_test}].started_at == to_us(@test1_started)
    end

    test "nil between_tests_finished_at falls back to finished_at" do
      test_data = %{
        started_at: @suite_started,
        finished_at: @suite_finished,
        modules: %{
      ...
      windows = TimeWindows.build(test_data)
      assert windows.tests[{OnlyMod, :only_test}].finished_at == to_us(@test1_finished)
    end
  end

  describe "attribute/3 — extended window via between_tests_finished_at" do
    test "timestamp between finished_at and between_tests_finished_at returns :high" do
      ...
      between_tests_finished = ~U[2026-03-09 10:01:04Z]
