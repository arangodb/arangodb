...
defmodule Toast.Diagnostics.CoredumpOverrideDirTest do
  ...
  describe "diagnose_core_pattern/1" do
    test "returns nil for simple relative pattern" do
      assert Discovery.diagnose_core_pattern("core") == nil
    end

    test "returns nil for fs pattern with existing directory" do
      assert Discovery.diagnose_core_pattern("/tmp/core_%e.%p") == nil
    end

    test "warns for fs pattern with non-existent directory" do
      result = Discovery.diagnose_core_pattern("/nonexistent/dir/core_%e.%p_%t")

      assert result =~ "does not exist"
      assert result =~ "/nonexistent/dir"
    end

    test "expands template variables before checking directory" do
      # Only the expanded dir (with glob) should appear next to "does not exist";
      # a leftover %e would yield dir "/no/such/%e/path".
      result = Discovery.diagnose_core_pattern("/no/such/%e/path/core.%p")

      assert result =~ "'/no/such/*/path' does not exist"
    end

    test "returns nil for piped pattern with apport handler" do
      assert Discovery.diagnose_core_pattern("|/usr/share/apport/apport -p%p") == nil
    end

    test "warns for piped pattern with unknown handler when no coredumpctl" do
      # This test depends on coredumpctl not being installed, which we
      # verified earlier. Skip if it happens to be available.
      if System.find_executable("coredumpctl") do
        :skip
      else
        result = Discovery.diagnose_core_pattern("|/usr/lib/unknown-handler %p")

        assert result =~ "unknown handler"
        assert result =~ "unknown-handler"
      end
    end
  end
