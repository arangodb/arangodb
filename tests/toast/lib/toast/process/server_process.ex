...
defmodule Toast.Process.ServerProcess do
  ...
  defp do_stop(state, timeout, from) do
    ...
    # Non-blocking: initiates SIGTERM. The erlexec kill_timeout only matters when exec-port
    # runs its finalize() cleanup after BEAM dies — it bounds how long exec-port waits after
    # SIGTERM before escalating to SIGKILL. Kept short so orphaned servers get force-killed
    # within finalize's ~10s budget even when deadlocked.
    try do
