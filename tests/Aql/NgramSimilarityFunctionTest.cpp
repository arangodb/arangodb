...
TEST_F(NgramSimilarityFunctionTest, test) {
...

  {
    ...
    // different length
    {
      ...
      std::vector<double> expected_rev{0.9f, 8.f / 9.f, 0.75f, 5.f / 7.f,
                                       4.f / 6.f};
      for (int i = 1; i <= 5; ++i) {
        SCOPED_TRACE(testing::Message("Ngram size is ") << i);
        AqlValue const ValidInt{AqlValueHintInt{i}};
        assertNgramSimilarity(__LINE__, expected[i - 1], &Attribute, &Target,
                              &ValidInt);
        assertNgramSimilarity(__LINE__, expected_rev[i - 1], &Target,
                              &Attribute, &ValidInt);
      }
    }
    // with gaps
    {
      ...
      std::vector<double> expected{9.f / 12.f, 7.f / 11.f, 4.f / 10.f,
                                   2.f / 9.f, 1.f / 8.f};
