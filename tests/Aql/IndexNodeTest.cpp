...
class IndexNodeTest
 ...
 protected:
  ...
  IndexNodeTest() : server(false) { server.startFeatures(); }
...
TEST_F(IndexNodeTest, constructIndexNode) {
  ...
  arangodb::aql::Query query(false, vocbase, arangodb::aql::QueryString("FOR d IN testCollection FILTER d.obj.a == 'a_val' SORT d.obj.c LIMIT 10 RETURN d"),
...

  {
    ...
    {// without properties
     {auto indNodeClone = dynamic_cast<arangodb::aql::IndexNode*>(
          indNode.clone(query.plan(), true, false));
  ...
  // with properties
  {
    arangodb::aql::Query queryClone(false, vocbase, arangodb::aql::QueryString("RETURN 1"),
                                    nullptr, arangodb::velocypack::Parser::fromJson("{}"),
                                    arangodb::aql::PART_MAIN);
    ...
    auto indNodeClone = dynamic_cast<arangodb::aql::IndexNode*>(
        indNode.clone(queryClone.plan(), true, true));
