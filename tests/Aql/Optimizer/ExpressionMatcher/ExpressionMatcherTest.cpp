...
TEST(ExpressionMatcherTest, any_matches_any) {
  for (auto i = size_t{0}; i < NODE_TYPE_CALCULATED_OBJECT_ELEMENT; ++i) {
    auto node = AstNode(AstNodeType(i));

    auto result = Any{}.apply(&node);
    ASSERT_TRUE(result.isSuccess());
  }
}
...
TEST(ExpressionMatcherTest, matches_filter_expression_into_variable) {
  ...
  {
    AstNode const* variable = nullptr;
    AstNode const* map = nullptr;
        ...
        expansion(                                               //
            iterator(AnyVariable{},                              //
                     attributeAccess(Reference{.name = "path"},  //
                                     {"edges", "vertices"})),    //
            into(variable, Reference{.name = "3_"}),             //
            NoOp{},                                              //
            NoOp{},                                              //
            into(map, Any{})),                                   //
        AnyValue{},                                              //
