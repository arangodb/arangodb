...
class MultiDependencySingleRowFetcherTest
 ...
 protected:
  ...
  auto buildFetcher(std::vector<std::deque<SharedAqlItemBlockPtr>> inputData,
    ...
    for (auto blockDeque : inputData) {
      auto dep = std::make_unique<WaitingExecutionBlockMock>(
 ...
 private:
  // This vector is not read anywhere, it just serves as a data lake for the
  // ExecutionBlocks generated by the tests s.t. they are garbage collected
  // after the test execution is done.
  std::vector<std::unique_ptr<ExecutionBlock>> _blocks;
  // The dependencies, they are referenced by _proxy, modifing this will modify
  // the proxy
  std::vector<ExecutionBlock*> _dependencies{};
  RegIdSet inputRegister = RegIdSet{0};
  DependencyProxy<BlockPassthrough::Disable> _proxy{
      _dependencies, itemBlockManager, inputRegister, 1, nullptr};
};

INSTANTIATE_TEST_CASE_P(
    MultiDependencySingleRowFetcherTest, MultiDependencySingleRowFetcherTest,
...
TEST_P(MultiDependencySingleRowFetcherTest,
       one_block_upstream_all_deps_differ) {
  std::vector<std::deque<SharedAqlItemBlockPtr>> data;
  for (size_t i = 0; i < numberDependencies(); ++i) {
    std::deque<SharedAqlItemBlockPtr> blockDeque{};
...
TEST_P(MultiDependencySingleRowFetcherTest,
       many_blocks_upstream_all_deps_equal) {
  std::vector<std::deque<SharedAqlItemBlockPtr>> data;
  for (size_t i = 0; i < numberDependencies(); ++i) {
    std::deque<SharedAqlItemBlockPtr> blockDeque{};
    blockDeque.emplace_back(buildBlock<1>(itemBlockManager, {{1}, {2}, {3}}));
...
TEST_P(MultiDependencySingleRowFetcherTest,
       many_blocks_upstream_all_deps_differ) {
  std::vector<std::deque<SharedAqlItemBlockPtr>> data;
  for (int i = 0; i < static_cast<int>(numberDependencies()); ++i) {
    std::deque<SharedAqlItemBlockPtr> blockDeque{};
    blockDeque.emplace_back(buildBlock<1>(
        itemBlockManager, {{1 * (i + 1)}, {2 * (i + 1)}, {3 * (i + 1)}}));
    blockDeque.emplace_back(buildBlock<1>(
        itemBlockManager, {{4 * (i + 1)}, {5 * (i + 1)}, {6 * (i + 1)}}));
    blockDeque.emplace_back(buildBlock<1>(
        itemBlockManager, {{7 * (i + 1)}, {8 * (i + 1)}, {9 * (i + 1)}}));
    data.emplace_back(std::move(blockDeque));
  }
...
TEST_P(MultiDependencySingleRowFetcherTest,
       many_blocks_upstream_all_deps_differ_sequentially) {
  // Difference to the test above is that we first fetch all from Dep1, then
  // Dep2 then Dep3
  std::vector<std::deque<SharedAqlItemBlockPtr>> data;
  for (int i = 0; i < static_cast<int>(numberDependencies()); ++i) {
    std::deque<SharedAqlItemBlockPtr> blockDeque{};
  ...
  for (size_t dep = 0; dep < numberDependencies(); ++dep) {
    AqlCallSet set{};
    set.calls.emplace_back(
        AqlCallSet::DepCallPair{dep, AqlCallList{AqlCall{}}});
    testWaiting(testee, stack, set);
...
TEST_P(
  ...
  for (int i = 0; i < static_cast<int>(numberDependencies()); ++i) {
    std::deque<SharedAqlItemBlockPtr> blockDeque{};
    blockDeque.emplace_back(buildBlock<1>(
        itemBlockManager, {{1 * (i + 1)}, {2 * (i + 1)}, {0}, {3 * (i + 1)}},
        {{2, 0}}));
    blockDeque.emplace_back(
        buildBlock<1>(itemBlockManager,
...
TEST_P(
  ...
  for (size_t dep = 0; dep < numberDependencies(); ++dep) {
    ...
    // offset 10
    set.calls.emplace_back(
        AqlCallSet::DepCallPair{dep, AqlCallList{AqlCall{10}}});
    testWaiting(testee, stack, set);
