  ...
  void assertNodesMatch(VPackSlice actualNodes, std::vector<std::string> expectedNodes) {
    ...
    for (size_t i = 0; i < expectedNodes.size(); ++i) {
      if (actualNodes.length() > i) {
        ...
        EXPECT_TRUE(node.get("type").isEqualString(expectedNodes.at(i))) << node.get("type").toString() << " != " << expectedNodes.at(i);
      } else {
        EXPECT_TRUE(false) << "Too few nodes. expected: " << expectedNodes.size() << ", got: " << actualNodes.length();
      }
    }
    EXPECT_EQ(actualNodes.length(), expectedNodes.size()) << "Non matching amount of nodes.";
  }

  DistributeQueryRuleTest() {
    // We can register them, but then the API will call count, and the servers do not respond.
    // Now we just get "no endpoint found" but this seems to be okay :shrug:
    // server.registerFakedDBServer("DB1");
    // server.registerFakedDBServer("DB2");
    server.createCollection(server.getSystemDatabase().name(), "collection", {{"s123", "DB1"}, {"s234", "DB2"}}, TRI_col_type_e::TRI_COL_TYPE_DOCUMENT);
  }
};
