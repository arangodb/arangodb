...
TEST_F(RemoveFollowerTest,
       collection_still_exists_if_missing_job_is_finished_move_to_finished) {
...
TEST_F(RemoveFollowerTest,
       condition_still_holds_for_the_mentioned_collections_move_to_finished) {
  TestStructType createTestStructure = [&](Slice const& s,
                                           std::string const& path) {
    std::unique_ptr<Builder> builder = std::make_unique<Builder>();
    if (s.isObject()) {
  ...
  When(Method(mockAgent, write))
      .AlwaysDo([&](query_t const& q,
        ...
        EXPECT_TRUE(
            ...
            "delete");
        EXPECT_EQ(typeName(writes.get("/arango/Target/Finished/1")), "object");
        Slice servers = writes.get(
            "/arango/Plan/Collections/database/collection1/shards/s1");
