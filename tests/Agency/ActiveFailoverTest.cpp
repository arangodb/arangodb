...
TEST_F(ActiveFailover,
       the_state_is_already_good_and_failservers_is_still_in_the_snapshot) {
  const char* tt =
      R"=({"arango":{"Supervision":{"Health":{"SNGL-leader":{"Status":"GOOD"}}}}})=";
  ...
  When(Method(mockAgent, write))
      .AlwaysDo([&](query_t const& q,
        ...
        EXPECT_TRUE(typeName(precond.get(
                    ...
                    "object");
        EXPECT_TRUE(precond.get("/arango/Supervision/Health/SNGL-leader/Status")
                        .get("old")
                        .copyString() == "BAD");
        EXPECT_TRUE(
...
TEST_F(ActiveFailover, server_is_healthy_again_job_finishes) {
  ...
  When(Method(mockAgent, write))
      .Do([&](query_t const& q,
        ...
        EXPECT_FALSE(
            writes.hasKey("/arango" + asyncReplLeader));  // no change to leader
        return fakeWriteResult;
      });
...
TEST_F(ActiveFailover, current_leader_is_different_from_server_in_job) {
  const char* health =
      R"=({"arango":{"Plan":{"AsyncReplication":{"Leader":"SNGL-follower1"}},
    "Target":{"ToDo":{"1":{"jobId":"1","type":"activeFailover"}}}}})=";
  VPackBuilder mod =
      VPackCollection::merge(base.slice(), createBuilder(health).slice(), true);
...
TEST_F(ActiveFailover, follower_with_best_tick_value_used) {
  ...
  trans_ret_t fakeTransient{
      true, "", 1, 0, std::make_shared<Builder>(createBuilder(transient))};
  ...
  When(Method(mockAgent, write))
      .Do([&](query_t const& q,
        ...
        EXPECT_TRUE(typeName(writes.get("/arango/Target/ToDo/1").get("op")) ==
                    "string");
        EXPECT_EQ(typeName(writes.get("/arango/Target/Finished/1")), "object");
        EXPECT_TRUE(typeName(writes.get(
                        "/arango/Plan/AsyncReplication/Leader")) == "string");
        EXPECT_EQ(
        ...
        EXPECT_TRUE(precond.get("/arango/Supervision/Health/SNGL-leader/Status")
                        ...
                        .copyString() == "FAILED");
        EXPECT_TRUE(
            precond.get("/arango/Supervision/Health/SNGL-follower1/Status")
                .get("old")
                .copyString() == "GOOD");
        EXPECT_TRUE(precond.get("/arango/Plan/AsyncReplication/Leader")
                        .get("old")
                        .copyString() == LEADER);
