...
SECTION("in case of a timeout the job should be aborted") {
  ...
  When(Method(mockAgent, write)).Do([&](query_t const& q, consensus::AgentInterface::WriteMode w) -> write_ret_t {
    try {
      ...
      auto writes = q->slice()[0][0];
      REQUIRE(std::string(writes.get("/arango/Target/Pending/1").get("op").typeName()) ==
                  "string");
      REQUIRE(std::string(writes.get("/arango/Target/ToDo/1").get("op").typeName()) ==
                  "string");
      REQUIRE(writes.get("/arango/Target/Pending/1").get("op").copyString() ==
                  "delete");
      REQUIRE(writes.get("/arango/Supervision/Shards/s99").get("op").copyString() ==
                  "delete");
      REQUIRE(std::string(writes.get("/arango/Target/Failed/1").typeName()) ==
                  "object");
      ...
      REQUIRE(getKeySet(writes) == expectedKeys);
