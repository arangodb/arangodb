...
SECTION("a successfully started job should finish immediately and set everything") {
  ...
  When(Method(mockAgent, transact)).Do([&](query_t const& q) -> trans_ret_t {
    ...
    auto free = writes.get(planEntry)[1].copyString();
    REQUIRE(free.compare(0,4,FREE_SERVER) == 0);
    ...
    auto healthStat = std::string("/arango/Supervision/Health/") + free + "/Status";
...
SECTION("the job should handle distributeShardsLike") {
  ...
  When(Method(mockAgent, transact)).Do([&](query_t const& q) -> trans_ret_t {
    ...
    REQUIRE(writes.get("/arango/Plan/Collections/" + DATABASE + "/" + COLLECTION + "/shards/" + SHARD)[1].copyString().compare(0,4,FREE_SERVER) == 0);
