...
TEST_F(FailedFollowerTest, creating_a_job_should_create_a_job_in_todo) {
  ...
  auto failedFollower =
      FailedFollower(*baseStructure, &agent, jobId, "unittest", DATABASE,
                     COLLECTION, SHARD, SHARD_FOLLOWER1);
...
TEST_F(FailedFollowerTest, if_collection_is_missing_job_should_just_finish) {
  ...
  AgentInterface& agent = mockAgent.get();
  auto failedFollower =
      FailedFollower(*agency->get(PREFIX), &agent, JOB_STATUS::TODO, jobId);
...
TEST_F(FailedFollowerTest, if_follower_is_healthy_again_we_fail_the_job) {
  ...
  agency = agency->placeAt(
      "/arango/Supervision/Health/" + SHARD_FOLLOWER1 + "/Status", "GOOD");
  agency = agency->placeAt("/arango/Target/ToDo/1", createJob().slice());
...
TEST_F(FailedFollowerTest,
       if_there_is_no_healthy_free_server_at_start_just_finish) {
  std::string jobId = "1";
...
TEST_F(FailedFollowerTest, abort_any_moveshard_job_blocking) {
  ...
  auto moveShard = MoveShard(*baseStructure->get(PREFIX), &moveShardAgent, "2",
                             "strunz", DATABASE, COLLECTION, SHARD,
...
TEST_F(FailedFollowerTest, successfully_started_job_should_finish_immediately) {
  ...
  When(Method(mockAgent, transact)).Do([&](velocypack::Slice q) -> trans_ret_t {
    ...
    EXPECT_TRUE(preconditions.get(healthStat).get("old").copyString() ==
                "GOOD");
    EXPECT_TRUE(
        std::string(preconditions.get(planEntry).get("old").typeName()) ==
        "array");
