...
TEST_F(FailedFollowerTest, creating_a_job_should_create_a_job_in_todo) {
  ...
  auto failedFollower =
      FailedFollower(mockSupervision.get(), baseStructure, &agent, jobId,
                     "unittest", DATABASE, COLLECTION, SHARD, SHARD_FOLLOWER1);
...
TEST_F(FailedFollowerTest, if_collection_is_missing_job_should_just_finish) {
  ...
  When(Method(mockAgent, waitFor)).AlwaysReturn(AgentInterface::raft_commit_t::OK);
  AgentInterface& agent = mockAgent.get();
  auto failedFollower = FailedFollower(mockSupervision.get(), agency(PREFIX),
...
TEST_F(FailedFollowerTest, abort_any_moveshard_job_blocking) {
  ...
  auto moveShard = MoveShard(mockSupervision.get(), baseStructure(PREFIX),
