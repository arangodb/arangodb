...
// RBAC mode: when RBAC_JWT=1, per-user matrix requests authenticate with a
// per-user JWT (preferred_username) instead of Basic auth, because arangod only
// forwards a JWT to the external RBAC service (Basic auth -> empty token -> deny).
const RBAC_JWT = process.env.RBAC_JWT === '1';
let jwtSecretGlobal = null;

function generateUserJwt(secret, username) {
  const header  = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
  const payload = Buffer.from(JSON.stringify({ iss: 'arangodb', preferred_username: username })).toString('base64url');
  const signingInput = `${header}.${payload}`;
  const sig = createHmac('sha256', secret).update(signingInput).digest('base64url');
  return `${signingInput}.${sig}`;
}

// Build the per-user Authorization header: JWT bearer in RBAC mode, else Basic.
function userAuthHeader(username) {
  if (RBAC_JWT) {
    return `bearer ${generateUserJwt(jwtSecretGlobal, username)}`;
  }
  return `Basic ${Buffer.from(`${username}:${username}`).toString('base64')}`;
}
...
async function doSetup(endpoint, rootPassword) {
  // In RBAC mode arangod rejects Basic auth (no forwardable token), so setup
  // authenticates with a superuser JWT minted from RBAC_JWT_SECRET_FILE.
  const authHeader = (RBAC_JWT && process.env.RBAC_JWT_SECRET_FILE)
    ? `bearer ${generateSuperuserJwt(readFileSync(process.env.RBAC_JWT_SECRET_FILE, 'utf8').trim())}`
    : `Basic ${Buffer.from(`root:${rootPassword}`).toString('base64')}`;
...
async function runCollectionTest(endpoint, superuserToken, test) {
  ...
  for (const coll of LEVELS) {
    for (const wc of LEVELS) {
      ...
      for (const db of LEVELS) {
        const username = `${db}${wc}${coll}`;
        const authHeader = userAuthHeader(username);
...
async function main() {
  ...
  jwtSecretGlobal = secret;  // used by userAuthHeader() in RBAC mode
