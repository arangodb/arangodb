...
| `body`     | `object \| null`   | no       | JSON body sent with the request |
| `headers`  | `object`           | no       | Extra HTTP headers added to every test request |
...
If `setup` returns an object (or any value), the runner assigns it to
`ctx.data` before invoking `teardown`.  This lets `teardown` clean up
resources whose identifiers were not known ahead of time (e.g. a document
`_id` generated by ArangoDB during the setup call).  After `teardown`
completes, `ctx.data` is reset to `undefined` so it does not bleed into
the next matrix cell.
...
### Using `ctx.data` to pass setup results to teardown

When the resource identifier is not known ahead of time — for example when
ArangoDB generates the `_id` — `setup` can return an object.  The runner
binds the return value to `ctx.data` so `teardown` can use it:
...
export default [
  {
    name:   'Insert document with server-generated key',
    type:   'collection',
    method: 'POST',
    path:   '/_db/d/_api/document/c',
    body:   { value: 99 },   // no _key – ArangoDB generates one

    // Create a sentinel document and return its _id for teardown.
    setup: async (ctx) => {
      const r = await ctx.request('POST', '/_db/d/_api/document/c', { value: 0 });
      // Return an object; the runner stores it as ctx.data.
      return { id: r.body._id };
    },

    // ctx.data contains whatever setup returned.
    teardown: async (ctx) => {
      if (ctx.data?.id) {
