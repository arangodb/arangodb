...
| `setup`    | `async (ctx) => …` | no       | Hook executed **before** each matrix cell; runs as superuser |
| `teardown` | `async (ctx) => …` | no       | Hook executed **after** each matrix cell; runs as superuser |
