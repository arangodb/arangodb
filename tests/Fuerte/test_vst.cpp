...
TEST(VelocyStream_11, ChunkHeader) {
  ...
  fu::vst::Chunk chunk;
  fu::vst::parser::ChunkState state =
      fu::vst::parser::readChunkVST1_1(chunk, ptr, chunkData.size());
...
TEST(VelocyStream_11, MultiChunk) {
  ...
  fu::vst::parser::ChunkState state =
      fu::vst::parser::readChunkVST1_1(resultChunk, ptr, chunk0.size());
  ASSERT_EQ(state, fu::vst::parser::ChunkState::Complete);
...
TEST(VelocyStream_11, prepareForNetworkSingleChunk) {
  ...
  ASSERT_EQ(data, std::string(reinterpret_cast<char const*>(result[2].data()),
                              result[2].size()));
}
...
TEST(VelocyStream_11, prepareForNetworkMultipleChunks) {
  ...
  ASSERT_EQ(resultChunk.header.messageLength(),
  ...
  ASSERT_EQ(data.substr(expectedLength0, expectedLength1),
            std::string(reinterpret_cast<char const*>(result[4].data()),
                        result[4].size()));
  ...
  ASSERT_EQ(data.substr(expectedLength1, expectedLength2),
            std::string(reinterpret_cast<char const*>(result[6].data()),
                        result[6].size()));
}
