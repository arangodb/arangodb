...
TEST_F(ReplicatedLogTest, multiple_follower) {
  ...
  auto follower_1 = std::make_shared<DelayedFollowerLog>(
      defaultLogger(), _logMetricsMock, _optionsMock, followerId_1, std::move(coreB),
      LogTerm{1}, leaderId);
  auto follower_2 = std::make_shared<DelayedFollowerLog>(
      defaultLogger(), _logMetricsMock, _optionsMock, followerId_2, std::move(coreC),
      LogTerm{1}, leaderId);
...
TEST_F(ReplicatedLogTest,
  ...
  auto coreB = makeLogCore(LogId{2});
  auto follower = std::make_shared<DelayedFollowerLog>(
      defaultLogger(), _logMetricsMock, _optionsMock,  followerId, std::move(coreB),
      LogTerm{3}, leaderId);
