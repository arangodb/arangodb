...
struct CollectionGroupsSupervisionTest : ::testing::Test {
  ...
  CollectionGroup prepareSmartGraphGroup(std::vector<std::string> vertexNames,
    ...
    group.target.id = ag::CollectionGroupId{12};
    group.target.version = 1;
    for (auto const& c : vertexNames) {
      group.target.collections[c] = {};
      group.targetCollections[c].groupId = group.target.id;
      group.targetCollections[c].immutableProperties.isSmart = true;
    }

    for (auto const& c : edgeNames) {
      group.target.collections[c] = {};
      group.targetCollections[c].groupId = group.target.id;
      group.targetCollections[c].immutableProperties.isSmart = true;
      group.targetCollections[c].immutableProperties.type =
          TRI_col_type_e::TRI_COL_TYPE_EDGE;
      group.targetCollections[c].immutableProperties.isSmartChild = false;

      std::vector<DataSourceId> shadows = {};
      {
        auto localName = StaticStrings::FullLocalPrefix + c;
        group.target.collections[localName] = {};
        group.targetCollections[localName].groupId = group.target.id;
        group.targetCollections[localName].immutableProperties.isSmart = true;
        group.targetCollections[localName].immutableProperties.type =
            TRI_col_type_e::TRI_COL_TYPE_EDGE;
        group.targetCollections[localName].immutableProperties.isSmartChild =
            true;
      }
      if (!isDisjoint) {
        for (auto const& prefix :
             {StaticStrings::FullFromPrefix, StaticStrings::FullToPrefix}) {
          auto localName = prefix + c;
          group.target.collections[localName] = {};
          group.targetCollections[localName].groupId = group.target.id;
          group.targetCollections[localName].immutableProperties.isSmart = true;
          group.targetCollections[localName].immutableProperties.type =
          ...
          shadows.emplace_back(
              group.targetCollections[localName].immutableProperties.id);
        }
      }

      group.targetCollections[c].immutableProperties.shadowCollections =
          std::move(shadows);
    }
    group.target.attributes.mutableAttributes.replicationFactor =
        replicationFactor;
...
TEST_F(CollectionGroupsSupervisionTest, add_satellite_collection) {
  constexpr auto numberOfShards = 1;
  constexpr auto replicationFactor = 0;  // aka satellite

  CollectionGroup group =
      prepareGroup({"A"}, numberOfShards, replicationFactor);
  ...
  EXPECT_EQ(action.sheaves.size(), numberOfShards);
  EXPECT_EQ(action.spec.shardSheaves.size(), numberOfShards);
  EXPECT_EQ(action.collections.size(), 1);
  ...
  for (auto const& [unused, sheaf] : action.sheaves) {
    ...
    EXPECT_TRUE(sheaf.leader.has_value()) << "Did not pick a leader";
    EXPECT_TRUE(sheaf.participants.contains(
        sheaf.leader.value_or("NO_LEADER_SELECTED")))
        << "The leader is not a participant";
  }
}

TEST_F(CollectionGroupsSupervisionTest, add_smart_edge_collection) {
  constexpr auto numberOfShards = 3;
  constexpr auto replicationFactor = 2;

  CollectionGroup group = prepareSmartGraphGroup(
      {"V"}, {"E"}, false, numberOfShards, replicationFactor);
  ...
  EXPECT_EQ(action.sheaves.size(), numberOfShards);
  for (auto const& [unused, sheaf] : action.sheaves) {
    EXPECT_EQ(sheaf.participants.size(), replicationFactor)
        << "We have enough healthy servers, so every sheaf should have the "
           "replication factor as participants";
    EXPECT_TRUE(sheaf.leader.has_value()) << "Did not pick a leader";
    EXPECT_TRUE(sheaf.participants.contains(
        sheaf.leader.value_or("NO_LEADER_SELECTED")))
        << "The leader is not a participant";
  ...
  EXPECT_EQ(action.collections.size(), 5);
  for (auto const& [name, c] : action.collections) {
    if (name == "E") {
      EXPECT_EQ(c.shardList.size(), 0)
          << "The virtual smart edge collection does not have shards";
    } else {
      EXPECT_EQ(c.shardList.size(), numberOfShards);
    }
  }
}

TEST_F(CollectionGroupsSupervisionTest, add_disjoint_smart_edge_collection) {
  constexpr auto numberOfShards = 3;
  constexpr auto replicationFactor = 2;
  CollectionGroup group = prepareSmartGraphGroup(
      {"V"}, {"E"}, true, numberOfShards, replicationFactor);
  ...
  EXPECT_EQ(action.collections.size(), 3);
  for (auto const& [name, c] : action.collections) {
    if (name == "E") {
      EXPECT_EQ(c.shardList.size(), 0)
          << "The virtual smart edge collection does not have shards";
