...
SECTION("tst_1") {
  ...
  size_t len = 0;
  char* result = TRI_normalize_utf8_to_NFC(TRI_UNKNOWN_MEM_ZONE, (const char*) decomposed, strlen((const char*) decomposed),&len);
/*
  size_t outLength;
  char* uni = TRI_EscapeUtf8StringZ (TRI_UNKNOWN_MEM_ZONE, (const char*) decomposed, strlen((const char*) decomposed), true, &outLength);
  printf("\nOriginal: %s\nEscaped: %s\n", decomposed, uni);

  char* uni2 = TRI_EscapeUtf8StringZ (TRI_UNKNOWN_MEM_ZONE, (const char*) composed, strlen((const char*) composed), true, &outLength);
  printf("\nOriginal: %s\nEscaped: %s\n", composed, uni2);
*/  
...
SECTION("tst_2") {
  ...
  int32_t len = 0;
  char* result = TRI_tolower_utf8(TRI_UNKNOWN_MEM_ZONE, (const char*) gruessgott1, (int32_t) strlen((const char*) gruessgott1), &len);
  ...
  len = 0;
  result = TRI_tolower_utf8(TRI_UNKNOWN_MEM_ZONE, (const char*) gruessgott2, (int32_t) strlen((const char*) gruessgott2), &len);
