...
TEST_F(LoggerTest, test_fds) {
  ...
  logger2.logMessage(LogMessage(__FUNCTION__, __FILE__, __LINE__, LogLevel::WARN,
                                0, "some warning message", 0, true));
...
TEST_F(LoggerTest, test_fds_after_reopen) {
  ...
  logger2.logMessage(LogMessage(__FUNCTION__, __FILE__, __LINE__, LogLevel::WARN,
                                0, "some other warning message", 0, true));
...
TEST_F(LoggerTest, testTimeFormats) {
...

  {
    ...
    out.clear();
    LogTimeFormats::writeTime(out, LogTimeFormats::TimeFormat::UnixTimestampMillis,
                              tp, startTp);
    EXPECT_EQ("1481464963.000", out);

    out.clear();
    LogTimeFormats::writeTime(out, LogTimeFormats::TimeFormat::UnixTimestampMicros,
                              tp, startTp);
    ...
    out.clear();
    LogTimeFormats::writeTime(out, LogTimeFormats::TimeFormat::UTCDateStringMillis,
                              tp, startTp);
