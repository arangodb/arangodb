...
TEST(PromiseRegistryTest,
     cannot_erase_a_promise_that_does_not_exist_in_the_list) {
  auto promise_registry = PromiseRegistryOnThread{};

  auto promise_not_in_list = PromiseInList(std::source_location::current());
  EXPECT_TRUE(promise_registry.erase(&promise_not_in_list).fail());
}
