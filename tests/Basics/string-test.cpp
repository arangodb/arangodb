...
SECTION("tst_lower_ascii_no_change") {
  ...
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "this is supposed to stay the same");
  CHECK("this is supposed to stay the same" == std::string(result));
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "this is also supposed to stay the same");
  CHECK("this is also supposed to stay the same" == std::string(result));
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  // punctuation should not change
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "01234567890,.;:-_#'+*~!\"§$%&/()[]{}=?\\|<>");
  CHECK(std::string("01234567890,.;:-_#'+*~!\"§$%&/()[]{}=?\\|<>") == std::string(result));
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  // whitespace should not change
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, ("  \t \n \r \n"));
  CHECK(std::string("  \t \n \r \n") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
...
SECTION("tst_lower_ascii") {
  ...
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "This MUST be converted into LOWER CASE!");
  CHECK(std::string("this must be converted into lower case!") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "SCREAMING OUT LOUD");
...
SECTION("tst_lower_ascii_non_ascii") {
  ...
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "äöüÄÖÜß");
  CHECK(std::string("äöüÄÖÜß") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "코리아닷컴");
  CHECK(std::string("코리아닷컴") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_LowerAsciiString(TRI_UNKNOWN_MEM_ZONE, "своих партнеров");
...
SECTION("tst_upper_ascii_no_change") {
  ...
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "THIS IS SUPPOSED TO STAY THE SAME");
  CHECK(std::string("THIS IS SUPPOSED TO STAY THE SAME") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "THIS IS ALSO SUPPOSED TO STAY THE SAME");
  CHECK(std::string("THIS IS ALSO SUPPOSED TO STAY THE SAME") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  // punctuation should not change
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "01234567890,.;:-_#'+*~!\"§$%&/()[]{}=?\\|<>");
  CHECK(std::string("01234567890,.;:-_#'+*~!\"§$%&/()[]{}=?\\|<>") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  // whitespace should not change
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "  \t \n \r \n");
  CHECK(std::string("  \t \n \r \n") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
...
SECTION("tst_upper_ascii") {
  ...
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "This must be converted into upper CASE!");
  CHECK(std::string("THIS MUST BE CONVERTED INTO UPPER CASE!") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "silently whispering");
...
SECTION("tst_upper_ascii_non_ascii") {
  ...
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "äöüÄÖÜß");
  CHECK(std::string("äöüÄÖÜß") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "코리아닷컴");
  CHECK(std::string("코리아닷컴") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
  
  result = TRI_UpperAsciiString(TRI_UNKNOWN_MEM_ZONE, "своих партнеров");
  CHECK(std::string("своих партнеров") == result);
  TRI_FreeString(TRI_UNKNOWN_MEM_ZONE, result);
}
