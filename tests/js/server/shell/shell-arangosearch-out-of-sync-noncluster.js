...
function ArangoSearchOutOfSyncSuite () {
  ...
  return {
    ...
    testInsertIntoOutOfSyncIgnored : function () {
      ...
      // should not produce any errors - but not insert into the link
      c.insert(docs);
    ...
    testRemoveFromOutOfSync : function () {
      ...
      // query results should not have changed, as the link
      // participates in the remove/insert ops.
    ...
    testRemoveFromOutOfSyncIgnored : function () {
      ...
      // due to the failure point set, the link will not carry
      // out the removes/inserts!
      db._query("FOR doc IN UnitTestsCollection1 UPDATE doc WITH { value: 1 } IN UnitTestsCollection1");
      internal.debugClearFailAt();
      
      // although the link did not carry out the remove and
