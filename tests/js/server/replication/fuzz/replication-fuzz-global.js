...
function ReplicationSuite () {
 ...
 return db !== '_system';
          ...
          let pickCollection = function () {
            let collections;
            while (true) {
              collections = db._collections().filter(function (c) {
 return c.name()[0] !== '_' && c.type() === 2;
});
 ...
 return c.name()[0] !== '_' && c.type() === 3;
          ...
          let insertOverwrite = function () {
            let collection = pickCollection();
            collection.insert({ _key: "test",
value: Date.now() }, { overwrite: true });
          };
          ...
          let insertEdge = function () {
            ...
            collection.insert({ _from: "test/v1",
_to: "test/v2",
value: Date.now() });
          };
...
func: dropDatabase }
          ...
          if (!isCluster) {
            ops.push({ name: "renameCollection",
func: renameCollection });
          ...
          db._databases().forEach(function (d) {
            ...
            db._collections().filter(function (c) {
 return c.name()[0] !== '_';
}).forEach(function (c) {
