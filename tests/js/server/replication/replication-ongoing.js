    ...
    testViewData: function () {
      ...
      compare(
        ...
        function (state) { // masterFunc2
          ...
          db._collection(cn).save({ _key: "testxxx", "value": -1, "text": txt });
          state.checksum = collectionChecksum(cn);
          state.count = collectionCount(cn);
        ...
        function (state) { // slaveFuncFinal
          ...
          res = db._query("FOR doc IN " + view.name() + " OPTIONS { waitForSync: true } SEARCH PHRASE(doc.text, 'foxx jumps over', 'text_en')  RETURN doc").toArray();
