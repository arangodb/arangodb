...
payload: [ { name: "foo" }, { name: "bar" } ] },
        { name: "cpp",
count: 4,
payload: [ { name: "baz" }, { name: "qux" } ] },
        { name: "php",
count: 2,
payload: [ ] },
        { name: "ruby",
count: 5,
payload: [ { name: "test" }, { name: 23 } ] },
        { name: "python",
count: 3,
payload: [ { name: "one" }, { name: "two" }, { name: "three" }, { name: "four" } ] }
      ]
    }
  ];
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testQuestionMarkWithFilterAndQuantifier: function () {
      ...
      result = AQL_EXECUTE("LET values = @values RETURN values[? @lower..@upper FILTER CURRENT >= 3]", { values,
lower: 9,
upper: 11 }).json;
      assertEqual([ true ], result);
...
upper: 10 }).json;
      ...
      result = AQL_EXECUTE("LET values = @values RETURN values[? @value FILTER CURRENT >= 3]", { values,
value: 8 }).json;
      assertEqual([ false ], result);
