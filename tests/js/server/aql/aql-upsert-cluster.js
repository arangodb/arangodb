...
function parallelizeGatherSleep () {

  // Regression test for https://arangodb.atlassian.net/browse/BTS-1401
  return {
    setUpAll: function() {
      db._createDatabase("test");
      db._useDatabase("test");
      db._create("coll", {"replicationFactor" : 3, "numberOfShards" : 40});
      db._query("FOR i IN 1..2  INSERT { value: i } INTO coll");
    },
    ...
    testSleepQuery: function() {
      db._query("FOR i in coll  LET x = SLEEP(35) INSERT {} INTO coll", {}, {optimizer: {rules: ["+parallelize-gather"]}});
      db._query("FOR i in coll  LET x = SLEEP(35) INSERT {} INTO coll", {}, {optimizer: {rules: ["-parallelize-gather"]}});
    }
  }
}

function parallelizeGatherErrorCodes () {
...
jsunity.run(parallelizeGatherSleep);
jsunity.run(parallelizeGatherErrorCodes);
