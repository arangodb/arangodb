...
sparse: true });
      ...
      let nodeTypes = nodes.map(function (n) {
 return n.type;
});
...
sparse: true });
      ...
      let nodes = AQL_EXPLAIN(query).plan.nodes;
      let indexes = nodes.filter(function (n) {
...
sparse: true });
 ...
 return n.type === 'IndexNode';
      ...
      let collections = nodes.filter(function (n) {
...
sparse: true });
 ...
 return n.type === 'IndexNode';
      ...
      let sorts = nodes.filter(function (n) {
 return n.type === 'SortNode';
});
      assertEqual(0, sorts.length);

      let filters = nodes.filter(function (n) {
