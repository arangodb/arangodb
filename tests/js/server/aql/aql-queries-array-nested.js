...
function nestedArraySimpleSuite () {
  ...
  var runQueries = function (q, expected, useIndexForSimple, useIndexForArray) {
    ...
    var idx = c.ensureIndex({ type: "hash",
fields: [ "value" ] });
...
fields: [ "tags[*].name" ] });
    ...
    testNestedAnotherSubAttribute: function () {
      c.insert({ persons: [ { name: { first: "Jane",
last: "Doe" },
gender: "f" }, { name: { first: "Joe",
last: "Public" },
gender: "m" } ] });
      c.insert({ persons: [ { name: { first: "Jack",
last: "White" },
gender: "m" }, { name: { first: "John",
last: "Black" },
gender: "m" } ] });
      c.insert({ persons: [ { name: { first: "Janet",
last: "Smith" },
gender: "f" } ] });
      c.insert({ persons: [ { name: { first: "Jill",
last: "Jones" },
gender: "f" }, { name: { first: "Jeff",
last: "Jackson" },
gender: "m" } ] });
      c.insert({ persons: [ { name: { first: "Jimbo",
last: "Jonas" },
gender: "m" }, { name: { first: "Jay",
last: "Jameson" } } ] });
      c.insert({ persons: [ { name: { first: "Jack",
last: "Rabbit" },
gender: "m" }, { name: { first: "Jane",
last: "Jackson" },
gender: "f" } ] });
      c.insert({ persons: [ { name: { first: "Janet",
last: "Black" },
gender: "f" }, { name: { first: "James",
last: "Smith" },
gender: "m" } ] });
      c.insert({ persons: [ { name: { first: "Jill",
last: "Doe" },
...
function nestedArrayInArraySuite () {
  ...
  return {
    ...
    testIndexUsage: function () {
      ...
      var result = AQL_EXECUTE(query, { "@c1": cn1,
"@c2": cn2 });
...
fields: [ "values[*]" ] });
      result = AQL_EXECUTE(query, { "@c1": cn1,
"@c2": cn2 });
      assertEqual(expected.sort(), result.json.sort());
      assertEqual(0, result.warnings.length);
      assertTrue(indexUsed(query, { "@c1": cn1,
"@c2": cn2 }));
    }
  };
