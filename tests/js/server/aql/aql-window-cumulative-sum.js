...
function WindowCumulativeSumTestSuite () {
  let validateResult = function (result, size, from, to, preceding, following) {
    ...
    result.forEach((value, index) => {
      ...
      assertEqual(expectedSum, value, { from,
to,
preceding,
following,
size,
...
`;
      ...
      invalid.forEach((value) => {
        try {
          db._query(query, { preceding: value,
following: 1 });
          fail();
        } catch (err) {
...
`;
      ...
      invalid.forEach((value1) => {
        invalid.forEach((value2) => {
          try {
            db._query(query, { preceding: value1,
following: value2 });
            fail();
          } catch (err) {
            assertEqual(err.errorNum, errors.ERROR_BAD_PARAMETER.code, { value1,
value2 });
          }
        });
      });
