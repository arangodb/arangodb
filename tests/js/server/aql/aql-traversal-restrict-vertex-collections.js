...
value: i };
  ...
  for (let i = 0; i < 10; ++i) {
    ec.save({_from: vc1Name + "/node_" + i,
_to: vc2Name + "/node_" + i});
    ec.save({_from: vc2Name + "/node_" + i,
_to: vc1Name + "/node_" + i});
    if (i < 9) {
      ec.save({_from: vc1Name + "/node_" + i,
_to: vc1Name + "/node_" + (i + 1)});
    }
    if (i > 0) {
      ec.save({_from: vc2Name + "/node_" + i,
_to: vc2Name + "/node_" + (i - 1)});
    }
  }
};
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testNoRestriction: function () {
      ...
      const actual = db._query(query, {'@vc1': vc1Name,
'@vc2': vc2Name,
'@ec': ecName}).toArray();
      const expected = [
        vc1Name + "/node_4",
