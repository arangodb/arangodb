...
let explain = function (query, params) {
  return helper.removeClusterNodes(helper.getCompactPlan(AQL_EXPLAIN(query, params, { optimizer: { rules: [ "-all", "+use-indexes" ] } })).map(function (node) {
 return node.type;
}));
};
    ...
    testEqMultiAll1: function () {
      for (var i = 1; i <= 5; ++i) {
        for (var j = 1; j <= 5; ++j) {
          ...
          var actual = getQueryResults(query, { "a": i,
"b": j });

          assertEqual(expected, actual);

          assertEqual([ "SingletonNode", "IndexNode", "CalculationNode", "FilterNode", "CalculationNode", "ReturnNode" ], explain(query, { a: i,
b: j }));
        }
      }
    },
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testEqMultiAll2: function () {
      for (var i = 1; i <= 5; ++i) {
        for (var j = 1; j <= 5; ++j) {
          var expected = [ [ i, j ] ];
          var actual = getQueryResults("FOR v IN " + collection.name() + " FILTER @a == v.a && @b == v.b RETURN [ v.a, v.b ]", { "a": i,
"b": j });
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testPartialCoverage: function () {
      collection.save({ "a": 20,
"c": 1 });
      collection.save({ "a": 20,
"c": 2 });
      collection.save({ "a": 21,
"c": 1 });
      collection.save({ "a": 21,
"c": 2 });
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testMultiRefMulti3: function () {
      ...
      var expected = [ [ 2, 3 ] ];
      var actual = getQueryResults(query, { "a": 2,
"b": 3 });

      assertEqual(expected, actual);

      assertEqual([ "SingletonNode", "IndexNode", "CalculationNode", "FilterNode", "CalculationNode", "ReturnNode" ], explain(query, { a: 2,
b: 3 }));
    },
