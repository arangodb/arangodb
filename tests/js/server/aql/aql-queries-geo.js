...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testLegacyNear1: function () {
      var expected = [ { "distance": "111194.92664",
"latitude": -1,
"longitude": 0 }, { "distance": "111194.92664",
"latitude": 0,
"longitude": -1 }, { "distance": "111194.92664",
"latitude": 0,
"longitude": 1 }, { "distance": "111194.92664",
"latitude": 1,
"longitude": 0 }, { "distance": 0,
"latitude": 0,
"longitude": 0 } ];
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testLegacyNear2: function () {
      var expected = [ { "latitude": -10,
"longitude": 24 }, { "latitude": -10,
...
// //////////////////////////////////////////////////////////////////////////////
    ...
    testLegacyNear3: function () {
      var expected = [ { "distance": "14891044.54146",
"latitude": 40,
"longitude": -40 },
                       { "distance": "14853029.30724",
"latitude": 40,
"longitude": -39 },
                       { "distance": "14815001.47646",
"latitude": 40,
"longitude": -38 } ];
      ...
      expected = [ { "distance": "4487652.12954",
"latitude": -37,
"longitude": 26 },
                   { "distance": "4485565.93668",
"latitude": -39,
"longitude": 20 },
                   { "distance": "4484371.86154",
"latitude": -38,
"longitude": 23 } ];
...
fields: ["latitude", "longitude"] });
      ...
      for (lat = -40; lat <= 40; ++lat) {
        for (lon = -40; lon <= 40; ++lon) {
          locationsNon.save({ "latitude": lat,
"longitude": lon });
        }
      }
    },
...
"coordinates": [
  ...
  let emea = [ // TODO implement multi-polygon
    ...
    { "type": "Polygon",
"coordinates": [[[35, 10], [45, 45], [15, 40], [10, 20], [35, 10]], [[20, 30], [30, 20], [35, 35], [20, 30]]] },
    { "type": "LineString",
...
coordinates: [[[-1, -1], [2, -1], [2, 2], [-1, 2], [-1, -1]],
      ...
      const query = "RETURN GEO_CONTAINS(@polygon, @cc)";
      valid.forEach(c => {
        assertEqual(getQueryResults(query, {"polygon": polygon,
"cc": c}), [true]);
      });
      assertEqual(getQueryResults(query, {"polygon": polygon,
"cc": [0.5, 0.5]}), [false]);
...
"cc": [3.0, 3.0]}), [false]);
    ...
    testContainsMultiPolygon: function () {
      let multipoly = {"type": "MultiPolygon",
"coordinates": [[[[0, 0], [1, 0], [0, 1], [0, 0]]], [[[2, 2], [3, 2], [2, 3], [2, 2]]]] };
      let valid = [[0.01, 0.01], [0.01, 0.99], [0.99, 0.01], [0.49, 0.49],
                   [2.01, 2.01], [2.01, 2.99], [2.99, 2.01], [2.49, 2.49]];

      const query = "RETURN GEO_CONTAINS(@polygon, @cc)";
      valid.forEach(c => {
        assertEqual(getQueryResults(query, {"polygon": multipoly,
"cc": c}), [true]);
      });
      assertEqual(getQueryResults(query, {"polygon": multipoly,
"cc": [1.0, 1.0]}), [false]);
...
"cc": [3.0, 3.0]}), [false]);
    ...
    testIntersectsMultiPolygon2: function () {
      let multipoly = {"type": "MultiPolygon",
"coordinates": [[[[0, 0], [1, 0], [0, 1], [0, 0]]], [[[2, 2], [3, 2], [2, 3], [2, 2]]]] };
      let valid = [{"type": "Polygon",
"coordinates": [[[2.5, 2], [3, 2.0], [3, 2.5], [2.5, 2.5], [2.5, 2]]]},
                   {"type": "LineString",
"coordinates": [[2, 2.5], [2.5, 3.0]]},
                   {"type": "LineString",
"coordinates": [[-0, 0.5], [0.5, 1], [0.5, -0.5]]}];
...
"coordinates": [[2.5, 1], [3, 0.05]]}}), [false]);
    },
...
function geoLegacyComparison () {
  ...
  return {
    setUp: function () {
      ...
      legacy.ensureIndex({type: "geo",
fields: ["geo"],
geoJson: true,
                          legacyPolygons: true});
      newStyle = db._create(cnNewStyle);
      newStyle.ensureIndex({type: "geo",
fields: ["geo"],
geoJson: true});
    },
...
fields: ["value"],
unique: false});
      coll2.ensureIndex({type: "persistent",
fields: ["value"],
unique: false});
      makeData(1000);
      let query = `FOR e IN ${cn2}
