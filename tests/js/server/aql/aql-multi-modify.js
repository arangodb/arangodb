...
"@e": cn3 });
      ...
      var nodes = AQL_EXPLAIN(q, { "@cn": cn1,
"@e": cn3 }).plan.nodes, found = false;
    ...
    testWithDeclarationsAndSingleModificationMultipleCollections: function () {
      ...
      var actual = AQL_EXECUTE(q, { "@cn1": cn1,
"@cn2": cn2 });
      assertEqual([ [ ] ], actual.json);
      assertEqual(1, actual.stats.writesExecuted);
      assertEqual(1, c1.count());
...
"@e": cn3 });
    ...
    testEdgeAfterModification: function () {
      ...
      assertQueryError(errors.ERROR_QUERY_ACCESS_AFTER_MODIFICATION.code, q, { "@cn": cn3,
from: cn2 + "/1",
to: cn2 + "/2" });
    },
...
"@cn2": cn2 });
      ...
      var nodes = AQL_EXPLAIN(q, { "@cn1": cn1,
"@cn2": cn2 }).plan.nodes, found = false;
      nodes.forEach(function (node) {
        if (node.type === 'RemoveNode') {
          found = true;
        }
...
"@cn2": cn2 });
    ...
    testInsertRemove2: function () {
      ...
      assertEqual(2010, c1.count());
      assertEqual(0, c2.count());
      const actual = AQL_EXECUTE("FOR i IN @@cn1 INSERT { value: i } INTO @@cn2 LET x = i._key REMOVE x IN @@cn1", { "@cn1": cn1,
"@cn2": cn2 }).json;
      assertEqual([ ], actual);
      assertEqual(0, c1.count());
      assertEqual(2010, c2.count());
    ...
    testMultiInsert: function () {
      var actual = AQL_EXECUTE("FOR i IN 1..2010 INSERT { value: i * 3, _key: CONCAT('one-', i) } INTO @@cn1 LET one = NEW._key INSERT { value: i * 5, _key: CONCAT('two-', i) } INTO @@cn2 LET two = NEW._key RETURN [ one, two ]", { "@cn1": cn1,
"@cn2": cn2 }).json;
...
e: "2" } ];
    ...
    testMultiRemove2: function () {
      AQL_EXECUTE("FOR i IN 1..2000 INSERT { _key: CONCAT('test', i) } IN @@cn1 INSERT { _key: CONCAT('test', i) } IN @@cn2", { "@cn1": cn1,
"@cn2": cn2 });
      assertEqual(2000, c1.count());
      assertEqual(2000, c2.count());

      AQL_EXECUTE("FOR i IN 1..2000 REMOVE { _key: CONCAT('test', i) } IN @@cn1 REMOVE { _key: CONCAT('test', i) } IN @@cn2", { "@cn1": cn1,
"@cn2": cn2 });
      assertEqual(0, c1.count());
      assertEqual(0, c2.count());
    }
