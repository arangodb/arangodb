...
const fetchCollectionChecksum = (serverUrl, shardId) => {
  let result = request({ method: "GET",
    url: serverUrl + `/_api/collection/${encodeURIComponent(shardId)}/checksum?withData=true`});
  assertEqual(200, result.statusCode);
  return JSON.parse(result.body).checksum;
};
  ...
  do {
    ...
    Object.entries(shardInfo).forEach(
      ([shard, [leader, follower]]) => {
        const leaderChecksum = fetchCollectionChecksum(getServerUrl(leader), shard);
        const followerChecksum = fetchCollectionChecksum(getServerUrl(follower), shard);

        if (leaderChecksum !== followerChecksum) {
          console.error(`Leader and follower have different checksums for shard ${shard}`);
          console.error("Leader: ", leaderChecksum);
          console.error("Follower: ", followerChecksum);
          failed = true;
        }
      });
    if (failed) {
