...
/* global getOptions, runSetup, assertTrue, assertEqual */
...
function FillBlockCacheSuite() {
  ...
  return {
    ...
    testQuery: function() {
      ...
      result = db._query("FOR doc IN @@collection RETURN doc.value1", { "@collection": cn }, { fillBlockCache: true }).toArray();
      assertEqual(250 * 1000, result.length);
      
      const newValue3 = db._engineStats()["rocksdb.block-cache-usage"];
      assertTrue(newValue3 >= newValue2 + 100 * 1000 * 1000, { newValue2, newValue3 });
    },
