...
function testSuite () {
  ...
  return {
    tearDownAll: function () {
      let result = request({ url: baseUrl() + "/_admin/debug/failat/startListeningEarly",
method: "delete",
auth: { bearer: jwtRoot } });
      ...
      let iterations = 0;
      while (iterations++ < 60) {
        let result = request({ url: baseUrl() + "/_api/collection",
method: "get",
auth: { bearer: jwtRoot } });
    ...
    testForbiddenWithoutJWT: function () {
      ["/_api/version", "/_admin/version", "/_admin/status", "/_api/collection", "/_admin/aardvark"].forEach((url) => {
        let result = request({ url: baseUrl() + url,
method: "get" });
