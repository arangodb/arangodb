...
function queryAnalysisSuite () {
  ...
  const queryWithBindBody = {query: "FOR x IN 1..5 LET y = SLEEP(@value) RETURN x",
bindVars: {value: 4}};
  ...
  return {
    setUp: function () {
      arango.PUT(properties, { enable: true,
...
trackSlowQueries: true});
    ...
    test_should_track_at_most_n_slow_queries: function () {
      let max = 3;
      arango.PUT_RAW(properties, {slowQueryThreshold: 0.1,
maxSlowQueries: max});
...
trackSlowQueries: false});
    ...
    test_should_truncate_the_query_string_to_at_least_64_bytes: function () {
      arango.PUT(properties, {slowQueryThreshold: 2,
maxQueryStringLength: 12});
