...
"action": "function () { return 1; }" };
    ...
    test_returns_an_error_when_using_a_non_declared_collection: function () {
      let cmd = api;
      let body = { "collections": { "write": cn1 },
"action": `function () { require("internal").db.${cn2}.save({ }); }` };
      let doc = arango.POST_RAW(cmd, body);
    ...
    test_returns_an_error_when_using_a_collection_in_invalid_mode_1: function () {
      let cmd = api;
      let body = { "collections": { "read": cn1 },
"action": `function () { require("internal").db.${cn1}.save({ }); }` };
      let doc = arango.POST_RAW(cmd, body);
    ...
    test_returns_an_error_when_using_a_collection_in_invalid_mode_2: function () {
      let cmd = api;
      let body = { "collections": { "read": [ cn1, cn2 ] },
"action": `function () { require("internal").db.${cn1}.save({ }); }` };
      let doc = arango.POST_RAW(cmd, body);
...
"action": `function () { var c = require("internal").db.UnitTestsTransactions; c.save({ }); c.save({ }); return c.count(); }` };
    ...
    test_aborting__single: function () {
      let cmd = api;
      let body = { "collections": { "write": cn },
"action": `function () { var c = require("internal").db.UnitTestsTransactions; c.save({ }); c.save({ }); throw "doh!"; }` };
      let doc = arango.POST_RAW(cmd, body);
...
"action": `function () { var c1 = require("internal").db.${cn1}; var c2 = require("internal").db.${cn2}; c1.save({ }); c1.save({ }); c2.save({ }); return [ c1.count(), c2.count() ]; }` };
    ...
    test_aborting__multi: function () {
      let cmd = api;
      let body = { "collections": { "write": [ cn1, cn2 ] },
"action": `function () { var c1 = require("internal").db.${cn1}; var c2 = require("internal").db.${cn2}; c1.save({ }); c1.save({ }); c2.save({ }); throw "doh!"; }` };
      let doc = arango.POST_RAW(cmd, body);
