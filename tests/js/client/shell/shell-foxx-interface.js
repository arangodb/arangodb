...
function foxxInterfaceSuite () {
  ...
  return {
    ...
    testFoxxInterfaceHead: function () {
      ...
      assertEqual(res.code, 200, res.parsedBody);
      // doesn't work for VST in 3.6 - assertUndefined(res.body);
      // doesn't work for VST in 3.6 - assertEqual(res.headers['content-length'], vpackObjectSize);
      assertEqual(res.headers['content-type'], vpackMime);
      assertEqual(res.headers['test'], 'header');
    ...
    testFoxxInterfaceHeadBinary: function () {
      ...
      assertEqual(res.code, 200, res.parsedBody);
      // doesn't work for VST in 3.6 - assertTrue(res.body instanceof Buffer);
      // doesn't work for VST in 3.6 - assertEqual(res.body.length, 0);
      
      // doesn't work for VST in 3.6 - assertEqual(res.headers['content-length'], pixelGif.length);
      assertEqual(res.headers['content-type'], binaryMime);
      assertEqual(res.headers['test'], 'header');
      assertEqual(res.headers['request-type'], 'HEAD_RAW');
    },
    ...
    testFoxxInterfaceHeadText: function () {
      ...
      assertEqual(res.code, 200, res.parsedBody);
      // doesn't work for VST in 3.6 - assertUndefined(res.body);
      // doesn't work for VST in 3.6 - assertEqual(res.headers['content-length'], pixelStr.length);
      assertEqual(res.headers['content-type'], textMime);
      assertEqual(res.headers['test'], 'header');
      assertEqual(res.headers['request-type'], 'HEAD_RAW');
    }
