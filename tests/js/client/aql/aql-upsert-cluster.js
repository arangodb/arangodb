...
/*global assertEqual, assertFalse, assertNull, assertTrue */
...
function query_execute(query, bindVars = null, options = {}) {
  return db._query({query, bindVars: bindVars, options: options});
};
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertOnlyUpdate : function () {
      var actual = query_execute("FOR i IN 0..9 UPSERT { _key: CONCAT('test', i) } INSERT { new: true, value2: i } UPDATE { value2: i, new: false, value: OLD.value + 1 } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });

      let sorted = actual.toArray().sort(sorter);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertOnlyInsert : function () {
      var actual = query_execute("FOR i IN 50..59 UPSERT { _key: CONCAT('test', i) } INSERT { new: true, value2: i, _key: CONCAT('test', i) } UPDATE { value2: i, new: false, value: OLD.value + 1 } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });
      let sorted = actual.toArray().sort(sorter);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertMixed : function () {
      var actual = query_execute("FOR i IN 0..39 UPSERT { _key: CONCAT('test', i) } INSERT { new: true, value2: i, _key: CONCAT('test', i) } UPDATE { value2: i, new: false, value: OLD.value + 1 } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });
      let sorted = actual.toArray().sort(sorter);

      assertEqual(40, sorted.length);
      for (var i = 0; i < 40; ++i) {
        var doc = sorted[i];
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertOnlyUpdateNonKey : function () {
      var actual = query_execute("FOR i IN 0..9 UPSERT { value : i } INSERT { new: true, value2: i, value: i } UPDATE { value2: i, new: false } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertOnlyInsertNonKey : function () {
      var actual = query_execute("FOR i IN 50..59 UPSERT { value: i } INSERT { new: true, value2: i, value: i } UPDATE { value2: i, new: false } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });
      let sorted = actual.toArray().sort(sorter);
...
////////////////////////////////////////////////////////////////////////////////
    ...
    testUpsertMixedNonKey : function () {
      var actual = query_execute("FOR i IN 0..39 UPSERT { value: i } INSERT { new: true, value2: i, value: i } UPDATE { value2: i, new: false } IN @@cn1 RETURN { old: OLD, new: NEW }", { "@cn1": cn1 });
      let sorted = actual.toArray().sort(sorter);
