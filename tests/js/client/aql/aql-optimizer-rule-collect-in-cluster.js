...
function optimizerCollectInClusterSuite(isSearchAlias) {
  ...
  return {
    ...
    testDistinct: function () {
      ...
      if (isSearchAlias) {
        ...
        */  
        ...
        // let indexResults = db._query(indexQuery);
        // assertEqual(1000, indexResults.toArray().length);
        // for (let i = 0; i < 1000; ++i) {
        //   assertEqual(i, indexResults.toArray()[i]);
        // }
    ...
    testDistinctMulti: function () {
      ...
      let results = db._query(query, null, {optimizer: {rules: ["-interchange-adjacent-enumerations"]}});
      let res = results.toArray();
      assertEqual(1000, res.length);
      for (let i = 0; i < 1000; ++i) {
        assertEqual(i, res[i]);
    ...
    testDistinctMultiView: function () {
      ...
      if (isSearchAlias) {
        ...
        if (isEnterprise) {
          ...
          indexResults = db._query(indexQueryEE);
          let res = indexResults.toArray();
          assertEqual(1000, res.length);
          for (let i = 0; i < 1000; ++i) {
            assertEqual(i, res[i]);
...
function optimizerCollectInClusterSingleShardSuite(isSearchAlias) {
  ...
  return {
    ...
    testSingleCount: function () {
      ...
      let results = db._query(query, null, opt);
      let res = results.toArray();           
      assertEqual(1, res.length);
      assertEqual(1000, res[0]);
    ...
    testSingleDistinctMulti: function () {
      ...
      let results = db._query(query, null, opt2);
      let res = results.toArray();
      assertEqual(1000, res.length);
      for (let i = 0; i < 1000; ++i) {
        assertEqual(i, res[i]);
      ...
      let plan = db._createStatement({query: query, bindVars: null, options: opt2}).explain().plan;
      let nodeTypes = plan.nodes.map(function (node) {
        return node.type;
      });
