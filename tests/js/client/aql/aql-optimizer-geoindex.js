  ...
  return {
    ...
    testLegacyRuleBasics : function () {
      if (enabled.basics) {
        ...
        queries.forEach(function(query) {
          var result = db._createStatement(query.string).explain();
    ...
    testLegacyRuleRemoveNodes : function () {
      if (enabled.removeNodes) {
        ...
        queries.forEach(function(query, qindex) {
          var result = db._query(query[0]);
          let res = result.toArray();
          expect(expected[qindex].length).to.be.equal(res.length);
          var pairs = res.map(function(res){
    ...
    testLegacyRuleSorted : function() {
      if (enabled.sorted) {
        ...
        var distances = result.toArray().map(d => { return parseFloat(d.toFixed(5)); });
        old=0;
  ...
  return {
    ...
    testQueries : function () {
      ...
      queries.forEach(function(q) {
        let result = db._query(q, bind, noOpt);
        assertEqual([ "1138", "1173", "1256" ], result.toArray());

        result = db._query(q, bind, doOpt);
        assertEqual([ "1138", "1173", "1256" ], result.toArray());

        let plan = db._createStatement({query: q, bindVars: bind}).explain();
        hasIndexNode(plan);
      });
    }
  };
...
function optimizerRuleTestSuite() {
  ...
  return {
    ...
    testRuleRemoveNodes: function () {
      if (enabled.removeNodes) {
        ...
        queries.forEach(function (query, qindex) {
          var result = db._query(query[0]);
          let res = result.toArray();
          expect(expected[qindex].length).to.be.equal(res.length, query[0]);
          var pairs = res.map(function (res) {
            return [res.lat, res.lon];
          });
    ...
    testSelfReference1: function () {
      ...
      const result = db._createStatement({query: query.string, bindVars: query.bindVars}).explain();
      hasNoIndexNode(result, query);
      hasFilterNode(result, query);
    },
