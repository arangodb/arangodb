...
function optimizerIndexesGroupSortTestSuite() {
  ...
  create_collection_with_persistent_index = function (row_count) {
  ...
  copy_collection = function (collection_name) {
  ...
  assert_query_plan_uses_group_sort = function (query, collection_name) {
    const stmt = db._createStatement(query)
    stmt.bind({ "@collection": collection_name })
  ...
  assert_query_does_sorting_correctly = function (query, collection_name) {
  ...
  return {
    ...
    test_indexed_group_sort_gives_same_results_as_unindexed_sort: function () {
      for (row_count of [1, 2, 10, 100, 800, 1000, 1001, 2501, 10000]) {
    ...
    test_uses_index_in_sort_after_index_creation: function () {
      ...
      var plan = query_plan(query, collection.name());
    ...
    test_does_not_use_index_when_sort_registers_are_not_in_same_order_as_index: function () {
      ...
      var plan = query_plan("FOR i IN @@collection SORT i.second_index_field RETURN i.second_index_field", collection.name());
      assertFalse(["use-indexes", "use-index-for-sort"].every((rule) =>
        plan.rules.indexOf(rule) >= 0), plan.rules);

      var plan = query_plan("FOR i IN @@collection SORT i.second_index_field, i.first_index_field RETURN i.second_index_field", collection.name());
      assertFalse(["use-indexes", "use-index-for-sort"].every((rule) =>
        plan.rules.indexOf(rule) >= 0), plan.rules);
    },
    ...
    test_sorting_in_different_direction_than_index: function () {
      ...
      var plan = query_plan("FOR i IN @@collection SORT i.first_index_field DESC, i.non_indexed_field DESC RETURN i.second_index_field", collection.name());
      assertTrue(["use-indexes", "use-index-for-sort"].every((rule) =>
        plan.rules.indexOf(rule) >= 0), plan.rules);
      ...
      var plan = query_plan("FOR i IN @@collection SORT i.first_index_field, i.second_index_field DESC, i.non_indexed_field DESC RETURN i.second_index_field", collection.name());
      assertFalse(["use-indexes", "use-index-for-sort"].every((rule) =>
        plan.rules.indexOf(rule) >= 0), plan.rules);
    },
    ...
    test_limit: function () {
      ...
      for (count of [0, 1, 100, 1000, 1001, 2000]) {
        ...
        for (offset of [0, 1, 100, 1000, 1001, 2000]) {
