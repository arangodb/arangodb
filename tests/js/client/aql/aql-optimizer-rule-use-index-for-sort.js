  ...
  return {
    ...
    testRuleHasEffect : function () {
      ...
      queries.forEach(function(query, i) {
        let result = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort}).explain();
        ...
        QResults[1] = db._query(query, { }, paramIndexFromSort).toArray();
    ...
    testRuleHasEffectButSortsStill : function () {
      ...
      queries.forEach(function(query, i) {
        var j;
        var result = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort}).explain();
    ...
    testSortIndexable: function () {
      ...
      QResults[0] = db._query(query, { }, paramNone).toArray().sort(sortArray);

      // -> use-index-for-sort alone.
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort}).explain();
      QResults[1] = db._query(query, { }, paramIndexFromSort).toArray().sort(sortArray);
      ...
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort_RemoveCalculations}).explain();
      QResults[2] = db._query(query, { }, paramIndexFromSort_RemoveCalculations).toArray().sort(sortArray);
      // our rule should have been applied.
    ...
    testSortMoreThanIndexed: function () {
      ...
      QResults[3] = db._query(query, { }, paramIndexRange).toArray();
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexRange}).explain();
    ...
    testRangeSupersedesSort: function () {
      ...
      // -> combined use-index-for-sort and use-index-range
      QResults[2] = db._query(query, { }, paramIndexFromSort_IndexRange).toArray().sort(sortArray);
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort_IndexRange}).explain();
      ...
      // -> use-index-range alone.
      QResults[3] = db._query(query, { }, paramIndexRange).toArray().sort(sortArray);
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexRange}).explain();
      assertEqual([ secondRuleName ], removeAlwaysOnClusterRules(XPresult.plan.rules));
      // the sortnode and its calculation node should be there.
      hasSortNode(XPresult);
      ...
      QResults[4] = db._query(query, { }, paramIndexFromSort_IndexRange_RemoveCalculations).toArray().sort(sortArray);

      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort_IndexRange_RemoveCalculations}).explain();
    ...
    testRangeSupersedesSort2: function () {
      ...
      // -> combined use-index-for-sort, remove-unnecessary-calculations-2 and use-index-range
      QResults[4] = db._query(query, { }, paramIndexFromSort_IndexRange_RemoveCalculations).toArray();
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexFromSort_IndexRange_RemoveCalculations}).explain();
    ...
    testRangeEquals: function () {
      ...
      // -> use-index-range alone.
      QResults[1] = db._query(query, { }, paramIndexRange).toArray();
      XPresult    = db._createStatement({query: query, bindVars:  { }, options:  paramIndexRange}).explain();
      assertEqual([ secondRuleName ], removeAlwaysOnClusterRules(XPresult.plan.rules));
      // the sortnode and its calculation node should be there.
      hasCalculationNodes(XPresult, 2);
    ...
    testRangeLessThan: function () {
      ...
      // -> use-index-range alone.
      QResults[1] = db._query(query, { }, paramIndexRange).toArray().sort(sortArray);
    ...
    testSortAscEmptyCollection : function () {
      ...
      var rules = db._createStatement("FOR v IN " + colName + " SORT v.d ASC RETURN v").explain().plan.rules;
      assertNotEqual(-1, rules.indexOf(ruleName));

      var nodes = db._createStatement("FOR v IN " + colName + " SORT v.d ASC RETURN v").explain().plan.nodes;
      var seen = false;
    ...
    testSortDescEmptyCollection : function () {
      ...
      var rules = db._createStatement("FOR v IN " + colName + " SORT v.d DESC RETURN v").explain().plan.rules;
      assertNotEqual(-1, rules.indexOf(ruleName));
      
      var nodes = db._createStatement("FOR v IN " + colName + " SORT v.d DESC RETURN v").explain().plan.nodes;
      var seen = false;
      nodes.forEach(function(node) {
        if (node.type === "IndexNode") {
          seen = true;
