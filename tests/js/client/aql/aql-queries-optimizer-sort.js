  ...
  let explain = function (query, params) {
    return helper.removeClusterNodes(
      helper.getCompactPlan(
        db._createStatement({query: query, bindVars: params, options: {optimizer: {rules: ["-all", "+use-index-for-sort", "+use-indexes", "+remove-redundant-sorts"]}}}).explain()
      ).map(function (node) {
        return node.type;
      }));
  };
...
function sortTestsuite () {
  ...
  let explain = function (query, params) {
    return helper.removeClusterNodes(helper.getCompactPlan(db._createStatement({query: query, bindVars: params, options: { optimizer: { rules: [ "-all", "+use-index-for-sort", "+use-indexes", "+remove-redundant-sorts" ] }}}).explain()).map(function(node) { return node.type; }));
  };
  ...
  return {
    ...
    setUpAll : function () {
      ...
      testStringsSorted=db._query("FOR t IN @bla SORT t RETURN t", {"bla": testStrings}).toArray();
    },
