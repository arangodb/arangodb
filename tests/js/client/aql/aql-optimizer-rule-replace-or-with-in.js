...
function NewAqlReplaceORWithINTestSuite () {
  ...
  var isRuleUsed = function (query, params) {
   var result = db._createStatement({query: query, bindVars: params, options:  { optimizer: { rules: [ "-all", "+" + ruleName ] } }}).explain();
   assertTrue(result.plan.rules.indexOf(ruleName) !== -1, query);
   result = db._createStatement({query: query, bindVars: params, options:  { optimizer: { rules: [ "-all" ] } }}).explain();
   assertTrue(result.plan.rules.indexOf(ruleName) === -1, query);
  };
