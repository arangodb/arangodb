...
const assertRuleIsUsed = (query) => {
  ...
  const planDisabled = db._createStatement({query: query, bindVars:  { }, options:  paramDisabled}).explain();
  expect(planDisabled.plan.rules.indexOf(ruleName)).to.equal(-1, query);

  // assert the rule is used when it's enabled
  const planEnabled = db._createStatement({query: query, bindVars:  { }, options:  paramEnabled}).explain();
...
const assertResultsAreUnchanged = (query) => {
  ...
  const resultDisabled = db._createStatement({query: query, bindVars:  { }, options:  paramDisabled}).execute().toArray();
  const resultEnabled = db._createStatement({query: query, bindVars:  { }, options:  paramEnabled}).execute().toArray();
...
describe('Rule optimize-traversals', () => {
  ...
  it('should remove unused variables', () => {
    ...
    queries.forEach((query) => {
      const result = db._createStatement({query: query[0], bindVars:  { }, options:  paramEnabled}).explain();
  ...
  it('should not take effect in these queries', () => {
    ...
    queries.forEach(function (query) {
      const result = db._createStatement({query: query, bindVars:  { }, options:  paramEnabled}).explain();
  ...
  describe('using condition vars', () => {
    it('taking invalid start form subquery', () => {
      ...
      expect(result.plan.rules.indexOf(ruleName)).to.not.equal(-1, query);
      expect(db._query(query).toArray().length).to.equal(0);
    });
  ...
  describe('filtering on own output', () => {
    it('should filter with v on path', () => {
      ...
      let plan = db._createStatement({query: query, bindVars:  bindVars, options:  paramEnabled}).explain();
      ...
      plan = db._createStatement({query: query, bindVars:  bindVars, options:  paramEnabled}).explain();
      expect(plan.plan.rules.indexOf(ruleName)).to.equal(-1, query);
  ...
  describe('various filter optimizations', () => {
    ...
    const checkDoesOptimize = (conditions, shouldOptimize) => {
      for (let c of conditions) {
        ...
        try {
          const expl = db._createStatement({query: q, bindVars:  { }}).explain();
          if (shouldOptimize) {
