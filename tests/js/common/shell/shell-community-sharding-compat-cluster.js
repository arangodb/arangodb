...
numberOfShards: 5 });
    ...
    testCreateWithEnterpriseSharding: function () {
      let c = db._create(name1, { shardingStrategy: enterpriseCompat,
numberOfShards: 5 });
...
numberOfShards: 5 });
    ...
    testCreateWithCommunitySharding: function () {
      let c = db._create(name1, { shardingStrategy: communityCompat,
numberOfShards: 5 });
...
value: i });
    ...
    testDistributeShardsLikeWithCommunitySharding: function () {
      let c1 = db._create(name1, { shardingStrategy: communityCompat,
numberOfShards: 5 });
...
numberOfShards: 5 });
    ...
    testEdgeCreateWithCommunitySharding: function () {
      let c = db._createEdgeCollection(name1, { shardingStrategy: communityCompat,
numberOfShards: 5 });
      assertEqual(communityCompat, c.properties()["shardingStrategy"]);
      assertEqual(5, c.properties()["numberOfShards"]);
      assertEqual(["_key"], c.properties()["shardKeys"]);
...
_to: "v/test" + i });
    ...
    testEdgeCreateWithEnterpriseSharding: function () {
      if (!isEnterprise) {
        try {
          db._createEdgeCollection(name1, { shardingStrategy: enterpriseCompat,
numberOfShards: 5 });
        } catch (err) {
          assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
        }
...
_to: "v/test" + i });
    ...
    testEdgeDistributeShardsLikeWithNonDefaultKeys: function () {
      let c1 = db._createEdgeCollection(name1, { shardingStrategy: communityCompat,
shardKeys: ["value"],
numberOfShards: 5 });
