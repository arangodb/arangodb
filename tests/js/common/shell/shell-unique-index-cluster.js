...
result: true }
    ...
    tests.forEach(function (test) {
      ...
      var collection = internal.db._create(cn, { shardKeys: test.shardKeys,
numberOfShards: 4 });
      if (test.result) {
        // index creation should work
        var idx = collection.ensureIndex({ type: indexType,
fields: test.indexKeys,
unique: true });
        ...
        for (i = 0; i < n;) {
          ...
          if (i === 0) {
            ...
            assertNotEqual(-1, db._createStatement(query).explain().plan.nodes.map(function (node) {
 return node.type;
}).indexOf("IndexNode"));
          }
        ...
        try {
          collection.ensureIndex({ type: indexType,
fields: test.indexKeys,
unique: true });
          fail();
        } catch (err) {
