...
value: i });
    ...
    testCreateWithInvalidSharding: function () {
      try {
        db._create(name1, { shardingStrategy: "peng",
numberOfShards: 5 });
        fail();
      } catch (err) {
    ...
    testCreateWithNoneSharding: function () {
      try {
        db._create(name1, { shardingStrategy: "none",
numberOfShards: 5 });
        fail();
      } catch (err) {
        assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
    ...
    testCreateWithEnterpriseSharding: function () {
      if (!isEnterprise) {
        try {
          db._create(name1, { shardingStrategy: "enterprise-compat",
numberOfShards: 5 });
        } catch (err) {
          assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
        }
    ...
    testCreateWithCommunitySharding: function () {
      let c = db._create(name1, { shardingStrategy: hash,
numberOfShards: 5 });
...
value: i });
    ...
    testDistributeShardsLikeWithCommunitySharding: function () {
      let c1 = db._create(name1, { shardingStrategy: hash,
numberOfShards: 5 });
      let c2 = db._create(name2, { distributeShardsLike: name1 });
...
shardKeys: ["one", "two"] });
      try {
        db._create(name2, { distributeShardsLike: name1,
shardKeys: ["three"] });
        fail();
      } catch (err) {
        assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
...
_to: "v/test" + i });
    ...
    testEdgeCreateWithInvalidSharding: function () {
      try {
        db._createEdgeCollection(name1, { shardingStrategy: "peng",
numberOfShards: 5 });
        fail();
      } catch (err) {
        assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
    ...
    testEdgeCreateWithNoneSharding: function () {
      try {
        db._createEdgeCollection(name1, { shardingStrategy: "none",
numberOfShards: 5 });
        fail();
      } catch (err) {
        assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
    ...
    testEdgeCreateWithCommunitySharding: function () {
      let c = db._createEdgeCollection(name1, { shardingStrategy: hash,
numberOfShards: 5 });
      assertEqual(hash, c.properties()["shardingStrategy"]);
      assertEqual(5, c.properties()["numberOfShards"]);
      assertEqual(["_key"], c.properties()["shardKeys"]);
...
_to: "v/test" + i });
    ...
    testEdgeCreateWithEnterpriseSharding: function () {
      if (!isEnterprise) {
        try {
          db._createEdgeCollection(name1, { shardingStrategy: "enterprise-compat",
numberOfShards: 5 });
        } catch (err) {
          assertEqual(ERRORS.ERROR_BAD_PARAMETER.code, err.errorNum);
        }
...
numberOfShards: 5 });
      ...
      let keys = [];
      for (let i = 0; i < 1000; ++i) {
        keys.push(c.insert({ _from: "v/test" + i,
_to: "v/test" + i })._key);
      }
...
shardKeys: ["value"] });
      ...
      let keys = [];
      for (let i = 0; i < 1000; ++i) {
        keys.push(c2.insert({ value: "test" + i,
_from: "v/test" + i,
_to: "v/test" + i })._key);
      }
...
_to: "v/test" + i })._key);
    ...
    testEdgeDistributeShardsLikeWithNonDefaultKeys: function () {
      let c1 = db._createEdgeCollection(name1, { shardingStrategy: hash,
shardKeys: ["value"],
numberOfShards: 5 });
      let c2 = db._createEdgeCollection(name2, { distributeShardsLike: name1,
shardKeys: ["value"] });
