...
function PruneExpressionsSuite () {
  ...
  return {
    setUpAll: function () {
      ...
      require("@arangodb/aql/functions").register(udf, function () {
 return '0';
});
...
numberOfShards: 1 });
      ...
      db[vn].insert({ _key: "test1",
value: 1 });
      db[vn].insert({ _key: "test2",
value: 2 });
      db[vn].insert({ _key: "test3",
value: 3 });

      db[en].insert({ _from: vn + "/test1",
_to: vn + "/test2",
value: 1 });
      db[en].insert({ _from: vn + "/test2",
_to: vn + "/test3",
value: 0 });
    },
