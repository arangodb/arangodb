...
TEST_F(InvalidGeoJSONInputTest, empty_object) {
  ...
  ASSERT_TRUE(geo::geojson::parsePolygon(vpack, shape, false).is(TRI_ERROR_BAD_PARAMETER));

  ASSERT_TRUE(geo::geojson::parseRegion(vpack, shape, false).is(TRI_ERROR_BAD_PARAMETER));
}
...
TEST_F(InvalidGeoJSONInputTest, bad_multipolygon_no_coordinates) {
  ...
  ASSERT_TRUE(geo::geojson::parseMultiPolygon(vpack, shape, false).is(TRI_ERROR_BAD_PARAMETER));
}
...
TEST_F(ValidGeoJSONInputTest, valid_point_as_region) {
  ...
  ASSERT_EQ(geo::geojson::Type::POINT, geo::geojson::type(vpack));
  ASSERT_TRUE(geo::geojson::parseRegion(vpack, shape, false).ok());
...
TEST_F(ValidGeoJSONInputTest, valid_multipolygon) {
  ...
  ASSERT_EQ(geo::geojson::Type::MULTI_POLYGON, geo::geojson::type(vpack));
  ASSERT_TRUE(geo::geojson::parseMultiPolygon(vpack, shape, false).ok());
