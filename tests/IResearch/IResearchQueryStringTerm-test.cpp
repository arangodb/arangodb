...
TEST_CASE("IResearchQueryTestStringTerm", "[iresearch][iresearch-query]") {
  ...
  // _rev attribute
  {
    ...
    std::string const query = "FOR d IN testView SEARCH d._rev == '" + static_cast<std::string>(revRef) + "' RETURN d";
  ...
  // _key attribute
  {
    ...
    std::string const query = "FOR d IN testView SEARCH d._key == '" + key.toString() + "' RETURN d";
  ...
  // _id attribute
  {
    ...
    std::string const query = "FOR d IN testView SEARCH d._id == '" + id + "' RETURN d";
...
//  }
  ...
  // subquery, d.name == (FOR i IN collection_1 SEARCH i.name == 'A' RETURN i)[0]), unordered
  {
    ...
    REQUIRE(queryResult.result.is(TRI_ERROR_BAD_PARAMETER)); // unsupported type: object
  }
