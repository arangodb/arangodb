...
TEST_CASE("IResearchQueryTestComplexBoolean", "[iresearch][iresearch-query]") {
  ...
  // (prefix || phrase || exists || !field)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER STARTS_WITH(d.prefix, 'abc') || ANALYZER(PHRASE(d['duplicated'], 'z'), 'test_analyzer') || EXISTS(d.same) || d['value'] != 3.14 SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
  ...
  // field && prefix && !exists
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.same == 'xyz' && STARTS_WITH(d['prefix'], 'abc') && NOT EXISTS(d.value) SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // (field && prefix) || (phrase && exists)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER (d['same'] == 'xyz' && STARTS_WITH(d.prefix, 'abc')) || (ANALYZER(PHRASE(d['duplicated'], 'z'), 'test_analyzer') && EXISTS(d.value)) SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // (field && prefix) || (phrase && exists)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER (d['same'] == 'xyz' && STARTS_WITH(d.prefix, 'abc')) || (ANALYZER(PHRASE(d['duplicated'], 'z'), 'test_analyzer') && EXISTS(d.value)) SORT BM25(d) ASC, TFIDF(d) DESC, d.seq limit 5 RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // (field || exists) && (starts_with || phrase || range)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER (d.same == 'xyz' || EXISTS(d['value'])) && (STARTS_WITH(d.prefix, 'abc') || ANALYZER(PHRASE(d['duplicated'], 'z'), 'test_analyzer') || d.seq >= -3) SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
