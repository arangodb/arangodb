...
TEST_CASE("IResearchQueryTestIn", "[iresearch][iresearch-query]") {
  ...
  // test array
  {
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ [ -1, 0 ], [ 1, \"abc\" ] ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
  ...
  // test bool
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ true ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
  ...
  // test bool via []
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d['value'] IN [ true, false ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test numeric
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ 123, 1234 ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test numeric, limit 2
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ 123, 1234 ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq LIMIT 2 RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test null
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ null ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test null via []
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d['value'] IN [ null, null ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test object
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ { \"a\": 7, \"b\": \"c\" } ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_QUERY_PARSE == result.code);
  }
  ...
  // test string
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.value IN [ \"abc\", \"xyz\" ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
