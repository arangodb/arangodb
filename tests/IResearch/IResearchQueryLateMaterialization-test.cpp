...
TEST_P(IResearchQueryLateMaterializationTest, test_1) {
  auto const query = std::string("FOR d IN ") + viewName2 +
                     " SEARCH d.value IN [1, 2, 11, 12] LET a = NOOPT(d.foo) "
                     "LET e = SUM(FOR c IN " +
                     viewName1 +
                     " LET p = CONCAT(c.foo, c.foo) RETURN p) SORT CONCAT(a, "
                     "e) LIMIT 10 RETURN d";
...
TEST_P(IResearchQueryLateMaterializationTest, test_3) {
  auto const query =
      std::string("FOR d IN ") + viewName2 +
      " SEARCH d.value IN [1, 2, 11, 12] SORT d.value DESC LET c = BM25(d) * 2 "
...
TEST_P(IResearchQueryLateMaterializationTest, test_4) {
  auto const query = std::string("FOR d IN ") + viewName2 +
                     " SEARCH d.value IN [1, 2, 11, 12] SORT RAND(), d.value "
...
TEST_P(IResearchQueryLateMaterializationTest, test_5) {
  auto const query = std::string("FOR d IN ") + viewName2 +
                     " SEARCH d.value IN [1, 2, 11, 12] SORT d.value DESC, "
...
TEST_P(IResearchQueryLateMaterializationTest, test_12) {
  auto const query = std::string("FOR c IN ") + viewName1 +
                     ...
                     " SEARCH d.value IN [c.value, c.value + 1] SORT d.value "
                     "DESC LIMIT 10 RETURN d";
