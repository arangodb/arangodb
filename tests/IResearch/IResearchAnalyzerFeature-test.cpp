...
TEST_F(IResearchAnalyzerFeatureTest, test_persistence) {
  ...
  // read valid configuration (different parameter options)
  {
    ...
    feature.start();  // features doesn't load persisted analyzers

    EXPECT_TRUE(feature.visit(
    [](arangodb::iresearch::IResearchAnalyzerFeature::AnalyzerPool::ptr const& ) -> bool {
       return false;
    }));
  }
  ...
  // add new records
  {
...

    {
      ...
      EXPECT_TRUE(feature.emplace(
        result, arangodb::StaticStrings::SystemDatabase + "::valid",
  ...
  // emplace on single-server (should persist)
  {
    ...
    arangodb::iresearch::IResearchAnalyzerFeature feature(server);
    EXPECT_TRUE(
      feature.emplace(result, arangodb::StaticStrings::SystemDatabase + "::test_analyzerA",
                      "TestAnalyzer", VPackParser::fromJson("\"abc\"")->slice(),
                      {irs::frequency::type()}).ok()
    );
    EXPECT_TRUE(result.first);
    EXPECT_NE(nullptr, feature.get(arangodb::StaticStrings::SystemDatabase + "::test_analyzerA"));
...
TEST_F(IResearchAnalyzerFeatureTest, test_start) {
  ...
  // test feature start load configuration (inRecovery)
  {
    auto before = StorageEngineMock::recoveryStateResult;
    StorageEngineMock::recoveryStateResult = arangodb::RecoveryState::IN_PROGRESS;
    auto restore = irs::make_finally([&before]() -> void {
  ...
  {
    {
      ...
      EXPECT_TRUE(feature.emplace(
        result, arangodb::StaticStrings::SystemDatabase + "::test_analyzer",
        "identity",
    ...
    arangodb::iresearch::IResearchAnalyzerFeature feature(server);
    feature.prepare();  // add static analyzers
    feature.start(); // feature doesn't load analyzers on startup
  ...
  // test feature start load configuration
  {
    arangodb::iresearch::IResearchAnalyzerFeature feature(server);
    feature.prepare();  // add static analyzers
    feature.start(); // feature doesn't load analyzers on startup
