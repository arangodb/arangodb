...
TEST_F(IResearchAnalyzerFeatureTest, test_tokens) {
  ...
  // test tokenization
  {
    ...
    AqlValueWrapper result(impl(&exprCtx, nullptr, *args));
  ...
  // Zero count (less than expected)
  {
    ...
    EXPECT_THROW(AqlValueWrapper(impl(&exprCtx, nullptr, args)), arangodb::basics::Exception);
  }
  // test invalid arg count
  ...
  // test double data type
  {
    ...
    auto result = AqlValueWrapper(impl(&exprCtx, nullptr, *args));
    EXPECT_TRUE(result->isArray());
