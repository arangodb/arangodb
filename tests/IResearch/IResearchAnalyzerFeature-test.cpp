...
struct TestIndex : public arangodb::Index {
  ...
  std::unique_ptr<arangodb::IndexIterator> iteratorForCondition(
      ...
      arangodb::IndexIteratorOptions const& /* opts */, arangodb::ReadOwnWrites) override {
    return nullptr;
  }
  void load() override {}
...
TEST_F(IResearchAnalyzerFeatureTest, test_emplace_duplicate_valid) {
  ...
  auto pool = feature.get(analyzerName(), arangodb::QueryAnalyzerRevisions::QUERY_LATEST);
  ASSERT_NE(pool, nullptr);
  EXPECT_EQ(arangodb::iresearch::Features(irs::IndexFeatures::FREQ), pool->features());
  {
    arangodb::iresearch::IResearchAnalyzerFeature::EmplaceResult result;
    EXPECT_TRUE(feature
                    .emplace(result, analyzerName(), "TestAnalyzer",
  ...
  auto poolOther =
      feature.get(analyzerName(), arangodb::QueryAnalyzerRevisions::QUERY_LATEST);
...
TEST_F(IResearchAnalyzerFeatureTest, test_emplace_creation_properties_too_large) {
  ...
  prop.close();
  auto res = feature.emplace(result, analyzerName(), "TestAnalyzer", prop.slice(),
                             arangodb::iresearch::Features({}, irs::IndexFeatures::FREQ));
  EXPECT_FALSE(res.ok());
  EXPECT_EQ(TRI_ERROR_BAD_PARAMETER, res.errorNumber());
  EXPECT_EQ(feature.get(analyzerName(), arangodb::QueryAnalyzerRevisions::QUERY_LATEST), nullptr);
...
TEST_F(IResearchAnalyzerFeatureTest, test_emplace_add_static_analyzer) {
  ...
  auto res =
      feature.emplace(result, "identity", "identity", VPackSlice::noneSlice(),
  ...
  ASSERT_NE(pool, nullptr);
  EXPECT_EQ(arangodb::iresearch::Features(arangodb::iresearch::FieldFeatures::NORM,
                                          irs::IndexFeatures::FREQ),
            pool->features());
...
TEST_F(IResearchAnalyzerFeatureCoordinatorTest, test_ensure_index_add_factory) {
  // add index factory
  {
    struct IndexTypeFactory : public arangodb::IndexTypeFactory {
      ...
      virtual bool equal(arangodb::velocypack::Slice lhs, arangodb::velocypack::Slice rhs,
...
TEST_F(IResearchAnalyzerFeatureTest, test_normalize) {
  ...
  // normalize 'identity' (with prefix)
  {
    ...
    auto normalized =
        arangodb::iresearch::IResearchAnalyzerFeature::normalize(analyzer,
                                                                 active.name(), true);
...
TEST_F(IResearchAnalyzerFeatureTest, test_analyzer_features) {
  ...
  // norm, version 0
  {
    ...
    ASSERT_TRUE(arangodb::iresearch::IResearchAnalyzerFeature::createAnalyzerPool(
                    ...
                    .ok());
    ASSERT_NE(nullptr, pool);
    ASSERT_EQ((arangodb::iresearch::Features{arangodb::iresearch::FieldFeatures::NORM,
                                             irs::IndexFeatures::FREQ}),
              pool->features());
    ASSERT_EQ(irs::IndexFeatures::FREQ, pool->indexFeatures());
...
TEST_F(IResearchAnalyzerFeatureTest, test_analyzer_equality) {
  ...
  // different revision - this is still the same analyzer!
  {
    ...
    ASSERT_TRUE(arangodb::iresearch::IResearchAnalyzerFeature::createAnalyzerPool(
                    rhs, "db::test", "TestAnalyzer",
                    VPackParser::fromJson("\"abc\"")->slice(),
                    arangodb::AnalyzersRevision::MIN + 1, arangodb::iresearch::Features{},
                    arangodb::iresearch::LinkVersion::MIN, false)
                    .ok());
    ASSERT_NE(nullptr, rhs);
    ASSERT_EQ(*lhs, *rhs);
...
TEST_F(IResearchAnalyzerFeatureTest, test_remove) {
  ...
  arangodb::network::ConnectionPool::Config poolConfig(
      server.server().getFeature<arangodb::MetricsFeature>());
  ...
  // remove existing (dbserver)
  {
    ...
    newServer.getFeature<arangodb::ClusterFeature>().agencyCache().applyTestTransaction(bogus);
...
TEST_F(IResearchAnalyzerFeatureTest, test_prepare) {
  ...
  feature.visit([&expected, &feature](arangodb::iresearch::AnalyzerPool::ptr const& analyzer) -> bool {
    ...
    EXPECT_EQ(itr->second.features,
              feature
                  .get(analyzer->name(), arangodb::QueryAnalyzerRevisions::QUERY_LATEST)
                  ->features());
    expected.erase(itr);
    return true;
  });
...
TEST_F(IResearchAnalyzerFeatureTest, test_tokens) {
  ...
  // test typed analyzer tokenization BTS-357
  {
    ...
    std::string expected123[] = {"oL/wAAAAAAAA", "sL/wAAAAAA==", "wL/wAAA=",
                                 "0L/w",         "oIAAAAAAAAAA", "sIAAAAAAAA==",
                                 "wIAAAAA=",     "0IAA",         "oL/wAAAAAAAA",
...
TEST_F(IResearchAnalyzerFeatureTest, test_visit) {
  struct ExpectedType {
    ...
    ExpectedType(irs::string_ref const& name, irs::string_ref const& properties,
                 arangodb::iresearch::Features const& features, irs::string_ref const& type)
  ...
  // create system vocbase (before feature start)
  {
    ...
    std::shared_ptr<arangodb::LogicalCollection> unused;
    arangodb::OperationOptions options(arangodb::ExecContext::current());
    arangodb::methods::Collections::createSystem(*system, options,
                                                 arangodb::tests::AnalyzerCollectionName,
                                                 false, unused);
  }
  ...
  std::shared_ptr<arangodb::LogicalCollection> unused;
  arangodb::OperationOptions options(arangodb::ExecContext::current());
  arangodb::methods::Collections::createSystem(*vocbase0, options,
                                               arangodb::tests::AnalyzerCollectionName,
                                               false, unused);
  arangodb::methods::Collections::createSystem(*vocbase1, options,
                                               arangodb::tests::AnalyzerCollectionName,
                                               false, unused);
...
TEST_F(IResearchAnalyzerFeatureTest, custom_analyzers_toVelocyPack) {
  ...
  EXPECT_EQUAL_SLICES(VPackParser::fromJson(R"({"locale":"ru_RU","case":"upper","accent":true})")
                          ->slice(),
...
TEST_F(IResearchAnalyzerFeatureTest, custom_analyzers_vpack_create) {
  ...
  // non default values for stem, accent and case
  {
    ...
    EXPECT_TRUE(feature
                    ...
                    .ok());
    EXPECT_TRUE(result.first);
    EXPECT_EQUAL_SLICES(VPackParser::fromJson(R"({"locale":"ru_RU","case":"upper","stopwords":[],"accent":true,"stemming":false})")
                            ->slice(),
                        result.first->properties());
  }
