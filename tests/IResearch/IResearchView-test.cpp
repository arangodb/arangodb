...
TEST_F(IResearchViewTest, test_tracked_cids) {
  ...
  ASSERT_TRUE((nullptr != viewImpl));
  std::shared_ptr<arangodb::Index> index =
      arangodb::iresearch::IResearchMMFilesLink::factory().instantiate(*logicalCollection,
                                                                       linkJson->slice(),
                                                                       42, false);
  ASSERT_TRUE((false == !index));
  auto link = std::dynamic_pointer_cast<arangodb::iresearch::IResearchLink>(index);
...
TEST_F(IResearchViewTest, test_update_partial) {
  ...
  // modify meta params
  {
    ...
    // for persistence
    {
      ...
      builder.openObject();
      view->properties(builder, arangodb::LogicalDataSource::Serialization::Peristence);
      builder.close();
  ...
  // test rollback on meta modification failure (as an example invalid value for 'cleanupIntervalStep')
  {
    ...
    // for persistence
    {
      ...
      builder.openObject();
      logicalView->properties(builder, arangodb::LogicalDataSource::Serialization::Peristence);
      builder.close();
