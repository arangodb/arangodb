...
struct DocIdScorer : public irs::sort {
  ...
  struct Prepared : public irs::prepared_sort_base<uint64_t, void> {
    ...
    virtual irs::score_function prepare_scorer(irs::sub_reader const& segment,
...
TEST_F(IResearchViewTest, test_defaults) {
  ...
  // new view definition with links to missing collections
  {
    ...
    arangodb::LogicalView::ptr view;
    auto res =
        arangodb::iresearch::IResearchView::factory().create(view, vocbase,
...
TEST_F(IResearchViewTest, test_vocbase_inventory) {
  ...
  // check vocbase inventory
  {
    ...
    ASSERT_TRUE(logicalView
                    ->properties(viewDefinition, arangodb::LogicalDataSource::Serialization::Inventory)
                    .ok());
...
TEST_F(IResearchViewTest, test_drop_collection) {
  ...
  EXPECT_TRUE(
      (true == logicalView->properties(viewUpdateJson->slice(), true, true).ok()));
...
TEST_F(IResearchViewTest, test_drop_cid) {
  ...
  // cid in list of collections for snapshot (view definition updated, not persisted until recovery is complete)
  {
    ...
    auto* view = dynamic_cast<arangodb::iresearch::IResearchView*>(logicalView.get());
    ASSERT_TRUE((false == !view));
    auto index = StorageEngineMock::buildLinkMock(arangodb::IndexId{__LINE__},
...
TEST_F(IResearchViewTest, test_insert) {
  ...
  // in recovery (skip operations before or at recovery tick)
  {
...

    {
      ...
      EXPECT_TRUE(
          link->insert(trx, arangodb::LocalDocumentId(1), docJson->slice()).ok());
      StorageEngineMock::recoveryTickResult = 42;
      EXPECT_TRUE(
          link->insert(trx, arangodb::LocalDocumentId(2), docJson->slice()).ok());
  ...
  // not in recovery (FindOrCreate)
  {
...

    {
      ...
      EXPECT_TRUE(
          (link->insert(trx, arangodb::LocalDocumentId(2), docJson->slice()).ok()));
      EXPECT_TRUE(
          (link->insert(trx, arangodb::LocalDocumentId(1), docJson->slice()).ok()));  // 2nd time
      EXPECT_TRUE(
          (link->insert(trx, arangodb::LocalDocumentId(2), docJson->slice()).ok()));  // 2nd time
      EXPECT_TRUE((trx.commit().ok()));
      EXPECT_TRUE(link->commit().ok());
    }
  ...
  // not in recovery (SyncAndReplace)
  {
    ...
    arangodb::transaction::Methods trx(arangodb::transaction::StandaloneContext::Create(vocbase),
                                       ...
                                       arangodb::transaction::Options());
    auto* snapshot =
        view->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::SyncAndReplace);
    EXPECT_TRUE((4 == snapshot->docs_count()));
  }
  ...
  // in recovery (skip operations before or at recovery tick)
  {
...

    {
      ...
      EXPECT_TRUE(
          link->insert(trx, arangodb::LocalDocumentId(2), docJson->slice()).ok());
      EXPECT_TRUE(
          link->insert(trx, arangodb::LocalDocumentId(3), docJson->slice()).ok());

      // skip tick operations before recovery tick
      StorageEngineMock::recoveryTickResult = 41;
      EXPECT_TRUE(link->remove(trx, arangodb::LocalDocumentId(1), VPackSlice::noneSlice())
                      .ok());
      StorageEngineMock::recoveryTickResult = 42;
      EXPECT_TRUE(link->insert(trx, arangodb::LocalDocumentId(2), VPackSlice::noneSlice())
                      .ok());

      // apply remove after recovery tick
      StorageEngineMock::recoveryTickResult = 43;
      EXPECT_TRUE(link->remove(trx, arangodb::LocalDocumentId(3), VPackSlice::noneSlice())
                      .ok());
...
TEST_F(IResearchViewTest, test_query) {
  ...
  // ordered iterator
  {
    ...
    // fill with test data
    {
      ...
      for (size_t i = 0; i < 12; ++i) {
        EXPECT_TRUE(
            (link->insert(trx, arangodb::LocalDocumentId(i), doc->slice()).ok()));
      }
  ...
  // snapshot isolation
  {
    ...
    arangodb::transaction::Methods trx0(arangodb::transaction::StandaloneContext::Create(vocbase),
                                        ...
                                        arangodb::transaction::Options());
    auto* snapshot0 =
        view->snapshot(trx0, arangodb::iresearch::IResearchView::SnapshotMode::FindOrCreate);
    ...
    arangodb::transaction::Methods trx1(arangodb::transaction::StandaloneContext::Create(vocbase),
                                        ...
                                        arangodb::transaction::Options());
    auto* snapshot1 =
        view->snapshot(trx1, arangodb::iresearch::IResearchView::SnapshotMode::FindOrCreate);
...
TEST_F(IResearchViewTest, test_register_link) {
  ...
  // known link
  {
    ...
    try {
      link1 = StorageEngineMock::buildLinkMock(arangodb::IndexId{1}, *logicalCollection,
                                               linkJson->slice());
    ...
    link1 = StorageEngineMock::buildLinkMock(arangodb::IndexId{1},
                                             *logicalCollection, linkJson->slice());
    EXPECT_FALSE(persisted);
...
TEST_F(IResearchViewTest, test_unregister_link) {
  ...
  // link removed before view (in recovery)
  {
    ...
    persisted = false;
    EXPECT_TRUE(
        (true == vocbase.dropCollection(logicalCollection->id(), true, -1).ok()));
...
TEST_F(IResearchViewTest, test_transaction_snapshot) {
  ...
  // no snapshot in TransactionState (force == true, waitForSync = false)
  {
    arangodb::transaction::Methods trx(arangodb::transaction::StandaloneContext::Create(vocbase),
                                       ...
                                       arangodb::transaction::Options());
    EXPECT_TRUE(nullptr ==
                viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::Find));
    auto* snapshot =
        viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::FindOrCreate);
  ...
  // no snapshot in TransactionState (force == true, waitForSync = true)
  {
    ...
    EXPECT_TRUE(nullptr ==
                viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::Find));
    auto* snapshot =
        viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::SyncAndReplace);
    EXPECT_TRUE(snapshot ==
                viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::Find));
    EXPECT_TRUE(snapshot ==
                viewImpl->snapshot(trx, arangodb::iresearch::IResearchView::SnapshotMode::FindOrCreate));
...
TEST_F(IResearchViewTest, test_update_overwrite) {
    ...
    // initial update (overwrite)
    {
      ...
      // for persistence
      {
        ...
        EXPECT_TRUE((slice.hasKey("deleted") && slice.get("deleted").isBoolean() &&
                     false == slice.get("deleted").getBoolean()));  // has system properties
        EXPECT_TRUE((meta.init(slice, error) && expectedMeta == meta));
        EXPECT_TRUE((true == metaState.init(slice, error) && expectedMetaState == metaState));
  ...
  // test rollback on meta modification failure (as an example invalid value for 'cleanupIntervalStep')
  {
    ...
    EXPECT_TRUE((TRI_ERROR_BAD_PARAMETER ==
                 logicalView->properties(updateJson->slice(), true, false).errorNumber()));
...
TEST_F(IResearchViewTest, test_update_partial) {
  ...
  // test rollback on meta modification failure (as an example invalid value for 'cleanupIntervalStep')
  {
    ...
    expectedMeta._cleanupIntervalStep = 52;
    EXPECT_TRUE((TRI_ERROR_BAD_PARAMETER ==
                 logicalView->properties(updateJson->slice(), true, true).errorNumber()));
  ...
  // add a new link (in recovery)
  {
    ...
    // not for persistence
    {
      ...
      EXPECT_TRUE(slice.get("deleted").isNone());  // no system properties
      EXPECT_TRUE(slice.isObject());
      EXPECT_TRUE((true == slice.hasKey("links") && slice.get("links").isObject() &&
...
TEST_F(IResearchViewTest, test_remove_referenced_analyzer) {
  ...
  // remove existing (properties don't match
  {
    // add analyzer
    {
      ...
      ASSERT_TRUE(analyzers
                      .emplace(result, vocbase->name() + "::test_analyzer3",
                               "TestAnalyzer", VPackParser::fromJson("\"abcd\"")->slice())
                      .ok());
      ASSERT_NE(nullptr, analyzers.get(vocbase->name() + "::test_analyzer3",
                                       arangodb::QueryAnalyzerRevisions::QUERY_LATEST));
    }
...
TEST_F(IResearchViewTest, create_view_with_stored_value) {
  // default
  {
    ...
    EXPECT_EQ(arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
                  std::string("obj.e") +
                  arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
                  "obj.f.f1" + arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
                  "obj.g",
