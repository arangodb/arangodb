...
protected:
  ...
  IResearchViewTest(): engine(server), server(nullptr, nullptr) {
    ...
    system = irs::memory::make_unique<TRI_vocbase_t>(TRI_vocbase_type_e::TRI_VOCBASE_TYPE_NORMAL, 0, systemDatabaseArgs);
...
TEST_F(IResearchViewTest, test_drop_database) {
  ...
  ASSERT_TRUE((TRI_ERROR_NO_ERROR == databaseFeature->createDatabase(0, "testDatabase" TOSTRING(__LINE__), arangodb::velocypack::Slice::emptyObjectSlice(), vocbase)));
