...
TEST_F(IResearchViewTest, test_defaults) {
  ...
  // new view definition with links
  {
    ...
    logicalView->visitCollections([&cids](TRI_voc_cid_t cid) -> bool {
      cids.emplace(cid);
      return true;
    });
...
TEST_F(IResearchViewTest, test_drop_cid) {
  ...
  // cid in list of collections for snapshot (view definition updated, not persisted until recovery is complete)
  {
    ...
    // collection not in view after drop (in recovery)
    {
      ...
      EXPECT_TRUE((view->visitCollections([&actual](TRI_voc_cid_t cid) -> bool {
        actual.emplace(cid);
        return true;
      })));
...
TEST_F(IResearchViewTest, test_emplace_cid) {
  ...
  // emplace (already in list)
  {
    ...
    {
      std::unordered_set<TRI_voc_cid_t> expected = {TRI_voc_cid_t{42}};
      std::set<TRI_voc_cid_t> actual;
...
TEST_F(IResearchViewTest, test_register_link) {
  ...
  // new link in recovery
  {
    ...
    {
      std::set<TRI_voc_cid_t> cids;
      view->visitCollections([&cids](TRI_voc_cid_t cid) -> bool {
        cids.emplace(cid);
        return true;
      });
    ...
    // link addition does modify view meta
    {
      ...
      std::set<TRI_voc_cid_t> actual;
      view->visitCollections([&actual](TRI_voc_cid_t cid) -> bool {
        actual.emplace(cid);
        return true;
      });
  ...
  // known link
  {
...

    {
      std::unordered_set<TRI_voc_cid_t> expected = {TRI_voc_cid_t{100},
                                                             TRI_voc_cid_t{123}};
      std::set<TRI_voc_cid_t> actual = {TRI_voc_cid_t{123}};
      view->visitCollections([&actual](TRI_voc_cid_t cid) -> bool {
        actual.emplace(cid);
        return true;
      });
...
TEST_F(IResearchViewTest, test_unregister_link) {
  ...
  // view deallocated before link removed
  {
...

    {
      ...
      EXPECT_TRUE((false == logicalCollection->getIndexes().empty()));
      std::set<TRI_voc_cid_t> cids;
      viewImpl->visitCollections([&cids](TRI_voc_cid_t cid) -> bool {
        cids.emplace(cid);
        return true;
      });
      EXPECT_TRUE((1 == cids.size()));
...
TEST_F(IResearchViewTest, test_tracked_cids) {
  ...
  // test empty before open (TRI_vocbase_t::createView(...) will call open())
  {
    ...
    std::set<TRI_voc_cid_t> actual;
    viewImpl->visitCollections([&actual](TRI_voc_cid_t cid) -> bool {
      actual.emplace(cid);
      return true;
    });
    EXPECT_TRUE((actual.empty()));
  ...
  // test add via link before open (TRI_vocbase_t::createView(...) will call open())
  {
    ...
    std::set<TRI_voc_cid_t> actual;
    std::set<TRI_voc_cid_t> expected = {TRI_voc_cid_t{100}};
    viewImpl->visitCollections([&actual](TRI_voc_cid_t cid) -> bool {
      actual.emplace(cid);
      return true;
    });
