...
TEST_CASE("IResearchQueryTestOr", "[iresearch][iresearch-query]") {
  ...
  // d.name == 'A' OR d.name == 'Q', d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'A' OR d.name == 'Q' SORT d.seq DESC RETURN d"
    );
  ...
  // d.name == 'X' OR d.same == 'xyz', BM25(d) DESC, TFIDF(d) DESC, d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'X' OR d.same == 'xyz' SORT BM25(d) DESC, TFIDF(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'K' OR d.value <= 100 OR d.duplicated == abcd, TFIDF(d) DESC, d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'K' OR d.value <= 100 OR d.duplicated == 'abcd' SORT TFIDF(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'A' OR d.name == 'Q' OR d.same != 'xyz', d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'A' OR d.name == 'Q' OR d.same != 'xyz' SORT d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'F' OR EXISTS(d.duplicated), BM25(d) DESC, d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'F' OR EXISTS(d.duplicated) SORT BM25(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'D' OR STARTS_WITH(d.prefix, 'abc'), TFIDF(d) DESC, d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'D' OR STARTS_WITH(d.prefix, 'abc') SORT TFIDF(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'D' OR STARTS_WITH(d.prefix, 'abc'), BM25(d) DESC, d.seq DESC
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'D' OR STARTS_WITH(d.prefix, 'abc') SORT BM25(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // d.name == 'D' OR STARTS_WITH(d.prefix, 'abc'), BM25(d) DESC, d.seq DESC, LIMIT 3
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER d.name == 'D' OR STARTS_WITH(d.prefix, 'abc') SORT BM25(d) DESC, d.seq DESC LIMIT 3 RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
  ...
  // STARTS_WITH(d['prefix'], 'abc') OR EXISTS(d.duplicated) OR d.value < 100 OR d.name >= 'Z', BM25(d) DESC, TFIDF(d) DESC, d.seq DESC
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER STARTS_WITH(d['prefix'], 'abc') OR EXISTS(d.duplicated) OR d.value < 100 OR d.name >= 'Z' SORT TFIDF(d) DESC, d.seq DESC RETURN d"
    );
  ...
  // PHRASE(d['duplicated'], 'v', 2, 'z', 'test_analyzer') OR STARTS_WITH(d['prefix'], 'abc') OR d.value > 100 OR d.name >= 'Z', BM25(d) DESC, TFIDF(d) DESC, d.seq DESC
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER ANALYZER(PHRASE(d.duplicated, 'v', 1, 'z'), 'test_analyzer') OR STARTS_WITH(d['prefix'], 'abc') OR d.value < 100 OR d.name >= 'Z' SORT TFIDF(d) DESC, d.seq DESC RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
