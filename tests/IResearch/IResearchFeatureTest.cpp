...
TEST_F(IResearchFeatureTest, test_options_default) {
  ...
  auto* consolidationThreadsIdle =
      opts->get<UInt32Parameter>("--arangosearch.consolidation-threads-idle");
  ...
  auto* commitThreadsIdle =
      opts->get<UInt32Parameter>("--arangosearch.commit-threads-idle");
...
TEST_F(IResearchFeatureTest, test_start) {
  ...
  irs::Finally cleanup =
      [&functions]() noexcept { functions.unprepare(); };
  ...
  std::map<std::string_view, std::pair<std::string_view, FunctionType>> expected =
      {
          // filter functions
...
TEST_F(IResearchFeatureTest, test_upgrade0_1_no_directory) {
  ...
  irs::Finally versionFilenameRestore =
      [&versionFilename]() noexcept {
