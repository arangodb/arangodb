...
TEST_CASE("IResearchQueryTestValue", "[iresearch][iresearch-query]") {
  ...
  // test empty array (true)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER [ ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
  ...
  // test non-empty array (true)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER [ 'abc', 'def' ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test numeric range (true)
  {
    ...
    auto result = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER [ 1 .. 42 ] SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == result.code);
    auto slice = result.result->slice();
    CHECK(slice.isArray());
  ...
  // test boolean (false)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER false SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
  ...
  // test boolean (true)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER true SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test numeric (false)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER 0 SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test numeric (true)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER 3.14 SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test null
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER null SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test null via bind parameter
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER @param SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d",
  ...
  // test null via bind parameter
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER 1 - @param SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d",
  ...
  // test non-empty object (true)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER { 'a': 123, 'b': 'cde' } SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test non-empty string (true)
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER 'abc' SORT BM25(d) ASC, TFIDF(d) DESC, d.seq RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test non-empty string (true), LIMIT 5
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER 'abc' SORT BM25(d) ASC, TFIDF(d) DESC, d.seq LIMIT 5 RETURN d"
    );
    REQUIRE(TRI_ERROR_NO_ERROR == queryResult.code);
    auto slice = queryResult.result->slice();
    CHECK(slice.isArray());
  ...
  // test non-empty string (true), LIMIT 5
  {
    ...
    auto queryResult = arangodb::tests::executeQuery(
      vocbase,
      "FOR d IN testView FILTER @param SORT BM25(d) ASC, TFIDF(d) DESC, d.seq LIMIT 5 RETURN d",
