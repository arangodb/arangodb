...
TEST_CASE("IResearchViewCoordinatorTest",
          "[iresearch][iresearch-view-coordinator]") {
  IResearchViewCoordinatorSetup s;
  UNUSED(s);

  SECTION("test_type") {
    CHECK((arangodb::LogicalDataSource::Type::emplace(arangodb::velocypack::StringRef(
  ...
  SECTION("visit_collections") {
    ...
    CHECK(true == view->visitCollections(
                      [&begin](TRI_voc_cid_t cid) { return *begin++ = cid; }));
  ...
  SECTION("test_defaults") {
    ...
    // create database
    {
      ...
      CHECK((ci->createDatabaseCoordinator(vocbase->name(), VPackSlice::emptyObjectSlice(), 0.0)
                 .ok()));
    }
    ...
    // view definition with LogicalView (for persistence)
    {
      ...
      // +system, +properties
      {
        ...
        CHECK((slice.hasKey("isSystem") && slice.get("isSystem").isBoolean() &&
               false == slice.get("isSystem").getBoolean()));
        CHECK((slice.get("name").copyString() == "testView"));
        CHECK((slice.get("type").copyString() ==
      ...
      // -system, +properties
      {
        ...
        CHECK((slice.hasKey("links") && slice.get("links").isObject() &&
    ...
    // new view definition with links with invalid definition
    {
      ...
      CHECK((ci->createCollectionCoordinator(vocbase->name(), collectionId, 0, 1,
    ...
    // new view definition with links
    {
      ...
      arangodb::LogicalView::ptr logicalView;
      CHECK((arangodb::iresearch::IResearchViewCoordinator::factory()
      ...
      CHECK((false == logicalView->visitCollections(
  ...
  SECTION("test_create_drop_view") {
    ...
    // create and drop view (no id specified)
    {
      ...
      CHECK((TRI_ERROR_ARANGO_DUPLICATE_NAME ==
             ci->createViewCoordinator(vocbase->name(), viewId, json->slice()).errorNumber()));
  ...
  SECTION("test_drop_with_link") {
    ...
    auto logicalCollection = ci->getCollection(vocbase->name(), collectionId);
    REQUIRE((false == !logicalCollection));
    CHECK((ci->createViewCoordinator(vocbase->name(), viewId, viewCreateJson->slice())
               .ok()));
    auto logicalView = ci->getView(vocbase->name(), viewId);
    REQUIRE((false == !logicalView));
...

    {
      ...
      // not authorised (NONE collection) as per https://github.com/arangodb/backlog/issues/459
      {
        ...
        user.grantCollection(vocbase->name(),
                             "testCollection", arangodb::auth::Level::NONE);  // for missing collections User::collectionAuthLevel(...) returns database auth::Level
      ...
      // authorised (RO collection)
      {
        ...
        auto& user =
            userMap
                ...
                .first->second;
        user.grantCollection(vocbase->name(),
                             "testCollection", arangodb::auth::Level::RO);  // for missing collections User::collectionAuthLevel(...) returns database auth::Level
        userManager->setAuthInfo(userMap);  // set user map to avoid loading configuration from system database
  ...
  SECTION("test_update_properties") {
    ...
    // create view
    {
      ...
      // update properties - full update
      {
        ...
        CHECK(nullptr != std::dynamic_pointer_cast<arangodb::iresearch::IResearchViewCoordinator>(
                             fullyUpdatedView));
  ...
  SECTION("test_update_links_partial_remove") {
    ...
    // check properties
    {
      ...
      // testCollection1
      {
        ...
        std::string error;
        CHECK(actualMeta.init(value, false, error));
        CHECK(error.empty());
  ...
  SECTION("test_update_links_partial_add") {
    ...
    // add link (collection not authorized)
    {
      ...
      REQUIRE((false == !logicalCollection1));
      arangodb::LogicalView::ptr logicalView;
      REQUIRE((arangodb::LogicalView::create(logicalView, *vocbase, viewJson->slice())
                   .ok()));
      REQUIRE((false == !logicalView));

      CHECK((true == logicalCollection1->getIndexes().empty()));
      CHECK((true == logicalView->visitCollections(
                         [](TRI_voc_cid_t) -> bool { return false; })));
      ...
      auto resetExecContext = irs::make_finally([origExecContext]() -> void {
        ExecContext::CURRENT = origExecContext;
      });
  ...
  SECTION("test_drop_link") {
    ...
    // update link
    {
      ...
      // check indexes
      {
        ...
        REQUIRE(updatedCollection);
        auto link =
            arangodb::iresearch::IResearchLinkHelper::find(*updatedCollection, *view);
        REQUIRE((link));
        linkId = link->id();
      ...
      // drop link
      CHECK((arangodb::methods::Indexes::drop(
      ...
      // visit collections
      CHECK(view->visitCollections([](TRI_voc_cid_t) { return false; }));
    ...
    // add link (collection not authorized)
    {
      ...
      REQUIRE((false == !logicalCollection1));
      arangodb::LogicalView::ptr logicalView;
      REQUIRE((arangodb::LogicalView::create(logicalView, *vocbase,
  ...
  SECTION("test_update_overwrite") {
    ...
    // modify meta params with links to missing collections
    {
      ...
      CHECK((TRI_ERROR_ARANGO_DATA_SOURCE_NOT_FOUND ==
             logicalView->properties(viewUpdateJson->slice(), false).errorNumber()));
      logicalView = ci->getView(vocbase->name(), viewId);
      REQUIRE((false == !logicalView));
      CHECK((true == logicalView->visitCollections(
    ...
    // add authorised link (existing collection not authorized)
    {
      ...
      CHECK((ci->createCollectionCoordinator(vocbase->name(), collectionId0, 0, 1,
                                             false, collection0Json->slice(), 0.0)
                 .ok()));
      auto logicalCollection0 = ci->getCollection(vocbase->name(), collectionId0);
      REQUIRE((false == !logicalCollection0));
      ...
      CHECK((ci->createCollectionCoordinator(vocbase->name(), collectionId1, 0, 1,
                                             false, collection1Json->slice(), 0.0)
                 .ok()));
      auto logicalCollection1 = ci->getCollection(vocbase->name(), collectionId1);
      REQUIRE((false == !logicalCollection1));
      ...
      // subsequent update (overwrite) not authorised (NONE collection)
      {
        ...
        user.grantCollection(vocbase->name(),
                             "testCollection1", arangodb::auth::Level::RO);  // for missing collections User::collectionAuthLevel(...) returns database auth::Level
        userManager->setAuthInfo(userMap);  // set user map to avoid loading configuration from system database
      ...
      // subsequent update (overwrite) authorised (RO collection)
      {
        ...
        auto& user =
            userMap
                ...
                .first->second;
        user.grantCollection(vocbase->name(),
                             "testCollection0", arangodb::auth::Level::RO);  // for missing collections User::collectionAuthLevel(...) returns database auth::Level
        user.grantCollection(vocbase->name(),
                             "testCollection1", arangodb::auth::Level::RO);  // for missing collections User::collectionAuthLevel(...) returns database auth::Level
        userManager->setAuthInfo(userMap);  // set user map to avoid loading configuration from system database
