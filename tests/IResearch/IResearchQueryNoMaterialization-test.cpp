  ...
  void executeAndCheck(std::string const& queryString,
    ...
    for (auto const node : nodes) {
      if (node.hasKey("type") && node.get("type").isString() &&
          node.get("type").stringRef() == "EnumerateViewNode") {
        ...
        for (auto const cf : columnFields) {
          ...
          if (cf.hasKey("fieldNumber")) {
            ...
            auto it = fields.find(std::make_pair(arangodb::iresearch::IResearchViewNode::SortColumnNumber,
            ...
            for (auto const f : fs) {
              ...
              auto it = fields.find(
                  std::make_pair(columnNumber, f.get("fieldNumber").getNumber<size_t>()));
              ASSERT_TRUE(it != fields.end());
              fields.erase(it);
            }
          }
...
TEST_P(IResearchQueryNoMaterializationTest, maxMatchColumnPriority) {
  ...
  std::vector<VPackValue> expectedValues{VPackValue(0),  VPackValue(1),
                                         VPackValue(2),  VPackValue(3),
                                         VPackValue(10), VPackValue(11),
                                         VPackValue(12), VPackValue(13)};
...
TEST_P(IResearchQueryNoMaterializationTest, sortAndStoredValues) {
  ...
  executeAndCheck(queryString, expectedValues, 2,
...
TEST_P(IResearchQueryNoMaterializationTest, storedFieldExistence) {
  ...
  std::vector<VPackValue> expectedValues{VPackValue("ex0"), VPackValue("ex2"),
                                         VPackValue("ex_10"), VPackValue("ex_12")};
...
TEST_P(IResearchQueryNoMaterializationTest, testStoredValuesRecord) {
  ...
  arangodb::ManagedDocumentResult insertedDoc;
  {
    ...
    EXPECT_TRUE(trx.begin().ok());
    auto const res = logicalCollection->insert(&trx, doc->slice(), insertedDoc, opt);
    EXPECT_TRUE(res.ok());
...

  {
    ...
    std::string const columns[] = {
        arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER + std::string("_id"),
        ...
        arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
            std::string("foo") + arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
            "str" + arangodb::iresearch::IResearchViewStoredValues::FIELDS_DELIMITER +
            "value",
