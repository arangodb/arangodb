...
* **Collection lookup access restriction** (`RestIndexHandler::collection`,
  ...
  **V1**, looking up a collection by name additionally enforces a `Read`
  access check via `ExecContext::current().canUseCollection(...)`; if this
  fails every request returns a `403`/`TRI_ERROR_FORBIDDEN`.
