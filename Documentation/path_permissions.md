...
| X  |    |    | PUT    | `/_admin/cluster/numberOfServers`                            | RestAdminClusterHandler    | canUseHard(Maintenance)                  | AdminMaintenance, HARD              | SA/SW/LEG, only coordinator              |                        |
...
| X  |    |    | GET    | `/_admin/license`                                            | RestLicenseHandler(EE)     | canUseHard(License)                      | AdminLicense, HARD                  |                                          |                        |
| X  |    |    | PUT    | `/_admin/license`                                            | RestLicenseHandler(EE)     | canUseHard(License)                      | AdminLicense, HARD                  |                                          |                        |
...
| X  |    |    | GET    | `/_admin/metrics`                                            | RestMetricsHandler         | canUseHard(Monitoring)                   | AdminMonitoring, HARD               |                                          |                        |
...
| X  |    |    | GET    | `/_admin/statistics`                                         | RestAdminStatisticsHandler | canUseHard(Monitoring)                   | AdminMonitoring, HARD               |                                          |                        |
| X  |    |    | GET    | `/_admin/statistics-description`                             | RestAdminStatisticsHandler | canUseHard(Monitoring)                   | AdminMonitoring, HARD               |                                          |                        |
| X  |    |    | GET    | `/_admin/status`                                             | RestAdminStatusHandler     | canUseHard(Monitoring)                   | AdminMonitoring HARD                |                                          |                        |
...
| X  |    |    | GET    | `/_admin/system-report`                                      | RestSystemReportHandler    | canUseHard(MonitoringInternal)           | AdminMonitoringInternali, HARD      |                                          |                        |
...
| X  |    |    | GET    | `/_admin/usage-metrics`                                      | RestUsageMetricsHandler    | canUseHard(MonitoringInternal)           | AdminMonitoringInternal HARD        |                                          |                        |
| X  |    |    | GET    | `/_admin/version`                                            | RestVersionhandler         | -/canUseHard(MonitoringInternal)         | AUTHEN, details (2)                 |                                          |                        |
...
| X  |    |    | GET    | `/_api/key-generators`                                       | RestKeyGeneratorsHandler   | -                                        | AUTHEN                              |                                          |                        |
| X  |    |    | GET    | `/_api/log`                                                  | RestLogHandler             | canUseAdmin(AdminReadReplicatedLog       | AdminReadReplicatedLog              | (replication2 + cluster only)            |                        |
| X  |    |    | POST   | `/_api/log`                                                  | RestLogHandler             | canUseAdmin(AdminWriteReplicatedLog      | AdminWriteReplicatedLog             | (replication2 + cluster only)            |                        |
| X  |    |    | DELETE | `/_api/log`                                                  | RestLogHandler             | canUseAdmin(AdminWriteReplicatedLog      | AdminWriteReplicatedLog             | (replication2 + cluster only)            |                        |
| X  |    |    | GET    | `/_api/log-internal`                                         | RestLogInternalHandler     | isSuperUser                              | SUPER                               | (replication2 + cluster only)            |                        |
| X  |    |    | GET    | `/_api/query/slow`                                           | RestQueryHandler           | _system + isSuperUser (if for all DBs)   | AUTHEN, for all DBs _system + SUPER |                                          |                        |
| X  |    |    | GET    | `/_api/query/current`                                        | RestQueryHandler           | _system + isSuperUser (if for all DBs)   | AUTHEN, for all DBs _system + SUPER |                                          |                        |
| X  |    |    | GET    | `/_api/query/properties`                                     | RestQueryHandler           | -                                        | AUTHEN                              |                                          |                        |
| X  |    |    | GET    | `/_api/query/registry`                                       | RestQueryHandler           | isSuperUser                              | SUPER                               |                                          |                        |
| X  |    |    | GET    | `/_api/query/rules`                                          | RestQueryHandler           | -                                        | AUTHEN                              |                                          |                        |
...
| X  |    |    | GET    | `/_api/query-cache/entries`                                  | RestQueryCacheHandler      | -                                        | AUTHEN                              |                                          |                        |
| X  |    |    | GET    | `/_api/query-cache/properties`                               | RestQueryCacheHandler      | -                                        | AUTHEN                              |                                          |                        |
| X  |    |    | PUT    | `/_api/query-cache/properties`                               | RestQueryCacheHandler      | _system + canUseAdmin(AdminQueryCache)   | _system + AdminQueryCache           |                                          |                        |
| X  |    |    | DELETE | `/_api/query-cache`                                          | RestQueryCacheHandler      | _system + canUseAdmin(AdminQueryCache)   | _system + AdminQueryCache           |                                          |                        |
| X  |    |    | GET    | `/_api/query-plan-cache`                                     | RestQueryPlanCacheHandler  | canUseColl(...)                          | AUTHEN, details (5)                 |                                          |                        |
| X  |    |    | DELETE | `/_api/query-plan-cache`                                     | RestQueryPlanCacheHandler  | canUseDb(Write)                          | AUTHEN, DB RW                       | needs an RBAC solution?                  | FIXME                  |
...
| X  |    |    | GET    | `/_api/ttl/properties`                                       | RestTtlHandler             | _system                                  | AUTHEN, _system                     |                                          |                        |
...
| X  |    |    | GET    | `/_api/version`                                              | RestVersionHandler         | canUseHard(MonitoringInt) for details    | AUTHEN, details (2)                 |                                          |                        |
|    |    |    | GET    | `/_api/view`                                                 |                            |                                          |                                     |                                          |                        |
|    |    |    | POST   | `/_api/view`                                                 |                            |                                          |                                     |                                          |                        |
...
| X  |    |    | GET    | `/_api/wal/lastTick`                                         | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
| X  |    |    | GET    | `/_api/wal/open-transactions`                                | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
| X  |    |    | GET    | `/_api/wal/range`                                            | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
| X  |    |    | GET    | `/_api/wal/tail`                                             | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
| X  |    |    | PUT    | `/_api/wal/tail`                                             | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
| X  |    |    | DELETE | `/_api/wal/tail`                                             | RestWalAccessHandler       | -                                        | SUPER                               | only DBServer/Single, not on coord       |                        |
|    |    |    | GET    | `/_api/transaction`                                          | RestTransactionHandler     | same user or isSuperUser, same db        | AUTHEN, only see same user and db   | Coord/Single for users, DBServer internal|                        |
