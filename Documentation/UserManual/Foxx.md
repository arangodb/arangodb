...
This will start the ArangoDB server in a **development mode** using the directory `/home/user/apps` as the workspace and `/tmp/fancy_db` as your database directory. Production application are installed using the Foxx manager and should not be changed. In development mode the server
