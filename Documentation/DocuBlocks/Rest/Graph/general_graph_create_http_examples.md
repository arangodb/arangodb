...
Only has effect in Enterprise Edition and required to create a Hybrid.
(Disjoint) Smart Graph. The collection names that are going to be created
as SatelliteCollections. Each array entry must be set as String and must
be a valid collection name. The collection type cannot be modified later.
