...
template<typename T, typename... Ts>
constexpr const auto index_of_type_v =
    detail::index_of_type<0, T, Ts...>::value;
...
struct is_braces_constructible<
    constructing_wrapper<T, Args...>,
    std::void_t<decltype(T{std::declval<Args>()...})>> : std::true_type {};

}  // namespace detail

template<class T, typename... Args>
struct is_braces_constructible : detail::is_braces_constructible<
                                     detail::constructing_wrapper<T, Args...>> {
...
std::tuple<Ts...> unpack_opt_tuple(std::tuple<std::optional<Ts>...> t) {
  return tuple_map(std::move(t), [](auto&& x) {
