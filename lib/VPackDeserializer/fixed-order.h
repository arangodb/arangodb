...
struct fixed_order_deserializer_executor_visitor {
  ...
  E& error_store;
  explicit fixed_order_deserializer_executor_visitor(value_type& value_store,
                                                     E& error_store)
  ...
  bool operator()(E e) {
    ...
    error_store = std::move(
        e.wrap("in fixed order array at position "s + std::to_string(I))
            .trace(I));
    return false;
  }
};
...
struct deserialize_plan_executor<fixed_order_deserializer<Ds...>, H> {
 ...
 private:
  ...
  static auto unpack_internal(
      ::arangodb::velocypack::deserializer::slice_type s,
    ...
    bool result =
        ([&error, &c](
             ::arangodb::velocypack::deserializer::array_iterator const& iter,
             auto& value) {
          if (error) {
            return false;
          }
          ...
          if (iter != iter.end()) {
            return deserialize<Ds, hints::hint_list_empty, ctx>(
                       *iter, {}, std::forward<ctx>(c))
                .visit(detail::fixed_order_deserializer_executor_visitor<
                       I, typename Ds::constructed_type, deserialize_error>(
                    value, error));
          } else {
            error = deserialize_error{
                "bad array length, found: "s + std::to_string(I) +
                ", expected: " + std::to_string(expected_array_length)};
            return false;
          }
