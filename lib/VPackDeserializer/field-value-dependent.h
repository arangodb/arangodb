...
template<typename F>
class is_value_deserializer_pair<
    F, std::void_t<typename F::value, typename F::deserializer>>
    : public std::true_type {
  static_assert(is_deserializer_v<typename F::deserializer>);
};

template<typename F>
constexpr bool is_value_deserializer_pair_v =
    is_value_deserializer_pair<F>::value;
...
struct field_value_dependent {
  ...
  using constructed_type =
      std::variant<typename VSs::deserializer::constructed_type...>;
...
struct field_value_dependent_executor<I, E, VD, VDs...> {
  ...
  static auto unpack(::arangodb::velocypack::deserializer::slice_type s,
                     ::arangodb::velocypack::deserializer::slice_type v,
                     C&& ctx) -> unpack_result {
    using namespace std::string_literals;
    if (values::value_comparator<V>::compare(v)) {
      using hint =
          hints::hint_list<hints::has_field_with_value<E::name, V>,
          ...
          .visit(::arangodb::velocypack::deserializer::detail::gadgets::visitor{
              ...
              [](deserialize_error&& e) {
                return unpack_result{
                    std::move(e.wrap("with value `"s + to_string(V{}) + "`")
                                  .annotate(E::name, to_string(V{})))};
              }});
    }

    return field_value_dependent_executor<I + 1, E, VDs...>::unpack(
        s, v, std::forward<C>(ctx));
  }
};
...
struct field_value_dependent_executor<I, E> {
  ...
  static auto unpack(::arangodb::velocypack::deserializer::slice_type s,
    ...
    if (s.isNone()) {
      return unpack_result{
          deserialize_error{"field `"s + E::name + "` not found"}};
    }
    return unpack_result{std::move(
        deserialize_error{"unrecognized value `"s + v.toJson() + "`"}.trace(
            E::name))};
  }
};
}  // namespace detail
...
struct deserialize_plan_executor<field_value_dependent<N, VSs...>, H> {
  using executor_type =
      deserialize_plan_executor<field_value_dependent<N, VSs...>, H>;
  using plan_result_tuple_type =
      plan_result_tuple<field_value_dependent<N, VSs...>>;
  ...
  static auto unpack(::arangodb::velocypack::deserializer::slice_type s,
    ...
    return ::arangodb::velocypack::deserializer::detail::
        field_value_dependent_executor<0, executor_type, VSs...>::unpack(
               s, value_slice, std::forward<C>(ctx))
            .visit(
                ::arangodb::velocypack::deserializer::detail::gadgets::visitor{
