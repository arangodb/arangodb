...
class MerkleTree {
 protected:
  ...
  static constexpr std::uint64_t nodeCountUpToDepth(std::uint64_t maxDepth);
  static constexpr std::uint64_t allocationSize(std::uint64_t maxDepth);
 ...
 protected:
  ...
  std::mutex& lock(std::uint64_t index) const;
