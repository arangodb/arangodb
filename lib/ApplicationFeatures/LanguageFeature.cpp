...
void* LanguageFeature::prepareIcu(std::string const& binaryPath, std::string const& binaryExecutionPath, std::string& path, std::string const& binaryName) {
  ...
  if (path.empty() || !TRI_IsRegularFile(path.c_str())) {
    if (!path.empty()) {
      LOG_TOPIC(WARN, arangodb::Logger::STARTUP) << "failed to locate '" << fn << "' at '"<< path << "'";
    }
    ...
    if (!TRI_IsRegularFile(path.c_str())) {
      ...
      LOG_TOPIC(FATAL, arangodb::Logger::STARTUP) << msg;
      FATAL_ERROR_EXIT();
    }
    else {
  ...
  if (icuDataPtr == nullptr) {
    LOG_TOPIC(FATAL, arangodb::Logger::STARTUP) << "failed to load '" << fn << "' at '" << path << "' - " << TRI_last_error();
    FATAL_ERROR_EXIT();
  }
  return icuDataPtr;
}
...
void LanguageFeature::prepare() {
  ...
  if (!Utf8Helper::DefaultUtf8Helper.setCollatorLanguage(_language, _icuDataPtr)) {
    LOG_TOPIC(FATAL, arangodb::Logger::STARTUP) << "error initializing ICU with the contents of '" << p << "'";
    FATAL_ERROR_EXIT();    
  }
}
...
void LanguageFeature::start() {
  ...
  LOG_TOPIC(DEBUG, arangodb::Logger::STARTUP) << "using default language '" << languageName << "'";
}
