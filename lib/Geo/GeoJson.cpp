...
Result parseRegion(VPackSlice const& vpack, ShapeContainer& region, bool legacy) {
  if (vpack.isObject()) {
    Type t = type(vpack);
    switch (t) {
      case Type::POINT: {
...
///   ]
/// }
Result parsePolygon(VPackSlice const& vpack, ShapeContainer& region, bool legacy) {
