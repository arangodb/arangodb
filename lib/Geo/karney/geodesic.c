...
static real epsilon, realmin, pi, degree, NaN, tiny, tol0, tol1, tol2, tolb,
    xthresh;
...
static real minx(real a, real b) { return (b < a) ? b : a; }

static real maxx(real a, real b) { return (a < b) ? b : a; }
...
static real LatFix(real x) { return fabs(x) > 90 ? NaN : x; }
...
static real SinCosSeries(boolx sinp, real sinx, real cosx, const real c[],
                         int n);
static void Lengths(const struct geod_geodesic* g, real eps, real sig12,
                    real ssig1, real csig1, real dn1, real ssig2, real csig2,
                    real dn2, real cbet1, real cbet2, real* ps12b, real* pm12b,
                    real* pm0, real* pM12, real* pM21,
                    /* Scratch area of the right size */
                    real Ca[]);
static real Astroid(real x, real y);
static real InverseStart(const struct geod_geodesic* g, real sbet1, real cbet1,
                         real dn1, real sbet2, real cbet2, real dn2, real lam12,
                         real slam12, real clam12, real* psalp1, real* pcalp1,
                         ...
                         /* Scratch area of the right size */
                         real Ca[]);
static real Lambda12(const struct geod_geodesic* g, real sbet1, real cbet1,
                     real dn1, real sbet2, real cbet2, real dn2, real salp1,
                     real calp1, real slam120, real clam120, real* psalp2,
                     real* pcalp2, real* psig12, real* pssig1, real* pcsig1,
                     real* pssig2, real* pcsig2, real* peps, real* pgomg12,
...
void geod_init(struct geod_geodesic* g, real a, real f) {
  ...
  g->b = g->a * g->f1;
  g->c2 =
      (sq(g->a) + sq(g->b) * (g->e2 == 0 ? 1
                                         : (g->e2 > 0 ? atanhx(sqrt(g->e2))
                                                      : atan(sqrt(-g->e2))) /
                                               sqrt(fabs(g->e2)))) /
...
static void geod_lineinit_int(struct geod_geodesicline* l,
                              const struct geod_geodesic* g, real lat1,
                              real lon1, real azi1, real salp1, real calp1,
                              unsigned caps) {
  ...
  if (l->caps & CAP_C1p) C1pf(eps, l->C1pa);
...
void geod_gendirectline(struct geod_geodesicline* l,
                        const struct geod_geodesic* g, real lat1, real lon1,
                        real azi1, unsigned flags, real a12_s12,
                        unsigned caps) {
...
void geod_directline(struct geod_geodesicline* l, const struct geod_geodesic* g,
                     real lat1, real lon1, real azi1, real s12, unsigned caps) {
  geod_gendirectline(l, g, lat1, lon1, azi1, GEOD_NOFLAGS, s12, caps);
}

real geod_genposition(const struct geod_geodesicline* l, unsigned flags,
                      real s12_a12, real* plat2, real* plon2, real* pazi2,
                      real* ps12, real* pm12, real* pM12, real* pM21,
                      real* pS12) {
  real lat2 = 0, lon2 = 0, azi2 = 0, s12 = 0, m12 = 0, M12 = 0, M21 = 0,
       S12 = 0;
  ...
  if (flags & GEOD_ARCMODE) {
    ...
    real tau12 = s12_a12 / (l->b * (1 + l->A1m1)), s = sin(tau12),
         c = cos(tau12);
    /* tau2 = tau1 + tau12 */
    B12 = -SinCosSeries(TRUE, l->stau1 * c + l->ctau1 * s,
                        l->ctau1 * c - l->stau1 * s, l->C1pa, nC1p);
  ...
  if (outmask & GEOD_LONGITUDE) {
    ...
    lam12 =
        omg12 +
        l->A3c * (sig12 +
                  (SinCosSeries(TRUE, ssig2, csig2, l->C3a, nC3 - 1) - l->B31));
    lon12 = lam12 / degree;
    lon2 = flags & GEOD_LONG_UNROLL
               ? l->lon1 + lon12
               : AngNormalize(AngNormalize(l->lon1) + AngNormalize(lon12));
  }

  if (outmask & GEOD_LATITUDE) lat2 = atan2dx(sbet2, l->f1 * cbet2);

  if (outmask & GEOD_AZIMUTH) azi2 = atan2dx(salp2, calp2);

  if (outmask & (GEOD_REDUCEDLENGTH | GEOD_GEODESICSCALE)) {
    real B22 = SinCosSeries(TRUE, ssig2, csig2, l->C2a, nC2),
  ...
  if (outmask & GEOD_LATITUDE) *plat2 = lat2;
  if (outmask & GEOD_LONGITUDE) *plon2 = lon2;
  if (outmask & GEOD_AZIMUTH) *pazi2 = azi2;
  if (outmask & GEOD_DISTANCE) *ps12 = s12;
  if (outmask & GEOD_REDUCEDLENGTH) *pm12 = m12;
  if (outmask & GEOD_GEODESICSCALE) {
    if (pM12) *pM12 = M12;
    if (pM21) *pM21 = M21;
  }
  if (outmask & GEOD_AREA) *pS12 = S12;
...
void geod_position(const struct geod_geodesicline* l, real s12, real* plat2,
                   real* plon2, real* pazi2) {
  geod_genposition(l, FALSE, s12, plat2, plon2, pazi2, 0, 0, 0, 0, 0);
}

real geod_gendirect(const struct geod_geodesic* g, real lat1, real lon1,
                    real azi1, unsigned flags, real s12_a12, real* plat2,
                    real* plon2, real* pazi2, real* ps12, real* pm12,
                    real* pM12, real* pM21, real* pS12) {
  ...
  geod_lineinit(
      ...
      outmask | (flags & GEOD_ARCMODE ? GEOD_NONE : GEOD_DISTANCE_IN));
  return geod_genposition(&l, flags, s12_a12, plat2, plon2, pazi2, ps12, pm12,
                          pM12, pM21, pS12);
}

void geod_direct(const struct geod_geodesic* g, real lat1, real lon1, real azi1,
                 real s12, real* plat2, real* plon2, real* pazi2) {
  geod_gendirect(g, lat1, lon1, azi1, GEOD_NOFLAGS, s12, plat2, plon2, pazi2, 0,
                 0, 0, 0, 0);
}

static real geod_geninverse_int(const struct geod_geodesic* g, real lat1,
                                real lon1, real lat2, real lon2, real* ps12,
                                real* psalp1, real* pcalp1, real* psalp2,
                                real* pcalp2, real* pm12, real* pM12,
                                real* pM21, real* pS12) {
  ...
  unsigned outmask =
      (ps12 ? GEOD_DISTANCE : 0U) | (pm12 ? GEOD_REDUCEDLENGTH : 0U) |
      (pM12 || pM21 ? GEOD_GEODESICSCALE : 0U) | (pS12 ? GEOD_AREA : 0U);
  ...
  if (cbet1 < -sbet1) {
    ...
    if (fabs(sbet2) == -sbet1) cbet2 = cbet1;
  }
  ...
  if (meridian) {
    ...
    Lengths(g, g->n, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
            &s12x, &m12x, 0, outmask & GEOD_GEODESICSCALE ? &M12 : 0,
    ...
    if (sig12 < 1 || m12x >= 0) {
      ...
      if (sig12 < 3 * tiny) sig12 = m12x = s12x = 0;
      m12x *= g->b;
      s12x *= g->b;
      a12 = sig12 / degree;
    } else
  ...
  if (!meridian && sbet1 == 0 && /* and sbet2 == 0 */
    ...
    m12x = g->b * sin(sig12);
    if (outmask & GEOD_GEODESICSCALE) M12 = M21 = cos(sig12);
    a12 = lon12 / g->f1;
    ...
    real dnm = 0;
    sig12 = InverseStart(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, slam12,
                         clam12, &salp1, &calp1, &salp2, &calp2, &dnm, Ca);

    if (sig12 >= 0) {
      /* Short lines (InverseStart sets salp2, calp2, dnm) */
      s12x = sig12 * g->b * dnm;
      m12x = sq(dnm) * g->b * sin(sig12 / dnm);
      if (outmask & GEOD_GEODESICSCALE) M12 = M21 = cos(sig12 / dnm);
      a12 = sig12 / degree;
      ...
      for (tripn = FALSE, tripb = FALSE; numit < maxit2; ++numit) {
        ...
        real dv = 0, v = Lambda12(g, sbet1, cbet1, dn1, sbet2, cbet2, dn2,
                                  salp1, calp1, slam12, clam12, &salp2, &calp2,
                                  &sig12, &ssig1, &csig1, &ssig2, &csig2, &eps,
                                  &domg12, numit < maxit1, &dv, Ca);
        ...
        if (numit < maxit1 && dv > 0) {
          real dalp1 = -v / dv;
          real sdalp1 = sin(dalp1), cdalp1 = cos(dalp1),
      ...
      Lengths(g, eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2,
              &s12x, &m12x, 0, outmask & GEOD_GEODESICSCALE ? &M12 : 0,
              outmask & GEOD_GEODESICSCALE ? &M21 : 0, Ca);
      m12x *= g->b;
      s12x *= g->b;
  ...
  if (outmask & GEOD_DISTANCE) s12 = 0 + s12x; /* Convert -0 to 0 */

  if (outmask & GEOD_REDUCEDLENGTH) m12 = 0 + m12x; /* Convert -0 to 0 */
  ...
  if (outmask & GEOD_AREA) {
    ...
    if (calp0 != 0 && salp0 != 0) {
      real
          ...
          ssig1 = sbet1,
          csig1 = calp1 * cbet1, ssig2 = sbet2, csig2 = calp2 * cbet2,
          k2 = sq(calp0) * g->ep2, eps = k2 / (2 * (1 + sqrt(1 + k2)) + k2),
          /* Multiplier = a^2 * e^2 * cos(alpha0) * sin(alpha0). */
    ...
    if (!meridian &&
      ...
      /* alp12 = alp2 - alp1, used in atan2 so no need to normalize */
      real salp12 = salp2 * calp1 - calp2 * salp1,
  ...
  if (swapp < 0) {
    swapx(&salp1, &salp2);
    swapx(&calp1, &calp2);
    if (outmask & GEOD_GEODESICSCALE) swapx(&M12, &M21);
  }
...
real geod_geninverse(const struct geod_geodesic* g, real lat1, real lon1,
                     real lat2, real lon2, real* ps12, real* pazi1, real* pazi2,
                     real* pm12, real* pM12, real* pM21, real* pS12) {
  real salp1, calp1, salp2, calp2,
      a12 = geod_geninverse_int(g, lat1, lon1, lat2, lon2, ps12, &salp1, &calp1,
                                &salp2, &calp2, pm12, pM12, pM21, pS12);
...
void geod_inverseline(struct geod_geodesicline* l,
                      const struct geod_geodesic* g, real lat1, real lon1,
                      real lat2, real lon2, unsigned caps) {
  real salp1, calp1,
      a12 = geod_geninverse_int(g, lat1, lon1, lat2, lon2, 0, &salp1, &calp1, 0,
                                0, 0, 0, 0, 0),
...
void geod_inverse(const struct geod_geodesic* g, real lat1, real lon1,
                  real lat2, real lon2, real* ps12, real* pazi1, real* pazi2) {
...
void Lengths(const struct geod_geodesic* g, real eps, real sig12, real ssig1,
             real csig1, real dn1, real ssig2, real csig2, real dn2, real cbet1,
             real cbet2, real* ps12b, real* pm12b, real* pm0, real* pM12,
             real* pM21,
             /* Scratch area of the right size */
             real Ca[]) {
  ...
  if (ps12b) {
    ...
    *ps12b = A1 * (sig12 + B1);
    ...
    int l;
    for (l = 1; l <= nC2; ++l) Cb[l] = A1 * Ca[l] - A2 * Cb[l];
  ...
  if (pM12 || pM21) {
    ...
    if (pM12) *pM12 = csig12 + (t * ssig2 - csig2 * J12) * ssig1 / dn1;
    if (pM21) *pM21 = csig12 - (t * ssig1 - csig1 * J12) * ssig2 / dn2;
  }
}
...
real InverseStart(const struct geod_geodesic* g, real sbet1, real cbet1,
                  real dn1, real sbet2, real cbet2, real dn2, real lam12,
                  real slam12, real clam12, real* psalp1, real* pcalp1,
                  /* Only updated if return val >= 0 */
                  real* psalp2, real* pcalp2,
  ...
  real sbet12a;
  boolx shortline =
      cbet12 >= 0 && sbet12 < (real)(0.5) && cbet2 * lam12 < (real)(0.5);
  ...
  salp1 = cbet2 * somg12;
  calp1 = comg12 >= 0 ? sbet12 + cbet2 * sbet1 * sq(somg12) / (1 + comg12)
  ...
  if (shortline && ssig12 < g->etol2) {
    ...
    if (g->f >= 0) { /* In fact f == 0 does not get here */
      ...
      /* x = dlat, y = dlong */
      real cbet12a = cbet2 * cbet1 - sbet2 * sbet1,
           bet12a = atan2(sbet12a, cbet12a);
      real m12b, m0;
      /* In the case of lon12 = 180, this repeats a calculation made in
       * Inverse. */
      Lengths(g, g->n, pi + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2, dn2,
    ...
    if (y > -tol1 && x > -1 - xthresh) {
      ...
      real k = Astroid(x, y);
      real omg12a =
          lamscale * (g->f >= 0 ? -x * k / (1 + k) : -y * (1 + k) / k);
      somg12 = sin(omg12a);
      comg12 = -cos(omg12a);
...
real Lambda12(const struct geod_geodesic* g, real sbet1, real cbet1, real dn1,
              real sbet2, real cbet2, real dn2, real salp1, real calp1,
              real slam120, real clam120, real* psalp2, real* pcalp2,
              real* psig12, real* pssig1, real* pcsig1, real* pssig2,
              real* pcsig2, real* peps, real* pdomg12, boolx diffp,
              real* pdlam12,
              /* Scratch area of the right size */
              real Ca[]) {
  real salp2 = 0, calp2 = 0, sig12 = 0, ssig1 = 0, csig1 = 0, ssig2 = 0,
       csig2 = 0, eps = 0, domg12 = 0, dlam12 = 0;
  real salp0, calp0;
...
void geod_polygon_addpoint(const struct geod_geodesic* g,
                           struct geod_polygon* p, real lat, real lon) {
  ...
  if (p->num == 0) {
    ...
    geod_geninverse(g, p->lat, p->lon, lat, lon, &s12, 0, 0, 0, 0, 0,
                    p->polyline ? 0 : &S12);
    accadd(p->P, s12);
    if (!p->polyline) {
      accadd(p->A, S12);
...
unsigned geod_polygon_compute(const struct geod_geodesic* g,
                              const struct geod_polygon* p, boolx reverse,
  ...
  geod_geninverse(g, p->lat, p->lon, p->lat0, p->lon0, &s12, 0, 0, 0, 0, 0,
                  &S12);
  ...
  area0 = 4 * pi * g->c2;
  if (crossings & 1) accadd(t, (t[0] < 0 ? 1 : -1) * area0 / 2);
...
unsigned geod_polygon_testpoint(const struct geod_geodesic* g,
                                const struct geod_polygon* p, real lat,
                                real lon, boolx reverse, boolx sign, real* pA,
                                real* pP) {
  ...
  for (i = 0; i < (p->polyline ? 1 : 2); ++i) {
    real s12, S12 = 0; /* Initialize S12 to stop Visual Studio warning */
    geod_geninverse(g, i == 0 ? p->lat : lat, i == 0 ? p->lon : lon,
                    i != 0 ? p->lat0 : lat, i != 0 ? p->lon0 : lon, &s12, 0, 0,
                    0, 0, 0, p->polyline ? 0 : &S12);
    perimeter += s12;
    if (!p->polyline) {
      tempsum += S12;
  ...
  if (pP) *pP = perimeter;
  if (p->polyline) return num;

  area0 = 4 * pi * g->c2;
  if (crossings & 1) tempsum += (tempsum < 0 ? 1 : -1) * area0 / 2;
  /* area is with the clockwise sense.  If !reverse convert to
   * counter-clockwise convention. */
...
unsigned geod_polygon_testedge(const struct geod_geodesic* g,
                               const struct geod_polygon* p, real azi, real s,
                               boolx reverse, boolx sign, real* pA, real* pP) {
  real perimeter, tempsum, area0;
  int crossings;
  unsigned num = p->num + 1;
...
void geod_polygonarea(const struct geod_geodesic* g, real lats[], real lons[],
                      int n, real* pA, real* pP) {
  int i;
  struct geod_polygon p;
  geod_polygon_init(&p, FALSE);
  for (i = 0; i < n; ++i) geod_polygon_addpoint(g, &p, lats[i], lons[i]);
