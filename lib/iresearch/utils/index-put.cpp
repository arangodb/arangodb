...
struct WikiDoc : Doc {
  explicit WikiDoc(const analyzer_factory_f& analyzer_factory,
    ...
    // body: text
    body = std::make_shared<TextField>("body", TEXT_INDEX_FEATURES,
                                       text_features, analyzer_factory());
...
int put(const std::string& path, const std::string& dir_type,
  ...
  if (commit_interval_ms) {
    thread_pool.run([&consolidation_cv, &consolidation_mutex, &batch_provider,
                     commit_interval_ms, &writer,
                     consolidation_threads]() -> void {
  ...
  for (size_t i = consolidation_threads; i; --i) {
    thread_pool.run([&dir, &policy, &batch_provider, &consolidation_mutex,
                     &consolidation_cv, consolidation_interval_ms,
                     &writer]() -> void {
...
int put(int argc, char* argv[]) {
  ...
  cmdput.add(DIR_TYPE, 0, "Directory type (fs|mmap)", false,
             std::string("mmap"));
  cmdput.add(FORMAT, 0, "Format (1_0|1_1|1_2|1_2simd)", false,
             std::string("1_0"));
  cmdput.add(INPUT, 0, "Input file", true, std::string());
  cmdput.add(BATCH_SIZE, 0, "Lines per batch", false, size_t(0));
  cmdput.add(CONSOLIDATE_ALL, 0, "Consolidate all segments into one", false,
             false);
  ...
  cmdput.add(CONSOLIDATION_INTERVAL, 0, "Consolidation interval (msec)", false,
             DEFAULT_CONSOLIDATION_INTERVAL_MSEC);
  cmdput.add(ANALYZER_TYPE, 0, "Text analyzer type", false,
             DEFAULT_ANALYZER_TYPE);
  cmdput.add(ANALYZER_OPTIONS, 0, "Text analyzer options", false,
             DEFAULT_ANALYZER_OPTIONS);
