...
TEST(Norm2HeaderTest, Construct) {
  ...
  AssertNorm2Header(buf, sizeof(uint32_t), std::numeric_limits<uint32_t>::max(),
...
TEST(Norm2HeaderTest, ResetByValue) {
  auto AssertNumBytes = [](auto value) {
    ...
    irs::bstring buf;
    irs::Norm2Header::Write(hdr, buf);
    AssertNorm2Header(buf, sizeof(value_type),
...
TEST(Norm2HeaderTest, ReadInvalid) {
  ...
  ASSERT_FALSE(irs::Norm2Header::Read(irs::kEmptyStringView<irs::byte_type>)
                   .has_value());
