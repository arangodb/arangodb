...
TEST(ConsolidationTierTest,
     PreferConsolidationOverCleanupWhenDoesntMeetThreshold) {
  irs::IndexMeta meta;
...
TEST(ConsolidationTierTest, SkewedSegments) {
  ...
  //  test correct selection of candidates
  {
    ...
    const std::vector<std::vector<size_t>> expected_tiers{
        {0, 1, 2, 3, 4, 10, 11}, {5, 6, 7, 8, 9, 12, 15, 16}};
    ...
    for (size_t i = 0; i < expected_tiers.size(); i++) {
      ...
      policy(candidates, reader, consolidating_segments, true);
      AssertCandidates(
          reader, expected_tier, candidates,
          "Line: " + std::to_string(__LINE__) + ", i = " + std::to_string(i));
      candidates.clear();
      policy(candidates, reader, consolidating_segments, true);
      AssertCandidates(
          reader, expected_tier, candidates,
...
TEST(ConsolidationTierTest, SkewedSegmentsAndMaxConsolidationSize) {
  ...
  /*
      ...
      In the below table, we can see the skew is over 0.4 until the last
  segment. However, at that time we're already over the maxSegmentBytes of 132.
  So, we shouldn't consolidate this candidate.
  ...
  skew:             1.000   0.667   0.571   0.533   0.516   0.508   0.406 0.316
      ...
      Instead, we would start removing segments from the candidate from the left
  until we're back within the maxSegmentBytes limit which happens when we get
  here.
  ...
  */
  std::vector<SEGMENT_SIZE> segmentSizes{1, 2, 4, 8, 16, 32, 43, 49};

  auto getAttributes = [](int segment, tier::SegmentAttributes& attrs) {
  ...
  auto result = tier::findBestConsolidationCandidate<SEGMENT_SIZE>(
      segmentSizes, maxSegmentsBytes, maxSkewThreshold, getAttributes, best);
  ASSERT_TRUE(result);
...
TEST(ConsolidationTierTest, SkewOverThresholdDontConsolidate) {
  ...
  std::vector<std::vector<SEGMENT_SIZE>> testcases{{50, 100, 2000, 4000},
                                                   {1, 2, 4, 8, 16, 32, 64},
                                                   {1, 1, 12},
                                                   {12, 12},
                                                   {12},
...
TEST(ConsolidationTierTest, NewSegmentAdditions) {
  ...
  //  sum of sizes of segments on its left, then the skew will be >= 0.5
  //  which is over the default threshold.
  std::vector<SEGMENT_SIZE> segmentSizes{1, 2, 4, 8, 16, 32, 64};
  ...
  segmentSizes.emplace_back(64);
  result = tier::findBestConsolidationCandidate<SEGMENT_SIZE>(
      segmentSizes, maxSegmentsBytes, maxSkewThreshold, getAttributes, best);
  ASSERT_TRUE(result);
...
TEST(ConsolidationTierTest, PopLeftTest) {
  ...
  std::vector<int> segmentSizes{1, 1, 2, 3, 3, 4};
  ...
  auto result = tier::findBestConsolidationCandidate<int>(
      segmentSizes, maxSegmentsBytes, maxSkewThreshold, getAttributes, best);
  ASSERT_TRUE(result);
  ...
  for (auto res_itr = best.first(), src_itr = segmentSizes.cbegin() + 1;
       res_itr != best.last() + 1; res_itr++, src_itr++) {
...
TEST(ConsolidationTierTest, CleanupSmokeTest) {
  ...
  std::vector<std::vector<SegmentType>> testcases{
      {
          {100, 100},  //  100% live
          {55, 100},   //  55% live
          {66, 100},   //  66%
      ...
      {{90, 100}, {80, 100}, {70, 100}, {60, 100}, {51, 100}}};

  std::vector<std::vector<SegmentType>> expectedCandidates{
      {{49, 100},  //  49% - under the default threshold of 50%
       {50, 100}},
      {}};

  auto getSegmentAttributes = [](const SegmentType& seg,
  ...
  for (size_t i = 0; i < testcases.size(); i++) {
    ...
    auto result = tier::findBestCleanupCandidate<SegmentType>(
        testcase, minDeletionPercentage, getSegmentAttributes, best);

    ASSERT_EQ(result, expectedCandidate.size() > 0);

    if (result) {
      ASSERT_EQ(std::distance(best.first(), best.last() + 1),
...
TEST(ConsolidationTierTest, SingleSegmentCleanup) {
  ...
  std::vector<SegmentType> testcase{{44, 100}};
...
TEST(ConsolidationTierTest, CleanupBeforeConsolidation) {
  ...
  //  test correct selection of candidates
  {
    ...
    const std::vector<std::vector<size_t>> expected_tiers{{4}, {0, 1, 2, 3}};
...
TEST(ConsolidationTierTest, CombinedLivePercentageWithinThreshold) {
  ...
  //  Segments with individual- and combined- live % over 50%
  std::vector<SegmentType> segments{{50, 90}, {60, 100}};
  ...
  tier::ConsolidationCandidate<SegmentType> best;
  auto result = tier::findBestCleanupCandidate<SegmentType>(
      segments, minDeletionPercentage, getSegmentAttributes, best);
  ...
  result = tier::findBestCleanupCandidate<SegmentType>(
      segments, minDeletionPercentage, getSegmentAttributes, best);
  ASSERT_TRUE(result);
...
TEST(ConsolidationTierTest, NullChecks) {
...

  {
    ...
    std::vector<SegmentType> segments{{0, 90}, {0, 100}};

    //  No cleanup candidates should be selected here.
    tier::ConsolidationCandidate<SegmentType> best;
    tier::findBestCleanupCandidate<SegmentType>(segments, minDeletionPercentage,
                                                getSegmentAttributes, best);
    tier::findBestConsolidationCandidate<SegmentType>(
        segments, maxSegmentsBytes, maxSkewThreshold, getSegmentAttributes,
        best);
  }

  //  Zero docs count
  {
    std::vector<SegmentType> segments{{0, 0}, {0, 0}};
