...
void LoggerOptionsProvider::declareOptions(std::shared_ptr<ProgramOptions> opts,
  ...
  opts->addOption("--log",
                  ...
                  "trace.",
                  new VectorParameter<StringParameter>(&options.levels),
  ...
  opts->addOption(
      ...
      new BooleanParameter(&options.useColor),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Dynamic));

  opts->addOption("--log.escape-control-chars",
                  "Escape control characters in log messages.",
                  new BooleanParameter(&options.useControlEscaped))
...
verbose level (e.g. `debug` or `trace`).)");
  ...
  opts->addOption("--log.escape-unicode-chars",
                  "Escape Unicode characters in log messages.",
                  new BooleanParameter(&options.useUnicodeEscaped))
      .setIntroducedIn(30900)
...
verbose level (e.g. `debug` or `trace`).)");
  ...
  opts->addOption(
          ...
          new VectorParameter<StringParameter>(&options.structuredLogParams))
...
`/_admin/log/structured` HTTP API.)");
  ...
  opts->addOption("--log.output,-o",
                  ...
                  new VectorParameter<StringParameter>(&options.output))
...
appropriate log topics to the `info` log level.)");
  ...
  opts->addOption("--log.max-entry-length",
                  ...
                  new UInt32Parameter(&options.maxEntryLength))
...
the maximum size of log messages.)");
  ...
  opts->addOption("--log.use-local-time",
                  "Use the local timezone instead of UTC.",
                  new BooleanParameter(&options.useLocalTime),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setDeprecatedIn(30500)
...
Use `--log.time-format local-datestring` instead.)");
  ...
  opts->addOption("--log.use-microtime",
                  "Use Unix timestamps in seconds with microsecond precision.",
                  new BooleanParameter(&options.useMicrotime),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setDeprecatedIn(30500)
      .setLongDescription(R"(This option is deprecated.
Use `--log.time-format timestamp-micros` instead.)");
  ...
  opts->addOption("--log.time-format", "The time format to use in logs.",
...
`local-datestring`      | 2019-03-28T10:55:23      | local date and time in format YYYY-MM-DDTHH:MM:SS)");
  ...
  opts->addOption("--log.ids", "Log unique message IDs.",
                  new BooleanParameter(&options.showIds))
...
'rocksdb'` (where `144fe` is the log ID).)");
  ...
  opts->addOption("--log.role", "Log the server role.",
                  new BooleanParameter(&options.showRole))
...
| `message`  | the actual log message payload)");
  ...
  opts->addOption("--log.file-group",
                  "The group to use for a new log file. The user must be a "
                  "member of this group.",
                  new StringParameter(&options.fileGroup));
#endif

  opts->addOption("--log.prefix", "Prefix log message with this string.",
                  new StringParameter(&options.prefix),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
...
`2020-07-23T09:46:03Z --> [17493] INFO ...`)");
  ...
  opts->addOption(
      ...
      new StringParameter(&options.file),
...
If you set this option to `auto`, the hostname is automatically determined.)");
  ...
  opts->addOption("--log.process",
                  "Show the process identifier (PID) in log messages.",
                  new BooleanParameter(&options.processId),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setIntroducedIn(30800);
  ...
  opts->addOption(
      ...
      new BooleanParameter(&options.threadId),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));
  ...
  opts->addOption("--log.performance",
                  "Shortcut for `--log.level performance=trace`.",
                  new BooleanParameter(&options.performance),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
      .setDeprecatedIn(30500);
  ...
  opts->addOption("--log.force-direct",
                  "Do not start a separate thread for logging.",
                  new BooleanParameter(&options.forceDirect),
                  arangodb::options::makeDefaultFlags(
                      arangodb::options::Flags::Uncommon))
...
off to an extra logging thread, which asynchronously writes the log messages.)");
  ...
  opts->addOption(
          ...
          new UInt32Parameter(&options.maxQueuedLogMessages),
          arangodb::options::makeDefaultFlags(
              arangodb::options::Flags::Uncommon))
...
full, log entries are written synchronously until the queue has space again.)");
  ...
  opts->addOption(
      ...
      new BooleanParameter(&options.logRequestParameters),
      arangodb::options::makeDefaultFlags(arangodb::options::Flags::Uncommon));

  opts->addObsoleteOption("log.content-filter", "", true);
  opts->addObsoleteOption("log.source-filter", "", true);
  opts->addObsoleteOption("log.application", "", true);
...
void LoggerOptionsProvider::validateOptions(
  ...
  if (options.performance) {
    options.levels.push_back("performance=trace");
  }
  ...
  if (!options.fileMode.empty()) {
    try {
      int result = std::stoi(options.fileMode, nullptr, 8);
...
#ifdef ARANGODB_HAVE_SETGID
  if (!options.fileGroup.empty()) {
    bool valid = false;
    int gidNumber = NumberUtils::atoi_positive<int>(
        options.fileGroup.data(),
        options.fileGroup.data() + options.fileGroup.size(), valid);

    if (valid && gidNumber >= 0) {
#ifdef ARANGODB_HAVE_GETGRGID
      std::optional<gid_t> gid = FileUtils::findGroup(options.fileGroup);
      if (!gid) {
        LOG_TOPIC("174c2", FATAL, arangodb::Logger::FIXME)
            << "unknown numeric gid '" << options.fileGroup << "'";
        FATAL_ERROR_EXIT();
      }
#endif
    } else {
      ...
      if (gid) {
        ...
        LOG_TOPIC("11a2c", FATAL, arangodb::Logger::FIXME)
            << "cannot convert groupname '" << options.fileGroup
