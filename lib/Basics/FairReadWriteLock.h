 ...
 public:
  ...
  bool tryWriteLock(double timeout, 
    ...
    if (_whoEntered != 0 ||
      ...
      try {
        ...
        while (true) {
          expired = !cond.wait_for(guard, std::chrono::microseconds(10000), [this]() -> bool { 
  ...
  bool tryWriteLock(double timeout = 0.000001) {
    ...
    if (_whoEntered != 0 ||
      ...
      bool expired = true;
      try {
        expired = !cond.wait_until(guard, deadline, [this]() -> bool { 
          return _whoEntered == 0 && (_readQueue.empty() || _nextPreferredPhase == Phase::WRITE); 
        });
      } catch (...) {
        // we must always remove the queued writer
  ...
  bool tryReadLock(double timeout,
    ...
    if (_whoEntered == -1 ||
      ...
      try {
        ...
        while (true) {
          expired = !_readCondition.wait_for(guard, std::chrono::microseconds(10000), [this]() -> bool { 
  ...
  bool tryReadLock(double timeout = 0.000001) {
    ...
    if (_whoEntered == -1 ||
      ...
      bool expired = true;
      try {
        expired = !_readCondition.wait_until(guard, deadline, [this]() -> bool { 
          return _whoEntered >= 0 && (_writeQueue.empty() || _nextPreferredPhase == Phase::READ); 
        });
      } catch (...) {
