...
struct elsa<ErrorCode> {
  constexpr auto operator()(ErrorCode const& value, std::size_t seed) const
      -> std::size_t {
...
constexpr static frozen::unordered_map<ErrorCode, const char*, 363>
    ErrorMessages = {
        ...
        {TRI_ERROR_CLUSTER_UNKNOWN_DISTRIBUTESHARDSLIKE,  // 1486
         "must not have a distributeShardsLike attribute pointing to an "
         "unknown collection"},
        ...
        {TRI_ERROR_CLUSTER_SHARD_LEADER_RESIGNED,  // 1491
         "a (former) shard leader refuses to perform an operation, because it "
         "has resigned in the meantime"},
        ...
        {TRI_ERROR_KEY_MUST_BE_PREFIXED_WITH_SMART_GRAPH_ATTRIBUTE,  // 4003
         "in smart vertex collections _key must be a string and prefixed with "
         "the value of the smart graph attribute"},
