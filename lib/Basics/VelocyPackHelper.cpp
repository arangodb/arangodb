...
bool VelocyPackHelper::velocyPackToFile(std::string const& filename,
  ...
  int fd = TRI_TRACKED_CREATE_FILE(tmp.c_str(), O_CREAT | O_TRUNC | O_EXCL | O_RDWR | TRI_O_CLOEXEC,
...
uint64_t VelocyPackHelper::hashByAttributes(VPackSlice slice,
  ...
  if (slice.isObject()) {
    ...
    if (pos != std::string::npos) {
      ...
      VPackBuilder temporaryBuilder;
      temporaryBuilder.add(VPackValuePair(subKey.data(), subKey.length(),
...
void VelocyPackHelper::sanitizeNonClientTypes(VPackSlice input, VPackSlice base,
  ...
  if (sanitizeExternals && input.isExternal()) {
    ...
    sanitizeNonClientTypes(input.resolveExternal(), base, output, options,
                           sanitizeExternals, sanitizeCustom);
...
VPackBuffer<uint8_t> VelocyPackHelper::sanitizeNonClientTypesChecked(
  ...
  if (resolveExt) {                    // resolve
    ...
    sanitizeNonClientTypes(input, VPackSlice::noneSlice(), builder, options,
                           sanitizeExternals, sanitizeCustom);
  } else {
    builder.add(input);
  }
