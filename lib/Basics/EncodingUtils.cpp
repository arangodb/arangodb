...
int uncompressStream(z_stream& strm, T& uncompressed) noexcept {
  ...
  try {
    ...
    // we can get here only if uncompressed.reserve() or uncompressed.append() throw
    uncompressed.clear();
    ret = Z_MEM_ERROR;
  }
...
template <typename T>
ErrorCode encoding::gzipUncompress(uint8_t const* compressed, size_t compressedLength,
                                   T& uncompressed) {
  uncompressed.clear();
...
template <typename T>
ErrorCode encoding::gzipInflate(uint8_t const* compressed, size_t compressedLength,
                                T& uncompressed) {
  uncompressed.clear();
...
template <typename T>
ErrorCode encoding::gzipDeflate(uint8_t const* uncompressed, size_t uncompressedLength,
                                T& compressed) {
  compressed.clear();
  ...
  z_stream strm;
  memset(&strm, 0, sizeof(strm));
  strm.next_in = const_cast<Bytef*>(reinterpret_cast<Bytef const*>(uncompressed));
...
template ErrorCode encoding::gzipUncompress<arangodb::velocypack::Buffer<uint8_t>>(
    uint8_t const* compressed, size_t compressedLength, arangodb::velocypack::Buffer<uint8_t>& uncompressed);
...
template ErrorCode encoding::gzipInflate<arangodb::basics::StringBuffer>(
    uint8_t const* compressed, size_t compressedLength, arangodb::basics::StringBuffer& uncompressed);

template ErrorCode encoding::gzipDeflate<arangodb::velocypack::Buffer<uint8_t>>(
    uint8_t const* uncompressed, size_t uncompressedLength, arangodb::velocypack::Buffer<uint8_t>& compressed);

template ErrorCode encoding::gzipDeflate<arangodb::basics::StringBuffer>(
    uint8_t const* uncompressed, size_t uncompressedLength, arangodb::basics::StringBuffer& compressed);
