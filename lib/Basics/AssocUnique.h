...
class AssocUnique {
 ...
 public:
  ...
  typedef std::function<bool(UserData*, Key const*, uint64_t hash, Element const&)> IsEqualKeyElementFuncType;
 ...
 public:
  ...
  Element findByKey(UserData* userData, Key const* key) const {
    ...
    for (; i < n && b._table[i] && !_isEqualKeyElement(userData, key, hash, b._table[i]); ++i)
      ;
    if (i == n) {
      for (i = 0; i < k && b._table[i] &&
  ...
  Element remove(UserData* userData, Element const& element) {
    ...
    for (; i < n && b._table[i] && !_isEqualElementElement(userData, element, b._table[i]); ++i)
      ;
    if (i == n) {
      for (i = 0; i < k && b._table[i] &&
