    ...
    if (b->_table[i].prev == INVALID_INDEX) {
      ...
      if (j == INVALID_INDEX) {
        ...
        if (useHashCache) {
          ...
          b->_table[i].writeHashCache(_helper.HashElement(userData, b->_table[i].value, true));
        }
#ifdef TRI_CHECK_MULTI_POINTER_HASH
        check(userData, false, false);
#endif
        ...
        for (i = 0; i < b._nrAlloc; i++) {
          if (b._table[i].value) {
            ...
            if (b._table[i].prev == INVALID_INDEX) {
              ...
              uint64_t hashByKey = _helper.HashElement(userData, b._table[i].value, true);
              ...
              uint64_t hashByElm = _helper.HashElement(userData, b._table[i].value, false);
