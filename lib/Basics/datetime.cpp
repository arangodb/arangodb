...
std::
    vector<std::pair<std::string, format_func_t>> const
        sortedDateMap =
            {
                {"%&",
                ...
                {"%yyyyyy",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   auto ymd = year_month_day(floor<date::days>(tp));
                ...
                {"%wwww",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   ...
                   wrk.append(::weekDayNames[wd.c_encoding()]);
                 }},
                ...
                {"%mmm",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   auto ymd = year_month_day(floor<date::days>(tp));
                   wrk.append(
                       ::monthNamesShort[static_cast<unsigned>(ymd.month()) -
                                         1]);
                 }},
                {"%www",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                ...
                {"%xxx",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   ...
                   uint64_t daysSinceFirst =
                       duration_cast<date::days>(tp - sys_days(firstDayInYear))
                           .count();
                ...
                {"%t",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   ...
                   auto diff =
                       duration_cast<duration<double, std::milli>>(diffDuration)
                           .count();
                   wrk.append(
                       std::to_string(static_cast<int64_t>(std::round(diff))));
                 }},
                {"%z",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   std::string formatted =
                       format("%FT%TZ", floor<milliseconds>(tp));
                ...
                {"%a",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   ...
                   wrk.append(std::to_string(
                       static_cast<unsigned>(lastMonthDay.day())));
                 }},
                {"%%",
                 [](std::string& wrk, arangodb::tp_sys_clock_ms const& tp) {
                   wrk.append("%");
...
bool arangodb::basics::parseIsoDuration(
  ...
  ret.minutes =
      NumberUtils::atoi_unchecked<int>(begin, begin + durationParts.length(13));

  begin = duration.data() + durationParts.position(15);
  ret.seconds =
      NumberUtils::atoi_unchecked<int>(begin, begin + durationParts.length(15));
