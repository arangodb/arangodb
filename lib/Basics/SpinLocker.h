...
class SpinLocker {
 ...
 public:
  SpinLocker(Mode mode, Lock& lock, bool doLock = true, Effort effort = Effort::Succeed)
  ...
  SpinLocker(Mode mode, Lock& lock, std::size_t maxAttempts) : _lock(lock), _mode(mode), _locked(false) {
    if (_mode == Mode::Read) {
