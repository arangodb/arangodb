  ...
  if (canLock == 0) {
    ...
    if (res != TRI_ERROR_NO_ERROR) {
      ...
      LOG_TOPIC(WARN, arangodb::Logger::FIXME)
          ...
          << TRI_errno_string(canLock) << ". a possible reason is that the filesystem does not support file-locking";
    }
  ...
  // check PATH variable
    ...
    if (p != nullptr) {
      std::vector<std::string> files =
          basics::StringUtils::split(std::string(p), ':', '\0');
...
#define LOCAL_MAX_PATH_BUFFER 2049
  ...
  */
  ...
  if ((dwReturnValue > LOCAL_MAX_PATH_BUFFER) || (dwReturnValue == 0)) {
    // something wrong
    LOG_TOPIC(TRACE, arangodb::Logger::FIXME)
        << "GetTempPathA failed: LOCAL_MAX_PATH_BUFFER=" << LOCAL_MAX_PATH_BUFFER
