...
bool copyDirectoryRecursive(std::string const& source, std::string const& target,
  ...
  handle = _wfindfirst(reinterpret_cast<wchar_t*>(f.getTerminatedBuffer()), &oneItem);
  ...
  do {
    rcs.clear();
    icu::UnicodeString d(reinterpret_cast<wchar_t*>(oneItem.name), static_cast<int32_t>(wcslen(oneItem.name)));
...
std::string slurpProgram(std::string const& program) {
  ...
  FILE* fp = _wpopen(reinterpret_cast<wchar_t*>(uprog.getTerminatedBuffer()), L"r");
#else
