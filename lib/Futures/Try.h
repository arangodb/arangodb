...
template<typename T>
class Try {
  static_assert(!std::is_reference<T>::value, "Do not use with reference types");
...
public:
  ...
  explicit Try(in_place_t, Args&&... args) noexcept(std::is_nothrow_constructible<T, Args&&...>::value)
...
/// Specialization of Try for void value type. Encapsulates either success or an exception.
template<>
class Try<void> {
 public:
  ...
  /// Set an exception value into this Try object.
  /// This has the effect of clearing any existing exception stored
  void set_exception(std::exception_ptr const& e) noexcept {
    _exception = e;
  }
