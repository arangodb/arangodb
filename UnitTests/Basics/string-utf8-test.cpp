...
#define TEST_STRING(str) \
  TRI_EscapeUtf8String(TRI_CORE_MEM_ZONE, str, strlen(str), true, &outLength, true);

#define TEST_STRING_L(str, len) \
  TRI_EscapeUtf8String(TRI_CORE_MEM_ZONE, str, len, true, &outLength, true);
...
BOOST_AUTO_TEST_CASE (tst_japanese) {
  ...
  char* unescaped = TRI_UnescapeUtf8String(TRI_CORE_MEM_ZONE, result, strlen(result), &outLength, true);
