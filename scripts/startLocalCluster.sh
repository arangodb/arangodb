...
if [ -z "$ONGOING_PORTS" ] ; then
  CO_BASE=$(( $PORT_OFFSET + 8529 ))
  DB_BASE=$(( $PORT_OFFSET + 8629 ))
  AG_BASE=$(( $PORT_OFFSET + 4001 ))
  SE_BASE=$(( $PORT_OFFSET + 8729 ))
else
  CO_BASE=$(( $PORT_OFFSET + 8529 ))
  DB_BASE=$(( $PORT_OFFSET + 8529 + $NRCOORDINATORS ))
  AG_BASE=$(( $PORT_OFFSET + 8529 + $NRCOORDINATORS + $NRDBSERVERS ))
  SE_BASE=$(( $PORT_OFFSET + 8529 + $NRCOORDINATORS + $NRDBSERVERS + $NRAGENTS ))
fi
